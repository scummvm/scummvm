...
#ifdef NDS
	return (
	  (((IPC->rtc.hours > 11 ? IPC->rtc.hours - 40 : IPC->rtc.hours) & 0x1F) << 11) | ((IPC->rtc.minutes & 0x3F) << 5) | ((IPC->rtc.seconds >> 1) & 0x1F));
#else
	return 0;
#endif
}
...
#ifdef NDS
	return (
	  (((IPC->rtc.year + 20) & 0x7F) << 9) | ((IPC->rtc.month & 0xF) << 5) | (IPC->rtc.day & 0x1F));
#else
	return 0;
#endif
}
