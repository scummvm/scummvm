...
int handleInput(struct mapledev *pad, int &mouse_x, int &mouse_y,
	...
	for (int i = 0; i < 4; i++, pad++)
		if (pad->func & MAPLE_FUNC_CONTROLLER) {
			...
			if (!(buttons & 8))
				newkey = Common::KEYCODE_F5;
			else if (!(buttons & 512))
				newkey = ' ';
			else if (!(buttons & 1024))
				newkey = numpadmap[(buttons >> 4) & 15];
			...
			if (pad->cond.controller.ltrigger > 200)
				newkey = 1005;
			else if (pad->cond.controller.rtrigger > 200)
				newkey = 1006;
			...
			for (int p = 0; p < 6; p++) {
				...
				if (shift & 0x80)
					rmb++;
				if (shift & 0x11)
					shiftFlags |= Common::KBD_CTRL;
				if (shift & 0x44)
					shiftFlags |= Common::KBD_ALT;
				if (shift & 0x22)
					shiftFlags |= Common::KBD_SHIFT;
				if (key >= 4 && key <= 0x1d)
					newkey = key + ('a' - 4);
					...
					case 0x28:
					case 0x58:
						newkey = Common::KEYCODE_RETURN;
						break;
					case 0x29:
						newkey = Common::KEYCODE_ESCAPE;
						break;
					case 0x2a:
						newkey = Common::KEYCODE_BACKSPACE;
						break;
					case 0x2b:
						newkey = Common::KEYCODE_TAB;
						break;
					case 0x2c:
						newkey = Common::KEYCODE_SPACE;
						break;
					case 0x4c:
...
bool OSystem_Dreamcast::pollEvent(Common::Event &event) {
	...
	if (_ms_cur_x < 0)
		_ms_cur_x = 0;
	if (_ms_cur_x >= _screen_w)
		_ms_cur_x = _screen_w - 1;
	if (_ms_cur_y < 0)
		_ms_cur_y = 0;
	if (_ms_cur_y >= _screen_h)
		_ms_cur_y = _screen_h - 1;
	...
	if (e < 0) {
		...
		if (e < 1000) {
			...
			event.kbd.ascii = (e >= 'a' && e <= 'z' && (event.kbd.flags & Common::KBD_SHIFT) ? e & ~0x20 : e);
			processed = true;
		} else if (down) {
			if (e == 1005)
