...
void draw_solid_quad(float x1, float y1, float x2, float y2,
	...
	mypoly.cmd = TA_CMD_POLYGON | TA_CMD_POLYGON_TYPE_OPAQUE | TA_CMD_POLYGON_SUBLIST | TA_CMD_POLYGON_STRIPLENGTH_2 | TA_CMD_POLYGON_PACKED_COLOUR | TA_CMD_POLYGON_GOURAUD_SHADING;
	mypoly.mode1 = TA_POLYMODE1_Z_ALWAYS | TA_POLYMODE1_NO_Z_UPDATE;
	mypoly.mode2 = TA_POLYMODE2_BLEND_SRC | TA_POLYMODE2_FOG_DISABLED;
	mypoly.texture = 0;
...
void draw_trans_quad(float x1, float y1, float x2, float y2,
	...
	mypoly.cmd = TA_CMD_POLYGON | TA_CMD_POLYGON_TYPE_TRANSPARENT | TA_CMD_POLYGON_SUBLIST | TA_CMD_POLYGON_STRIPLENGTH_2 | TA_CMD_POLYGON_PACKED_COLOUR | TA_CMD_POLYGON_GOURAUD_SHADING;
	mypoly.mode1 = TA_POLYMODE1_Z_ALWAYS | TA_POLYMODE1_NO_Z_UPDATE;
...
static bool loadIcon(Game &game, Dir *dirs, int num_dirs) {
	...
	for (int i = 0; i < num_dirs; i++)
		if (!strcmp(dirs[i].name, game.dir) && dirs[i].deficon[0]) {
...
static bool uniqueGame(const char *base, const char *dir,
	...
	while (cnt--)
	  ...
	  this is a workaround for the detector bug in toon... */
		    ...
		    !scumm_stricmp(base, games->filename_base) && lang == games->language && plf == games->platform)
			return false;
		else
			games++;
	return true;
...
static int findGames(Game *games, int max, bool use_ini) {
	...
	if (use_ini) {
		...
		for (Common::ConfigManager::DomainMap::const_iterator i = game_domains.begin(); curr_game < max && i != game_domains.end(); i++) {
	...
	while ((curr_game < max || use_ini) && curr_dir < num_dirs) {
		...
		if (!dirs[curr_dir].name[0] || dirs[curr_dir].name[strlen(dirs[curr_dir].name) - 1] != '/')
...
int gameMenu(Game *games, int num_games) {
	...
	for (;;) {
		...
		for (int i = top_game, cnt = 0; cnt < 10 && i < num_games; i++, cnt++) {
			...
			drawGameLabel(games[i], pal, 50.0, y, (cnt == selector_pos ? 0xffff00 : 0xffffff));
			y += 40.0;
		}
