...
vmsaveResult writeSaveGame(const char *gamename, const char *data, int size,
	...
	if (lastvm >= 0 && (res = trySave(gamename, data, size, filename, icon, lastvm)) == VMSAVE_OK)
		return res;
...
bool readSaveGame(char *&buffer, int &size, const char *filename) {
	if (lastvm >= 0 && tryLoad(buffer, size, filename, lastvm))
		return true;
...
bool deleteSaveGame(const char *filename) {
	if (lastvm >= 0 && tryDelete(filename, lastvm))
		return true;
...
public:
	...
	bool readSaveGame(const char *filename) { return ::readSaveGame(buffer, _size, filename); }
};
