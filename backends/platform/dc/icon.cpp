...
void Icon::create_vmicon(void *buffer) {
	...
	for (int n = 0; n < 16; n++) {
		int p = palette[n];
		pal[n] = ((p >> 16) & 0xf000) | ((p >> 12) & 0x0f00) | ((p >> 8) & 0x00f0) | ((p >> 4) & 0x000f);
	}
...
void Icon::draw(float x1, float y1, float x2, float y2, int pal,
	...
	mypoly.mode2 = TA_POLYMODE2_BLEND_SRC_ALPHA | TA_POLYMODE2_BLEND_DST_INVALPHA | TA_POLYMODE2_FOG_DISABLED | TA_POLYMODE2_ENABLE_ALPHA | TA_POLYMODE2_TEXTURE_MODULATE_ALPHA | TA_POLYMODE2_U_SIZE_32 | TA_POLYMODE2_V_SIZE_32;
	mypoly.texture = TA_TEXTUREMODE_CLUT4 | TA_TEXTUREMODE_CLUTBANK4(pal) | TA_TEXTUREMODE_ADDRESS(texture);
...
bool Icon::load_image2(const void *data, int len) {
	...
	if (len < 40)
		return false;
	memcpy(&hdr, data, 40);
	if (hdr.size != 40 || /* hdr.sizeimg<=0 || */ hdr.w < 0 || hdr.h < 0 || hdr.bitcnt < 0 || hdr.used < 0)
		return false;
	if (!hdr.used)
	...
	for (int i = hdr.used; i < 16; i++)
		palette[i] = 0;
	const unsigned char *mask = ((const unsigned char *)data) + hdr.size + (hdr.used << 2) + 32 * 32 / 2;
		...
		for (int y = 0; y < 32; y++)
			for (int x = 0; x < 32 / 8; x++) {
				...
				for (int z = 0; z < 4; z++) {
					...
					if (mbits & 64)
						pbits = (pbits & ~0xf) | unused;
					if (mbits & 128)
						pbits = (pbits & 0xf) | (unused << 4);
					*pix++ = pbits;
					mbits <<= 2;
				}
			}
