...
bool DLObject::relocate(int fd, unsigned long offset, unsigned long size) {
	...
	if (lseek(fd, offset, SEEK_SET) < 0 || read(fd, rela, size) != (ssize_t)size) {
...
bool DLObject::load(int fd) {
	...
	if (read(fd, &ehdr, sizeof(ehdr)) != sizeof(ehdr) || memcmp(ehdr.e_ident, ELFMAG, SELFMAG) || ehdr.e_type != 2 || ehdr.e_machine != 42 || ehdr.e_phentsize < sizeof(phdr) || ehdr.e_shentsize != sizeof(*shdr) || ehdr.e_phnum != 1) {
	...
	if (lseek(fd, ehdr.e_phoff, SEEK_SET) < 0 || read(fd, &phdr, sizeof(phdr)) != sizeof(phdr)) {
		seterror("Program header load failed.");
		return false;
	}

	if (phdr.p_type != 1 || phdr.p_vaddr != 0 || phdr.p_paddr != 0 || phdr.p_filesz > phdr.p_memsz) {
	...
	if (lseek(fd, phdr.p_offset, SEEK_SET) < 0 || read(fd, segment, phdr.p_filesz) != (ssize_t)phdr.p_filesz) {
	...
	if (lseek(fd, ehdr.e_shoff, SEEK_SET) < 0 || read(fd, shdr, ehdr.e_shnum * sizeof(*shdr)) != (ssize_t)(ehdr.e_shnum * sizeof(*shdr))) {
	...
	for (int i = 0; i < ehdr.e_shnum; i++) {
		...
		if (shdr[i].sh_type == 2 && shdr[i].sh_entsize == sizeof(Elf32_Sym) && shdr[i].sh_link < ehdr.e_shnum && shdr[shdr[i].sh_link].sh_type == 3 && symtab_sect < 0)
			symtab_sect = i;
	}
	...
	if (lseek(fd, shdr[symtab_sect].sh_offset, SEEK_SET) < 0 || read(fd, symtab, shdr[symtab_sect].sh_size) != (ssize_t)shdr[symtab_sect].sh_size) {
	...
	if (lseek(fd, shdr[shdr[symtab_sect].sh_link].sh_offset, SEEK_SET) < 0 || read(fd, strtab, shdr[shdr[symtab_sect].sh_link].sh_size) != (ssize_t)shdr[shdr[symtab_sect].sh_link].sh_size) {
	...
	for (int i = 0; i < ehdr.e_shnum; i++)
		if (shdr[i].sh_type == 4 && shdr[i].sh_entsize == sizeof(Elf32_Rela) && (int)shdr[i].sh_link == symtab_sect && shdr[i].sh_info < ehdr.e_shnum && (shdr[shdr[i].sh_info].sh_flags & 2))
...
bool DLObject::open(const char *path) {
	...
	if (ctors_start == NULL || ctors_end == NULL || dtors_start == NULL || dtors_end == NULL) {
...
void *DLObject::symbol(const char *name) {
	...
	Elf32_Sym *s = (Elf32_Sym *)symtab;
	for (int c = symbol_cnt; c--; s++)
		if ((s->st_info >> 4 == 1 || s->st_info >> 4 == 2) && strtab[s->st_name] == '_' && !strcmp(name, strtab + s->st_name + 1)) {
			DBG(("=> %p\n", (void *)s->st_value));
