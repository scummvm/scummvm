...
void GLESBaseTexture::initGLExtensions() {
	const char *ext_string = reinterpret_cast<const char *>(glGetString(GL_EXTENSIONS));
...
void GLESTexture::fillBuffer(uint32 color) {
	...
	if (_pixelFormat.bytesPerPixel == 1 || (_pixelFormat.bytesPerPixel == 2 && ((color & 0xff) == ((color >> 8) & 0xff))))
...
void GLESTexture::drawTexture(GLshort x, GLshort y, GLshort w, GLshort h) {
	...
	if (!_dirty_rect.isEmpty()) {
		...
		if (dwidth == _surface.w) {
			...
			byte *src = _pixels + _dirty_rect.top * _surface.pitch + _dirty_rect.left * _surface.format.bytesPerPixel;
			byte *dst = _buf;
...
void GLESFakePaletteTexture::drawTexture(GLshort x, GLshort y, GLshort w,
	...
	if (!_dirty_rect.isEmpty()) {
		...
		byte *src = _pixels + _dirty_rect.top * _surface.pitch + _dirty_rect.left;
		uint16 *dst = _buf;
