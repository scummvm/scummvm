...
void iOSMetalGraphicsManager::initSurface() {
	...
	MTL::CommandQueue *commandQueue = sys->getMetalCommandQueue();
	
	// Create a target to render to
	_renderTarget = new Metal::MetalRenderbufferTarget(sys->getMetalTargetTexture());
...
void iOSMetalGraphicsManager::notifyResize(const int width, const int height) {
	MTL::Texture *newtargetTexture =
		dynamic_cast<OSystem_iOS7 *>(g_system)->getMetalTargetTexture();
	_renderTarget->updateRenderBuffer(newtargetTexture);
...
bool iOSMetalGraphicsManager::loadVideoMode(uint requestedWidth, uint requestedHeight, const Graphics::PixelFormat &format) {
...
void iOSMetalGraphicsManager::refreshScreen() {
	dynamic_cast<OSystem_iOS7 *>(g_system)->refreshScreen(false);
}
