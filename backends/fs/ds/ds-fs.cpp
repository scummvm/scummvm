...
*/
	...
	if (_zipFile->restartFile()) {
		do {
			...
			if ((_zipFile->isDirectory() && ((mode == Common::FSNode::kListDirectoriesOnly) || (mode == Common::FSNode::kListAll))) || (!_zipFile->isDirectory() && ((mode == Common::FSNode::kListFilesOnly) || (mode == Common::FSNode::kListAll)))) {
...
bool GBAMPFileSystemNode::getChildren(AbstractFSList &dirList, ListMode mode, bool hidden) const {
	...
	while (entryType != TYPE_NO_MORE) {
		...
		if (((entryType == TYPE_DIR) && ((mode == Common::FSNode::kListDirectoriesOnly) || (mode == Common::FSNode::kListAll))) || ((entryType == TYPE_FILE) && ((mode == Common::FSNode::kListFilesOnly) || (mode == Common::FSNode::kListAll)))) {
