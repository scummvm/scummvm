...
enum HardwareKeyType {
	kHardwareKeyTypeKeyboard,
	kHardwareKeyTypeGesture
};
...
struct HardwareKey {
	...
	const HardwareKeyType type;

	/**
	* The KeyState that is generated by the back-end
	* when this key is pressed.
	* This is only relevant when type == kHardwareKeyTypeKeyboard
	*/
	KeyState key;
	 ...
	 * This is only relevant when type == kHardwareKeyTypeGesture
	 */
	GestureCode gesture;

	HardwareKey(String i, KeyState ky = KeyState(), String desc = "")
		: id(i), key(ky), description(desc), type(kHardwareKeyTypeKeyboard) { }

	HardwareKey(String i, GestureCode g, String desc = "")
		: id(i), gesture(g), description(desc), type(kHardwareKeyTypeGesture) { }
};
...
public:
	...
	const HardwareKey *findHardwareKey(const GestureCode gesture) const {
		List<const HardwareKey *>::const_iterator it;
	...
	/**
	 * Add hardware keys to the set out of a gesture table.
	 * @param gestures table of available gestures
	 */
	void addHardwareKeys(const GestureTableEntry gestures[], const ModifierTableEntry modifiers[]) {
		const GestureTableEntry *gesture;

		for (gesture = gestures; gesture->hwId; gesture++) {
			addHardwareKey(new HardwareKey(gesture->hwId, gesture->code, gesture->desc));
		}
	}
private:
	...
	void checkForKey(const HardwareKey *key) {
		...
		for (it = _keys.begin(); it != _keys.end(); it++) {
			const HardwareKey *entry = (*it);
			if (entry->id == key->id)
				error("Error adding HardwareKey '%s' - id of %s already in use!", key->description.c_str(), entry->id.c_str());
			else if (entry->type == kHardwareKeyTypeKeyboard && entry->key == key->key)
				error("Error adding HardwareKey '%s' - key already in use!", key->description.c_str());
			else if (entry->type == kHardwareKeyTypeGesture && entry->gesture == key->gesture)
				error("Error adding HardwareKey '%s' - gesture already in use!", key->description.c_str());
		}
	}
