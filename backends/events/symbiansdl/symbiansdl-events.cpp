...
bool SymbianSdlEventSource::remapKey(SDL_Event &ev, Common::Event &event) {
	...
	for (TInt loop = 0; loop < GUI::ACTION_LAST; loop++) {
		if (GUI::Actions::Instance()->getMapping(loop) == (uint)ev.key.keysym.sym && GUI::Actions::Instance()->isEnabled(loop)) {
			...
			case GUI::ACTION_ZONE:
				if (ev.type == SDL_KEYDOWN) {
					for (int i = 0; i < TOTAL_ZONES; i++)
						if ((_km.x / MULTIPLIER) >= _zones[i].x && (_km.y / MULTIPLIER) >= _zones[i].y && (_km.x / MULTIPLIER) <= _zones[i].x + _zones[i].width && (_km.y / MULTIPLIER <= _zones[i].y + _zones[i].height)) {
