...
bool WebOSSdlEventSource::handleMouseButtonUp(SDL_Event &ev,
	...
	if (_fingerDown[ev.button.which]) {
		...
		if (ev.button.which == 0 && _dragging) {
			...
			if (ev.button.which == 0 && _doClick && !_fingerDown[1] && !_fingerDown[2]) {
				event.type = Common::EVENT_LBUTTONUP;
				processMouseEvent(event, _curX, _curY);
				// update KbdMouse
				_km.x = _curX * MULTIPLIER;
				...
				if (_queuedDragTime > 0)
					_queuedDragTime = 0;
			} else if (ev.button.which == 1 && _fingerDown[0] && _fingerDown[1] && !_fingerDown[2]) {
			...
			} else if (ev.button.which == 2 && _fingerDown[0] && _fingerDown[1]) {
...
bool WebOSSdlEventSource::handleMouseMotion(SDL_Event &ev,
	...
	if (_fingerDown[ev.motion.which]) {
		...
		case 0:
			...
			if (_doClick && (ABS(_dragDiffX[0]) > MOUSE_DEADZONE_PIXELS || ABS(_dragDiffY[0]) > MOUSE_DEADZONE_PIXELS)) {
				_doClick = false;
				if (_queuedDragTime > 0)
					_queuedDragTime = 0;
			}
		...
		case 1:
			...
			if (_fingerDown[0] && !_fingerDown[2]) {
				...
				if (ABS(_dragDiffY[0]) > _swipeDistY && ABS(_dragDiffY[1]) > _swipeDistY) {
				...
				if (ABS(_dragDiffX[0]) > _swipeDistX && ABS(_dragDiffX[1]) > _swipeDistX) {
					// Virtually lift fingers to prevent repeat triggers
					_fingerDown[0] = _fingerDown[1] = false;
					if (_dragDiffX[0] < 0 && _dragDiffX[1] < 0) {
						...
						_queuedEscapeUpTime = g_system->getMillis() + QUEUED_KEY_DELAY;
		...
		case 2:
			...
			if (_fingerDown[0] && _fingerDown[1]) {
				...
				if (_dragDiffX[0] > _swipeDistX && _dragDiffX[1] > _swipeDistX && _dragDiffX[2] > _swipeDistX) {
					// Virtually lift fingers to prevent repeat triggers
					...
					dialog.runModal();
					return true;
				} else if (_dragDiffY[0] > _swipeDistY && _dragDiffY[1] > _swipeDistY && _dragDiffY[2] > _swipeDistY) {
					...
					_queuedSpaceUpTime = g_system->getMillis() + QUEUED_KEY_DELAY;
				}
			}
		}
	}
