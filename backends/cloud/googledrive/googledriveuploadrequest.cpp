...
#define GOOGLEDRIVE_API_FILES "https://www.googleapis.com/upload/drive/v3/files"
	...
	void GoogleDriveUploadRequest::partUploadedCallback(Networking::JsonResponse response) {
		...
		if (json->isObject()) {
			...
			if (Networking::CurlJsonRequest::jsonContainsString(object, "id", "GoogleDriveUploadRequest") && Networking::CurlJsonRequest::jsonContainsString(object, "name", "GoogleDriveUploadRequest") && Networking::CurlJsonRequest::jsonContainsString(object, "mimeType", "GoogleDriveUploadRequest")) {
				//finished
