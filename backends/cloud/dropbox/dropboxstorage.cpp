...
#define DROPBOX_API_FILES_DOWNLOAD "https://content.dropboxapi.com/2/files/download"
	...
	DropboxStorage::DropboxStorage(Common::String accessToken, Common::String userId)
	  : _token(accessToken)
	  , _uid(userId) {}
	...
	void DropboxStorage::codeFlowComplete(Networking::JsonResponse response) {
		...
		if (!Networking::CurlJsonRequest::jsonContainsString(result, "access_token", "DropboxStorage::codeFlowComplete") || !Networking::CurlJsonRequest::jsonContainsString(result, "uid", "DropboxStorage::codeFlowComplete")) {
