...
#define DROPBOX_API_FILES_UPLOAD_SESSION "https://content.dropboxapi.com/2/files/upload_session/"
	...
	void DropboxUploadRequest::partUploadedCallback(Networking::JsonResponse response) {
		...
		if (json->isObject()) {
			...
			if (Networking::CurlJsonRequest::jsonContainsString(object, "path_lower", "DropboxUploadRequest") && Networking::CurlJsonRequest::jsonContainsString(object, "server_modified", "DropboxUploadRequest") && Networking::CurlJsonRequest::jsonContainsIntegerNumber(object, "size", "DropboxUploadRequest")) {
				//finished
