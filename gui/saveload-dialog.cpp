...
SaveLoadCloudSyncProgressDialog::SaveLoadCloudSyncProgressDialog(bool canRunInBackground)
  : Dialog("SaveLoadCloudSyncProgress")
  , _close(false) {
...
SaveLoadChooserDialog::SaveLoadChooserDialog(const Common::String &dialogName, const bool saveMode)
  ...
  , _metaEngine(0)
  , _delSupport(false)
  , _metaInfoSupport(false)
  , _thumbnailSupport(false)
  , _saveDateSupport(false)
  , _playTimeSupport(false)
  , _saveMode(saveMode)
...
SaveLoadChooserSimple::SaveLoadChooserSimple(const String &title, const String &buttonLabel, bool saveMode)
  : SaveLoadChooserDialog("SaveLoadChooser", saveMode)
  , _list(0)
  , _chooseButton(0)
  , _deleteButton(0)
  , _gfxWidget(0) {
...
void SaveLoadChooserSimple::updateSelection(bool redraw) {
	...
	if (_list->isEditable()) {
		...
		if (startEditMode) {
			...
			if (_chooseButton->isEnabled() && _list->getSelectedString() == _("Untitled saved game") && _list->getSelectionColor() == ThemeEngine::kFontColorAlternate) {
				_list->setEditString("");
