...
namespace Versailles {
	...
	void Toolbar::handleToolbarEvents(const Graphics::Surface *original) {
		...
		while (!exitToolbar) {
			...
			redrawToolbar = false;
			if (_engine->checkKeysPressed(2, Common::KEYCODE_ESCAPE, Common::KEYCODE_SPACE) || _engine->getCurrentMouseButton() == 2) {
				_engine->waitMouseRelease();
				exitToolbar = true;
				break;
			}
			...
			if (_backup_selected_object != nullptr && !(_mouse_in_options || _mouse_in_view_object) && !_engine->getCurrentMouseButton()) {
