...
#define IMG_CB(name) void CryOmni3DEngine_Versailles::img_##name(ZonFixedImage *fimg)
		...
		while (1) {
			...
			if (fimg->_zoneUse) {
				if (fimg->_currentZone == 0) {
					...
					ZonFixedImage::CallbackFunctor *functor = new Common::Functor1Mem<ZonFixedImage *, void, CryOmni3DEngine_Versailles>(this,
	...
	IMG_CB(32120) {
		...
		// Already painted
		if (_gameVariables[GameVariables::kSketchState] == 3 || _gameVariables[GameVariables::kSketchState] == 4) {
			fimg->_exit = true;
			return;
		}
		...
		while (1) {
			...
			if (fimg->_zoneUse) {
				...
				if (fimg->_currentZone == 1) { // on gold paint
					...
					if (fimg->_usedObject->idOBJ() == 111 && _gameVariables[GameVariables::kBrushColor] == 1) {
						...
						fimg->changeCallback(functor);
						break;
					} else if (fimg->_usedObject->idOBJ() == 112 && _gameVariables[GameVariables::kBrushColor] == 2) {
	...
	IMG_CB(34174c) {
		...
		if (_gameVariables[GameVariables::kCollectVaubanBlueprint1] && _gameVariables[GameVariables::kCollectVaubanBlueprint2]) {
			// Safe is empty
			ZonFixedImage::CallbackFunctor *functor = new Common::Functor1Mem<ZonFixedImage *, void, CryOmni3DEngine_Versailles>(this,
		...
		if (!_gameVariables[GameVariables::kCollectVaubanBlueprint1] && _gameVariables[GameVariables::kCollectVaubanBlueprint2] == 1) {
	...
	IMG_CB(41802) {
		// Dispatch to the correct state
		if (_gameVariables[GameVariables::kInkSpilled] && !_gameVariables[GameVariables::kCollectedPaperOnTable]) {
	...
	IMG_CB(43143) {
		...
		if (!_gameVariables[GameVariables::kCollectLampoonArchitecture] && (_currentLevel == 5 || currentGameTime() >= 3)) {
			ZonFixedImage::CallbackFunctor *functor = new Common::Functor1Mem<ZonFixedImage *, void, CryOmni3DEngine_Versailles>(this,
	...
	IMG_CB(44161) {
		...
		if (_gameVariables[GameVariables::kCollectQuill] == 1 && !_inventory.inInventoryByNameID(126)) {
			...
			fimg->changeCallback(functor);
			return;
		} else if (_gameVariables[GameVariables::kUsedVaubanBlueprint1] == 1 && _gameVariables[GameVariables::kUsedVaubanBlueprint2] == 1) {
			// We have used vauban blueprints: display the solution
			ZonFixedImage::CallbackFunctor *functor = new Common::Functor1Mem<ZonFixedImage *, void, CryOmni3DEngine_Versailles>(this,
			                                                                                                                     &CryOmni3DEngine_Versailles::img_44161f);
			fimg->changeCallback(functor);
			return;
		} else if (_gameVariables[GameVariables::kCollectQuill] == 1 && _inventory.inInventoryByNameID(126)) {
		...
		while (1) {
			...
			if (fimg->_zoneUse) {
				...
				if (keyCode == Common::KEYCODE_BACKSPACE) {
					...
					if (keyCode >= Common::KEYCODE_a && keyCode <= Common::KEYCODE_z && _epigraphContent.contains(keyCode - Common::KEYCODE_a + 'A')) {
	...
	IMG_CB(88001) {
		if (!_inventory.inInventoryByNameID(121) && _gameVariables[GameVariables::kMedalsDrawerStatus] == 3) {
			ZonFixedImage::CallbackFunctor *functor = new Common::Functor1Mem<ZonFixedImage *, void, CryOmni3DEngine_Versailles>(this,
		...
		while (1) {
			...
			if (fimg->_zoneUse && !_inventory.inInventoryByNameID(121)) {
				// Open the drawer
	...
	IMG_CB(88001b) {
		...
		while (1) {
			...
			if (fimg->_usedObject && fimg->_usedObject->idOBJ() == 114 && fimg->_currentZone == 0) {
	...
	IMG_CB(88001c) {
		...
		while (1) {
			...
			if (fimg->_usedObject && fimg->_usedObject->idOBJ() == 113 && fimg->_currentZone == 0) {
	...
	IMG_CB(88003) {
		// Dispatch to the correct state
		if (_gameVariables[GameVariables::kBombState] >= 1 && _gameVariables[GameVariables::kBombState] <= 5) {
			...
			ZonFixedImage::CallbackFunctor *functor = new Common::Functor1Mem<ZonFixedImage *, void, CryOmni3DEngine_Versailles>(this, callback);
			fimg->changeCallback(functor);
			return;
		}
		...
		while (1) {
			...
			if (fimg->_currentZone == 0 && fimg->_usedObject && fimg->_usedObject->idOBJ() == 145) {
	...
	IMG_CB(88003b) {
		...
		while (1) {
			...
			if (fimg->_currentZone == 0 && fimg->_usedObject && fimg->_usedObject->idOBJ() == 97) {
	...
	IMG_CB(88003c) {
		...
		while (1) {
			...
			if (fimg->_currentZone == 1 && fimg->_usedObject && fimg->_usedObject->idOBJ() == 116) {
	...
	IMG_CB(88003d) {
		...
		while (1) {
			...
			if (fimg->_currentZone == 2 && fimg->_usedObject && fimg->_usedObject->idOBJ() == 135) {
	...
	IMG_CB(88003e) {
		...
		while (1) {
			...
			if (fimg->_currentZone == 3 && fimg->_usedObject && fimg->_usedObject->idOBJ() == 136) {
...
*/
	...
	FILTER_EVENT(2, 1) {
		if (*event == 22101 && _inventory.selectedObject()) {
			...
			if (idOBJ == 96 || idOBJ == 101 || idOBJ == 115 || idOBJ == 125 || idOBJ == 127) {
	...
	FILTER_EVENT(2, 5) {
		if (*event == 22501 && _inventory.selectedObject()) {
			...
			_inventory.deselectObject();
		} else if (*event >= 1 && *event <= 9999 && _inventory.inInventoryByNameID(96) && !_inventory.inInventoryByNameID(101)) {
	...
	FILTER_EVENT(2, 9) {
		if (*event == 22902 && _inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 105) {
			...
			_inventory.deselectObject();
		} else if (*event >= 1 && *event <= 9999 && currentGameTime() == 3 && _placeStates[9].state != 2) {
			setPlaceState(9, 2);
		}
		return true;
	}
	...
	FILTER_EVENT(2, 12) {
		if (*event == 22121 && _inventory.selectedObject()) {
			...
			if (_inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 107 && _gameVariables[GameVariables::kSketchState] == 2) {
	...
	FILTER_EVENT(3, 3) {
		if (*event == 23030 && _inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 118 && _gameVariables[GameVariables::kDecipherScore]) {
	...
	FILTER_EVENT(3, 10) {
		if (*event == 23101 && _inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 120) {
	...
	FILTER_EVENT(3, 17) {
		if (*event == 18) {
			if (_inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 123) {
	...
	FILTER_EVENT(3, 22) {
		if (*event == 33220) {
			if (!_gameVariables[GameVariables::kCollectLampoonArchitecture]) {
				if (_inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 119) {
					// Using pool cue
					_inventory.removeByNameID(119);
		...
		if (*event >= 20000 && *event < 30000 && _inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 118) {
			...
			if (_dialogsMan["LULLY-DONNE-MISSION1-JOUEUR"] != 'Y' || _gameVariables[GameVariables::kDecipherScore]) {
				_inventory.deselectObject();
			} else {
	...
	FILTER_EVENT(3, 23) {
		...
		if (_inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 140) {
	...
	bool CryOmni3DEngine_Versailles::filterEventLevel3Obj23151() {
		if (_inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 115) {
			_dialogsMan["{JOUEUR-MONTRE-PAMPHLET-ARCHITECTURE}"] = 'Y';
		} else if (_inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 121 && _gameVariables[GameVariables::kGotMedalsSolution]) {
	...
	FILTER_EVENT(4, 12_13_14) {
		...
		if (!_inventory.selectedObject() || _inventory.selectedObject()->idOBJ() != 130) {
	...
	FILTER_EVENT(4, 15) {
		if (*event == 17 && (_dialogsMan["BONTEMPS-VU-PAPIER-CROISSY"] == 'N' || _dialogsMan["BONTEMPS-VU-PAMPHLET-GOUVERNEMENT"] == 'N')) {
			// Closed
			displayMessageBoxWarp(2);
			return false;
		}
		...
		if (*event == 35162) {
			...
			if (_placeStates[16].state != 0) {
				if (_gameVariables[GameVariables::kLadderState] == 0) {
					...
					_gameVariables[GameVariables::kLadderState] = 1;
					filterEventLevel5UpdatePlaceStates();
				} else if (_gameVariables[GameVariables::kLadderState] == 1 && _inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 108) {
					// Put back the ladder
			...
			// Don't move the ladder when there is a guard
			if (_placeStates[16].state != 0) {
				if (!_gameVariables[GameVariables::kOpenedCurtain] && _inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 133) {
			...
			if (_gameVariables[GameVariables::kLadderState] == 1 && _inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 108) {
	...
	FILTER_EVENT(5, 27) {
		if (*event == 25270) {
			...
			// Handled here
			return false;
		} else if (*event > 0 && *event < 10000 && currentGameTime() == 1 && _gameVariables[GameVariables::kCollectCord]) {
			setGameTime(2, 5);
		}
	...
	FILTER_EVENT(6, Orangery) {
		if (*event == 36000) {
			if (_inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 143) {
	...
	FILTER_EVENT(6, 19) {
		if (*event == 26190 && _inventory.selectedObject() && _placeStates[19].state == 0) {
	...
	FILTER_EVENT(7, 2) {
		if (*event == 37021) {
			if (_inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 103) {
				// Light the candle
			...
			if (_inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 102) {
				// Now you can go
	...
	FILTER_EVENT(7, 10_11_13) {
		if (*event == 37131) {
			if (_inventory.selectedObject() && _inventory.selectedObject()->idOBJ() == 143 && !_inventory.inInventoryByNameID(136)) {
				collectObject(136);
