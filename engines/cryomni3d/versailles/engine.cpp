...
namespace Versailles {
	...
	CryOmni3DEngine_Versailles::CryOmni3DEngine_Versailles(OSystem *syst,
	  ...
	  , _musicVolumeFactor(1.)
	  , _musicCurrentFile(nullptr)
	  , _countingDown(false)
	  , _countdownNextEvent(0) {
	}
	...
	Common::Error CryOmni3DEngine_Versailles::run() {
		...
		for (uint lvl = 1; lvl <= 7; lvl++) {
			...
			fallbackFilesAnimacti->addSubDirectoryMatching(gameDataDir, Common::String::format("animacti/level%d", lvl), 2);
			fallbackFilesWarp->addSubDirectoryMatching(gameDataDir, Common::String::format("warp/level%d/cyclo", lvl), 2);
			fallbackFilesWarp->addSubDirectoryMatching(gameDataDir, Common::String::format("warp/level%d/hnm", lvl), 2);
			fallbackFilesImgFix->addSubDirectoryMatching(gameDataDir, Common::String::format("img_fix/level%d", lvl), 2);
		...
		_inventory.init(50, new Common::Functor1Mem<uint, void, Toolbar>(&_toolbar, &Toolbar::inventoryChanged));
	...
	void CryOmni3DEngine_Versailles::loadCursorsPalette() {
		...
		_cursorPalette = new byte[3 * (bmpDecoder.getPaletteColorCount() + bmpDecoder.getPaletteStartIndex())];
		memset(_cursorPalette, 0, 3 * (bmpDecoder.getPaletteColorCount() + bmpDecoder.getPaletteStartIndex()));
	...
	void CryOmni3DEngine_Versailles::initNewLevel(int level) {
		...
		if (level >= 1 && level <= 7) {
			...
			levelFilesAnimacti->addSubDirectoryMatching(gameDataDir, Common::String::format("animacti/level%d", level), 1);
			levelFilesWarp->addSubDirectoryMatching(gameDataDir, Common::String::format("warp/level%d/cyclo", level), 1);
			levelFilesWarp->addSubDirectoryMatching(gameDataDir, Common::String::format("warp/level%d/hnm", level), 1);
			levelFilesImgFix->addSubDirectoryMatching(gameDataDir, Common::String::format("img_fix/level%d", level), 1);
	...
	bool CryOmni3DEngine_Versailles::handleWarpMouse(uint *actionId,
		...
		if (getCurrentMouseButton() == 2 || getNextKey().keycode == Common::KEYCODE_SPACE) {
