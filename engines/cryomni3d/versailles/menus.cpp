...
namespace Versailles {
	...
	void CryOmni3DEngine_Versailles::drawMenuTitle(Graphics::ManagedSurface *surface, byte color) {
		...
		if (getLanguage() == Common::FR_FRA || getLanguage() == Common::ES_ESP) {
			titleX = 144;
			titleY = 160;
			subtitleX = 305;
			subtitleY = 160;
			...
			if (drawState > 0) {
				...
				for (Common::Array<int>::iterator it = menuEntries.begin(); it != menuEntries.end(); it++) {
					if (*it == 30 && !ConfMan.getBool("subtitles")) {
						*it = 31;
					} else if (*it == 32 && (ConfMan.getBool("mute") || ConfMan.getBool("music_mute"))) {
						*it = 33;
					}
#if 0
				else if (*it == 34) {
			...
			if (pollEvents() || forceEvents) { // always call pollEvents
				...
				if (it != menuEntries.end()) {
					...
					if (boxes.hitTest(volumeBox, mouse)) {
						...
						} else if (getDragStatus() == 2 && !_mixer->hasActiveChannelOfType(Audio::Mixer::kMusicSoundType) && _mixer->getVolumeForSoundType(Audio::Mixer::kMusicSoundType) > 0) {
							// Finished dragging
				...
				if (selectedMsg == 27 || selectedMsg == 28 || selectedMsg == 40 || selectedMsg == 42) {
					...
					if (!_isPlaying || _isVisiting) {
						...
						} else if (getLanguage() == Common::ES_ESP || getLanguage() == Common::IT_ITA) {
