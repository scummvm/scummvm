...
RivenScriptManager::RivenScriptManager(MohawkEngine_Riven *vm) : _vm(vm),
...
void RivenScript::applyCardPatches(MohawkEngine_Riven *vm, uint32 cardGlobalId, uint16 scriptType, uint16 hotspotId) {
	...
	// switchCard(534);
	// playSound(112, 256, 0);
	if (cardGlobalId == 0x2E900 && scriptType == kMouseDownScript && hotspotId == 3 && !vm->isGameVariant(GF_DVD)) {
		shouldApplyPatches = true;
		RivenSimpleCommand::ArgumentArray arguments;
	...
	if (cardGlobalId == 0xE2E && scriptType == kMouseDownScript && hotspotId == 16 && vm->isGameVariant(GF_25TH)) {
		shouldApplyPatches = true;
		_commands.clear();
...
RivenCommand::RivenCommand(MohawkEngine_Riven *vm) : _vm(vm) {
}

RivenCommand::~RivenCommand() {
}

RivenSimpleCommand::RivenSimpleCommand(MohawkEngine_Riven *vm, RivenCommandType type, const ArgumentArray &arguments) : RivenCommand(vm),
                                                                                                                        _type(type),
...
RivenSwitchCommand::RivenSwitchCommand(MohawkEngine_Riven *vm) : RivenCommand(vm),
                                                                 _variableId(0) {
}
...
RivenStackChangeCommand::RivenStackChangeCommand(MohawkEngine_Riven *vm, uint16 stackId, uint32 globalCardId,
                                                 bool byStackId, bool byStackCardId) : RivenCommand(vm),
                                                                                       _stackId(stackId),
...
RivenTimerCommand::RivenTimerCommand(MohawkEngine_Riven *vm, const Common::SharedPtr<RivenStack::TimerProc> &timerProc) : RivenCommand(vm),
