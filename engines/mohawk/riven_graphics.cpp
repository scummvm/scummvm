...
public:
	TransitionEffect(OSystem *system, Graphics::Surface *mainScreen, Graphics::Surface *effectScreen,
	                 RivenTransition type, uint duration, const Common::Rect &rect) : _system(system),
...
public:
	TransitionEffectWipe(OSystem *system, Graphics::Surface *mainScreen, Graphics::Surface *effectScreen,
	                     RivenTransition type, uint duration, const Common::Rect &rect) : TransitionEffect(system, mainScreen, effectScreen, type, duration, rect) {
...
public:
	TransitionEffectPan(OSystem *system, Graphics::Surface *mainScreen, Graphics::Surface *effectScreen,
	                    RivenTransition type, uint duration, const Common::Rect &rect, int16 offset) : TransitionEffect(system, mainScreen, effectScreen, type, duration, rect) {
...
RivenGraphics::RivenGraphics(MohawkEngine_Riven *vm) : GraphicsManager(),
                                                       _vm(vm),
...
void RivenGraphics::updateScreen() {
	if (_dirtyScreen) {
		...
		if (_scheduledTransition == kRivenTransitionNone || _transitionMode == kRivenTransitionModeDisabled) {
...
WaterEffect::WaterEffect(MohawkEngine_Riven *vm, uint16 sfxeID) : _vm(vm) {
...
RivenTransitionMode RivenGraphics::sanitizeTransitionMode(int mode) {
	if (mode != kRivenTransitionModeDisabled && mode != kRivenTransitionModeFastest && mode != kRivenTransitionModeNormal && mode != kRivenTransitionModeBest) {
...
FliesEffect::FliesEffect(MohawkEngine_Riven *vm, uint16 count, bool fireflies) : _vm(vm) {
...
void FliesEffect::selectAlphaMap(bool horGridOffset, bool vertGridoffset, const uint16 **alphaMap, uint *width, uint *height) {
	...
	for (uint i = 0; i < ARRAYSIZE(alphaSelector); i++) {
		if (alphaSelector[i].horizontalGridOffset == horGridOffset && alphaSelector[i].verticalGridOffset == vertGridoffset && alphaSelector[i].isLarge == _parameters->isLarge) {
