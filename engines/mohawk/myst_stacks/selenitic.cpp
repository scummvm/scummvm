...
Selenitic::Selenitic(MohawkEngine_Myst *vm) : MystScriptParser(vm, kSeleniticStack),
...
void Selenitic::o_soundLock_init(uint16 var, const ArgumentsArray &args) {
	for (uint i = 0; i < _vm->getCard()->_resources.size(); i++) {
		...
		} else if (_vm->getCard()->_resources[i]->hasType(kMystAreaImageSwitch) && _vm->getCard()->_resources[i]->getImageSwitchVar() == 28) {
