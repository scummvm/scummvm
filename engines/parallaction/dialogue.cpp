...
DialogueManager::DialogueManager(Parallaction *vm, ZonePtr z) : _vm(vm), _z(z) {
	...
	if (!isNpc) {
		...
		for (int i = 1; i < kNumberOfVoiceDatas; ++i) {
			if (characterVoiceDatas[i].characterName == _z->u._filename) {
...
bool DialogueManager::displayQuestion() {
	...
	if (_questionerVoiceID < kNumberOfVoiceDatas - 1 && characterVoiceDatas[_questionerVoiceID + 1].characterName.empty()) {
