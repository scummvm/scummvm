...
public:
	BraInventoryObjects(Common::ReadStream &stream)
	  : BraFont(stream) {
	}
...
Font *DosDisk_ns::createFont(const char *name, Cnv *cnv) {
	...
	else if (!scumm_stricmp(name, "topaz"))
		f = new DosMonospacedFont(cnv);
	else if (!scumm_stricmp(name, "slide"))
		f = new DosMonospacedFont(cnv);
	else
