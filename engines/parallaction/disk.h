...
public:
	...
	virtual Script* loadLocation(const char *name) override;
	virtual Script* loadScript(const char* name) override;
};
...
protected:
	Gfx	 *_gfx;
	virtual Common::SeekableReadStream *tryOpenFile(const char* name) override;
	virtual void decodeCnv(byte *data, uint16 numFrames, uint16 width, uint16 height, Common::SeekableReadStream *stream) override;
