...
void KyraEngine_MR::malcolmSceneStartupChat() {
	...
	_cnvFile->seek(index1*6, Seek::CUR);
	_cnvFile->seek(index2*4, Seek::CUR);
	_cnvFile->seek(index*2, Seek::CUR);
	_cnvFile->seek(_cnvFile->readUint16LE(), Seek::SET);
