...
void RegionManager::updateOverRegion() {
	...
	for (ScreenRegionList::iterator it = _allScreenRegions->begin(); it != _allScreenRegions->end(); ++it) {
		if ((g_sludge->_evtMan->mouseX() >= (*it)->x1 - cameraX) && (g_sludge->_evtMan->mouseY() >= (*it)->y1 - cameraY) && (g_sludge->_evtMan->mouseX() <= (*it)->x2 - cameraX) && (g_sludge->_evtMan->mouseY() <= (*it)->y2 - cameraY)) {
			_overRegion = (*it);
			return;
		}
	}
