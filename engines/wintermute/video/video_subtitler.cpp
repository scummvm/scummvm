...
VideoSubtitler::VideoSubtitler(BaseGame *inGame)
  : BaseClass(inGame) {
	_lastSample = -1;
	_currentSubtitle = 0;
	_showSubtitle = false;
...
bool VideoSubtitler::loadSubtitles(const Common::String &filename, const Common::String &subtitleFile) {
	...
	while (pos < fileSize) {
		...
		while (pos + lineLength < fileSize && buffer[pos + lineLength] != '\n' && buffer[pos + lineLength] != '\0') {
...
void VideoSubtitler::update(uint32 frame) {
	...
	if ((int32)frame != _lastSample) {
		...
		while (_currentSubtitle < _subtitles.size() && overdue && hasNext && nextStarted) {
			/*
