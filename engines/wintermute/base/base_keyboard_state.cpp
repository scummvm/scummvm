...
bool BaseKeyboardState::readKey(Common::Event *event) {
	...
	if (_currentCharCode >= 'a' && _currentCharCode <= 'z' && (event->kbd.hasFlags(Common::KBD_SHIFT) || event->kbd.flags & Common::KBD_CAPS)) {
		if (!(event->kbd.keycode >= Common::KEYCODE_F1 && event->kbd.keycode <= Common::KEYCODE_F12)) {
			_currentCharCode -= 'a' - 'A';
		}
	}
...
uint32 BaseKeyboardState::keyCodeToVKey(Common::Event *event) {
	...
	if ((event->kbd.keycode >= 'a' && event->kbd.keycode <= 'z') || (event->kbd.keycode >= '0' && event->kbd.keycode <= '9')) {
		return event->kbd.ascii;
	}

	if ((event->kbd.ascii >= '0' && event->kbd.ascii <= '9') && ((event->kbd.flags & Common::KBD_NUM) != 0)) {
		return event->kbd.ascii;
	}
