...
Mesh3DSOpenGL::Mesh3DSOpenGL() : _vertexData(nullptr), _vertexCount(0), _indexData(nullptr), _indexCount(0) {
}
...
void Mesh3DSOpenGL::computeNormals() {
...
bool Wintermute::Mesh3DSOpenGL::loadFrom3DS(Common::MemoryReadStream &fileStream) {
...
void Mesh3DSOpenGL::dumpVertexCoordinates(const char *filename) {
...
int Mesh3DSOpenGL::faceCount() {
	// .3ds files have only triangles anyways
	return _indexCount / 3;
}

uint16 *Mesh3DSOpenGL::getFace(int index) {
	return _indexData + 3 * index;
}

float *Mesh3DSOpenGL::getVertexPosition(int index) {
	return reinterpret_cast<float *>(&((_vertexData + index)->x));
}

int Wintermute::Mesh3DSOpenGL::vertexCount() {
	return _vertexCount;
}
