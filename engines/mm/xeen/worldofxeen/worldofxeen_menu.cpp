...
void CloudsMenuDialog::loadButtons() {
	...
	addButton(Common::Rect(93, 53, 227, 73), Res.KeyConstants.CloudsOfXeenMenu.KEY_START_NEW_GAME, &_buttonSprites, 0);
	addButton(Common::Rect(93, 78, 227, 98), Res.KeyConstants.CloudsOfXeenMenu.KEY_LOAD_GAME, &_buttonSprites, 1);
	addButton(Common::Rect(93, 103, 227, 123), Res.KeyConstants.CloudsOfXeenMenu.KEY_SHOW_CREDITS, &_buttonSprites, 2);
	if (g_vm->_gameWon[0])
		addButton(Common::Rect(93, 128, 227, 148), Res.KeyConstants.CloudsOfXeenMenu.KEY_VIEW_ENDGAME, &_buttonSprites, 3);
}
...
void CloudsMenuDialog::draw() {
	...
	w.frame();
	w.writeString(Common::String::format(Res.OPTIONS_MENU, Res.GAME_NAMES[0], g_vm->_gameWon[0] ? 117 : 92, 1992), _voiceText);
	drawButtons(&w);
}
...
void DarkSideMenuDialog::loadButtons() {
	addButton(Common::Rect(124, 87, 177, 97), Common::KEYCODE_s, nullptr, 0);
	addButton(Common::Rect(126, 98, 173, 108), Common::KEYCODE_l, nullptr, 1);
	addButton(Common::Rect(91, 110, 209, 120), Common::KEYCODE_c, nullptr, 2);
	addButton(Common::Rect(85, 121, 216, 131), Common::KEYCODE_o, nullptr, 3);
}
...
void WorldMenuDialog::loadButtons() {
	_buttonSprites.load("start.icn");
	addButton(Common::Rect(93, 53, 227, 73), Common::KEYCODE_s, &_buttonSprites, 0);
	addButton(Common::Rect(93, 78, 227, 98), Common::KEYCODE_l, &_buttonSprites, 1);
	addButton(Common::Rect(93, 103, 227, 123), Common::KEYCODE_c, &_buttonSprites, 2);
	addButton(Common::Rect(93, 128, 227, 148), Common::KEYCODE_o, &_buttonSprites, 3);
}
...
void WorldMenuDialog::draw() {
	...
	w.frame();
	w.writeString(Common::String::format(Res.OPTIONS_MENU, Res.GAME_NAMES[2], 117, 1993), _voiceText);
	drawButtons(&w);
}
...
void OtherOptionsDialog::loadButtons() {
	...
	// View Darkside Intro
	addButton(r, Common::KEYCODE_d, &_buttonSprites, 0);
	r.translate(0, 25);

	// View Clouds Intro
	if (g_vm->getGameID() == GType_WorldOfXeen) {
		addButton(r, Common::KEYCODE_c, &_buttonSprites, 1);
		r.translate(0, 25);
	} else {
	...
	// View Darkside End
	if (g_vm->_gameWon[1]) {
		addButton(r, Common::KEYCODE_e, &_buttonSprites, 2);
		r.translate(0, 25);
	} else {
		addButton(Common::Rect(), Common::KEYCODE_INVALID, nullptr, 2);
	}

	// View Clouds End
	if (g_vm->_gameWon[0]) {
		addButton(r, Common::KEYCODE_v, &_buttonSprites, 3);
		r.translate(0, 25);
	} else {
		addButton(Common::Rect(), Common::KEYCODE_INVALID, nullptr, 3);
	}

	// View World End
	if (g_vm->_gameWon[2]) {
		addButton(r, Common::KEYCODE_w, &_buttonSprites, 4);
	} else {
		addButton(Common::Rect(), Common::KEYCODE_INVALID, nullptr, 4);
	}

#ifdef USE_TTS
	uint index = 0;
	addNextTextToButtons(worldOtherOptionsButtons[_vm->_ttsLanguage], index, 5);
#endif
}
...
void OtherOptionsDialog::draw() {
	...
	w.writeString(Common::String::format(Res.OPTIONS_MENU,
		...
		w.getBounds().height() - 33, 1993), _voiceText);
	drawButtons(&w);
}
