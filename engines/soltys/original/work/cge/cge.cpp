...
static	char		UsrPath[13] = "\0É±%^þúÈ¼´ ÇÉ";
static	int		OldLev		= 0;
...
void SaveSound (void)
{
  CFILE cfg(ProgName(CFG_EXT), WRI);
...
static void QGame (void)
{
  ...
  OldLev = Lev;
  SaveSound();
  SaveGame(CFILE(UsrPath, WRI, RCrypt));
  Vga.Sunset();
  Finis = TRUE;
}
...
static void TakeName (void)
{
  if (GET_TEXT::Ptr) SNPOST_(SNKILL, -1, 0, GET_TEXT::Ptr);
    ...
    {
      GET_TEXT * tn = new GET_TEXT(Text[GETNAME_PROMPT], UsrPath, 8, KeyClick);
      if (tn)
	{
...
void LoadUser (void)
{
  ...
  if (STARTUP::Mode == 0) // user .SVG file found
    {
      LoadGame(CFILE(UsrPath, REA, RCrypt));
    }
  else
    {
...
Boolean ShowTitle (const char * name)
{
  ...
  if (STARTUP::Mode < 2)
    {
      #ifdef	DEMO
      strcpy(UsrPath, ProgName(SVG_EXT));
      usr_ok = TRUE;
      #else
      //-----------------------------------------
      #ifndef EVA
      ...
      HEART::Enable = FALSE;
      if (KEYBOARD::Last() == Enter && *UsrPath) usr_ok = TRUE;
      if (usr_ok) strcat(UsrPath, SVG_EXT);
      //Mouse.Off();
      VGA::ShowQ.Clear();
      VGA::CopyPage(0, 2);
      #endif
	...
	{
	  if (CFILE::Exist(UsrPath))
	    {
	      LoadGame(CFILE(UsrPath, REA, RCrypt), TRUE); // only system vars
	      VGA::SetColors(SysPal, 64);
	      Vga.Update();
	      if (FINIS)
		{
