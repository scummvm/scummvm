...
public:
	...
	int CD_count() const {
		return drive_IDs_.size();
	}
	bool is_CD_available(int cd_id) const {
		assert(cd_id >= 0 && cd_id < drive_IDs_.size());
		return drive_IDs_[cd_id] != NA_DRIVE_ID;
	}
	bool is_package_available(int idx) {
		assert(idx >= 0 && idx < packages_.size());
		return packages_[idx].is_open();
	}
