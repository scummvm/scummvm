...
public:
	...
	Mat3d(double angle, eAxis axis) {
		set(angle, axis);
	}

	Mat3d(const Vect3d &diag, const Vect3d &sym) {
		set(diag, sym);
	}
	...
	// for reading columns
	Vect3d xcol() const {
		return Vect3d(xx, yx, zx);
	}
	Vect3d ycol() const {
		return Vect3d(xy, yy, zy);
	}
	Vect3d zcol() const {
		return Vect3d(xz, yz, zz);
	}
	Vect3d col(int axis) const {
		return axis == X_AXIS ? Vect3d(xx, yx, zx) : (axis == Y_AXIS ? Vect3d(xy, yy, zy) : Vect3d(xz, yz, zz));
	}
	// for writing to columns
	xm_inline Mat3d &setXcol(const Vect3d &v);
	xm_inline Mat3d &setYcol(const Vect3d &v);
	xm_inline Mat3d &setZcol(const Vect3d &v);
	Mat3d &setCol(int axis, const Vect3d &v) {
		if (axis == X_AXIS) setXcol(v);
		else if (axis == Y_AXIS) setYcol(v);
		else setZcol(v);
		return *this;
	...
	// for reading a symmetric matrix
	Vect3d diag() const {
		return Vect3d(xx, yy, zz);
	}
	Vect3d sym()  const {
		return Vect3d(yz, zx, xy);
	}
	...
	friend Mat3d xpose(const Mat3d &M) {
		Mat3d N;
		return N.xpose(M);
	}
	...
	friend Mat3d invert(const Mat3d &M) {
		Mat3d N;
		N.invert(M);
		return N;
	}
	...
	//  Transforming operators  ///////////////
	friend Vect3d &operator*= (Vect3d &v, const Mat3d &M) {
		return M.xform(v);
	}
	friend Vect3d operator* (const Vect3d &v, const Mat3d &M) {
		Vect3d xv;
		return M.xform(v, xv);
	}
	friend Vect3d operator* (const Mat3d &M, const Vect3d &v) {
		Vect3d xv;
		return M.xform(v, xv);
	}
	...
	//  Transforming operators  ///////////////
	friend Vect3f &operator*= (Vect3f &v, const Mat3d &M) {
		return M.xform(v);
	}
	friend Vect3f operator* (const Vect3f &v, const Mat3d &M) {
		Vect3f xv;
		return M.xform(v, xv);
	}
	friend Vect3f operator* (const Mat3d &M, const Vect3f &v) {
		Vect3f xv;
		return M.xform(v, xv);
	}
...
public:
	...
	xm_inline MatXd &set(const Mat3d &R_, const Vect3d &d_) {
		R = R_;
		d = d_;
		return *this;
	}
	...
	xm_inline const Vect3d &trans() const {
		return d;
	}
	xm_inline Mat3d  &rot()        {
		return R;
	}
	xm_inline Vect3d &trans()      {
		return d;
	}
	...
	xm_inline MatXd &operator*= (const MatXd &M) {
		return postmult(M);
	}
	xm_inline MatXd operator* (const MatXd &N) const {
		MatXd M;
		return M.mult(*this, N);
	}
...
public:
	...
	xm_inline Vect4f() {}
	xm_inline Vect4f(float x_, float y_, float z_, float w_ = 1.0f) {
		x = x_;
		y = y_;
		z = z_;
		w = w_;
	}
	xm_inline explicit Vect4f(const Vect3f &v3, float w_) {
		x = v3.x;
		y = v3.y;
		z = v3.z;
		w = w_;
	...
	Vect4f &set(float x_, float y_, float z_, float w_ = 1.0f) {
		x = x_;
		y = y_;
		z = z_;
		w = w_;
	...
	xm_inline Vect4f &operator+= (const Vect4f &v) {
		return add(v);
	}
	xm_inline Vect4f &operator-= (const Vect4f &v) {
		return sub(v);
	}
	xm_inline Vect4f operator+ (const Vect4f &v) const {
		Vect4f u;
		return u.add(*this, v);
	}
	xm_inline Vect4f operator- (const Vect4f &v) const {
	...
	xm_inline Vect4f &operator*= (float s) {
		return scale(s);
	}
	xm_inline Vect4f &operator/= (float s) {
		return scale(1 / s);
	}
	xm_inline Vect4f operator* (float s) const {
		Vect4f u;
		return u.scale(*this, s);
	}
	xm_inline Vect4f operator/ (float s) const {
	...
	xm_inline friend Vect4f operator* (float s, const Vect4f &v) {
		Vect4f u;
		return u.scale(v, s);
	}
	...
	xm_inline friend void swap(Vect4f &u, Vect4f &v) {
		u.swap(v);
	}
