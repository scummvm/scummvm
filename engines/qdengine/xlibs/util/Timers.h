...
public:
	...
	void start(time_type delay = 0) {
		startTime_ = timer() + delay;    // в милисекундах
	}
	void stop() {
		startTime_ = 0;
	}
	void pause() {
		if (startTime_) startTime_ += timer().delta();    // вызывать каждый квант для оттягивания конца
	}

	// Время после startTime_, до startTime_ - 0
	time_type time() const {
		return startTime_ && timer() >= startTime_ ? timer() - startTime_ + 1 : 0;
	}

	// Время до startTime_ (уменьшается), после startTime_ 0
	time_type timeRest() const {
		return startTime_ > timer() ? startTime_ - timer() : 0;
	}
	...
	bool busy() const {
		return timer() <= startTime_;
	}
	bool finished() const {
		return startTime_ && timer() >= startTime_;
	}
...
protected:
	...
	static const SyncroTimer &timer() {
		return global_time;
	}
};
