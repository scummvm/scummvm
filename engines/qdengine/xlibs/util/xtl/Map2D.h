...
public:
	...
	// Map coords
	bool inside(const Vect2i &v) const {
	...
	T &operator()(const Vect2i &v) {
		return (*this)(v.x, v.y);
	}
	const T &operator()(const Vect2i &v) const {
		return (*this)(v.x, v.y);
	}

	// World coords
	bool lookable(int x, int y) const {
		return x >= 0 && y >= 0 && w2m(x) < sizeX() && w2m(y) < sizeY();
	}
	T &look(int x, int y) {
		return (*this)(w2m(x), w2m(y));
	}
	const T &look(int x, int y) const {
		return (*this)(w2m(x), w2m(y));
	}
	T &look(const Vect2i &v) {
		return look(v.x, v.y);
	}
	const T &look(const Vect2i &v) const {
		return look(v.x, v.y);
	}
	...
	static int w2m(int x) {
		return (x + tileSize / 2) >> tileSizeShl;
	}
	static float w2m(float x) {
		return x * (1.f / float(tileSize));
	}
	static int w2mFloor(int x) {
		return x >> tileSizeShl;
	}
	static int w2mCeil(int x) {
		return (x + tileSize - 1) >> tileSizeShl;
	}

	static Vect2i w2m(const Vect2i &v) {
		return Vect2i(w2m(v.x), w2m(v.y));
	}
	static Vect2f w2m(const Vect2f &v) {
		return Vect2i(w2m(v.x), w2m(v.y));
	}
	static Vect2i w2mFloor(const Vect2i &v) {
		return Vect2i(w2mFloor(v.x), w2mFloor(v.y));
	}
	static Vect2i w2mCeil(const Vect2i &v) {
		return Vect2i(w2mCeil(v.x), w2mCeil(v.y));
	}
	...
	static float m2w(float x) {
		return x * float(tileSize);
	}
	static Vect2i m2w(const Vect2i &v) {
		return Vect2i(m2w(v.x), m2w(v.y));
	}
	static Vect2f m2w(const Vect2f &v) {
		return Vect2i(m2w(v.x), m2w(v.y));
	}
