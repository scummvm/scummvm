...
public:
	...
	std::pair<const_iterator, const_iterator> equal_range(const key_type &key) const {
		...
		while (ve != MapVector.end() && ((!key_comp()(ve->first, key)) && (!key_comp()(key, ve->first)))) {
			ve++;
		}
	...
	size_type count(const key_type &key) const {
		...
		size_type sz = 0;
		while ((!key_comp()(vlb->first, key)) && (!key_comp()(key, vlb->first))) {
			sz++;
			vlb++;
		}
		return sz;
	...
	iterator erase(iterator where)  {
		return MapVector.erase(where);
	}
	iterator erase(iterator first, iterator last) {
		return MapVector.erase(first, last);
	}
	...
	allocator_type get_allocator() const {
		return MapVector.get_allocator();
	}
	...
	size_type max_size() const {
		return MapVector.max_size();
	}

	void swap(StaticMap &right) {
		MapVector.swap(right.MapVector);
	}
