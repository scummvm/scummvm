...
struct Rect {
	...
	VectType right_top() const {
		return VectType(left_ + width_, top_);
	}
	...
	*/
	...
	// сеттеры:
	void       left(ScalarType _left) {
		left_ = _left;
	}
	void       top(ScalarType _top) {
		top_ = _top;
	}
	void       width(ScalarType _width) {
		width_ = _width;
	}
	void       height(ScalarType _height) {
		height_ = _height;
	}

	// сеттеры (вычисляющие):
	void       right(ScalarType _right) {
		left_ = _right - width_;
	}
	void       bottom(ScalarType _bottom) {
	...
	*/
	...
	void       size(const VectType &_size) {
		width_ = _size.x;
		height_ = _size.y;
	}
