...
void
XmlRpcClient::setAuthorization(const char* login, const char* pass)
{
	_authorization.clear();
	if(login == 0 || *login == 0)
		return;

	_authorization = "Proxy-Authorization: Basic ";

	std::string authdata = login;
	authdata.push_back(':');
	if(pass && *pass)
		authdata += pass;

	int iostatus = 0;
	base64<char>().put(authdata.begin(), authdata.end(), std::back_inserter(_authorization), iostatus, base64<>::noline());

	XmlRpcUtil::log(3, "set proxy authorization: %s", _authorization.c_str());
}
...
{
	XmlRpcUtil::log(2, "XmlRpcClient::execute: method %s (_connectionState %d).", method, _connectionState);
...
XmlRpcClient::doConnect()
{
	...
	XmlRpcUtil::log(4, "XmlRpcClient::doConnect: fd %d done.", fd);
	return true;
}
...
XmlRpcClient::readHeader()
{
	...
	if ( ! XmlRpcSocket::nbRead(this->getfd(), _header, &_eof) ||
		...
		XmlRpcUtil::log(5, "header: %s", _header.c_str());
