...
public:
	...
	void setVersion(int version) {
		version_ = version;    // Для сложной конверсии: вручную записывать, выставлять и кастить архив к XPrmIArchive
	}
	int version() const {
		return version_;
	}
...
private:
	...
	void closeBracket();
	bool isContainer() {
		return parserStack_.empty() ? false : parserStack_.back().isContainer;
	}
};
