...
public:
	...
	static void         editorAddGroup(const char *) {
		xassert(0);
	}
	static bool         editorAllowDrag() {
		return true;
	}
	...
	virtual bool        editorVisible() const {
		return true;
	}
	static bool         editorDynamicGroups() {
		return false;
	}
...
public:
	...
	template<class T>
	Serializer editorSerializer(T *self, const char *name, const char *nameAlt, bool protectedName) {
		return Serializer(name_, name, nameAlt);
	}
};
...
public:
	...
	string         editorGroupName() const {
		return type_ ? FactorySelector<T>::Factory::instance().nameAlt(typeid(*type_).name()) : "";
	}
	void                editorSetGroup(const char *group);
	static const char  *editorGroupsComboList() {
		return FactorySelector<T>::Factory::instance().comboListAlt();
	}
	static bool         editorAllowDrag() {
		return false;
	}
