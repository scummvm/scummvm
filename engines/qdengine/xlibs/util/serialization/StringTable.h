...
public:
	...
	const char *comboList(bool canAlwaysDereference) const {
		return canAlwaysDereference ? comboListAlwaysDereference_.c_str() : comboList_.c_str();
	}
	...
	const char         *editorComboList() const {
		return comboList_.c_str();
	}
	const char         *editorGroupsComboList() const {
		return String::editorGroupsComboList();
	}
	string         editorGroupName(int index) const;
	void                editorGroupMoveBefore(int index, int beforeIndex);
	bool                editorDynamicGroups() const {
		return String::editorDynamicGroups();
	}
	bool                editorAllowRename() const {
		return String::editorAllowRename();
	}
	bool                editorAllowDrag() const {
		return String::editorAllowDrag();
	}
	size_t         editorSize() const {
		return size();
	}
