...
public:
	...
	bool visibleInEditor() const {
		return flags_ & LIBRARY_EDITABLE;
	}
	bool inPlaceEnabled() const {
		return flags_ & LIBRARY_IN_PLACE;
	}
	bool inPlaceCreated() const {
		return inPlaceCreated_;
	}
...
public:
	...
	EditorIterator editorBegin() {
		return EditorIterator(this, 0);
	}
	EditorIterator editorEnd() {
		return EditorIterator(this, (int)editorSize());
	}

	// editor methods
	virtual bool            editorDynamicGroups() const {
		return false;
	}
	virtual bool            editorAllowDrag() const {
		return false;
	}
	virtual bool            editorAllowRename() const {
		return true;
	}
	virtual int             editorFindElement(const char *elementName) const;

	virtual std::string     editorAddElement(const char *name, const char *group = "") {
		return "";
	};
	virtual void            editorAddGroup(const char *name) {
		xassert(0);
	};
	...
	virtual const char     *editorGroupsComboList() const {
		return 0;
	}
	virtual std::string     editorGroupName(int index) const {
		return "";
	}
	virtual int             editorGroupIndex(const char *name) {
		return -1;
	}
	void                    editorGroupMoveBefore(int index, int beforeIndex) {}
	virtual const char     *editorComboList() const {
		return "";
	}
	virtual std::size_t     editorSize() const {
		return 0;
	}

	virtual bool            editorElementVisible(int index) const {
		return true;
	}
	virtual const char     *editorElementName(int index) const {
		xassert(0);
		return "";
	}
	virtual void            editorElementSetName(int index, const char *newName) {
		xassert(0);
	}
	void                    editorElementSetName(const char *oldName, const char *newName);

	virtual Serializer   editorElementSerializer(int index, const char *name, const char *nameAlt, bool protectedName) {
		return Serializer();
	};
	Serializer           editorElementSerializer(const char *elementName, const char *name, const char *nameAlt, bool protectedName);

	virtual std::string     editorElementGroup(int index) const {
		return "";
	}
