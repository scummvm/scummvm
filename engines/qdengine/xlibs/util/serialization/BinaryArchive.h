...
public:
	...
	char *data() const {
		return saver_.buffer();
	}
	size_t size() const {
		return size_t(saver_.size());
	}
...
private:
	...
	public:
		...
		void write(const wstring &str) {
			write((const char *)str.c_str(), (int)(str.size() + 1) * sizeof(wchar_t));
		}
...
public:
	...
	void setIgnoreUnregisteredClasses(bool ignore) {
		ignoreUnregisteredClasses_ = ignore;
	}
	void setVersion(int version) {
		version_ = version;    // ƒл€ сложной конверсии: вручную записывать, выставл€ть и кастить архив к XPrmIArchive
	}
	int version() const {
		return version_;
	}
...
private:
	...
	public:
		...
		bool validToClose() const {
			return complex_ || curr_ == end_;    // ѕростые блоки должны быть вычитаны точно
		}
