...
private:
	...
	public:
		...
		void write(const string &str) {
			write(str.c_str(), (int)str.size() + 1);
		}
	...
	public:
		...
		bool in(const char *address, int size) const {
			return address_ <= address && address_ + size_ >= address + size;
		}
		// [address, size] лежит полностью вне this
		bool out(const char *address, int size) const {
			return address_ + size_ <= address || address_ >= address + size;
		}
	...
	void checkIn(const char *object, int size) {
		xassert("ѕопытка записать в InPlaceOArchive елемент, не принадлежащий структуре" && stack_.back().in(object, size));
	}

	void writeString(const string &str);
	template<class T> void writeValue(const T &value) {
		checkIn((char *)&value, sizeof(value));
	}
