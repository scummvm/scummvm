...
struct GridRectangle {
	...
	int operator == (const GridRectangle &r) const {
		return x0 == r.x0 && y0 == r.y0 && x1 == r.x1 && y1 == r.y1;
	}
	friend XStream &operator<= (XStream &s, const GridRectangle &r) {
		s <= r.x0 < " " <= r.y0 < " " <= r.x1 < " " <= r.y1 < " ";
		return s;
	}
	GridRectangle operator + (const GridRectangle &r) const {
		return GridRectangle(min(x0, r.x0), min(y0, r.y0), max(x1, r.x1), max(y1, r.y1));
	}
...
class GridElementType {
	...
	int &passCounter() const {
		return MT_IS_LOGIC() ? passCounterLogic_ : passCounterGraphics_;
	}
...
public:
	...
	int belongSquare(int x0, int y0, int sx, int sy) const {
		return 1;    // pseudo-virtual due to template using
	}
	const GridRectangle &getRectangle() const {
		return rectangle;
	}
	int inserted() const {
		return insert_counter;
	}
	void incrInsertion() const {
		++insert_counter;
	}
	void decrInsertion() const {
		--insert_counter;
	}
};
...
public:
	GridPassDispatcher() {
		passCounterLogic_ = passCounterGraphics_ = 0;
		scanned_ = false;
	}
	void scanNext() const {
		passCounter()++;    // Сбрасывает scanAgain
	}
	void scanAgain() const {
		passCounter()--;    // Делает следующий скан атоммарным с предыдущим, т.е не будут вызываться уже вызванные объекты
	}
	...
	void setScanned(bool scanned) const {
		if (MT_IS_LOGIC()) scanned_ = scanned;
	}

	static void setRectangle(const GridElementType &el, const GridRectangle &rect) {
		el.rectangle = rect;
	}
...
protected:
	class ScanLock {
	public:
		ScanLock(const GridPassDispatcher &grid) : grid_(grid) {
			grid_.setScanned(true);
		}
		~ScanLock() {
			grid_.setScanned(false);
		}
	private:
...
public:
	GridVector() {
		if (reserve_size) reserve(reserve_size);
	}
	void insert(T *obj) {
		push_back(obj);
		obj->incrInsertion();
...
public:
	...
	////////////////////////////////////////////////////////////////////////////////////
	template <class Op>
	void Scan(const Vect2i &position, int side, Op &op) const {
		Scan(position.x - side, position.y - side, position.x + side, position.y + side, op);
	}

	template <class Op>
	void Scan(int xc, int yc, int side, Op &op) const {
		Scan(xc - side, yc - side, xc + side, yc + side, op);
	}
	...
	template <class Op>
	bool ConditionScan(int xc, int yc, int side, Op &op) const {
		return ConditionScan(xc - side, yc - side, xc + side, yc + side, op);
	}
...
//	int m_mask_x, m_mask_y;
	...
	int mask_x(int x) const {
		return x;
	}
	int mask_y(int y) const {
		return y;
	}
	int clamp_x(int x) const {
		return x > 0 ? (x < size_x ? x : size_x - 1) : 0;
	}
	int clamp_y(int y) const {
		return y > 0 ? (y < size_y ? y : size_y - 1) : 0;
	}
	int insideMap(int x, int y) const {
...
//	int insideMap(int x, int y) const {  return 1; }
	...
	CellList &table(int x, int y) const {
		xassert(x >= 0 && x < size_x && y >= 0 && y < size_y);
		return cell_table[mask_y(y)][mask_x(x)];
	}
