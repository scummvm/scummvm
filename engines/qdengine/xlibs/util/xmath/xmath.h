...
public:
	...
	xm_inline int operator()(int m) {
		return m ? (*this)() % m : 0;    // May by used in random_shuffle
	}
	xm_inline int operator()(int min, int max) {
		return min + (*this)(max - min);
	}
	xm_inline float frnd(float x = 1.f) {
		return (float)((*this)() * 2 - max_value) * x / (float)max_value;
	}
	xm_inline float fabsRnd(float x = 1.f) {
		return (float)((*this)()) * x / (float)max_value;
	}
	xm_inline float fabsRnd(float min, float max) {
		return min + fabsRnd(max - min);
	}
	xm_inline float frand() {
...
xm_inline unsigned random(unsigned m) {
	return xm_random_generator(m);
}
xm_inline float frnd(float x) {
	return xm_random_generator.frnd(x);
}
xm_inline float fabsRnd(float x) {
	return xm_random_generator.fabsRnd(x);
}
