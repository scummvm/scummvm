...
struct sBox6f {
	...
	void invalidate() {
		min.set(FLT_INF, FLT_INF, FLT_INF);
		max.set(-FLT_INF, -FLT_INF, -FLT_INF);
	}
	...
	sBox6f &operator += (const Vect3f &v) {
		min += v;
		max += v;
		return *this;
	}
	...
	float radius2D() const {
		return Vect2f(max).distance(Vect2f(min)) * .5f;
	}
	Vect3f center() const {
		return (min + max) * .5f;
	}
