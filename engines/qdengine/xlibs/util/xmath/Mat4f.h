...
public:
	...
	void xformCoord(const Vect3f &in, Vect3f &out) const {
		Vect3f pv;
		xformCoord(in, pv, out);
	}
	void xformCoord(Vect3f &v) const {
		Vect3f in = v;
		Vect3f pv;
		xformCoord(in, pv, v);
	}
	...
	Vect4f  &row(int n)                     {
		return ((Vect4f *)this)[n];
	}

	Vect4f col(int axis) const {
		return Vect4f(*(&_11 + axis), *(&_21 + axis), *(&_31 + axis), *(&_41 + axis));
	}
	...
	const Vect4f &operator[](int i) const {
		return *(((Vect4f *)this) + i);
	}
	Vect4f &operator[](int i)       {
		return *(((Vect4f *)this) + i);
	}
