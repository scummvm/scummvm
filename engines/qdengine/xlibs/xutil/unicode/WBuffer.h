...
public:
	...
	int setRadix(int r) {
		int old = radix_;
		radix_ = (r <= 0 || r > 16 ? DEFRADIX : r);
		return old;
	}
	int setDigits(int d) {
		int old = digits_;
		digits_ = (d <= 0 || d > CB_LEN ? DEFDIGITS : d);
		return old;
	}
	...
	template<class T> WBuffer &operator< (T v) {
		return write(v);
	}
	template<class T> WBuffer &operator> (T &v) {
		return read(v);
	...
	WBuffer &operator+= (difftype v) {
		offset_ += v * sizeof(wchar_t);
		return *this;
	}
	WBuffer &operator-= (difftype v) {
		offset_ -= v * sizeof(wchar_t);
	...
	const BYTE &operator[](sizetype ind) const {
		return buffer_[ind];
	}
	BYTE &operator[](sizetype ind) {
		return buffer_[ind];
	}

	const wchar_t &operator()() const {
		return reinterpret_cast<const wchar_t &>(buffer_[offset_]);
	}
	wchar_t &operator()() {
		return reinterpret_cast<wchar_t &>(buffer_[offset_]);
	}

	template<class T> WBuffer &write(const T &v) {
		while (offset_ + sizeof(T) > size_) handleOutOfSize();
		memcpy(&buffer_[offset_], &v, sizeof(T));
		offset_ += sizeof(T);
		return *this;
	}
	template<class T> WBuffer &read(T &v) {
