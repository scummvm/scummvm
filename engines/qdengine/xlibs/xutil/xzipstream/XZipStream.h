...
struct XZipStream {
	...
	int openCurrentFile();
	bool isCurrentFileOpen() const {
		return curFileOpened_;
	}
	...
	int operator!() {
		if (archive_.getHandler() != INVALID_HANDLE_VALUE) return 1;    // Obsolete
		else return 0;
	}
	operator void *() {
		if (archive_.getHandler() != INVALID_HANDLE_VALUE) return 0;    // Obsolete
		else return this;
	}

	template<class T> XZipStream &read(T &v) {
		read(&v, sizeof(T));
		return *this;
	}
};
