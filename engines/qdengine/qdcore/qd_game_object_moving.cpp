...
bool qdGameObjectMoving::find_path(const Vect3f target, bool lock_target) {
	...
	if (g_engine->_gameVersion > 20040519 && path_vect.size() >= 2 && (movement_type() == qdGameObjectStateWalk::MOVEMENT_FOUR_DIRS || movement_type() == qdGameObjectStateWalk::MOVEMENT_EIGHT_DIRS)) {
		...
		if (g_engine->_gameVersion <= 20040519)
			_path[idx] = trg;
		else
			_path[idx - 1] = trg;
	}

	_cur_path_index = (g_engine->_gameVersion <= 20040519 || idx > 1) ? 1 : 0;
	_path_length = idx;
