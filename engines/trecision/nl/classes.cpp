...
#define POSINV 2
	...
	case ME_MLEFT:
		...
		//	Game area
		if (GAMEAREA(g_vm->_curMessage->_u16Param2)	&& !g_vm->_animMgr->_playingAnims[kSmackerAction]) {
...
void doIdle() {
	...
	case 0x1B:
		...
		&& (g_vm->_actor->_curAction < hWALKIN) && !FlagUseWithStarted && FlagShowCharacter && !g_vm->_animMgr->_playingAnims[kSmackerAction]
			) {
			actorStop();
			nextStep();
			Mouse(MCMD_ON);
