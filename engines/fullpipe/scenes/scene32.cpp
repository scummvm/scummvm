...
void sceneHandler32_spin(ExCommand *cmd) {
	...
	if ((g_vars->scene32_cactus->_movement && g_vars->scene32_cactus->_movement->_id == MV_CTS_DEFAULT) || g_vars->scene32_cactus->_statics->_staticsId == ST_CTS_GROWUP) {
		for (int i = 0; i < 12; i++) {
...
void sceneHandler32_trySit(ExCommand *cmd) {
	...
	if (g_vars->scene32_cactusIsGrowing || g_vars->scene32_cactus->_movement || g_vars->scene32_cactus->_statics->_staticsId != ST_CTS_EMPTY || (g_vars->scene32_cactusCounter >= 0 && g_vars->scene32_cactusCounter <= 20)) {
		ex->_messageKind = 0;
		ex->_excFlags |= 1;
	} else {
