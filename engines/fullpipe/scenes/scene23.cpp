...
void scene23_initScene(Scene *sc) {
	...
	if (g_fp->getObjectState(sO_LowerHatch_23) == g_fp->getObjectEnumState(sO_LowerHatch_23, sO_Opened)) {
		...
		if (g_fp->getObjectState(sO_LowerPipe_21) == g_fp->getObjectEnumState(sO_LowerPipe_21, sO_IsOpened)) {
			...
			if (g_fp->getObjectState(sO_UpperHatch_23) == g_fp->getObjectEnumState(sO_UpperHatch_23, sO_Opened) && (g_vars->scene23_giraffee->_flags & 4))
...
int scene23_updateCursor() {
	...
	if (g_fp->_objectIdAtCursor == PIC_SC23_BTN1 || g_fp->_objectIdAtCursor == PIC_SC23_BTN2 || g_fp->_objectIdAtCursor == PIC_SC23_BTN3 || g_fp->_objectIdAtCursor == PIC_SC23_BTN4 || g_fp->_objectIdAtCursor == ANI_CALENDWHEEL)
	...
	if (g_fp->_aniMan->isIdle() || !(g_fp->_aniMan->_flags & 0x100)) {
		if (!g_vars->scene23_topReached) {
			if (g_fp->_aniMan->_ox != 405 || g_fp->_aniMan->_oy != 220) {
				if (g_fp->_aniMan->_ox != 276 || g_fp->_aniMan->_oy != 438 || g_fp->_aniMan->_movement || g_fp->_aniMan->_statics->_staticsId != ST_MAN_RIGHT) {
...
void sceneHandler23_checkReachingTop() {
	if (g_fp->_aniMan->_movement || g_fp->_aniMan->_statics->_staticsId != ST_MAN_STANDLADDER || g_fp->_aniMan->_ox != 405 || g_fp->_aniMan->_oy != 220)
		g_vars->scene23_topReached = false;
	else
		g_vars->scene23_topReached = true;
}
...
void sceneHandler23_exitCalendar() {
	if (!g_fp->_aniMan->_movement && g_fp->_aniMan->_statics->_staticsId == ST_MAN_STANDLADDER && !g_fp->_aniMan->getMessageQueue() && !(g_fp->_aniMan->_flags & 0x100)) {
