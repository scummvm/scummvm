...
void sceneHandler27_clickBat(ExCommand *cmd) {
	...
	if (ABS(bx - g_fp->_aniMan->_ox) > 1 || ABS(by - g_fp->_aniMan->_oy) > 1 || g_fp->_aniMan->_movement || g_fp->_aniMan->_statics->_staticsId != ST_MAN_RIGHT) {
...
void sceneHandler27_knockBats(int bat1n, int bat2n) {
	...
	if (bat1->power != 0.0) {
		double rndF = (double)g_fp->_rnd.getRandomNumber(32767) * 0.03 / 32767.0 - 0.015 + atan2(bat2->currY - bat1->currY, bat2->currX - bat1->currX);
		...
		double rndF2 = (double)g_fp->_rnd.getRandomNumber(32767) * 0.03 / 32767.0 - 0.015 + atan2(bat1->currY - bat2->currY, bat1->currX - bat2->currX);
