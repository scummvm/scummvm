...
void scene11_initScene(Scene *sc) {
	...
	if (swingie == g_fp->getObjectEnumState(sO_Swingie, sO_IsSwinging) || swingie == g_fp->getObjectEnumState(sO_Swingie, sO_IsSwingingWithBoot)) {
	...
	} else if (swingie == g_fp->getObjectEnumState(sO_Swingie, sO_IsStandingInBoots) || swingie == g_fp->getObjectEnumState(sO_Swingie, sO_IsStandingInCorner)) {
...
void sceneHandler11_putBoot() {
	if (abs(353 - g_fp->_aniMan->_ox) > 1 || abs(498 - g_fp->_aniMan->_oy) > 1 || g_fp->_aniMan->_movement || g_fp->_aniMan->_statics->_staticsId != ST_MAN_RIGHT) {
...
int sceneHandler11(ExCommand *cmd) {
	...
	case MSG_SC11_SITSWINGER:
		if (g_fp->getObjectState(sO_Swingie) == g_fp->getObjectEnumState(sO_Swingie, sO_IsStandingInBoots) || g_fp->getObjectState(sO_Swingie) == g_fp->getObjectEnumState(sO_Swingie, sO_IsStandingInCorner)) {
			g_fp->setObjectState(sO_Swingie, g_fp->getObjectEnumState(sO_Swingie, sO_IsSitting));
		}
		break;
	...
	case 33: {
		...
		if (!g_vars->scene11_arcadeIsOn && !g_vars->scene11_swingIsSwinging) {
			if (g_vars->scene11_swingSpeed == 0.0 && g_vars->scene11_dudeOnSwing->_movement && g_vars->scene11_dudeOnSwing->_movement->_currDynamicPhaseIndex == 45) {
				g_vars->scene11_dudeOnSwing->changeStatics2(ST_KCH_STATIC);
			}
		}
	...
	case 29:
		if (g_vars->scene11_swingIsSwinging) {
			if (g_fp->_currentScene->getStaticANIObjectAtPos(g_fp->_sceneRect.left + cmd->_x, g_fp->_sceneRect.top + cmd->_y) == g_vars->scene11_swingie && cmd->_param == ANI_INV_BOOT)
				sceneHandler11_putBoot();
		} else {
			if (!g_vars->scene11_arcadeIsOn)
				goto LABEL_69;
