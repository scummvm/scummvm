...
void scene25_initScene(Scene *sc, int entranceId) {
	...
	if (entranceId == TrubaRight) {
		if (boardState == g_fp->getObjectEnumState(sO_Board_25, sO_FarAway)) {
			...
			if (boardState == g_fp->getObjectEnumState(sO_Board_25, sO_Nearby) || boardState == g_fp->getObjectEnumState(sO_Board_25, sO_WithDudeOnRight))
...
void sceneHandler25_putBoard() {
	if (g_fp->_aniMan->_statics->_staticsId == ST_MAN_STANDLADDER || g_fp->_aniMan->_statics->_staticsId == ST_MAN_LADDERDOWN_R) {
...
bool sceneHandler25_isOnLadder(ExCommand *cmd) {
	if ((g_fp->_aniMan->_movement && g_fp->_aniMan->_movement->_id == MV_MAN_GOLADDERDOWN) || g_fp->_aniMan->_statics->_staticsId == ST_MAN_GOLADDERD) {
