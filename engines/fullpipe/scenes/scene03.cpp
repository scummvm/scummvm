...
void sceneHandler03_pushEggStack() {
	...
	if (g_vars->swallowedEgg2->_value.intValue == ANI_INV_EGGBOOT && g_vars->swallowedEgg1->_value.intValue == ANI_INV_EGGAPL) {
...
void sceneHandler03_takeEgg(ExCommand *ex) {
	...
	if (mq && mq->getCount() > 0) {
		...
		if (g_vars->swallowedEgg1->_value.intValue == ANI_INV_EGGAPL && !g_vars->swallowedEgg2->_value.intValue && !g_vars->swallowedEgg3->_value.intValue && ex1) {
