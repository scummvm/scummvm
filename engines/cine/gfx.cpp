...
static const char *copyProtectionTextsFWDOS[] = {
	...
	"Du mu\341t nur die richtige Farbe des blinkenden Ausscnitts auf dem Bildschirm gem\204\341 der "
	"R\201ckseite der Anleitung angeben.\n"
	"Du hast zwei Versuche.",
	// Spanish
	"Protecci\242n\n"
	"Usando la ilustracion a color de la contreportade del manuel de Future Wars, encuentra el "
...
static const char *copyProtectionTextsFWAmigaAtari[] = {
	...
	"Situa la rejilla de plastico que hay en el paquete sobre la pagina cuyo numero se indica.\n"
	"Entonces identifica que ilustraci\241on es la que te solicita el programa "
...
static const char *copyProtectionTextsOS[] = {
	// English
	"Operation Stealth\n"
	"The following information is classified:\n"
	"Top Secret\n"
	"To gain access to this file match the proper color codes from your color code card.\n"
	"Refer to your system documentation for complete information on security procedures.",
	// French
	"Operation Stealth\n"
	...
	// German
	"Operation Stealth\n"
	"Die informationen dieses dossiers sino\n"
	"Streng Geheim\n"
...
static const char *openingCreditsFWEnglish[] = {
	...
	"Music composed by Jaun Baudlot",
...
static const char *openingCreditsFWSpanish[] = {
	"Dise\224o y programa Paul Cuisset",
	"Visual dise\224o Eric Chahi",
...
static const char *openingCreditsFWItalian[] = {
	"Disegno e programma Paul Cuisset",
	"Disegno visivo Eric Chahi",
	"Musica Jean Baudlot",
	"Future Wars: Adventures in Time\nCopyright 1990 Delphine Software",    // DOS only
	"Future Wars: Time Travellers",                                         // Amiga/Atari only
	"Versione IBM PC Daniel Morais"                                         // DOS only
};
...
static const char *endCreditsFW[] = {
	...
	// Spanish, first line
	"Dise\224o: Paul Cuisset",
	// Spanish, second line DOS
	"Versi\224n de IBM PC: Daniel Morais",
...
static const char *openingCreditsOSEnglish[] = {
	"Delphine Software\nCinematique\nCopyright MCMXC Delphine Software\n",
	"IBM Version by D. Morais",    // DOS only
	"Script\nP. Cuisset\nP. Chastel",
	"Illustrations\nImagex\nM. Bacque\nE. Lecoz",
	"Programming\nP. Cuisset\nP. Chastel\nJ. Martinez",
	"Music\nJ. Baudlot \nEffects\nM. Minier",
...
static const char *openingCreditsOSGerman[] = {
	"Delphine Software\nCinematique\nCopyright MCMXC Delphine Software\n",
	"IBM-Version von D. Morais",    // DOS only
	"Skript\nP. Cuisset\nP. Chastel",
	"Illustrationen\nImagex\nM. Bacque\nE. Lecoz",
	"Programm\nP. Cuisset\nP. Chastel\nJ. Martinez",
	"Musik\nJ. Baudlot \nSoundeffekte\nM. Minier",
...
static const char *openingCreditsOSSpanish[] = {
	"Delphine Software\nCinematique\nCopyright MCMXC Delphine Software\n",
	"Versi\242n de IBM por D. Morais",    // DOS only
	"Guion\nP. Cuisset\nP. Chastel",
	"Ilustraciones\nImagex\nM. Bacque\nE. Lecoz",
	"Programa\nP. Cuisset\nP. Chastel\nJ. Martinez",
	"M\242sica\nJ. Baudlot \nEfectos\nM. Minier",
...
static const char *openingCreditsOSItalian[] = {
	"Delphine Software\nCinematique\nCopyright MCMXC Delphine Software\n",
	"Versione IBM di D. Morais",    // DOS only
	"Copione\nP. Cuisset\nP. Chastel",
	"Illustrazioni\nImagex\nM. Bacque\nE. Lecoz",
	"Programma\nP. Cuisset\nP. Chastel\nJ. Martinez",
	"Musica\nJ. Baudlot \nEffetti\nM. Minier",
...
static const char *endCreditsOS[] = {
	...
	"El equipo de Delphine espera que lo hayan pasado genial con John Glames y les agradecemos su apoyo.\n"
	"Nos vemos pronto en nuestra pr\242xima aventura.",
	...
	"Il team di Delphine spera che vi siate divertiti con John Glames e vi ringrazia per il vostro supporto.\n"
	"A presto per la nostra prossima avventura."
};
...
void FWRenderer::loadBg16(const byte *bg, const char *name, unsigned int idx) {
	...
	} else if (g_cine->getPlatform() == Common::kPlatformDOS && scumm_stricmp(name, "FW.PI1") == 0) {	// Second copy protection screen
...
void OSRenderer::incrustSprite(const BGIncrust &incrust) {
	...
	if (_bgTable[incrust.bgIdx].bg) {
		...
		if ((incrust.objIdx == 100 || incrust.frame == 10) && scumm_stricmp(currentPrcName, "INTRO3.PRC") == 0
			...
			if (scumm_stricmp(_bgTable[incrust.bgIdx].name, "SALLE02.PI1") == 0) {
				...
				index = kMusic;
			} else if (scumm_stricmp(_bgTable[incrust.bgIdx].name, "CIEL03.PI1") == 0) {
				index = kTitleScreen;
			}
...
void OSRenderer::loadBg16(const byte *bg, const char *name, unsigned int idx) {
	...
	if (scumm_stricmp(name, "INTRO1.PI1") == 0) {		// Intro screen
		const char **openingCredits;
		switch (g_cine->_ttsLanguage) {
		case kEnglish:
			openingCredits = openingCreditsOSEnglish;
		...
		g_cine->sayText(openingCredits[kOpeningScreen], Common::TextToSpeechManager::QUEUE);

		if (g_cine->getPlatform() == Common::kPlatformDOS) {
			g_cine->sayText(openingCredits[kIBM], Common::TextToSpeechManager::QUEUE);
		}
	} else if (scumm_stricmp(name, "GEN1.PI1") == 0) {	// Closing credits
		...
		g_cine->sayText(copyProtectionTextsOS[g_cine->_ttsLanguage], Common::TextToSpeechManager::QUEUE);
		g_cine->_copyProtectionTextScreen = true;
		g_cine->_copyProtectionColorScreen = false;
