...
bool Obstacles::linePolygonIntersection(LineSegment lineA, VertexType lineAType, Polygon *polyB, Vector2 *intersectionPoint, int *intersectionIndex, int pathLengthSinceLastIntersection) {
	...
	for (int i = 0; i != polyB->verticeCount; ++i) {
		...
		if (lineLineIntersection(lineA, lineB, &newIntersectionPoint)) {
			if ((lineAType == TOP_RIGHT && lineBType == TOP_LEFT) || (lineAType == BOTTOM_RIGHT && lineBType == TOP_RIGHT) || (lineAType == BOTTOM_LEFT && lineBType == BOTTOM_RIGHT) || (lineAType == TOP_LEFT && lineBType == BOTTOM_LEFT)) {
				if ((pathLengthSinceLastIntersection > 2) || ((!(WITHIN_TOLERANCE(lineB.end.x, intersectionPoint->x) && WITHIN_TOLERANCE(lineB.end.y, intersectionPoint->y))) && (newIntersectionPoint != *intersectionPoint))) {
		...
		for (int currentPolygonIdx = 0; currentPolygonIdx < kPolygonCount && pathVertexAvailable; ++currentPolygonIdx) {
			...
			for (int polygonVertexIdx = 0; polygonVertexIdx < polygon->verticeCount && pathVertexAvailable; ++polygonVertexIdx) {
				...
				if (findPolygonVerticeByXZWithinTolerance(intersection.x, intersection.y, &polygonIndexIntersection, &vertexIndexIntersection, currentPolygonIdx)) {
					...
					if ((currentPolygonIdx == polygonIndexPath && vertexIndexIntersection == vertexIndexPath) || (currentPolygonIdx == polygonIndexStart && vertexIndexIntersection == vertexIndexStart)) {
						continue;
					}
					...
					if (WITHIN_TOLERANCE(intersection.x, start.x) && WITHIN_TOLERANCE(intersection.y, start.z)) {
					...
					if (currentPolygonIdx == polygonIndexStart || startIntersectionWithinTolerance) {
						...
						if ((polygonVertexType == TOP_LEFT && intersection.y < path[pathVertexIdx].y) || (polygonVertexType == TOP_RIGHT && intersection.x > path[pathVertexIdx].x) || (polygonVertexType == BOTTOM_RIGHT && intersection.y > path[pathVertexIdx].y) || (polygonVertexType == BOTTOM_LEFT && intersection.x < path[pathVertexIdx].x)) {
							pathVertexAvailable = false;
							break;
						}
					}
