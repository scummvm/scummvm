...
bool Debugger::cmdScene(int argc, const char **argv) {
	...
	uint i;
	for (i = 0; sceneList[i].chapter != 0; ++i) {
		if (sceneList[i].chapter == chapterIdNormalized && sceneList[i].set == _vm->_scene->getSetId() && sceneList[i].scene == _vm->_scene->getSceneId())
			break;
	}
...
bool Debugger::cmdOverlay(int argc, const char **argv) {
	...
	if (_vm->_kia->isOpen() || _vm->_esper->isOpen() || _vm->_spinner->isOpen() || _vm->_elevator->isOpen() || _vm->_vk->isOpen() || _vm->_scores->isOpen()) {
		...
		if (argc == 2) {
			...
			if (argName == "reset") {
				...
				for (itemIter = 0; overlaysList[itemIter].resourceId != 0; ++itemIter) {
					if ((overlaysList[itemIter].resourceId == chapterIdOverlaysAvailableInt) || (modeMixOverlaysAvailableFlg && overlaysList[itemIter].resourceId == 6)) {
...
bool Debugger::cmdObject(int argc, const char **argv) {
	...
	if (argc < 3) {
		...
		} else if ((modeName == "remove" && argc == 3) || (modeName == "flags" && argc == 6) || (modeName == "bounds" && argc == 9) || (modeName == "list" && argc == 3)) {
...
bool Debugger::cmdItem(int argc, const char **argv) {
	...
	if (argc < 3) {
		...
		} else if ((modeName == "remove" && argc == 3) || (modeName == "flags" && argc == 5) || (modeName == "bounds" && argc == 9) || (modeName == "list" && argc == 3)) {
...
bool Debugger::cmdRegion(int argc, const char **argv) {
	...
	if (argc < 4) {
		...
		} else if ((modeName == "remove" && argc == 4) || (modeName == "list" && argc == 4) || (modeName == "bounds" && argc == 8)) {
			if (regions->_regions[regionID].present) {
...
void Debugger::drawDebuggerOverlay() {
	...
	if (_viewActorsToggle || _specificActorsDrawn || _view3dObjectsToggle || _specific3dObjectsDrawn || _viewItemsToggle || _specificItemsDrawn) {
		drawSceneObjects();
	}
	...
	if (_viewFogs || _specificFogsDrawn)
		drawFogs();
	if (_viewRegionsNormalToggle || _specificRegionNormalDrawn || _viewRegionsExitsToggle || _specificRegionExitsDrawn) {
		drawRegions();
	}
	if (_viewWaypointsNormalToggle || _specificWaypointNormalDrawn || _viewWaypointsFleeToggle || _specificWaypointFleeDrawn || _viewWaypointsCoverToggle || _specificWaypointCoverDrawn) {
		drawWaypoints();
	}
	...
	if (count > 0) {
		for (int i = 0; i < count; ++i) {
			...
			case kSceneObjectTypeActor:
				if (_viewActorsToggle || (_specificActorsDrawn && findInDbgDrawList(debuggerObjTypeActor, sceneObject->id - kSceneObjectOffsetActors, -1, -1) != -1)) {
			...
			case kSceneObjectTypeItem:
				if (_viewItemsToggle || (_specificItemsDrawn && findInDbgDrawList(debuggerObjTypeItem, sceneObject->id - kSceneObjectOffsetItems, -1, -1) != -1)) {
			...
			case kSceneObjectTypeObject:
				if (_view3dObjectsToggle || (_specific3dObjectsDrawn && findInDbgDrawList(debuggerObjType3dObject, sceneObject->id - kSceneObjectOffsetObjects, _vm->_scene->getSetId(), _vm->_scene->getSceneId()) != -1)) {
...
void Debugger::drawLights() {
	// draw lights
	for (int i = 0; i < (int)_vm->_lights->_lights.size(); ++i) {
		if (_viewLights || (_specificLightsDrawn && findInDbgDrawList(debuggerObjTypeLight, i, _vm->_scene->getSetId(), _vm->_scene->getSceneId()) != -1)) {
			Light *light = _vm->_lights->_lights[i];
...
void Debugger::drawFogs() {
	...
	for (int i = 0; fog != nullptr; ++i) {
		if (_viewFogs || (_specificFogsDrawn && findInDbgDrawList(debuggerObjTypeFog, i, _vm->_scene->getSetId(), _vm->_scene->getSceneId()) != -1)) {
...
void Debugger::drawRegions() {
	if (_viewRegionsNormalToggle || _specificRegionNormalDrawn) {
		...
		for (int i = 0; i < 10; ++i) {
			...
			if (!region->present)
				continue;
			if (_viewRegionsNormalToggle || (_specificRegionNormalDrawn && findInDbgDrawList(debuggerObjTypeRegionNormal, i, _vm->_scene->getSetId(), _vm->_scene->getSceneId()) != -1)) {
	...
	if (_viewRegionsExitsToggle || _specificRegionExitsDrawn) {
		...
		for (int i = 0; i < 10; ++i) {
			...
			if (!region->present)
				continue;
			if (_viewRegionsExitsToggle || (_specificRegionExitsDrawn && findInDbgDrawList(debuggerObjTypeRegionExit, i, _vm->_scene->getSetId(), _vm->_scene->getSceneId()) != -1)) {
...
void Debugger::drawWaypoints() {
	if (_viewWaypointsNormalToggle || _specificWaypointNormalDrawn) {
		...
		for (int i = 0; i < _vm->_waypoints->_count; ++i) {
			...
			if (_viewWaypointsNormalToggle || (_specificWaypointNormalDrawn && findInDbgDrawList(debuggerObjTypeWaypointNorm, i, -1, -1) != -1)) {
	...
	if (_viewWaypointsCoverToggle || _specificWaypointCoverDrawn) {
		...
		for (int i = 0; i < (int)_vm->_combat->_coverWaypoints.size(); ++i) {
			...
			if (_viewWaypointsCoverToggle || (_specificWaypointCoverDrawn && findInDbgDrawList(debuggerObjTypeWaypointCover, i, -1, -1) != -1)) {
	...
	if (_viewWaypointsFleeToggle || _specificWaypointFleeDrawn) {
		...
		for (int i = 0; i < (int)_vm->_combat->_fleeWaypoints.size(); ++i) {
			...
			if (_viewWaypointsFleeToggle || (_specificWaypointFleeDrawn && findInDbgDrawList(debuggerObjTypeWaypoingFlee, i, -1, -1) != -1)) {
...
void Debugger::drawWalkboxes() {
	//draw walkboxes
	for (int i = 0; i < _vm->_scene->_set->_walkboxCount; ++i) {
		if (_viewWalkboxes || (_specificWalkboxesDrawn && findInDbgDrawList(debuggerObjTypeWalkbox, i, _vm->_scene->getSetId(), _vm->_scene->getSceneId()) != -1)) {
			Set::Walkbox *walkbox = &_vm->_scene->_set->_walkboxes[i];
...
void Debugger::drawScreenEffects() {
	//draw aesc
	for (uint i = 0; i < _vm->_screenEffects->_entries.size(); ++i) {
		if (_viewScreenEffects || (_specificEffectsDrawn && findInDbgDrawList(debuggerObjTypeEffect, i, _vm->_scene->getSetId(), _vm->_scene->getSceneId()) != -1)) {
			ScreenEffects::Entry &entry = _vm->_screenEffects->_entries[i];
			int j = 0;
...
int Debugger::findInDbgDrawList(DebuggerDrawnObjectType drObjType, int drObjId, int drObjSetId, int drObjSceneId) {
	...
	for (int i = 0; i < (int)_specificDrawnObjectsList.size(); ++i) {
		if ((drObjType == debuggerObjTypeUndefined || drObjType == _specificDrawnObjectsList[i].type) && (drObjId == -1 || drObjId == _specificDrawnObjectsList[i].objId) && (drObjSetId == -1 || _specificDrawnObjectsList[i].setId == -1 || drObjSetId == _specificDrawnObjectsList[i].setId) && (drObjSceneId == -1 || _specificDrawnObjectsList[i].sceneId == -1 || drObjSceneId == _specificDrawnObjectsList[i].sceneId)) {
...
void Debugger::updateTogglesForDbgDrawListInCurrentSetAndScene() {
	...
	_specificWalkboxesDrawn = false;
	for (int i = 0; i < (int)_specificDrawnObjectsList.size(); ++i) {
		if ((_specificDrawnObjectsList[i].sceneId == -1 || _specificDrawnObjectsList[i].sceneId == _vm->_scene->getSceneId()) && (_specificDrawnObjectsList[i].setId == -1 || _specificDrawnObjectsList[i].setId == _vm->_scene->getSetId())) {
