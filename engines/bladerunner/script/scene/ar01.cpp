...
void SceneScriptAR01::InitializeScene() {
	...
	if (Game_Flag_Query(kFlagSpinnerAtAR01) && !Game_Flag_Query(kFlagAR02toAR01) && !Game_Flag_Query(kFlagHC01toAR01)) {
	...
	} else if (Game_Flag_Query(kFlagSpinnerAtAR01) && Game_Flag_Query(kFlagAR02toAR01)) {
		...
		Game_Flag_Reset(kFlagAR02toAR01);
	} else if (Game_Flag_Query(kFlagSpinnerAtAR01) && Game_Flag_Query(kFlagHC01toAR01)) {
		Scene_Loop_Set_Default(kAR01LoopMainLoop);
	} else if (!Game_Flag_Query(kFlagSpinnerAtAR01) && Game_Flag_Query(kFlagHC01toAR01)) {
		Scene_Loop_Set_Default(kAR01LoopMainLoopNoSpinner);
	...
	if (actorId == kActorFishDealer) {
		...
		if (!Loop_Actor_Walk_To_XYZ(kActorMcCoy, -120.73f, 0.0f, 219.17f, 12, true, false, false)) {
			...
			if (!Game_Flag_Query(kFlagAR01FishDealerTalk)) {
				...
				if (Actor_Clue_Query(kActorMcCoy, kClueStrangeScale1) && !Actor_Clue_Query(kActorMcCoy, kClueFishLadyInterview) && (!_vm->_cutContent || !Game_Flag_Query(kFlagWrongInvestigation))) {
					...
					if (_vm->_cutContent) {
						...
						case 1:
							Global_Variable_Increment(kVariableFishDealerBanterTalk, 1);
							if (Player_Query_Agenda() == kPlayerAgendaSurly || Actor_Query_Friendliness_To_Other(kActorFishDealer, kActorMcCoy) <= 45) {
...
bool SceneScriptAR01::ClickedOnExit(int exitId) {
	...
	if (exitId == kAR01ExitSpinner) {
		...
		if (Actor_Query_Goal_Number(kActorIzo) >= kGoalIzoTakePhoto && Actor_Query_Goal_Number(kActorIzo) <= kGoalIzoEscape) {
...
void SceneScriptAR01::SceneFrameAdvanced(int frame) {
	...
	} else if (frame == 196 && !Game_Flag_Query(kFlagArrivedFromSpinner2)) {
...
void SceneScriptAR01::PlayerWalkedOut() {
	...
	if (!Game_Flag_Query(kFlagAR01toHC01) && !Game_Flag_Query(kFlagAR01toAR02)) {
#if BLADERUNNER_ORIGINAL_BUGS
...
void SceneScriptAR01::dialogueWithFishDealerBuyGoldfish() {
	...
	if (Global_Variable_Query(kVariableChinyen) >= 105 || Query_Difficulty_Level() == kGameDifficultyEasy) {
