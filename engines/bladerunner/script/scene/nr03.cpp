...
bool SceneScriptNR03::ClickedOnExit(int exitId) {
	...
	if (exitId == 2) {
		if (!Loop_Actor_Walk_To_XYZ(kActorMcCoy, -151.0f, -70.19f, -476.0f, 12, true, false, false)) {
			if (Actor_Query_Which_Set_In(kActorHanoi) == kSetNR03 && Actor_Query_Goal_Number(kActorHanoi) != kGoalHanoiNR03GoToDancer) {
...
void SceneScriptNR03::rotateActorOnTable(int frame) {
	...
	if (!Game_Flag_Query(kFlagNR03toNR05) && Actor_Query_Goal_Number(kActorGuzza) != kGoalGuzzaSitAtNR03) {
		angle = angle + M_PI;
