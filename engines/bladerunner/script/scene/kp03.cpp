...
void SceneScriptKP03::InitializeScene() {
	...
	if ((Actor_Query_Goal_Number(kActorSteele) != kGoalSteeleGone && !Game_Flag_Query(kFlagKP03BombExploded) && !Game_Flag_Query(kFlagKP03BombDisarmed)) && ((Game_Flag_Query(kFlagMcCoyIsHelpingReplicants) && Game_Flag_Query(kFlagKP05toKP03)) || (!Game_Flag_Query(kFlagMcCoyIsHelpingReplicants) && Game_Flag_Query(kFlagKP01toKP03)))) {
...
bool SceneScriptKP03::ClickedOn3DObject(const char *objectName, bool combatMode) {
	...
	if (Object_Query_Click("BRACK MID", objectName) && !Game_Flag_Query(kFlagKP03BombExploded)) {
		if (combatMode) {
			Scene_Loop_Set_Default(kKP03MainLoopBombExploded);
...
bool SceneScriptKP03::ClickedOnActor(int actorId) {
	if (actorId == kActorSteele && Actor_Query_Goal_Number(kActorSteele) == kGoalSteeleKP03Walk) {
		Actor_Face_Object(kActorSteele, "BRACK MID", true);
		saveSteele();
	}
	return false;
...
void SceneScriptKP03::SceneFrameAdvanced(int frame) {
	...
	if (Game_Flag_Query(kFlagKP03BombActive) && !Game_Flag_Query(kFlagKP03BombDisarmed) && !Game_Flag_Query(kFlagKP03BombExploded)) {
		float x, y, z;
		int bombTriggeredByActor = -1;

		Actor_Query_XYZ(kActorMcCoy, &x, &y, &z);
		if ((Game_Flag_Query(kFlagKP01toKP03) && -130.0f < x) || (Game_Flag_Query(kFlagKP05toKP03) && -130.0f > x)) {
			bombTriggeredByActor = kActorMcCoy;
		}

		Actor_Query_XYZ(kActorSteele, &x, &y, &z);
		if (Game_Flag_Query(kFlagMcCoyIsHelpingReplicants) && Actor_Query_Which_Set_In(kActorSteele) == kSetKP03) {
			if ((Game_Flag_Query(kFlagKP01toKP03) && -130.0f > x) || (Game_Flag_Query(kFlagKP05toKP03) && -130.0f < x)) {
				bombTriggeredByActor = kActorSteele;
			}
		} else if ((Game_Flag_Query(kFlagKP01toKP03) && -130.0f < x) || (Game_Flag_Query(kFlagKP05toKP03) && -130.0f > x)) {
			bombTriggeredByActor = kActorSteele;
		}
...
void SceneScriptKP03::PlayerWalkedIn() {
	...
	if (Actor_Query_Is_In_Current_Set(kActorSteele) && Actor_Query_Goal_Number(kActorSteele) != kGoalSteeleKP03Dead) {
		...
		if (!Game_Flag_Query(kFlagKP03BombExploded) && !Game_Flag_Query(kFlagKP03BombDisarmed) && Game_Flag_Query(kFlagKP01toKP03)) {
			Scene_Exits_Disable();
			Delay(1000);
