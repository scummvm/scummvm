...
void SceneScriptUG13::SceneLoaded() {
	...
	if (Global_Variable_Query(kVariableChapter) >= 3 && !Actor_Clue_Query(kActorMcCoy, kClueOriginalRequisitionForm) && Game_Flag_Query(kFlagCT04HomelessKilledByMcCoy) && (Actor_Clue_Query(kActorMcCoy, kClueShippingForm) || Actor_Clue_Query(kActorMcCoy, kClueWeaponsOrderForm))
#if BLADERUNNER_ORIGINAL_BUGS
#else
...
bool SceneScriptUG13::ClickedOnActor(int actorId) {
	if (actorId == kActorTransient && Global_Variable_Query(kVariableChapter) == 4) {
		...
		if (!Loop_Actor_Walk_To_XYZ(kActorMcCoy, -267.0f, 44.0f, -795.0f, 0, true, false, false)) {
			...
			if (_vm->_cutContent && Query_Difficulty_Level() == kGameDifficultyHard) {
				if (!Game_Flag_Query(kFlagCallWithGuzza)) {
					...
					if (Actor_Query_Goal_Number(kActorTransient) == 391 || Actor_Query_Goal_Number(kActorTransient) == 395 || Actor_Query_Goal_Number(kActorTransient) == 599) {
...
void SceneScriptUG13::SceneFrameAdvanced(int frame) {
	...
	if (Game_Flag_Query(kFlagUG13CallElevator) && frame > 29 && frame < 91) {
		Scene_Exit_Add_2D_Exit(0, 394, 205, 464, 281, 0);
	...
	if (Game_Flag_Query(kFlagUG08toUG13) && frame < 25) {
		Actor_Set_Invisible(kActorMcCoy, true);
	} else if (Game_Flag_Query(kFlagUG13toUG08) && frame >= 94 && frame <= 120) {
		Actor_Set_Invisible(kActorMcCoy, true);
	} else {
		Actor_Set_Invisible(kActorMcCoy, false);
	}
