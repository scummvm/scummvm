...
void SceneScriptCT12::InitializeScene() {
	...
	if (Global_Variable_Query(kVariableChapter) < 2 && Actor_Query_Goal_Number(kActorGaff) == kGoalGaffCT12WaitForMcCoy) {
	...
	if (Game_Flag_Query(kFlagCT01toCT12) && Game_Flag_Query(kFlagSpinnerAtCT01)) {
		...
		Scene_Loop_Set_Default(2);
		Game_Flag_Reset(kFlagCT01toCT12);
	} else if (Game_Flag_Query(kFlagCT05toCT12) && Game_Flag_Query(kFlagCT12GaffSpinner)) {
...
bool SceneScriptCT12::ClickedOnActor(int actorId) {
	...
	if (actorId == kActorOfficerGrayford && Global_Variable_Query(kVariableChapter) == 4 && Game_Flag_Query(kFlagUG18GuzzaScene) && Game_Flag_Query(kFlagUG18BriefcaseTaken)) {
...
void SceneScriptCT12::PlayerWalkedIn() {
	if (Global_Variable_Query(kVariableChapter) < 2 && !Game_Flag_Query(kFlagGaffApproachedMcCoyAboutZuben) && Actor_Query_Goal_Number(kActorGaff) == kGoalGaffCT12WaitForMcCoy) {
#if !BLADERUNNER_ORIGINAL_BUGS
		...
		if (Game_Flag_Query(kFlagGaffApproachedMcCoyAboutZuben) && Game_Flag_Query(kFlagZubenRetired)) {
		...
		} else if (Game_Flag_Query(kFlagGaffApproachedMcCoyAboutZuben) && Game_Flag_Query(kFlagZubenSpared)) {
