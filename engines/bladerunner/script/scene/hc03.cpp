...
void SceneScriptHC03::InitializeScene() {
	...
	if (Game_Flag_Query(kFlagHC03CageOpen) || Global_Variable_Query(kVariableChapter) > 3) {
	...
	} else if (Game_Flag_Query(kFlagHC03CageOpen) || Global_Variable_Query(kVariableChapter) > 3) {
...
void SceneScriptHC03::SceneLoaded() {
	...
	if (Game_Flag_Query(kFlagHC03CageOpen) || Game_Flag_Query(kFlagHC03TrapDoorOpen) || Global_Variable_Query(kVariableChapter) > 3) {
...
void SceneScriptHC03::SceneFrameAdvanced(int frame) {
	...
	if (!Game_Flag_Query(kFlagHC03TrapDoorOpened) && frame == 66) {
