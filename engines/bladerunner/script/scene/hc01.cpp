...
bool SceneScriptHC01::ClickedOnActor(int actorId) {
	if (actorId == kActorIzo && (Actor_Query_Goal_Number(kActorIzo) == kGoalIzoGoToHC01 || Actor_Query_Goal_Number(kActorIzo) == kGoalIzoDefault)) {
...
void SceneScriptHC01::dialogueWithIzo() {
	...
	if (Dialogue_Menu_Query_List_Size() == 0) {
		...
		if (Actor_Query_Friendliness_To_Other(kActorIzo, kActorMcCoy) < 50) {
			...
			if (Actor_Query_Friendliness_To_Other(kActorIzo, kActorMcCoy) < 47 && Query_Difficulty_Level() == kGameDifficultyEasy) {
				takePhotoAndRunAway();
			}
		}
		return;
