...
bool SceneScriptDR02::ClickedOn3DObject(const char *objectName, bool a2) {
	if (_vm->_cutContent) {
		if (!Game_Flag_Query(kFlagMcCoyCommentsOnEyeOfEyeworks) && (Object_Query_Click("U2 EYE", objectName))) {
...
bool SceneScriptDR02::ClickedOnExit(int exitId) {
	...
	if (_vm->_cutContent && (exitId == 3 || exitId == 4)) {
		if ((exitId == 3 && !Loop_Actor_Walk_To_XYZ(kActorMcCoy, -1162.45f, 0.78f, -167.45f, 0, true, false, false)) || (exitId == 4 && !Loop_Actor_Walk_To_XYZ(kActorMcCoy, -975.17f, 0.42f, -120.04f, 0, true, false, false))) {
			if (exitId == 3) {
