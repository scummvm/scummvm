...
bool SceneScriptUG16::ClickedOn3DObject(const char *objectName, bool a2) {
	if (Object_Query_Click("QUADPATCH05", objectName)) {
		if (!Loop_Actor_Walk_To_XYZ(kActorMcCoy, 194.0f, -35.0f, 160.8f, 0, true, false, false)) {
			...
			if (!Game_Flag_Query(kFlagUG16FolderFound) && Game_Flag_Query(kFlagLutherLanceAreDead)) {
	...
	if (Object_Query_Click("SCREEN 01", objectName)) {
		if (!Loop_Actor_Walk_To_XYZ(kActorMcCoy, 194.0f, -35.0f, 160.8f, 0, true, false, false)) {
			...
			if ((Game_Flag_Query(kFlagLutherLanceAreDead) || !Actor_Query_Is_In_Current_Set(kActorLuther)) && !Actor_Clue_Query(kActorMcCoy, kClueDNALutherLance) && !Game_Flag_Query(kFlagUG16ComputerOff)) {
				Delay(2000);
...
void SceneScriptUG16::PlayerWalkedIn() {
	...
	if (!Game_Flag_Query(kFlagUG16LutherLanceTalk1) && Actor_Query_Is_In_Current_Set(kActorLuther)) {
...
void SceneScriptUG16::dialogueWithLuther() {
	...
	if (Game_Flag_Query(kFlagUG16LutherLanceTalkReplicants1) || Game_Flag_Query(kFlagUG16LutherLanceTalkReplicants2)) {
	...
	if (Global_Variable_Query(kVariableCorruptedGuzzaEvidence) > 1 && !Actor_Clue_Query(kActorMcCoy, kClueFolder)) {
	...
	if (Actor_Clue_Query(kActorMcCoy, kClueDNATyrell) && !Actor_Clue_Query(kActorMcCoy, kClueFolder) && Game_Flag_Query(kFlagUG15LanceLuthorTrade)) {
