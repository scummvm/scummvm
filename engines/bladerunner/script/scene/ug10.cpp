...
void SceneScriptUG10::SceneLoaded() {
	...
	if (Global_Variable_Query(kVariableChapter) == 4 && !Game_Flag_Query(kFlagUG10GateOpen) && Game_Flag_Query(kFlagCT04HomelessBodyThrownAway) && !Game_Flag_Query(kFlagUG03DeadHomeless)) {
		Scene_Loop_Set_Default(1);
...
bool SceneScriptUG10::ClickedOnExit(int exitId) {
	...
	if (exitId == 0) {
		if ((!Game_Flag_Query(kFlagUG10GateOpen) && x > 125.0f) || Game_Flag_Query(kFlagUG10GateOpen)) {
		...
		if ((!Game_Flag_Query(kFlagUG10GateOpen) && x < 120.0f) || Game_Flag_Query(kFlagUG10GateOpen)) {
