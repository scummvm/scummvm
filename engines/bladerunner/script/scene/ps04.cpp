...
void SceneScriptPS04::SceneLoaded() {
	...
	if (Global_Variable_Query(kVariableChapter) == 2 && !Actor_Clue_Query(kActorMcCoy, kClueWeaponsOrderForm) && !Game_Flag_Query(kFlagPS04WeaponsOrderForm)) {
...
#if BLADERUNNER_ORIGINAL_BUGS
	if (itemId == kItemWeaponsOrderForm && Actor_Query_Is_In_Current_Set(kActorGuzza)) {
...
void SceneScriptPS04::dialogueWithGuzza() {
	...
	case 130: // REPORT IN
		if (Game_Flag_Query(kFlagZubenRetired) && !Game_Flag_Query(kFlagPS04GuzzaTalkZubenRetired)) {
...
#endif // BLADERUNNER_ORIGINAL_BUGS
		} else if (Game_Flag_Query(kFlagZubenSpared) && !Game_Flag_Query(kFlagPS04GuzzaTalkZubenEscaped)) {
		...
		} else if ((Actor_Clue_Query(kActorMcCoy, kClueChopstickWrapper) || Actor_Clue_Query(kActorMcCoy, kClueSushiMenu)) && Actor_Clue_Query(kActorMcCoy, kClueRunciterInterviewA) && Actor_Query_Friendliness_To_Other(kActorGuzza, kActorMcCoy) < 50 && !Game_Flag_Query(kFlagPS04GuzzaTalk1)) {
		...
		} else if ((Actor_Clue_Query(kActorMcCoy, kClueChopstickWrapper) || Actor_Clue_Query(kActorMcCoy, kClueSushiMenu)) && Actor_Clue_Query(kActorMcCoy, kClueRunciterInterviewA) && !Game_Flag_Query(kFlagPS04GuzzaTalk2)) {
			Actor_Says(kActorMcCoy, 3920, 13);
