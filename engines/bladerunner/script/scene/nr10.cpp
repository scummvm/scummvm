...
void SceneScriptNR10::InitializeScene() {
	...
	if (_vm->_cutContent && !Game_Flag_Query(kFlagMcCoyCommentsOnOldProjector) && !Actor_Query_Is_In_Current_Set(kActorDektora)) {
...
bool SceneScriptNR10::ClickedOn3DObject(const char *objectName, bool combatMode) {
	if (Object_Query_Click("BOX18", objectName) && combatMode && Game_Flag_Query(kFlagNR10McCoyBlinded)) {
...
void SceneScriptNR10::SceneFrameAdvanced(int frame) {
	...
	if (frame == 61 && Game_Flag_Query(kFlagNR10McCoyBlinded)) {
		Game_Flag_Reset(kFlagNR10McCoyBlinded);
