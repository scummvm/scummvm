...
bool SceneScriptHF06::ClickedOn3DObject(const char *objectName, bool a2) {
	if (Object_Query_Click("BOX28", objectName) || Object_Query_Click("BOX29", objectName) || Object_Query_Click("BOX30", objectName) || Object_Query_Click("HOOD BOX", objectName)) {
		if (!Loop_Actor_Walk_To_XYZ(kActorMcCoy, 14.33f, 367.93f, 399.0f, 0, true, false, false)) {
			...
			if (Actor_Query_In_Set(kActorDektora, kSetHF06) && Actor_Query_Goal_Number(kActorDektora) != kGoalDektoraGone) {
				Actor_Face_Actor(kActorDektora, kActorMcCoy, true);
				Actor_Says(kActorDektora, 210, 12);
				Actor_Says(kActorMcCoy, 2125, 12);
			} else if (Actor_Query_In_Set(kActorLucy, kSetHF06) && Actor_Query_Goal_Number(kActorLucy) != kGoalLucyGone) {
...
bool SceneScriptHF06::ClickedOnActor(int actorId) {
	if (actorId == kActorLucy && Actor_Query_Goal_Number(kActorLucy) != kGoalLucyGone) {
		Actor_Face_Actor(kActorLucy, kActorMcCoy, true);
	...
	} else if (actorId == kActorDektora && Actor_Query_Goal_Number(kActorDektora) != kGoalDektoraGone) {
		Actor_Face_Actor(kActorDektora, kActorMcCoy, true);
...
void SceneScriptHF06::ActorChangedGoal(int actorId, int newGoal, int oldGoal, bool currentSet) {
	if (actorId == kActorSteele && oldGoal != kGoalSteeleGone && newGoal == kGoalSteeleGone) {
...
void SceneScriptHF06::PlayerWalkedIn() {
	if (Game_Flag_Query(kFlagHF01TalkToLovedOne)) {
		...
		if (Global_Variable_Query(kVariableAffectionTowards) == kAffectionTowardsLucy && Actor_Query_Goal_Number(kActorLucy) != kGoalLucyGone) {
			actorId = kActorLucy;
		} else if (Global_Variable_Query(kVariableAffectionTowards) == kAffectionTowardsDektora && Actor_Query_Goal_Number(kActorDektora) != kGoalDektoraGone) {
	...
	if (Game_Flag_Query(kFlagHF01TalkToLovedOne) && !Game_Flag_Query(kFlagHF06SteelInterruption)) {
		steelInterruption();
	}
}
