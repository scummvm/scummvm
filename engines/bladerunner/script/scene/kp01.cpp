...
void SceneScriptKP01::InitializeScene() {
	if (Game_Flag_Query(kFlagKP04toKP01)) {
		...
		if (!Game_Flag_Query(kFlagKP01Entered) && !Game_Flag_Query(kFlagMcCoyIsHelpingReplicants)) {
...
void SceneScriptKP01::PlayerWalkedIn() {
	...
	if (!Game_Flag_Query(kFlagMcCoyIsHelpingReplicants) && !Game_Flag_Query(kFlagMcCoyAttackedReplicants) && Actor_Query_Goal_Number(kActorSteele) == kGoalSteeleKP01Wait && Actor_Query_Goal_Number(kActorSteele) != kGoalSteeleGone) {
