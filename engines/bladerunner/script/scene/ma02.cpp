...
void SceneScriptMA02::InitializeScene() {
	...
	if (Global_Variable_Query(kVariableChapter) >= 4 && Global_Variable_Query(kVariableChapter) == 5 && Game_Flag_Query(kFlagMcCoyIsHelpingReplicants)) {
	...
	if (Global_Variable_Query(kVariableChapter) == 5 && Game_Flag_Query(kFlagMcCoyIsHelpingReplicants) && !Actor_Clue_Query(kActorMcCoy, kClueCrystalsCigarette)) {
...
bool SceneScriptMA02::ClickedOnActor(int actorId) {
	if (actorId == kActorMaggie && Actor_Query_Goal_Number(kActorMaggie) == kGoalMaggieDead) {
...
void SceneScriptMA02::PlayerWalkedIn() {
	...
	if (Global_Variable_Query(kVariableChapter) == 5 && !Game_Flag_Query(kFlagMA02Chapter5Started)) {
