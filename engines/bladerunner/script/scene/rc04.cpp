...
void SceneScriptRC04::dialogueWithBulletBob() {
	...
	case 1280: // AMMO
		...
		if (Global_Variable_Query(kVariableChinyen) > 40 || Query_Difficulty_Level() == kGameDifficultyEasy) {
...
bool SceneScriptRC04::ClickedOnActor(int actorId) {
	...
	if (actorId == kActorBulletBob) {
		if (Global_Variable_Query(kVariableChapter) == 2 && !Game_Flag_Query(kFlagRC04McCoyShotBob)) {
			Loop_Actor_Walk_To_Waypoint(kActorMcCoy, 104, 0, false, false);
			Actor_Face_Actor(kActorMcCoy, kActorBulletBob, true);
			if (Game_Flag_Query(kFlagRC04Entered) && !Game_Flag_Query(kFlagRC04BobTalk2) && Actor_Query_Friendliness_To_Other(kActorBulletBob, kActorMcCoy) > 45) {
			...
			} else if (Game_Flag_Query(kFlagRC04Entered) && !Game_Flag_Query(kFlagRC04BobTalk1) && Actor_Query_Friendliness_To_Other(kActorBulletBob, kActorMcCoy) < 45) {
			...
			} else if (Actor_Query_Friendliness_To_Other(kActorBulletBob, kActorMcCoy) > 51 && !Game_Flag_Query(kFlagRC04BobTalk3)) {
				...
				if (Global_Variable_Query(kVariableChinyen) > 50 || Query_Difficulty_Level() == kGameDifficultyEasy) {
			...
			} else if (Actor_Clue_Query(kActorMcCoy, kClueLabShellCasings) || Actor_Clue_Query(kActorMcCoy, kClueShellCasings) || Actor_Clue_Query(kActorMcCoy, kClueRadiationGoggles) || Actor_Clue_Query(kActorMcCoy, kClueHoldensBadge) || Actor_Clue_Query(kActorMcCoy, kClueHasanInterview) || !Game_Flag_Query(kFlagRC04BobTalkAmmo)) {
...
void SceneScriptRC04::PlayerWalkedIn() {
	...
	if (Global_Variable_Query(kVariableChapter) == 2 && !Game_Flag_Query(kFlagRC04Entered) && !Player_Query_Combat_Mode()) {
	...
	if (Global_Variable_Query(kVariableChapter) == 4 && !Game_Flag_Query(kFlagRC04McCoyShotBob) && !Game_Flag_Query(kFlagNotUsed306)) {
