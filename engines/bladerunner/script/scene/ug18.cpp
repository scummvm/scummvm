...
void SceneScriptUG18::InitializeScene() {
	...
	if (Game_Flag_Query(kFlagCallWithGuzza) && !Game_Flag_Query(kFlagUG18GuzzaScene) && Global_Variable_Query(kVariableChapter) == 4) {
...
void SceneScriptUG18::ActorChangedGoal(int actorId, int newGoal, int oldGoal, bool currentSet) {
	...
	if (actorId == kActorSadik) {
		...
		case kGoalSadikUG18Decide:
			if (Actor_Query_Friendliness_To_Other(kActorClovis, kActorMcCoy) > 55 && Game_Flag_Query(kFlagMcCoyRetiredHuman)) {
...
void SceneScriptUG18::PlayerWalkedIn() {
	...
	if (Game_Flag_Query(kFlagCallWithGuzza) && !Game_Flag_Query(kFlagUG18GuzzaScene) && Actor_Query_Is_In_Current_Set(kActorGuzza)) {
		Scene_Exits_Disable();
		talkWithGuzza();
		talkWithClovis();
...
void SceneScriptUG18::talkWithGuzza() {
	...
	} else if (Global_Variable_Query(kVariableAffectionTowards) > 1 || Player_Query_Agenda() == kPlayerAgendaSurly) {
