...
bool SceneScriptKP02::ClickedOnExit(int exitId) {
	if (exitId == 0) {
		...
		if (!Loop_Actor_Walk_To_XYZ(kActorMcCoy, -1040.0f, -615.49f, 2903.0f, 0, true, false, false)) {
			if (Actor_Query_Goal_Number(kActorFreeSlotB) == kGoalFreeSlotBAct5KP02Attack || Actor_Query_Goal_Number(kActorFreeSlotA) == kGoalFreeSlotAAct5KP02Attack) {
...
void SceneScriptKP02::PlayerWalkedIn() {
	...
	if (Game_Flag_Query(kFlagMcCoyIsHelpingReplicants) && Actor_Query_Goal_Number(kActorSteele) != 599) {
