...
void SceneScriptMA06::SceneLoaded() {
	...
	if (_vm->_cutContent) {
		...
		} else if (Actor_Query_Goal_Number(kActorRachael) == kGoalRachaelAtEndOfAct3IfMetWithMcCoy && Game_Flag_Query(kFlagMA02RajifTalk)) {
...
void SceneScriptMA06::PlayerWalkedIn() {
	...
	if (_vm->_cutContent) {
		...
		//
		if ((Game_Flag_Query(kFlagMA07toMA06) || Game_Flag_Query(kFlagMA01toMA06)) && Actor_Query_Goal_Number(kActorRachael) == kGoalRachaelIsInsideMcCoysElevatorAct3) {
		...
		} else if (Game_Flag_Query(kFlagMA02toMA06) && Actor_Query_Goal_Number(kActorRachael) == kGoalRachaelIsInsideMcCoysElevatorAct4) {
			normalElevatorBusiness = false;
...
bool SceneScriptMA06::isElevatorOnDifferentFloor() {
	return (Game_Flag_Query(kFlagMA01toMA06) && !Game_Flag_Query(kFlagMA06toMA01)) || (Game_Flag_Query(kFlagMA02toMA06) && !Game_Flag_Query(kFlagMA06ToMA02)) || (Game_Flag_Query(kFlagMA07toMA06) && !Game_Flag_Query(kFlagMA06toMA07));
}
...
void SceneScriptMA06::activateElevator() {
	...
	while (true) {
		...
		if (floorLevel > 1) {
			...
			if (Global_Variable_Query(kVariableChapter) == 4 && Game_Flag_Query(kFlagMA02RajifTalk)) {
				Sound_Play(kSfxELEBAD1, 100, 0, 0, 50);
				Delay(500);
