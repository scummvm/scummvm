...
void SceneScriptTB05::SceneLoaded() {
	...
	if (!Actor_Clue_Query(kActorMcCoy, kClueTyrellSalesPamphletEntertainModel) && !Actor_Clue_Query(kActorMcCoy, kClueTyrellSalesPamphletLolita) && (Game_Flag_Query(kFlagGordoIsReplicant) || Game_Flag_Query(kFlagLucyIsReplicant))) {
...
bool SceneScriptTB05::ClickedOn3DObject(const char *objectName, bool a2) {
	if (Object_Query_Click("MONITOR05", objectName)) {
		if (!Loop_Actor_Walk_To_XYZ(kActorMcCoy, 122.54f, 147.12f, -197.17f, 0, true, false, false)) {
			...
			if (!Actor_Clue_Query(kActorMcCoy, kClueDNAMarcus) && !Game_Flag_Query(kFlagTB05MonitorIntro)) {
			...
			if (Game_Flag_Query(kFlagTB05MonitorIntro) && !Game_Flag_Query(kFlagTB05MonitorUnlockAttempt) && !Actor_Clue_Query(kActorMcCoy, kClueDNAMarcus)) {
			...
			if (Game_Flag_Query(kFlagTB05MonitorUnlockAttempt) && !Game_Flag_Query(kFlagTB05MonitorDone)) {
				if (Actor_Clue_Query(kActorMcCoy, kClueDogCollar1) || Actor_Clue_Query(kActorMcCoy, kClueDogCollar2)) {
					Actor_Clue_Acquire(kActorMcCoy, kClueDNAMarcus, true, -1);
					Actor_Voice_Over(2230, kActorVoiceOver);
					Item_Pickup_Spin_Effect(kModelAnimationDNADataDisc, 352, 333);
