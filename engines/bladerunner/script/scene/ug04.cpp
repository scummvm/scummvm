...
#if BLADERUNNER_ORIGINAL_BUGS
#else
	if ((Global_Variable_Query(kVariableChapter) == 3) || (Global_Variable_Query(kVariableChapter) > 3 && Random_Query(1, 4) == 1)) {
...
void SceneScriptUG04::PlayerWalkedIn() {
	...
	if (_vm->_cutContent && !Game_Flag_Query(kFlagUG04DispatchOnHoodooRats) && !Game_Flag_Query(kFlagMcCoyCommentsOnHoodooRats)) {
