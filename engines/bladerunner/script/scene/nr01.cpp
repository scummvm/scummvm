...
void SceneScriptNR01::InitializeScene() {
	...
	if (Game_Flag_Query(kFlagSteeleKnowsBulletBobIsDead) && Actor_Query_Goal_Number(kActorSteele) == kGoalSteeleNR01WaitForMcCoy) {
	...
	if (Game_Flag_Query(kFlagSpinnerAtNR01) && !Game_Flag_Query(kFlagArrivedFromSpinner1)) {
		if ((!Game_Flag_Query(kFlagNR01VisitedFirstTimeWithSpinner) && Global_Variable_Query(kVariableChapter) == 3) || Random_Query(1, 3) == 1) {
...
void SceneScriptNR01::PlayerWalkedOut() {
	Ambient_Sounds_Remove_All_Non_Looping_Sounds(true);
	Ambient_Sounds_Remove_All_Looping_Sounds(1);
	if (!Game_Flag_Query(kFlagNR01toUG06) && !Game_Flag_Query(kFlagNR01toNR02) && !Game_Flag_Query(kFlagNR01toNR03) && !Game_Flag_Query(kFlagNR01DektoraFall) && !Game_Flag_Query(kFlagChapter4Starting)) {
...
void SceneScriptNR01::DialogueQueueFlushed(int a1) {
	if (Actor_Query_Goal_Number(kActorSteele) == kGoalSteeleNR01TalkToGordo && Actor_Query_Goal_Number(kActorGordo) != kGoalGordoNR01Die && Actor_Query_Goal_Number(kActorGordo) != kGoalGordoNR01HostageShot && Actor_Query_Goal_Number(kActorGordo) != kGoalGordoNR01HostageDie) {
