...
void SceneScriptHF04::SceneLoaded() {
	...
	if (Actor_Query_Goal_Number(kActorLucy) == kGoalLucyHF03RunToHF042) {
		if (Actor_Clue_Query(kActorLucy, kClueMcCoyHelpedLucy) && Global_Variable_Query(kVariableHollowayArrest) != 3) {
...
void SceneScriptHF04::SceneFrameAdvanced(int frame) {
	...
	if (frame == 179 && Actor_Query_Goal_Number(kActorLucy) == kGoalLucyHF04WaitForMcCoy1) {
...
void SceneScriptHF04::PlayerWalkedIn() {
	if (Actor_Query_Goal_Number(kActorLucy) == kGoalLucyHF04Start || Actor_Query_Goal_Number(kActorLucy) == kGoalLucyHF04Run1) {
