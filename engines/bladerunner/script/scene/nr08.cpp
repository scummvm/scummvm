...
void SceneScriptNR08::SceneFrameAdvanced(int frame) {
	...
	if (Game_Flag_Query(kFlagNR08McCoyWatchingShow) && !Game_Flag_Query(kFlagNR08Faded)) {
...
void SceneScriptNR08::PlayerWalkedIn() {
	if (Actor_Query_Goal_Number(kActorDektora) == kGoalDektoraNR08Dance && !Game_Flag_Query(kFlagNR08DektoraShow)) {
