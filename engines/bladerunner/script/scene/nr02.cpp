...
bool SceneScriptNR02::ClickedOnActor(int actorId) {
	if (actorId == kActorGordo && Actor_Query_Goal_Number(kActorGordo) == kGoalGordoNR02WaitAtBar) {
...
bool SceneScriptNR02::ClickedOnItem(int itemId, bool a2) {
	if (itemId == kItemGordosLighterReplicant || itemId == kItemGordosLighterHuman) {
...
bool SceneScriptNR02::ClickedOnExit(int exitId) {
	if (exitId == 0) {
		if (!Loop_Actor_Walk_To_XYZ(kActorMcCoy, -283.0f, -24.0f, 326.0f, 0, true, false, false)) {
			if (Actor_Query_Goal_Number(kActorGordo) < kGoalGordoNR01WaitAndAttack || Actor_Query_Goal_Number(kActorGordo) > kGoalGordoNR01WaitAndTakeHostage) {
				Music_Stop(2);
			}
...
void SceneScriptNR02::SceneFrameAdvanced(int frame) {
	...
	if (!Music_Is_Playing() && (Actor_Query_Goal_Number(kActorGordo) < kGoalGordoNR02TalkAboutMcCoy || Actor_Query_Goal_Number(kActorGordo) > kGoalGordoNR02RunAway3)) {
		playNextMusic();
	}
	//return false;
}
...
void SceneScriptNR02::PlayerWalkedOut() {
	...
	// Gordo on the podium
	if (Actor_Query_Goal_Number(kActorGordo) < kGoalGordoNR02TalkAboutMcCoy && Actor_Query_Goal_Number(kActorGordo) >= kGoalGordoNR02NextAct) {
		Actor_Set_Goal_Number(kActorGordo, kGoalGordoNR02WaitForMcCoy);
	}
}

void SceneScriptNR02::DialogueQueueFlushed(int a1) {
	if (Player_Query_Current_Scene() == kSceneNR02 && Actor_Query_Goal_Number(kActorGordo) == kGoalGordoNR02TellJoke1) {
		Sound_Play(kSfxRIMSHOT1, 50, 0, 0, 50);
		Sound_Play(kSfxAUDLAFF3, 50, 0, 0, 50);
	}

	if (Player_Query_Current_Scene() == kSceneNR02 && Actor_Query_Goal_Number(kActorGordo) == kGoalGordoNR02TellJoke2) {
		Sound_Play(kSfxRIMSHOT2, 50, 0, 0, 50);
		Sound_Play(kSfxAUDLAFF5, 50, 0, 0, 50);
	}

	if (Player_Query_Current_Scene() == kSceneNR02 && Actor_Query_Goal_Number(kActorGordo) == kGoalGordoNR02TellJoke3) {
		Sound_Play(kSfxRIMSHOT5, 50, 0, 0, 50);
		Sound_Play(kSfxAUDLAFF6, 50, 0, 0, 50);
	}

	// Gordo on the podium
	if (Player_Query_Current_Scene() == kSceneNR02 && Actor_Query_Goal_Number(kActorGordo) > kGoalGordoNR02NextAct && Actor_Query_Goal_Number(kActorGordo) < kGoalGordoNR02TalkAboutMcCoy) {
		Actor_Set_Goal_Number(kActorGordo, kGoalGordoNR02NextAct);
		return; // true;
	}

	// Gordo on the podium, but McCoy is outside
	if (Actor_Query_Goal_Number(kActorGordo) > kGoalGordoNR02NextAct && Actor_Query_Goal_Number(kActorGordo) < kGoalGordoNR02TalkAboutMcCoy) {
		Actor_Set_Goal_Number(kActorGordo, kGoalGordoNR02WaitForMcCoy);
		return; // true;
	}
	//return false;
