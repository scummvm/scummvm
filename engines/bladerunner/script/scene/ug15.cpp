...
void SceneScriptUG15::InitializeScene() {
	if (Game_Flag_Query(kFlagUG17toUG15)) {
		...
		if (Game_Flag_Query(kFlagUG15RatShot) && (Random_Query(1, 10) == 10) && !(_vm->_cutContent && Query_Difficulty_Level() == kGameDifficultyEasy)) {
	...
	if (Game_Flag_Query(kFlagUG17toUG15) || Game_Flag_Query(kFlagUG16toUG15a)) {
...
void SceneScriptUG15::SceneFrameAdvanced(int frame) {
	...
	if (Game_Flag_Query(kFlagUG15BridgeWillBreak) && !Game_Flag_Query(kFlagUG15BridgeBroken)) {
		float x, y, z;
		Actor_Query_XYZ(kActorMcCoy, &x, &y, &z);
		if (-180.0f <= x && 220.0f > z && !Game_Flag_Query(kFlagUG15BridgeBreaks)) {
