...
bool SceneScriptCT09::ClickedOnActor(int actorId) {
	if (actorId == kActorDeskClerk) {
		if (Actor_Query_Goal_Number(kActorDeskClerk) == kGoalDeskClerkDefault && Actor_Query_Which_Set_In(kActorLeon) != kSetCT09) {
...
void SceneScriptCT09::PlayerWalkedIn() {
	...
	if (Global_Variable_Query(kVariableChapter) == 3 && !Game_Flag_Query(kFlagCT09Entered)) {
...
void SceneScriptCT09::DialogueQueueFlushed(int a1) {
	...
	if (Actor_Query_Goal_Number(kActorLeon) == kGoalLeonHoldingDeskClerk && !Game_Flag_Query(kFlagCT09LeonInterrupted)) {
		Player_Loses_Control();
