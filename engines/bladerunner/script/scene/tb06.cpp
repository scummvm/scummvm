...
void SceneScriptTB06::SceneLoaded() {
	...
	if (!Game_Flag_Query(kFlagTB06DogCollarTaken) && Actor_Query_Goal_Number(kActorPhotographer) != 199) {
...
bool SceneScriptTB06::ClickedOnItem(int itemId, bool a2) {
	...
	if (itemId == kItemDeadDogA || itemId == kItemDeadDogB || itemId == kItemDeadDogC) {
...
void SceneScriptTB06::PlayerWalkedIn() {
	if (!Game_Flag_Query(kFlagTB06Visited) && !Game_Flag_Query(kFlagTB06Introduction)) {
