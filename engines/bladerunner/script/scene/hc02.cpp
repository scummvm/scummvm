...
bool SceneScriptHC02::ClickedOnActor(int actorId) {
	if (actorId == kActorHawkersBarkeep) {
		if (!Loop_Actor_Walk_To_XYZ(kActorMcCoy, -150.51f, 0.14f, 62.74f, 0, true, false, false)) {
			...
			if (!Game_Flag_Query(kFlagHC02HawkersBarkeepIntroduction)) {
				...
				Item_Pickup_Spin_Effect(kModelAnimationVideoDisc, 229, 215);
			} else if (Actor_Clue_Query(kActorMcCoy, kClueHomelessManInterview1) && !Actor_Clue_Query(kActorMcCoy, kClueFlaskOfAbsinthe)
			           ...
			           && (Global_Variable_Query(kVariableChinyen) > 20 || Query_Difficulty_Level() == kGameDifficultyEasy)) {
				...
				if (Actor_Clue_Query(kActorMcCoy, kClueMaggieBracelet) && !Game_Flag_Query(kFlagHC02HawkersBarkeepBraceletTalk)) {
				...
				if (Global_Variable_Query(kVariableChinyen) > 5 || Query_Difficulty_Level() == kGameDifficultyEasy) {
					if (Query_Difficulty_Level() != kGameDifficultyEasy) {
