...
void SceneScriptRC03::InitializeScene() {
	...
	if (Game_Flag_Query(kFlagHC04toRC03) && Actor_Query_Goal_Number(kActorIzo) != kGoalIzoWaitingAtRC03) {
...
bool SceneScriptRC03::ClickedOnExit(int exitId) {
	...
	if (exitId == 4) { // To Bullet Bob's
		if (!Loop_Actor_Walk_To_XYZ(kActorMcCoy, -22.0f, 1.0f, -63.0f, 0, true, false, false)) {
			if (Global_Variable_Query(kVariableChapter) == 3 || Global_Variable_Query(kVariableChapter) == 5 || Game_Flag_Query(kFlagBulletBobDead)) {
...
void SceneScriptRC03::PlayerWalkedIn() {
	...
	if (Actor_Query_Goal_Number(kActorIzo) == kGoalIzoEscape) {
		...
		if (Game_Flag_Query(kFlagUG01toRC03) || Game_Flag_Query(kFlagRC04toRC03)) {
