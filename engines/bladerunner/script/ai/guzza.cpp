...
bool AIScriptGuzza::Update() {
	if (Global_Variable_Query(kVariableChapter) == 2) {
		...
		if (Actor_Query_Goal_Number(kActorGuzza) != kGoalGuzzaGoToHawkersCircle1 && !Game_Flag_Query(kFlagHC01GuzzaWalk) && Game_Flag_Query(kFlagHC01GuzzaPrepare)) {
...
void AIScriptGuzza::ClickedByPlayer() {
	if (Global_Variable_Query(kVariableChapter) == 2 && Game_Flag_Query(kFlagGuzzaIsMovingAround)) {
