...
bool AIScriptGenericWalkerA::Update() {
	...
	if (Player_Query_Current_Set() == kSetRC04 && Actor_Query_Goal_Number(kActorGenwalkerA) != kGoalGenwalkerABulletBobsTrackGun) {
...
bool AIScriptGenericWalkerA::UpdateAnimation(int *animation, int *frame) {
	...
	case kGenericWalkerAStatesIdle:
		...
		if (!_vm->_cutContent || (Global_Variable_Query(kVariableGenericWalkerAModel) < 6 && Global_Variable_Query(kVariableGenericWalkerAModel) != 2)) {
			_animationFrame = 0;
		}
		break;
