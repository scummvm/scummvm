...
bool AIScriptRachael::Update() {
	if (_vm->_cutContent) {
		...
		} else if (Global_Variable_Query(kVariableChapter) > 4 && Actor_Query_Goal_Number(kActorRachael) != kGoalRachaelAtEndOfAct4) {
...
void AIScriptRachael::ClickedByPlayer() {
	if (Actor_Query_Goal_Number(kActorRachael) == 305) {
		...
		if ((Player_Query_Agenda() == kPlayerAgendaSurly || Player_Query_Agenda() == kPlayerAgendaErratic) || (Player_Query_Agenda() != kPlayerAgendaPolite && Actor_Query_Friendliness_To_Other(kActorSteele, kActorMcCoy) > Actor_Query_Friendliness_To_Other(kActorClovis, kActorMcCoy))) {
			dialogue_agenda2();
		} else {
			dialogue_agenda1();
		}
