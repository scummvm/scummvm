...
bool AIScriptDektora::Update() {
	...
	if (chapter == 2) {
		if (!Game_Flag_Query(kFlagDektoraChapter2Started)) {
			...
			if (Game_Flag_Query(kFlagAR02DektoraWillBuyScorpions) && !Game_Flag_Query(kFlagAR02DektoraBoughtScorpions) && Player_Query_Current_Scene() != kSceneAR01 && Player_Query_Current_Scene() != kSceneAR02) {
...
void AIScriptDektora::CompletedMovementTrack() {
	...
	case kGoalDektoraWalkAroundAsReplicant:
		if (Random_Query(1, 7) == 1 && Actor_Query_Goal_Number(kActorEarlyQ) != 1 && Actor_Query_Goal_Number(kActorEarlyQ) != 101) {
	...
	case kGoalDektoraWalkAroundAsHuman:
		if (Random_Query(1, 5) == 1 && Actor_Query_Goal_Number(kActorEarlyQ) != 1 && Actor_Query_Goal_Number(kActorEarlyQ) != 101) {
			Game_Flag_Set(kFlagAR02DektoraWillBuyScorpions);
			Actor_Set_Goal_Number(kActorDektora, kGoalDektoraStartWalkingAround);
		} else {
			Actor_Set_Goal_Number(kActorDektora, kGoalDektoraStartWalkingAround);
...
void AIScriptDektora::OtherAgentEnteredThisSet(int otherActorId) {
	if (otherActorId == kActorMcCoy && Actor_Query_Goal_Number(kActorDektora) == kGoalDektoraNR08GoToNR10) {
		Scene_Exits_Disable();
...
bool AIScriptDektora::ShotAtAndHit() {
	if (Actor_Query_Goal_Number(kActorDektora) == kGoalDektoraNR11Burning || Actor_Query_Goal_Number(kActorDektora) == kGoalDektoraNR11BurningGoToMcCoy || Actor_Query_Goal_Number(kActorDektora) == kGoalDektoraNR11BurningGoToWindow) {
...
bool AIScriptDektora::GoalChanged(int currentGoalNumber, int newGoalNumber) {
	...
	case kGoalDektoraWalkAroundAsReplicant:
		...
		if (Random_Query(1, 2) == 1 && Game_Flag_Query(kFlagAR02DektoraWillBuyScorpions)) {
	...
	case kGoalDektoraWalkAroundAsHuman:
		...
		if (Game_Flag_Query(kFlagZubenRetired) && Game_Flag_Query(kFlagLucyIsReplicant)) {
...
bool AIScriptDektora::UpdateAnimation(int *animation, int *frame) {
	...
	case 17:
		*animation = kModelAnimationDektoraCombatPunchAttack;
		++_animationFrame;
		if (_animationFrame == 6 && Actor_Query_Goal_Number(kActorDektora) == kGoalDektoraNR10AttackMcCoy) {
...
bool AIScriptDektora::ChangeAnimationMode(int mode) {
	...
	case 30:
		if (_animationState != 27 && _animationState != 28) {
			_animationState = 27;
			_animationFrame = 0;
			_flag = false;
		}
...
void AIScriptDektora::checkCombat() {
	if (Actor_Query_In_Set(kActorDektora, kSetHF01) && Global_Variable_Query(kVariableChapter) == 5 && Actor_Query_Goal_Number(kActorDektora) != 450) {
