...
bool AIScriptHanoi::Update() {
	...
	if (Global_Variable_Query(kVariableChapter) == 3 && Actor_Query_Goal_Number(kActorHanoi) < kGoalHanoiDefault) {
		Actor_Set_Goal_Number(kActorHanoi, kGoalHanoiNR03GoToDefaultPosition);
	}

	if (Player_Query_Current_Scene() != kSceneNR03 && Actor_Query_Goal_Number(kActorHanoi) == kGoalHanoiNR08Left) {
		Actor_Set_Goal_Number(kActorHanoi, kGoalHanoiNR03GoToDefaultPosition);
	}

	if (Player_Query_Current_Scene() == kSceneNR03 && Actor_Query_Goal_Number(kActorHanoi) != kGoalHanoiNR03StartGuarding && Actor_Query_Goal_Number(kActorHanoi) != kGoalHanoiNR08WatchShow && Actor_Query_Goal_Number(kActorHanoi) != kGoalHanoiNR08Leave && Actor_Query_Goal_Number(kActorHanoi) != kGoalHanoiNR08Left) {
		...
		if (Actor_Query_Inch_Distance_From_Actor(kActorMcCoy, kActorHysteriaPatron1) < 120 && Actor_Query_Which_Set_In(kActorHanoi) == kSetNR03 && Actor_Query_Goal_Number(kActorHanoi) != kGoalHanoiNR03GoToDancer) {
...
void AIScriptHanoi::ClickedByPlayer() {
	if (Actor_Query_Goal_Number(kActorHanoi) == kGoalHanoiNR08WatchShow || Actor_Query_Goal_Number(kActorHanoi) == kGoalHanoiNR08Leave) {
		Actor_Face_Actor(kActorMcCoy, kActorHanoi, true);
...
void AIScriptHanoi::OtherAgentEnteredCombatMode(int otherActorId, int combatMode) {
	if (Player_Query_Current_Scene() == kSceneNR03 && otherActorId == kActorMcCoy && combatMode) {
...
bool AIScriptHanoi::UpdateAnimation(int *animation, int *frame) {
	...
	case 0:
		...
		*animation = 648;
		if (_var3 != 0) {
			...
			if (_animationFrame == 5 || _animationFrame == 15 || _animationFrame == 11 || _animationFrame == 0) {
	...
	case 1:
		if (_flag1) {
			*animation = 649;
			...
			if (_animationFrame > Slice_Animation_Query_Number_Of_Frames(*animation) - 1 || _animationFrame <= 0) {
				_animationFrame = 0;
	...
	case 14:
		*animation = 650;
		if (_animationFrame == 0 && _var1 // this is never set so it's always 0
		) {
			_animationState = 0;
		} else {
			++_animationFrame;
...
bool AIScriptHanoi::ChangeAnimationMode(int mode) {
	...
	case kAnimationModeIdle:
		if (_animationState == 2 || _animationState == 3) {
			_animationState = 3;
		} else {
			_animationState = 0;
		}
