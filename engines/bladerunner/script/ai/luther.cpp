...
bool AIScriptLuther::Update() {
	if (Actor_Query_Is_In_Current_Set(kActorLuther) && Player_Query_Combat_Mode() && Global_Variable_Query(kVariableLutherLanceShot) == 0 && !Game_Flag_Query(kFlagUG16PulledGun) && Global_Variable_Query(kVariableChapter) == 4) {
	...
	if (Actor_Query_Goal_Number(kActorLuther) == kGoalLutherDefault && Actor_Query_Goal_Number(kActorLuther) != kGoalLutherDead // A bug? this is redundant
	) {
	...
	if (Actor_Query_Goal_Number(kActorLuther) == kGoalLutherDyingStarted && !Game_Flag_Query(kFlagUG15LutherLanceStartedDying)) {
	...
	if (Actor_Query_Goal_Number(kActorLuther) == kGoalLutherDyingCheck && Global_Variable_Query(kVariableLutherLanceShot) < 2 && !Game_Flag_Query(kFlagUG16ComputerOff)) {
	...
	if (Actor_Query_Goal_Number(kActorLuther) == kGoalLutherDyingCheck && Global_Variable_Query(kVariableLutherLanceShot) > 1 && !Game_Flag_Query(kFlagUG16ComputerOff)) {
		Actor_Set_Targetable(kActorLuther, false);
		Actor_Set_Goal_Number(kActorLuther, kGoalLutherDie);
		Actor_Set_Targetable(kActorLuther, false);
		return false;
