...
bool AIScriptMcCoy::Update() {
	...
	case kGoalMcCoyBB11RunAway:
		if (Actor_Query_Inch_Distance_From_Waypoint(kActorMcCoy, 316) < 36 && !Game_Flag_Query(kFlagBB11SadikPunchedMcCoy)) {
...
void AIScriptMcCoy::ReceivedClue(int clueId, int fromActorId) {
	...
	if (Global_Variable_Query(kVariableCorruptedGuzzaEvidence) > 6 && Global_Variable_Query(kVariableChapter) > 3 && !Actor_Clue_Query(kActorMcCoy, kClueGuzzaFramedMcCoy)) {
		Delay(500);
...
void AIScriptMcCoy::Retired(int byActorId) {
	...
	if (Actor_Query_In_Set(kActorMcCoy, kSetHF05) && Actor_Query_In_Set(kActorOfficerLeary, kSetHF05) && Actor_Query_In_Set(kActorDektora, kSetHF05) && Actor_Query_Goal_Number(kActorDektora) != kGoalDektoraGone) {
	...
	if (Actor_Query_In_Set(kActorMcCoy, kSetHF05) && Actor_Query_In_Set(kActorOfficerGrayford, kSetHF05) && Actor_Query_In_Set(kActorDektora, kSetHF05) && Actor_Query_Goal_Number(kActorDektora) != kGoalDektoraGone) {
	...
	if (Actor_Query_In_Set(kActorMcCoy, kSetHF05) && Actor_Query_In_Set(kActorOfficerLeary, kSetHF05) && Actor_Query_In_Set(kActorLucy, kSetHF05) && Actor_Query_Goal_Number(kActorLucy) != kGoalLucyGone) {
	...
	if (Actor_Query_In_Set(kActorMcCoy, kSetHF05) && Actor_Query_In_Set(kActorOfficerGrayford, kSetHF05) && Actor_Query_In_Set(kActorLucy, kSetHF05) && Actor_Query_Goal_Number(kActorLucy) != kGoalLucyGone) {
...
bool AIScriptMcCoy::GoalChanged(int currentGoalNumber, int newGoalNumber) {
	...
	case 400:
		...
		} else if (affectionTowards == kAffectionTowardsDektora || affectionTowards == kAffectionTowardsLucy) {
			if (!Game_Flag_Query(kFlagMcCoyIsHelpingReplicants)) {
				Global_Variable_Set(kVariableAffectionTowards, kAffectionTowardsNone);
			}
		}
...
bool AIScriptMcCoy::UpdateAnimation(int *animation, int *frame) {
	...
	case 21:
		*animation = kModelAnimationMcCoyWithGunShooting;
		++_animationFrame;
		if (_animationFrame == 1 && Actor_Query_Goal_Number(kActorMcCoy) == kGoalMcCoyNR11Shoot && _NR10SteeleShooting) {
...
void AIScriptMcCoy::UG15fall() {
	if (Actor_Query_Which_Set_In(kActorMcCoy) == kSetUG15 && Actor_Query_Goal_Number(kActorMcCoy) != kGoalMcCoyUG15Fall && !Game_Flag_Query(kFlagUG15BridgeBroken)) {
		float x, y, z;
		Actor_Query_XYZ(kActorMcCoy, &x, &y, &z);
		if (220.0f > z && -210.0f < x && -70.0f > x) {
