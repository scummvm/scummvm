...
namespace pyrodactyl
{
	namespace stat
	{
		...
		{
			if (val == STATNAME_HEALTH) return STAT_HEALTH;
			else if (val == STATNAME_ATTACK) return STAT_ATTACK;
			else if (val == STATNAME_DEFENSE) return STAT_DEFENSE;
			else if (val == STATNAME_SPEED) return STAT_SPEED;
		...
		{
			if (val == STAT_HEALTH) return STATNAME_HEALTH;
			else if (val == STAT_ATTACK) return STATNAME_ATTACK;
			else if (val == STAT_DEFENSE) return STATNAME_DEFENSE;
			else if (val == STAT_SPEED) return STATNAME_SPEED;
