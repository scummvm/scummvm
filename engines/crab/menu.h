...
namespace pyrodactyl
{
	namespace ui
	{
		...
		protected:
			...
			enum PathType { PATH_DEFAULT, PATH_HORIZONTAL, PATH_VERTICAL } path_type;
			...
			int HandleKeyboard(const SDL_Event &Event)
			{
				...
				if (!element.empty())
				{
					...
					{
						if (gInput.Equals(IU_DOWN, Event) == SDL_PRESSED)    { Next(); latest_input = KEYBOARD; }
						else if (gInput.Equals(IU_UP, Event) == SDL_PRESSED) { Prev(); latest_input = KEYBOARD; }
					}

					if (path_type != PATH_VERTICAL)
					{
						if (gInput.Equals(IU_RIGHT, Event) == SDL_PRESSED)     { Next(); latest_input = KEYBOARD; }
						else if (gInput.Equals(IU_LEFT, Event) == SDL_PRESSED) { Prev(); latest_input = KEYBOARD; }
					}
		...
		public:
			...
			Menu() { hover_index = -1; use_keyboard = false; latest_input = MOUSE; path_type = PATH_DEFAULT; }
			~Menu(){}
