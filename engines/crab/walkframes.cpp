...
{
	if (state == PST_FIGHT) return set[WT_FIGHT].frames[DIRECTION_DOWN].CurrentFrame().clip;
	else if (state == PST_KO) return set[WT_KO].frames[DIRECTION_DOWN].CurrentFrame().clip;
...
{
	if (state == PST_FIGHT) UpdateClip(WT_FIGHT, DIRECTION_DOWN);
	else if (state == PST_KO) UpdateClip(WT_KO, DIRECTION_DOWN);
	else UpdateClip(WT_STAND, DIRECTION_DOWN);
}
...
bool WalkFrames::Type(const Vector2f &vel, Direction &dir, const pyrodactyl::people::PersonState &pst, const bool &first_x)
{
	...
	else if (first_x)
	{
		...
		if (vel.x > 0)      { dir = DIRECTION_RIGHT; cur = WT_WALK; }
		else if (vel.x < 0) { dir = DIRECTION_LEFT; cur = WT_WALK; }
		else if (vel.y > 0) { dir = DIRECTION_DOWN;	cur = WT_WALK; }
		else if (vel.y < 0) { dir = DIRECTION_UP; cur = WT_WALK; }
		else                { cur = WT_STAND; }
	}
	else
	{
		//If we prioritize the Y direction, Y velocity is checked first for direction and then Y velocity
		if (vel.y > 0)      { dir = DIRECTION_DOWN;	cur = WT_WALK; }
		else if (vel.y < 0) { dir = DIRECTION_UP; cur = WT_WALK; }
		else if (vel.x > 0) { dir = DIRECTION_RIGHT; cur = WT_WALK; }
		else if (vel.x < 0) { dir = DIRECTION_LEFT; cur = WT_WALK; }
		else                { cur = WT_STAND; }
	}
