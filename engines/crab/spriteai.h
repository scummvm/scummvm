...
namespace pyrodactyl
{
	namespace ai
	{
		...
		enum AIFightState { FIGHTSTATE_GETNEXTMOVE, FIGHTSTATE_GETINRANGE, FIGHTSTATE_EXECUTEMOVE, FIGHTSTATE_CANTFIGHT };

		//States of a fleeing sprite
		enum AIFleeState { FLEESTATE_GETNEARESTEXIT, FLEESTATE_RUNTOEXIT, FLEESTATE_DISAPPEAR, FLEESTATE_CANTFLEE };
		...
		struct SpriteAIData
		{
			...
			void Dest(const int &x, const int &y, const bool &Active = true) { dest.x = x; dest.y = y; dest.active = Active; }
