...
TMXMap::TMXMap()
{
	...
	path_rows = 0; path_cols = 0;

	w = 0; h = 0;
	sprite_layer = 0;
	grid = nullptr;

	movementCosts.no_walk = 0; movementCosts.open = 0; movementCosts.stairs = 0;
}
		...
		if (NodeValid(node))
		{
			...
			for (auto groupnode = node->first_node(); groupnode != NULL; groupnode = groupnode->next_sibling())
			{
				...
				if (name == "layer" || name == "imagelayer") //Is this a tile or an image layer
				{
					...
					l.pos.x *= tile_size.x; l.pos.y *= tile_size.y;
					l.pos.w *= tile_size.x; l.pos.h *= tile_size.y;
