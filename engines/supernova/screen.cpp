...
ScreenBuffer::ScreenBuffer()
    : _x(0), _y(0), _width(0), _height(0), _pixels(nullptr) {
}
...
Marquee::Marquee(Screen *screen, MarqueeId id, const char *text)
    : _text(text), _textBegin(text), _delay(0), _loop(false), _oldColor(nullptr), _screen(screen) {
...
Screen::Screen(SupernovaEngine *vm, ResourceManager *resMan)
    : _vm(vm), _resMan(resMan), _currentImage(nullptr), _viewportBrightness(255), _guiBrightness(255), _screenWidth(320), _screenHeight(200), _textColor(kColorBlack), _textCursorX(0), _textCursorY(0), _messageShown(false) {
