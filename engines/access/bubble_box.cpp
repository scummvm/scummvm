...
int BubbleBox::doBox_v1(int item, int box, int &btnSelected) {
	...
	while (!_vm->shouldQuit()) {
		...
		if ((_vm->_events->_mousePos.x >= _boxStartX) && (_vm->_events->_mousePos.x <= _boxEndX) && (_vm->_events->_mousePos.y >= _boxStartY) && (_vm->_events->_mousePos.y <= _boxEndY)) {
		...
		if ((_vm->_events->_mousePos.y >= ICON1Y) && (_vm->_events->_mousePos.y <= ICON1Y + 8) && (_vm->_events->_mousePos.x >= ICON1X)) {
			btnSelected = 1;
