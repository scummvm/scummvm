...
namespace Amazon {
	...
	AmazonRoom::AmazonRoom(AccessEngine *vm)
	  : Room(vm) {
	...
	void AmazonRoom::mainAreaClick() {
		...
		if (_selectCommand == -1) {
			if (player._roomNumber == 42 || player._roomNumber == 44 || player._roomNumber == 31 || player._roomNumber == 29) {
		...
		} else if (mousePos.x >= screen._windowXAdd && mousePos.x <= (screen._windowXAdd + screen._vWindowBytesWide) && mousePos.y >= screen._windowYAdd && mousePos.y <= (screen._windowYAdd + screen._vWindowLinesTall)) {
			if (checkBoxes1(pt) >= 0) {
				checkBoxes3();
			}
		}
	...
	void AmazonRoom::walkCursor() {
		...
		if (_vm->_events->_middleButton || (_vm->_player->_roomNumber == 29 && events._normalMouse != CURSOR_CROSSHAIRS)) {
