...
namespace Amazon {
	...
	PannedScene::PannedScene(AmazonEngine *vm)
	  : AmazonManager(vm) {
	...
	CampScene::CampScene(AmazonEngine *vm)
	  : PannedScene(vm) {
		_skipStart = false;
	}
	...
	Opening::Opening(AmazonEngine *vm)
	  : CampScene(vm) {
		_pCount = 0;
	}
	...
	void Plane::mWhileFly() {
		...
		while (!_vm->shouldQuit() && !events.isKeyMousePressed() && ((_vm->_scrollCol + screen._vWindowWidth) != _vm->_room->_playFieldWidth)) {
			events._vbCount = 4;
	...
	void Jungle::mWhileJWalk2() {
		...
		while (!_vm->shouldQuit() && !_vm->_events->isKeyMousePressed() && (_vm->_scrollCol + screen._vWindowWidth) != _vm->_room->_playFieldWidth) {
			_vm->_images.clear();
	...
	void Cast::doCast(int param1) {
		...
		while (!_vm->shouldQuit()) {
			...
			for (int idx = 0; idx < 5 && !_vm->shouldQuit() && !_vm->_events->isKeyMousePressed(); ++idx)
				_vm->_events->pollEventsAndWait();
	...
	void River::moveCanoe() {
		...
		if (_canoeDir) {
			...
			if (events._leftButton && pt.y >= 140) {
				...
				_chickenOutFl = true;
			} else if ((events._leftButton && pt.y <= _canoeYPos) || (!events._leftButton && _vm->_player->_move == UP)) {
				// Move canoe up
				if (_canoeLane > 0) {
					_canoeDir = -1;
					_canoeMoveCount = 0;
	...
	bool River::checkRiverCollide() {
		...
		for (RiverStruct *cur = _topList; cur <= _botList; ++cur) {
			...
			if ((cur->_lane == _canoeLane) || (cur->_lane == _canoeLane + 1)) {
				if ((cur->_riverX + cur->_width - 1) >= _canoeVXPos && cur->_riverX < (_canoeVXPos + 124)) {
					_vm->_sound->playSound(4);
					return true;
				}
			}
	...
	void River::doRiver() {
		...
		while (!_vm->shouldQuit()) {
			...
			if (_vm->_room->_function != FN_CLEAR1) {
				...
				_vm->_canSaveLoad = true;
				while (!_vm->shouldQuit() && _vm->_room->_function == FN_NONE && _vm->_events->_vbCount > 0) {
					_vm->_events->pollEventsAndWait();
				}
				_vm->_canSaveLoad = false;
			}
