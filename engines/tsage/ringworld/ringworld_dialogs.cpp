...
void InventoryDialog::execute() {
	...
	while (!g_vm->shouldQuit()) {
		...
		if (!event.handled && (event.eventType == EVENT_KEYPRESS || event.eventType == EVENT_CUSTOM_ACTIONSTART)) {
			if ((event.kbd.keycode == Common::KEYCODE_RETURN) || (event.customType == kActionEscape)) {
				// Exit the dialog
				//hiliteObj = &_btnOk;
				break;
			}
