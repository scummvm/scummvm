 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene4000::dispatch() {
		...
		if (!_action) {
			if ((RING_INVENTORY._peg._sceneNumber == 1) && g_globals->getFlag(34) && g_globals->getFlag(37) && !g_globals->getFlag(40)) {
				g_globals->_player.disableControl();
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene4025::dispatch() {
		if (!_action && (_peg1._armStrip == 7) && (_peg2._armStrip == 4) && (_peg3._armStrip == 8) && (_peg4._armStrip == 5) && (_peg5._armStrip == 6))
			setAction(&_action3);
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene4300::Hotspot9::doAction(int action) {
		switch (action) {
		case CURSOR_LOOK:
			if ((RING_INVENTORY._stasisBox2._sceneNumber == 4300) || (RING_INVENTORY._concentrator._sceneNumber == 1))
	...
	void Scene4300::Hotspot16::doAction(int action) {
		...
		case CURSOR_LOOK:
			...
			break;
		case CURSOR_USE:
			if ((RING_INVENTORY._stasisBox2._sceneNumber != 4300) && (RING_INVENTORY._concentrator._sceneNumber != 4300)) {
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene4301::Action1::process(Event &event) {
		...
		if ((event.eventType == EVENT_BUTTON_DOWN) && buttonsRect.contains(event.mousePos)) {
			...
			int buttonIndex = ((event.mousePos.y - buttonsRect.top) / 33) * 3 + ((event.mousePos.x - buttonsRect.left) / 33);
			...
			if (_state == 6) {
				// Six buttons pressed
				if ((_indexList[0] == 2) && (_indexList[1] == 3) && (_indexList[2] == 0) && (_indexList[3] == 4) && (_indexList[4] == 1) && (_indexList[5] == 5)) {
