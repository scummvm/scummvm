 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene20::postInit(SceneObjectList *OwnerList) {
		...
		if (g_globals->_sceneManager._previousScene == 30) {
			...
			if ((g_globals->getFlag(120) && g_globals->getFlag(116)) || (g_globals->getFlag(117) && g_globals->getFlag(119))) {
				// Successful evasion
				setAction(&_action3);
	...
	void Scene6100::Action5::dispatch() {
		...
		for (int idx = 0; idx < 4; ++idx) {
			...
			if (tempSet._float2 < 0) {
				...
				if (idx != 3) {
					scene->_objList[idx]->_floats._float1 = g_globals->_randomSource.getRandomNumber(199);
					scene->_objList[idx]->_floats._float2 = g_globals->_randomSource.getRandomNumber(999) + 750.0;
			...
			if ((idx != 3) && (scene->_fadePercent == 100) && (tempSet.sqrt(zeroSet) < 150.0)) {
