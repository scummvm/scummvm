 ...
 *--------------------------------------------------------------------------*/
	...
	bool Scene3150::Toilet::startAction(CursorType action, Event &event) {
		...
		case R2_PILLOW:
			...
			return true;
		case R2_FOOD_TRAY:
			if ((R2_INVENTORY.getObjectScene(R2_LIGHT_BULB) != 3150) && (R2_INVENTORY.getObjectScene(R2_SUPERCONDUCTOR_WIRE) == 3150)
	...
	void Scene3500::dispatch() {
		...
		if ((_mazeChangeAmount == 0) && !_postFixupFl) {
			...
			case MAZEDIR_NORTH:
				...
				if (((cellId == 2) || (cellId == 3) || (cellId == 6) || (cellId == 1)) || ((cellId == 25 || cellId == 26 || cellId == 5 || cellId == 14 || cellId == 15) && deltaX > 3)) {
					R2_GLOBALS._sound2.play(339);
