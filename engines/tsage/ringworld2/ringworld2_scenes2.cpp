 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene2000::process(Event &event) {
		if ((R2_GLOBALS._player._canWalk) && (event.eventType == EVENT_BUTTON_DOWN) && (R2_GLOBALS._events.getCursor() == CURSOR_CROSSHAIRS)) {
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene2455::postInit(SceneObjectList *OwnerList) {
		...
		} else if ((R2_INVENTORY.getObjectScene(R2_ALCOHOL_LAMP_3) == 2455) || (R2_INVENTORY.getObjectScene(R2_ALCOHOL_LAMP_2) == 2455)) {
			_lamp.postInit();
 ...
 *--------------------------------------------------------------------------*/
	...
	bool Scene2900::KnobLeft::startAction(CursorType action, Event &event) {
		...
		switch (action) {
		case CURSOR_USE:
			if (scene->_majorMinorFlag || scene->_altitudeChanging || scene->_xAmount != scene->_xComparison) {
	...
	void Scene2900::postInit(SceneObjectList *OwnerList) {
		...
		if (R2_GLOBALS._sceneManager._previousScene == 2350 && R2_GLOBALS._balloonPosition.x == 0 && R2_GLOBALS._balloonPosition.y == 0) {
