 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene160::Action1::signal() {
		...
		case 1:
			...
			breakFlag = true;
			do {
				if (!scene->_lineNum || ((scene->_lineNum != -1) && (((*scene->_creditsList.reverse_begin())->_position.y < 164) || !breakFlag))) {
					breakFlag = true;
 ...
 */
	void Scene205::processList(Star **ObjList, int count, const Common::Rect &bounds,
		...
		for (int idx = 0; idx < count; ++idx) {
			...
			if ((obj->_position.x <= 319) && (obj->_position.x >= 0) && (obj->_position.y <= 199) && (obj->_position.y >= 0)) {
				if (!pt.x && !pt.y) {
					pt.x = pt.y = 1;
				}
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene300::Action1::signal() {
		...
		switch (_actionIndex) {
		case 0:
			setAction(&scene->_sequenceManager2, this, 311, (R2_GLOBALS._player._characterIndex == R2_QUINN) ? (SceneObject *)&R2_GLOBALS._player : (SceneObject *)&scene->_quinn);
			_actionIndex = 2;
			break;
		case 1:
			setAction(&scene->_sequenceManager2, this, 312, (R2_GLOBALS._player._characterIndex == R2_QUINN) ? (SceneObject *)&R2_GLOBALS._player : (SceneObject *)&scene->_quinn);
			_actionIndex = 0;
			break;
		case 2:
	...
	bool Scene300::Doorway::startAction(CursorType action, Event &event) {
		if (action == CURSOR_USE) {
			if ((R2_GLOBALS._player._characterIndex == R2_QUINN) && (!R2_GLOBALS.getFlag(44) || R2_GLOBALS._player._characterScene[R2_SEEKER] == 500)) {
				Scene300 *scene = (Scene300 *)R2_GLOBALS._sceneManager._scene;
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene325::consoleAction(int id) {
		...
		case 21:
			...
			if (id == 2 || (id == 19 && _priorConsoleAction == 5 && R2_GLOBALS.getFlag(50) && R2_GLOBALS.getFlag(44) && !R2_GLOBALS.getFlag(51))) {
				_icon5.setIcon(13);
				_icon4.setPosition(Common::Point(52, 107));
				_icon4._sceneRegionId = 9;
				_icon4.setIcon(14);
 ...
 *--------------------------------------------------------------------------*/
	...
	bool Scene500::OxygenTanks::startAction(CursorType action, Event &event) {
		...
		case CURSOR_USE:
			if (R2_GLOBALS._player._characterIndex != R2_QUINN) {
				...
				return true;
			} else if ((R2_INVENTORY.getObjectScene(R2_REBREATHER_TANK) != 1) && (R2_GLOBALS._player._characterIndex != R2_SEEKER) && !R2_GLOBALS.getFlag(28)) {
				Scene500 *scene = (Scene500 *)R2_GLOBALS._sceneManager._scene;
	...
	void Scene500::postInit(SceneObjectList *OwnerList) {
		...
		else if ((R2_GLOBALS._player._characterScene[R2_QUINN] != 500) || (R2_GLOBALS._player._characterScene[R2_SEEKER] != 500))
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene825::dispatch() {
		if (R2_GLOBALS._sceneObjects->contains(&_vertLine4) && ((_vertLine4._frame == 1) || (_vertLine4._frame == 3)) && (_vertLine4._frame != _frame1)) {
			_sound2.play(25);
		}

		if (R2_GLOBALS._sceneObjects->contains(&_vertLine1) && (_vertLine1._frame == 3) && (_vertLine1._frame != _frame2)) {
			_sound3.play(26);
		}
	...
	void Scene825::doButtonPress(int buttonId) {
		if ((_menuId != 4) || (buttonId == 5)) {
			...
			case 3:
				...
				if (R2_GLOBALS.getFlag(4)) {
					if ((R2_INVENTORY.getObjectScene(R2_READER) != 800) || (R2_INVENTORY.getObjectScene(R2_OPTICAL_FIBER) != 800)) {
