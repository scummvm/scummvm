 ...
 */
	bool Ringworld2Game::canSaveGameStateCurrently() {
		...
		return g_globals->_gfxManagers.size() == 1 && R2_GLOBALS._animationCtr == 0 && (!R2_GLOBALS._sceneManager._scene || !((SceneExt *)R2_GLOBALS._sceneManager._scene)->_preventSaving);
	}
	...
	void SceneExt::remove() {
		...
		if (R2_GLOBALS._events.getCursor() >= EXITCURSOR_N && R2_GLOBALS._events.getCursor() <= SHADECURSOR_DOWN)
	...
	bool SceneExt::display(CursorType action, Event &event) {
		...
		case R2_SONIC_STUNNER:
			if ((R2_GLOBALS._scannerFrequencies[R2_QUINN] == 2)
			    || ((R2_GLOBALS._scannerFrequencies[R2_QUINN] == 1) && (R2_GLOBALS._scannerFrequencies[R2_SEEKER] == 2) && (R2_GLOBALS._sceneManager._previousScene == 300))) {
 ...
 */
	bool Ringworld2InvObjectList::SelectItem(int objectNumber) {
		...
		case R2_CABLE_HARNESS:
			if (currentItem == R2_CABLE_HARNESS || currentItem == R2_ATTRACTOR_UNIT) {
		...
		case R2_PURE_GRAIN_ALCOHOL:
			if (currentItem == R2_TANNER_MASK || currentItem == R2_PURE_GRAIN_ALCOHOL) {
	...
	void SceneArea::process(Event &event) {
		...
		if (!R2_GLOBALS._insetUp && _enabled && R2_GLOBALS._events.isCursorVisible()) {
			...
			if (_bounds.contains(mousePos)) {
				...
				_insideArea = true;
			} else if ((mousePos.y < 171) && _insideArea && (_cursorNum == cursor) && (_savedCursorNum != CURSOR_NONE)) {
	...
	void MazeUI::draw() {
		...
		_cellsVisible.y = ((_mapOffset.y % _cellSize.y) + _bounds.height() + (_cellSize.y - 1)) / _cellSize.y;
 ...
 */
	...
	AnimationPlayerExt::AnimationPlayerExt()
	  : AnimationPlayer() {
		_isActive = false;
		_canSkip = false;
	}
