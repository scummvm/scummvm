 ...
 *--------------------------------------------------------------------------*/
	...
	bool Scene550::Lyle::startAction(CursorType action, Event &event) {
		...
		switch (action) {
		case CURSOR_TALK:
			if ((BF_INVENTORY.getObjectScene(INV_SCHEDULE) == 1) || (BF_INVENTORY.getObjectScene(INV_9MM_BULLETS) == 1)) {
				if ((BF_INVENTORY.getObjectScene(INV_SCHEDULE) == 1) && (BF_INVENTORY.getObjectScene(INV_9MM_BULLETS) == 1)) {
	...
	bool Scene550::Vechile::startAction(CursorType action, Event &event) {
		...
		case CURSOR_USE:
			...
			} else if ((BF_INVENTORY.getObjectScene(INV_SCHEDULE) == 1) || (BF_INVENTORY.getObjectScene(INV_9MM_BULLETS) == 1)) {
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene560::process(Event &event) {
		if ((event.eventType == EVENT_BUTTON_DOWN) && (BF_GLOBALS._events.getCursor() == CURSOR_WALK) && (_field380) && !_action) {
 ...
 *--------------------------------------------------------------------------*/
	...
	Scene570::PasswordEntry::PasswordEntry()
	  : EventHandler() {
	...
	Scene570::IconManager::IconManager()
	  : EventHandler() {
