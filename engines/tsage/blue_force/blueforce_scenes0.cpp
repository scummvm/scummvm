			...
			if ((BF_GLOBALS._bookmark == bCalledToDrunkStop) && BF_GLOBALS.getFlag(beenToJRDay2))
				...
				case 342:
					...
					if (BF_GLOBALS._driveToScene != 330) {
						...
						scene->_sceneNumber = (BF_GLOBALS._dayNumber != 1) || (BF_GLOBALS._bookmark < bStartOfGame) || (BF_GLOBALS._bookmark >= bCalledToDomesticViolence) ? 342 : 340;
					}
					break;
 ...
 *--------------------------------------------------------------------------*/
	...
	bool Scene60::Compartment::startAction(CursorType action, Event &event) {
		...
		case CURSOR_LOOK:
			...
			break;
		case CURSOR_USE:
			if ((BF_INVENTORY.getObjectScene(INV_TICKET_BOOK) == 1) && (BF_INVENTORY.getObjectScene(INV_MIRANDA_CARD) == 1)) {
	...
	void Scene60::postInit(SceneObjectList *OwnerList) {
		...
		if ((_sceneNumber == 1810) && (BF_GLOBALS._dayNumber > 1) && (BF_GLOBALS._dayNumber < 5) && !BF_GLOBALS.getFlag(fWithLyle) && ((BF_GLOBALS._dayNumber != 4) && (BF_GLOBALS._bookmark >= bEndDayThree))) {
			_car.setup(1810, 1, 1, 164, 131, 1);
		}

		if ((_sceneNumber == 1410) && (BF_GLOBALS._bookmark == bBookedGreen) && !BF_GLOBALS.getFlag(fDriverOutOfTruck)) {
			_object1.postInit();
		...
		switch (BF_GLOBALS._dayNumber) {
		case 1:
			if (BF_GLOBALS.getFlag(onDuty) && (BF_GLOBALS._subFlagBitArr1 & 1) && (BF_GLOBALS._bookmark < bStartOfGame) && (BF_GLOBALS._sceneManager._previousScene != 342)) {
				setAction(&_action2);
			...
			break;
		case 2:
			if (BF_GLOBALS.getFlag(onDuty) && ((BF_GLOBALS._subFlagBitArr1 >> 7) & 1) && (BF_GLOBALS._sceneManager._previousScene != 550) && (BF_GLOBALS._bookmark < bInspectionDone)) {
				setAction(&_action3);
			}
		}
	}
