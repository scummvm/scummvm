 ...
 *--------------------------------------------------------------------------*/
	...
	bool Scene800::Doorway::startAction(CursorType action, Event &event) {
		...
		case CURSOR_USE:
			...
			else if (((BF_INVENTORY.getObjectScene(INV_SCHEDULE) == 1) && (BF_GLOBALS._dayNumber == 3)) || (BF_GLOBALS._bookmark == bDoneWithIsland))
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene810::dispatch() {
		...
		if (BF_GLOBALS._sceneObjects->contains(&_faxMachineInset) && (BF_GLOBALS._player._position.x != 67) && (BF_GLOBALS._player._position.y != 111)) {
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene830::postInit(SceneObjectList *OwnerList) {
		...
		switch (BF_GLOBALS._dayNumber) {
		case 1:
			if ((BF_GLOBALS._bookmark >= bEndOfWorkDayOne) && (BF_INVENTORY.getObjectScene(INV_CARTER_NOTE) != 1) && (BF_INVENTORY.getObjectScene(INV_CARTER_NOTE) != 3)) {
				_object6.postInit();
		...
		case 4:
			...
			break;
		case 5:
			if ((BF_GLOBALS._sceneManager._previousScene == 840) && (BF_INVENTORY.getObjectScene(INV_RENTAL_KEYS) == 1) && !BF_GLOBALS.getFlag(fWithLyle)) {
				_field18A4 = 0;
	...
	void Scene830::signal() {
		...
		case 831:
		case 8306:
			BF_GLOBALS._sceneManager.changeScene((BF_GLOBALS._bookmark < bStartOfGame) || (BF_GLOBALS._bookmark >= bCalledToDomesticViolence) ? 342 : 340);
			break;
		case 832:
	...
	void Scene830::process(Event &event) {
		if (BF_GLOBALS._player._enabled && (event.eventType == EVENT_BUTTON_DOWN) && (BF_GLOBALS._events.getCursor() == CURSOR_WALK) && !_field18A4) {
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene840::signal() {
		...
		case 8403:
			...
			break;
		case 8407:
			if (((BF_GLOBALS._dayNumber == 4) && (BF_GLOBALS._bookmark >= bEndDayThree)) || (BF_INVENTORY.getObjectScene(INV_WAVE_KEYS) == 1)) {
				_sceneMode = 8416;
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene870::postInit(SceneObjectList *OwnerList) {
		...
		if (BF_GLOBALS._dayNumber == 5) {
			if (!BF_GLOBALS.getFlag(fLyleOnIsland) && (BF_GLOBALS._bookmark != bFinishedWGreen) && (!BF_GLOBALS.getFlag(fTookTrailerAmmo) || (BF_GLOBALS._bookmark >= bInvestigateBoat))) {
				_lyle.postInit();
				_lyle.setVisage(835);
				_lyle.setObjectWrapper(new SceneObjectWrapper());
				_lyle.animate(ANIM_MODE_1, NULL);
			...
			if ((BF_INVENTORY.getObjectScene(INV_RAGS) == 0) && (BF_INVENTORY.getObjectScene(INV_FLARE) == 0) && (BF_INVENTORY.getObjectScene(INV_HANDCUFFS) == 355)) {
				_green.postInit();
		...
		default:
			...
			if ((BF_INVENTORY.getObjectScene(INV_HANDCUFFS) != 1) && (BF_INVENTORY.getObjectScene(INV_GRENADES) == 355)) {
				_object4.postInit();
				_object4.hide();
				_object5.postInit();
				_object5.hide();
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene880::postInit(SceneObjectList *OwnerList) {
		...
		if (BF_GLOBALS._dayNumber == 5) {
			...
			if ((BF_GLOBALS._bookmark != bFinishedWGreen) && (BF_GLOBALS._bookmark >= bInvestigateBoat) && BF_GLOBALS.getFlag(fTookTrailerAmmo)) {
