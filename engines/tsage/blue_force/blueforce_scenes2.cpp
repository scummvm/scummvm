 ...
 *--------------------------------------------------------------------------*/
	...
	bool Scene270::Grandma::startAction(CursorType action, Event &event) {
		...
		case CURSOR_TALK:
			...
			case 1:
				...
				} else if (BF_GLOBALS.getFlag(onDuty) || (BF_INVENTORY.getObjectScene(INV_BASEBALL_CARD) == 2) || (scene->_field386 != 0)) {
	...
	void Scene270::postInit(SceneObjectList *OwnerList) {
		...
		if (((BF_GLOBALS._bookmark >= bLauraToParamedics) && (BF_GLOBALS._dayNumber == 1) && (BF_INVENTORY.getObjectScene(INV_BASEBALL_CARD) != 2) && (BF_INVENTORY.getObjectScene(INV_BASEBALL_CARD) != 1)) || ((BF_GLOBALS._dayNumber == 3) && BF_GLOBALS.getFlag(fGotGreen355fTalkedToGrannyDay3))) {
	...
	void Scene270::process(Event &event) {
		if ((event.eventType == EVENT_BUTTON_DOWN) && (BF_GLOBALS._events.getCursor() == CURSOR_WALK) && (_field380 == 1) && !_action) {
		...
		if (BF_GLOBALS._player._enabled && !_focusObject && (event.mousePos.y < (UI_INTERFACE_Y - 1)) && !_field384 && !_field386) {
 ...
 *--------------------------------------------------------------------------*/
	...
	void Scene271::process(Event &event) {
		if ((event.eventType == EVENT_BUTTON_DOWN) && (BF_GLOBALS._events.getCursor() == CURSOR_WALK) && (_field796 == 1) && (!_action)) {
			_tempPos = event.mousePos;
			BF_GLOBALS._player.disableControl();
