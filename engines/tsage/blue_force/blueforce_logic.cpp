 ...
 */
	...
	void FollowerObject::dispatch() {
		...
		if ((_object->_flags & OBJFLAG_HIDE) || ((_object->_visage != 307) && ((_object->_visage != 308) || (_object->_strip != 1)))) {
			hide();
	...
	void FocusObject::process(Event &event) {
		if (BF_GLOBALS._player._enabled) {
			if (_bounds.contains(event.mousePos)) {
				...
				if ((event.eventType == EVENT_BUTTON_DOWN) && (BF_GLOBALS._events.getCursor() == CURSOR_WALK) && (event.btnState == BTNSHIFT_RIGHT)) {
 ...
 */
	...
	void SceneMessage::process(Event &event) {
		if ((event.eventType == EVENT_BUTTON_DOWN) || ((event.eventType == EVENT_KEYPRESS) && (event.kbd.keycode == Common::KEYCODE_RETURN))) {
			signal();
		}
	}
