...
MenuRecord::MenuRecord(const MenuRecordBounds *bounds, int numParams, ...) {
	...
	// Store position data
	_hsxstart = bounds->left;
	_hsxend = bounds->right;
...
const char *MenuRecord::getEntry(uint8 index) {
	if (index >= _numEntries)
		error("Invalid menuitem index specified: %d", index);
...
uint8 Menu::execute() {
	...
	while (mouse.lButton() || mouse.rButton()) {
		while (events.pollEvent()) {
			...
			if (index != _selectedIndex) {
				if (_selectedIndex != 0)
					toggleHighlightItem(_selectedIndex);
	...
	if ((_selectedMenu == NULL) || (_selectedIndex == 0))
		return MENUITEM_NONE;
...
uint8 Menu::getIndexAt(uint16 x, uint16 y) {
	...
	if (index > _selectedMenu->numEntries())
		index = _selectedMenu->numEntries();
	return index;
}
...
void Menu::toggleHighlightItem(uint8 index) {
	...
	const byte *colors = LureEngine::getReference().isEGA() ? &colorList[0] : &colorList[2];
	byte *p = _surfaceMenu->data().data() + (Surface::textY() + ((index - 1) * FONT_HEIGHT)) * _surfaceMenu->width() + Surface::textX();
...
uint16 PopupMenu::ShowItems(Action contextAction, uint16 roomNumber) {
	...
	for (ir = rooms.begin(); ir != rooms.end(); ++ir) {
		...
		if (numItems == MAX_NUM_DISPLAY_ITEMS)
			error("Out of space in ask list");
	...
	for (ih = hotspots.begin(); ih != hotspots.end(); ++ih) {
		...
		if ((hotspot.headerFlags != 15) && ((hotspot.headerFlags & fields.hdrFlagMask()) == 0))
			continue;
		...
		if (((hotspot.flags & HOTSPOTFLAG_ROOM_SPECIFIC) != 0) && (hotspot.roomNumber != roomNumber))
			continue;
		...
		while (e.pollEvent()) {
			if (engine.shouldQuit()) {
				...
				if (((keycode == Common::KEYCODE_KP8) || (keycode == Common::KEYCODE_UP)) && (selectedIndex > 0)) {
					--selectedIndex;
					refreshFlag = true;
				} else if (((keycode == Common::KEYCODE_KP2) || (keycode == Common::KEYCODE_DOWN)) && (selectedIndex < numEntries - 1)) {
					++selectedIndex;
					refreshFlag = true;
...
#else
			} else if ((e.type() == Common::EVENT_LBUTTONDOWN) || (e.type() == Common::EVENT_MBUTTONDOWN)) {
				//mouse.waitForRelease();
				bailOut = true;
				break;
#endif
