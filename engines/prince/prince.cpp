...
void PrinceEngine::setTTSVoice(uint8 textColor) const {
	...
	if (ttsMan != nullptr && (ConfMan.getBool("tts_enabled_speech") || ConfMan.getBool("tts_enabled_objects") || ConfMan.getBool("tts_enabled_missing_voice"))) {
		...
		for (int i = 0; i < kNumberOfCharacterDatas; ++i) {
			...
			if (characterVoiceDatas[i].textColor == textColor && 
				(characterVoiceDatas[i].locationNumber == 0 || characterVoiceDatas[i].locationNumber == _locationNr) &&
				(characterVoiceDatas[i].mobIndex == -1 || characterVoiceDatas[i].mobIndex == _dialogMob)) {
				id = i;
				break;
			}
		}
