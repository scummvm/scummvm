...
RouteItem *LinesManager::findRoute(int fromX, int fromY, int destX, int destY) {
	...
	if (bestRouteNum == 2) {
		...
		case DIR_DOWN:
			for (int deltaY = 0; deltaY < delta; deltaY++) {
				if (checkCollisionLine(fromX, deltaY + fromY, &foundDataIdx, &foundLineIdx, _lastLine + 1, _linesNumb) && _lastLine < foundLineIdx) {
...
int LinesManager::getMouseZone() {
	...
	if ((_vm->_events->_mousePos.y + _vm->_events->_mouseOffset.y) > 19) {
		...
		for (int squareZoneId = 0; squareZoneId <= 99; squareZoneId++) {
			if (_zone[squareZoneId]._enabledFl && _squareZone[squareZoneId]._enabledFl && _squareZone[squareZoneId]._left <= xp && _squareZone[squareZoneId]._right >= xp && _squareZone[squareZoneId]._top <= yp && _squareZone[squareZoneId]._bottom >= yp) {
...
void LinesManager::checkZone() {
	...
	if (_vm->_globals->_cityMapEnabledFl || _vm->_events->_startPos.x >= mouseX || (mouseY = _vm->_graphicsMan->_scrollOffset + 54, mouseX >= mouseY) || (mouseY = oldMouseY - 1, mouseY < 0 || mouseY > 59)) {
