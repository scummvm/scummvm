...
parse_rec *make_parserec(int obj, parse_rec *rec) {
	if (rec == NULL)
		rec = (parse_rec *)rmalloc(sizeof(parse_rec));
	rec->obj = obj;
	rec->info = D_NOUN;
...
static const char *it_sdesc(int item) {
	if (tnoun(item))
		return noun[item - first_noun].shortdesc;
	if (tcreat(item))
		return creature[item - first_creat].shortdesc;
	if (item < 0)
		return dict[-item];
	return NULL;
}
...
static rbool it_invisible(int item, rbool sdesc_flag) {
	if (sdesc_flag)
		...
		if (it_name(item) == 0 && it_adj(item) == 0)
			return 1;
...
static rbool it_appears_empty(int item) {
	...
	contloop(i, item) if (!it_invisible(i, sdesc_flag)) return 0;
	return 1;
}
...
rbool it_isweapon(int objnum) {
	...
	creatloop(i) if (matchclass(objnum, creature[i].weapon)) return 1;
	return 0;
}
...
rbool it_door(int obj, word nword) {
	if (aver >= AGX00)
		return 0; /* No doors under Magx */
	if (tdoor(obj))
		return 1; /* The basic door */
	if (it_loc(obj) == loc + first_room)
		return 0;
...
static void set_contents(int p, int newval) {
	if (troom(p))
		room[p - first_room].contents = newval;
	else if (p == 1)
		player_contents = newval;
	else if (p == 1000)
		player_worn = newval;
	else if (tnoun(p))
		noun[p - first_noun].contents = newval;
	else if (tcreat(p))
		creature[p - first_creat].contents = newval;
	else {
...
static void set_next(int p, int newval) {
	if (tnoun(p))
		noun[p - first_noun].next = newval;
	else if (tcreat(p))
		creature[p - first_creat].next = newval;
	else {
...
void it_reposition(int item, int newloc, rbool save_pos) {
	...
	nounloop(i) if (noun[i].nearby_noun == item) {
...
defined in exec.h */
{
	...
	if (obj2 == 1 || (aver > AGTME15 && aver < AGX00)) {
		...
		if (obj2 == 1) {
			if (is_within(obj1, 1, 0) || is_within(obj1, 1000, 0))
				net = 0;
			net += contweight(1);
			if (aver >= AGX00)
				net += contweight(1000);
	...
	if (obj2 == 1 || aver > AGTME15) {
		/* Pre-ME/1.56 interpreters didn't check this except for the player's
		   inventory */
		if (it_loc(obj1) == obj2 || (aver < AGX00 && is_within(obj1, obj2, 0)))
			net = 0;           /* Avoid double-counting */
   ...
   problems. */
{
	...
	if (it_isglobal(item))
		return 1; /* Global objects always in scope. */

	/* Flag objects in scope if their associated flag is set. */
	tmp = it_flagnum(item);
	if (tmp &&
	    (room[loc].flag_noun_bits & (1L << (tmp - 1))))
		return 1;
	...
	if (curloc == 1 || curloc == 1000 || curloc == loc + first_room)
		return 1;
	else
		return 0;
}
...
on is the valid light */
{
	...
	if (roomlight > 1) {
		if (!matchclass(first_noun + obj, roomlight))
			...
			return 1; /* The correct light _always_ illuminates the room */
	}
	if (!active)
		return 1;
...
active=0 indicates that extinguished light sources are okay. */
{
	...
	contloop(i, parent) {
		if (tnoun(i) && good_light(i - first_noun, roomlight, active))
			return 1;
...
rbool islit(void) {
	if (room[loc].light == 0)
		return 1;
	if (lightcheck(loc + first_room, room[loc].light, 1))
		return 1;
	if (lightcheck(1, room[loc].light, 1))
		return 1; /* Player carried light */
	if (lightcheck(1000, room[loc].light, 1))
		return 1; /* Worn light */
	return 0;
}
...
rbool genvisible(parse_rec *dobj_) {
	...
	if (dobj_->obj > 0)
		return visible(dobj_->obj);

	if (dobj_->info == D_INTERN) {
		if (dobj_->obj != -ext_code[wdoor])
			return 1;
		return islit(); /* If item is a is a door */
	}
	if (dobj_->info == D_GLOBAL || dobj_->info == D_NUM)
		return 1;
	if (dobj_->info == D_FLAG) {
		for (i = 0; i < MAX_FLAG_NOUN; i++) /* Flag nouns */
			if (flag_noun[i] != 0 && dobj_->obj == -flag_noun[i] && (room[loc].flag_noun_bits & (1L << i)) != 0)
				return 1;
		return 0;
	}
	if (dobj_->info == D_PIX) {
...
static integer find_related(word w) {
	...
	if (w == 0)
		return 0;
	item = 0;
	nounloop(i) if (noun[i].name == w) {
		if (visible(i + first_noun))
			return i + first_noun;
		else if (item == 0)
			item = i + first_noun;
	}
	creatloop(i) if (creature[i].name == w) {
		if (visible(i + first_creat))
...
static void add_to_scope(integer item) {
	...
	if (tnoun(item)) {
		...
		if (i != 0) {
			if (tnoun(i))
				noun[i - first_noun].scope = 1;
			else if (tcreat(i))
				creature[i - first_creat].scope = 1;
		}
	} else if (tcreat(item))
		creature[item - first_creat].scope = 1;
...
void compute_seen(void) {
	...
	nounloop(i)
	    noun[i]
	        .seen = noun[i].seen || noun[i].scope;
	creatloop(i)
	    creature[i]
	        .seen = creature[i].seen || creature[i].scope;
}
...
void recompute_score(void) {
	...
	tscore -= objscore;
	objscore = 0;
	nounloop(obj) if (noun[obj].points && !noun[obj].unused &&
	                  (visible(obj + first_noun) || is_within(obj + first_noun, treas_room, 0)))
	    objscore += noun[obj].points;
	creatloop(obj) if (!creature[obj].unused && creature[obj].points && visible(obj + first_creat))
...
/* Gets nth word associated with item */
{
	if (n == 1) {
		if (item < 0)
			...
			return noun[item - first_noun].adj;
		else if (tcreat(item))
			return creature[item - first_creat].adj;
	}
	if (n == 2)
		if (tnoun(item) || tcreat(item))
			return it_name(item);
...
void goto_room(int newroom) {
	...
	safecontloop(i, j, loc + first_room) if (it_group(i))
	...
	loc = newroom;
	if (loc != newroom)
		oldloc = loc; /* No backtracking unless v_go allows it */
	if (!room[loc].seen) {
		room[loc].seen = 1;
...
static void rundesc(int i, descr_ptr dp_[], const char *shortdesc, int msgid) {
	if (dp_[i].size > 0)
		...
		sysmsg(msgid, "$You$ see nothing unexpected.");
}
...
void it_describe(int dobj_) {
	if (troom(dobj_))
		...
		if (room[loc].locked_door)
			...
			sysmsg(22, "$You$ see a perfectly normal doorway.");
	} else
		sysmsg(194, "$You$ see nothing unexpected.");
	if (tnoun(dobj_) &&
...
/* Return 1 if we actually printed something, 0 if obj is invisible */
{
	...
	if (sdesc_flag && tnoun(obj) && noun[obj - first_noun].initdesc != 0) {
		...
		if (tnoun(obj) && noun[obj - first_noun].pos_prep != 0) {
			...
			writestr(")");
		} else if (parent >= first_noun && it_invisible(parent, sdesc_flag) && (it_name(parent) != 0 || it_adj(parent) != 0)) {
		...
		if (tnoun(obj) && noun[obj - first_noun].light && noun[obj - first_noun].on && PURE_OBJ_DESC)
...
/* Returns number of objects contained in obj that were listed */
{
	...
	contloop(i, obj) {
		if (print_obj(i, ind_lev))
			cnt++;
		if (it_open(i))
			print_contents(i, ind_lev + 1);
	}
	return cnt;
}
...
int it_contents(integer obj) {
	if (tnoun(obj))
		return noun[obj - first_noun].contents;
	else if (troom(obj))
		return room[obj - first_room].contents;
	else if (tcreat(obj))
		return creature[obj - first_creat].contents;
	else if (obj == 1)
...
rbool it_lockable(integer obj, word nword) {
	if (tnoun(obj))
		return noun[obj - first_noun].lockable;
	else if (it_door(obj, nword))
		return 1;
	else
		return 0;
}
...
rbool it_locked(integer obj, word nword) {
	if (tnoun(obj))
		return noun[obj - first_noun].locked;
	else if (it_door(obj, nword) && room[loc].locked_door)
		return 1;
	else
		return 0;
}
