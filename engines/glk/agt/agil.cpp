...
static void print_title(fc_type fc) {
	...
	if (buff == NULL) {
		...
		for (height = skip_line; buff[height] != NULL; height++)
			if (!emptyline((uchar *)buff[height])) {
				if (rspace(buff[height][0]))
					center_mode++;
				else
					center_mode--;
			}

		if (box_title || aver == AGTCOS)
			center_mode = TB_CENTER;
		else /* includes aver==AGT135 */
		    if (center_mode <= 0)
			center_mode = TB_CENTER;
		else
			center_mode = TB_NOCENT;

		if (!bold_mode)
			agt_textcolor(-1);
		agt_clrscr();
		textbox(buff + skip_line, height - skip_line, center_mode | (bold_mode ? 0 : TB_BOLD) | TB_TTL | (box_title ? TB_BORDER : 0));
		if (!bold_mode)
	...
	if (height <= screen_height - 5)
		sprintf(s, "AGiliTy: "
		           "The (Mostly) Universal AGT Interpreter  %s",
		        version_str);
	else
...
void agil_option(int optnum, char *optstr[], rbool setflag, rbool lastpass) {
	if (opt("ibm_char"))
		fix_ascii_flag = !setflag;
	else if (!lastpass)
		return; /* On the first pass through the game specific
                 file, we ignore all but the above options */
	else if (opt("tone"))
		PURE_TONE = setflag;
	else if (opt("input_bold"))
		PURE_INPUT = setflag;
	else if (opt("force_load"))
		FORCE_VERSION = setflag;
...
static rbool noise_word(word w) {
	...
	if (w == ext_code[wmy])
		return 1;
	if (aver >= AGT18 && aver <= AGT18MAX && w == ext_code[wis])
		return 1;
	return 0;
}
...
/* lookahead=the rest of the current input line *after* the period. */
{
	...
	if (!PURE_DOT)
		return 1; /* No words with periods in them, so it must
                   be punctuation. */
	...
	for (i = 0; i < dp; i++) {
		...
		if (restcnt > endword)
			continue; /* Dictionary entry is longer than
                       following text */

		/* Check to see if the dictionary entry can be found in the lookahead
		   buffer */
		if (strncasecmp(lookahead, dict[i] + prevcnt + 1, restcnt) != 0)
			continue;
	...
	for (k = 0;; k++) {
		...
		if (buff[k] != 0 && !isspace(buff[k]) && !punctuation) {
			...
			if (input[ip_] == -1)
				...
				input[ip_] = ext_code[w_any]; /* _Real_ 'ANY' */
			if (!noise_word(input[ip_]))
				ip_ += 1;
		...
		if (curr_lives > 1) { /* Resurrection code */
			if (curr_lives == max_lives)
				gen_sysmsg(151, "Hmmm.... so $you$'ve gotten $your$self killed. "
				                "Would you like me to try a resurrection?",
				           MSG_MAIN, NULL);
			else
				gen_sysmsg(152, "<Sigh>  $You$'ve died *again*. "
	...
	while (!done_flag && !quitflag) {
		...
		if (strncasecmp(s, "RESTART", 7) == 0)
			if (restart_state != NULL) {
				...
				writeln("Sorry, I'm unable to do that because of limited memory.");
		else if (strncasecmp(s, "RESTORE", 7) == 0)
			if (g_vm->loadGame().getCode() == Common::kNoError) {
				done_flag = 1;
			} else
				writeln("(RESTORE failed)");
 ...
 commands */
{
	for (ip = 0; ip >= 0 && ip < MAXINPUT && input[ip] != -1;) {
		if (!parse() || quitflag || winflag || deadflag || endflag)
			break;
		if (doing_restore)
			break;
...
static void mainloop(void) {
	...
	while (!quitflag) {
		...
		if (!menu_mode) {
			...
			agt_newline();
			if (!doing_restore)
				tokenise(s); /* Tokenizes into input */
			rfree(s);
			if (!doing_restore)
				parse_loop();
		} else
...
static int init(void) {
	...
	for (i = 0; i < last_cmd; i++) {
		if (command[i].nouncmd == -1 || command[i].objcmd == -1 || command[i].noun_adj == -1 || command[i].obj_adj == -1 || command[i].prep == -1)
			nomatch_aware = 1;
		if (command[i].verbcmd == ext_code[wlook] &&
		    (command[i].nouncmd > 0 || command[i].noun_adj > 0 || command[i].objcmd > 0 || command[i].obj_adj > 0 || command[i].prep > 0))
			smart_look = 0;
	}
