...
namespace Scott {
	...
	Scott::Scott(OSystem *syst, const GlkGameDescription &gameDesc)
		...
		while (!shouldQuit()) {
			...
			if (_items[LIGHT_SOURCE]._location != DESTROYED && _gameHeader._lightTime != -1) {
				...
				if (_gameHeader._lightTime < 1) {
					...
					if (_items[LIGHT_SOURCE]._location == CARRIED || _items[LIGHT_SOURCE]._location == MY_LOC) {
	...
	int Scott::matchUpItem(const char *text, int loc) {
		...
		while (ct <= _gameHeader._numItems) {
			if (!_items[ct]._autoGet.empty() && _items[ct]._location == loc && xstrncasecmp(_items[ct]._autoGet.c_str(), word, _gameHeader._wordLength) == 0)
				return ct;
			ct++;
		}
			...
			if (act[cc] >= 1 && act[cc] < 52) {
				...
				case 65: {
					...
					while (i <= _gameHeader._numItems) {
						if (_items[i]._location == _gameHeader._treasureRoom && _items[i]._text.hasPrefix("*"))
							n++;
						i++;
					}
					if (_options & YOUARE)
	...
	int Scott::performActions(int vb, int no) {
		...
		if (vb == 1 && no >= 1 && no <= 6) {
			int nl;
			if (_items[LIGHT_SOURCE]._location == MY_LOC || _items[LIGHT_SOURCE]._location == CARRIED)
				d = 0;
			if (d)
