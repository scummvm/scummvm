...
void freeStrings() {
	...
	if (_G(_wordsInInput) == 0) {
		if (_G(_unicodeWords) != nullptr || _G(_charWords) != nullptr) {
			g_vm->fatal("ERROR! Wordcount 0 but word arrays not empty!\n");
		}
		return;
	}
...
void printPendingError(void) {
	if (_G(_firstErrorMessage)) {
		g_vm->glk_put_string_stream_uni(g_vm->glk_window_get_stream(_G(_bottomWindow)), _G(_firstErrorMessage));
		delete _G(_firstErrorMessage);
		_G(_firstErrorMessage) = nullptr;
		_G(_stopTime) = 1;
	}
...
char **LineInput(void) {
	...
	do {
		...
		if (_G(_transcript)) {
			g_vm->glk_put_string_stream_uni(_G(_transcript), unibuf);
			g_vm->glk_put_char_stream_uni(_G(_transcript), 10);
		}

		_G(_charWords) = splitIntoWords(unibuf, ev.val1);

		if (_G(_wordsInInput) == 0 || _G(_charWords) == nullptr)
			g_vm->output(_G(_sys)[HUH]);
		else {
...
char **splitIntoWords(glui32 *string, int length) {
	...
	g_vm->glk_buffer_to_lower_case_uni(string, 256, length);
	g_vm->glk_buffer_canon_normalize_uni(string, 256, length);
...
int findExtraneousWords(int *index, int noun) {
	...
	if (list == _G(_nouns) && noun) {
		if (g_vm->mapSynonym(noun) == g_vm->mapSynonym(verb)) {
			*index = *index + 1;
			return 0;
		}
	}
...
int createAllCommands(Command *command) {
	...
	struct Command *next = command->_next;
	...
	while (next && next->_verb == _G(_gameHeader)._numWords + EXCEPT) {
		for (int i = 0; i <= _G(_gameHeader)._numItems; i++) {
			if (!_G(_items)[i]._autoGet.empty() && xstrncasecmp(_G(_items)[i]._autoGet.c_str(), _G(_charWords)[next->_nounWordIndex], _G(_gameHeader)._wordLength) == 0) {
...
int whichWord(const char *word, Common::StringArray list, int wordLength) {
	...
	while (ne < list.size()) {
		...
		if (*tp == '*')
			...
			n = ne;
		if (xstrncasecmp(word, tp, wordLength) == 0)
			return (n);
		ne++;
	}
	return (0);
