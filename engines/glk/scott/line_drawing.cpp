...
void diamondFill(uint8_t x, uint8_t y, int colour) {
	uint8_t buffer[2048];
...
void drawVectorPicture(int image) {
	...
	if (_G(_vectorImageShown) == image) {
		if (_G(_vectorState) == SHOWING_VECTOR_IMAGE) {
			...
			if (_G(_gliSlowDraw))
				g_vm->glk_request_timer_events(20);
	...
	uint8_t opcode = 0;
	while (p - _G(_lineImages)[image]._data < _G(_lineImages)[image]._size && opcode != 0xff) {
