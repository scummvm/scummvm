...
#pragma mark Rooms
	...
	do {
		string_length = *(ptr++);
		if (string_length == 0) {
			_G(_rooms)[ct]._text = ".\0";
		} else {
			for (int i = 0; i < string_length; i++) {
				_G(_rooms)[ct]._text += *(ptr++);
			}
		}
		ct++;
	} while (ct < nr + 1);
...
#pragma mark Items
	...
	do {
		string_length = *(ptr++);
		if (string_length == 0) {
			_G(_items)[ct]._text = ".\0";
		} else {
			for (int i = 0; i < string_length; i++) {
				_G(_items)[ct]._text += *(ptr++);
			}
			const char *p = strchr(_G(_items)[ct]._text.c_str(), '/');
			if (p) {
				_G(_items)[ct]._autoGet = Common::String(p);

				/* Some games use // to mean no auto get/drop word! */
				if (!_G(_items)[ct]._autoGet.hasPrefix("//") && !_G(_items)[ct]._autoGet.hasPrefix("/*")) {
					_G(_items)[ct]._text = Common::String(_G(_items)[ct]._text.c_str(), p);
					_G(_items)[ct]._autoGet.deleteChar(0);

					const char *t = strchr(_G(_items)[ct]._autoGet.c_str(), '/');
					if (t) {
						_G(_items)[ct]._autoGet = Common::String(_G(_items)[ct]._autoGet.c_str(), t);
					}
					ptr++;
				}
			}
