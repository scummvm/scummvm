...
void loadVectorData(GameInfo info, uint8_t *ptr) {
	...
	do {
		...
		do {
			byte = *(ptr++);
			if (ptr - _G(_entireFile) >= _G(_fileLength)) {
				error("Error! Image data for image %d cut off!\n", ct);
				if (_G(_gameHeader)._numRooms - ct > 1)
					g_vm->display(_G(_bottomWindow), "[This copy has %d broken or missing pictures. These have been patched out.]\n\n", _G(_gameHeader)._numRooms - ct);
				if (lp->_data >= ptr)
					lp->_size = 0;
				else
...
int tryLoading(GameInfo info, int dictStart, int loud) {
	...
	if (loud)
		debug("Offset after reading actions: %lx\n", static_cast<int>(ptr - _G(_entireFile) - _G(_fileBaselineOffset)));
	...
	if (loud)
		debug("Offset after reading dictionary: %lx\n", static_cast<int>(ptr - _G(_entireFile) - _G(_fileBaselineOffset)));
...
jumpSysMess:
	...
	do {
		c = *(ptr++);
		if (ptr - _G(_entireFile) > _G(_fileLength) || ptr < _G(_entireFile)) {
	...
	if (loud)
		debug("Offset after reading system messages: %lx\n", static_cast<int>(ptr - _G(_entireFile)));
