...
namespace Alan3 {
	...
	GlkIO::GlkIO(OSystem *syst, const GlkGameDescription &gameDesc)
	  : GlkAPI(syst, gameDesc)
	  , glkMainWin(nullptr)
	  , glkStatusWin(nullptr)
	  , onStatusLine(false)
	  , _saveSlot(-1)
	...
	void GlkIO::showImage(int image, int align) {
		...
		if ((glk_gestalt(gestalt_Graphics, 0) == 1) && (glk_gestalt(gestalt_DrawImage, wintype_TextBuffer) == 1)) {
