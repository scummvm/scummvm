...
uint objgetp(mcmcxdef *mctx, objnum objn, prpnum prop, dattyp *typptr) {
	...
	if (objflg(objptr) & OBJFINDEX) {
		...
		while (retval && ((prpflg(objptr + retval) & PRPFIGN) != 0 || ((prpflg(objptr + retval) & PRPFDEL) != 0 && (mctx->mcmcxflg & MCMCXF_NO_PRP_DEL) == 0)) && cur < cnt && indp[0] == indp[4] && indp[1] == indp[5]) {
			indp += 4;
			retval = osrp2(indp + 2);
		}
	...
	if (!retval)
		retval = ignprop; /* use ignored value if nothing else */
	if (retval && typptr)
		*typptr = prptype(objofsp(objptr, retval));
...
void objdelp(mcmcxdef *mctx, objnum objn, prpnum prop, int mark_only) {
	...
	if (!pofs)
		return; /* not defined - nothing to delete */
...
void objsetp(mcmcxdef *ctx, objnum objn, prpnum prop, dattyp typ,
	...
	ERRBEGIN(ctx->mcmcxgl->mcmcxerr) {
		...
		if (undoctx && objuok(undoctx)) {
			...
			if (p) {
				if (prpflg(p) & PRPFORG) {
					...
					cmd = OBJUCHG; /* change property */
			} else
				cmd = OBJUADD; /* prop didn't exist - adding it */

			/* write header, reserve space, and get a pointer to the space */
			up = objures(undoctx, cmd,
			             (ushort)(sizeof(mcmon) + sizeof(prpnum) + (p ? PRPHDRSIZ + prpsize(p) : 0)));
		...
		prptype(p) = typ;
		if (siz != 0)
			memcpy(prpvalp(p), val, (size_t)siz);
	}
...
uchar *objures(objucxdef *undoctx, uchar cmd, ushort siz) {
	...
	if (siz > undoctx->objucxsiz)
		errsig(undoctx->objucxerr, ERR_UNDOVF);
...
void obj1undo(mcmcxdef *mctx, objucxdef *undoctx) {
	...
	if (cmd == OBJUSAV)
		return; /* savepointer marker - nothing to do */
...
void objsetign(mcmcxdef *mctx, objnum objn) {
	...
	for (mut = (prpdef *)(objptr + objrst(objptr)),
	    cnt = objnprop(objptr) - objstat(objptr);
	     cnt;
	     mut = objpnxt(mut), --cnt) {
		...
		if (icnt) {
			...
			for (;;) {
				...
				if (indp[0] == p[0] && indp[1] == p[1])
					break;
				else if (indp[0] < p[0] || (indp[0] == p[0] && indp[1] < p[1]))
					first = (cur == first ? first + 1 : cur);
				else
					last = (cur == last ? last - 1 : cur);
			}
			...
			while (cur < icnt && (indp[0] <= p[0] || (indp[0] == p[0] && indp[1] <= p[1]))) {
				indp += 4;
				++cur;
			}
