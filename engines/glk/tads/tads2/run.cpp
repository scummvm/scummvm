...
static void run_delete(runcxdef *ctx, uchar *noreg *codepp,
	...
	if (vctx->voccxit == objn)
		vctx->voccxit = MCMONINV;
	if (vctx->voccxhim == objn)
		vctx->voccxhim = MCMONINV;
	if (vctx->voccxher == objn)
		vctx->voccxher = MCMONINV;
	...
	if (vctx->voccxlsd.vocolobj == objn || vctx->voccxlsi.vocolobj == objn || vctx->voccxlsa == objn || vctx->voccxlsv == objn || vctx->voccxlsp == objn) {
		/* forget the verb */
...
#undef CHECK_VAL
		...
		if (ref) {
			if (hp != dst)
				memmove(dst, hp, (size_t)osrp2(hp));
			dst += osrp2(dst);
		}
	}
...
int runeq(runcxdef *ctx) {
	...
	case DAT_SSTRING:
	case DAT_LIST:
		return (osrp2(val1.runsv.runsvstr) == osrp2(val2.runsv.runsvstr) && !memcmp(val1.runsv.runsvstr, val2.runsv.runsvstr,
...
int runmcmp(runcxdef *ctx) {
	if (runtostyp(ctx) == DAT_NUMBER) {
		...
		while (len1 && len2) {
			if (*str1 < *str2)
				return (-1); /* character from 1 is greater */
			else if (*str1 > *str2)
				return (1); /* char from 1 is less */
		...
		if (len1)
			return (1); /* match up to len2, but string 1 is longer */
		else if (len2)
			return (-1); /* match up to len1, but str2 is longer */
		else
...
static uint runindofs(runcxdef *ctx, uint indx, uchar *lstp) {
	...
	for (--indx; indx && lstsiz; --indx)
		lstadv(&lstp, &lstsiz);

	/* if we ran out of list, the index is out of range */
	if (!lstsiz)
		runsig(ctx, ERR_HIGHINX);
...
resume_from_error:
	...
	for (brkchk = 0;; ++brkchk) {
		...
		case OPCSWITCH: {
			...
			for (match = FALSE; i && !match; --i) {
				...
				switch (*p++) {
				case OPCPUSHNUM:
					match = (typmatch && val.runsv.runsvnum == osrp4s(p));
					p += 4;
					break;

				case OPCPUSHLST:
				case OPCPUSHSTR:
					match = (typmatch && osrp2(val.runsv.runsvstr) == osrp2(p) && !memcmp(val.runsv.runsvstr, p, (size_t)osrp2(p)));
					p += runrp2s(p);
					break;

				case OPCPUSHPN:
					match = (typmatch && val.runsv.runsvprp == osrp2(p));
					p += 2;
					break;

				case OPCPUSHOBJ:
				case OPCPUSHFN:
					match = (typmatch && val.runsv.runsvobj == osrp2(p));
					p += 2;
					break;
			...
			if (!match)
				p += 2;      /* if default, skip to default case */
		...
		default:
			if ((opc & OPCASI_MASK) == OPCASI_MASK) {
				...
				case OPCASIMUL:
					if (val.runstyp != DAT_NUMBER || val2.runstyp != DAT_NUMBER)
				...
				case OPCASIEXT:
					...
					case OPCASIBAND:
						if ((val.runstyp == DAT_TRUE || val.runstyp == DAT_NIL) && (val2.runstyp == DAT_TRUE || val2.runstyp == DAT_NIL)) {
							int a, b;
	...
	*/
	ERRCATCH(ctx->runcxerr, err) {
		...
		*/
		if (!dbgpresent() || ctx->runcxdbg->dbgcxfcn == 0 || !dbgu_err_resume(ctx->runcxdbg) || (ctx->runcxdbg->dbgcxflg & DBGCXFIND) != 0)
			errrse(ctx->runcxerr);
...
void runsign(runcxdef *ctx, int err) {
	...
	*/
	if (ctx->runcxdbg->dbgcxfcn == 0 || !dbgu_err_resume(ctx->runcxdbg))
