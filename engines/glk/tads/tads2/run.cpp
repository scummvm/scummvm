		...
		uchar *runfind(uchar *lst, runsdef *item) {
			...
			for (len = osrp2(lst) - 2, lst += 2; len; lst += curlen, len -= curlen) {
				if (*lst == item->runstyp) {
					...
					case DAT_LIST:
					case DAT_SSTRING:
						if (osrp2(lst + 1) == osrp2(item->runsv.runsvstr) && !memcmp(lst + 1, item->runsv.runsvstr, (size_t)osrp2(lst + 1)))
							return (lst);
						break;
					case DAT_NUMBER:
					...
					for (; rem1; lstadv(&p1, &rem1)) {
						...
						for (; rem2; lstadv(&p2, &rem2)) {
							if (*p1 == *p2) {
								...
								if (siz1 == siz2 && (siz1 == 0 || !memcmp(p1 + 1, p2 + 1, (size_t)siz1))) {
									found = TRUE;
									break;
								}
							}
		...
		resume_from_error:
			...
			for (brkchk = 0;; ++brkchk) {
				...
				*/
				...
				case OPCJSF:
					if (runtostyp(ctx) == DAT_NIL || (runtostyp(ctx) == DAT_NUMBER && (ctx->runcxsp - 1)->runsv.runsvnum == 0))
						p += runrp2s(p);
					else {
						runpop(ctx, &val);
						p += 2;
