...
#define runpopnum(ctx) \
	(runstkund(ctx), ((--((ctx)->runcxsp))->runstyp != DAT_NUMBER ? (runsig(ctx, ERR_REQNUM), (long)0) : ((ctx)->runcxsp->runsv.runsvnum)))

/* pop an object, signalling an error if not an object */
/* objnum runpopobj(runcxdef *ctx); */
#define runpopobj(ctx) \
	(runstkund(ctx), ((--(ctx)->runcxsp))->runstyp != DAT_OBJECT ? (runsig(ctx, ERR_REQVOB), (objnum)0) : ((ctx)->runcxsp->runsv.runsvobj))

/* pop an object or nil - returns MCMONINV if the value is nil */
#define runpopobjnil(ctx) \
	(runstkund(ctx), ((--(ctx)->runcxsp))->runstyp == DAT_OBJECT ? ((ctx)->runcxsp->runsv.runsvobj) : ((ctx)->runcxsp->runstyp == DAT_NIL ? MCMONINV : (runsig(ctx, ERR_REQVOB), (objnum)0)))

/* pop a list, signalling an error if not a list */
/* uchar *runpoplst(runcxdef *ctx); */
#define runpoplst(ctx) \
	(runstkund(ctx), ((--(ctx)->runcxsp))->runstyp != DAT_LIST ? (runsig(ctx, ERR_REQVLS), (uchar *)0) : (uchar *)((ctx)->runcxsp->runsv.runsvstr))

/* pop a property number, signalling an error if not a property number */
/* prpnum runpopprp(runcxdef *ctx); */
#define runpopprp(ctx) \
	(runstkund(ctx), ((--(ctx)->runcxsp))->runstyp != DAT_PROPNUM ? (runsig(ctx, ERR_REQVPR), (prpnum)0) : ((ctx)->runcxsp->runsv.runsvprp))

/* pop function pointer */
/* objnum runpopfn(runcxdef *ctx); */
#define runpopfn(ctx) \
	((objnum)(runstkund(ctx), ((--(ctx)->runcxsp))->runstyp != DAT_FNADDR ? (runsig(ctx, ERR_REQVFN), (objnum)0) : ((ctx)->runcxsp->runsv.runsvobj)))

/* pop a string value */
/* char *runpopstr(runcxdef *ctx); */
#define runpopstr(ctx) \
	(runstkund(ctx), ((--((ctx)->runcxsp))->runstyp != DAT_SSTRING ? (runsig(ctx, ERR_REQSTR), (uchar *)0) : ((ctx)->runcxsp->runsv.runsvstr)))

/* pop a logical value - TRUE for DAT_TRUE, FALSE for DAT_NIL */
/* int runpoplog(runcxdef *ctx); */
#define runpoplog(ctx) \
	((--((ctx)->runcxsp))->runstyp == DAT_TRUE ? TRUE : (ctx)->runcxsp->runstyp == DAT_NIL ? FALSE : (runsig(ctx, ERR_REQLOG), 0))
...
#define runhres(ctx, siz, below) \
	((uint)((ctx)->runcxhtop - (ctx)->runcxhp) > (uint)(siz) ? DISCARD 0 : (runhcmp(ctx, siz, below, (runsdef *)0, (runsdef *)0, (runsdef *)0), DISCARD 0))

/* reserve space, with various amounts of saving */
#define runhres1(ctx, siz, below, val1) \
	((uint)((ctx)->runcxhtop - (ctx)->runcxhp) > (uint)(siz) ? DISCARD 0 : (runhcmp(ctx, siz, below, val1, (runsdef *)0, (runsdef *)0), DISCARD 0))

#define runhres2(ctx, siz, below, val1, val2) \
	((uint)((ctx)->runcxhtop - (ctx)->runcxhp) > (uint)(siz) ? DISCARD 0 : (runhcmp(ctx, siz, below, val1, val2, (runsdef *)0), DISCARD 0))

#define runhres3(ctx, siz, below, val1, val2, val3) \
	((uint)((ctx)->runcxhtop - (ctx)->runcxhp) > (uint)(siz) ? DISCARD 0 : (runhcmp(ctx, siz, below, val1, val2, val3), DISCARD 0))
