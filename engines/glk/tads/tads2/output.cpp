...
static void t_outline(out_stream_info *stream, int nl,
	...
	if (!scrquiet || stream != &G_std_disp) {
         ...
         */
		if (stream->use_more_mode && scrfp == 0 && G_os_moremode && nl != 0 && nl != 4 && stream->linecnt++ >= G_os_pagelength) {
...
static void outflushn_stream(out_stream_info *stream, int nl) {
	...
	if (nl == 3) {
		...
		case 1:
             ...
             */
			if (stream->linecol != 0 || !stream->just_did_nl || stream->html_pre_level != 0) {
         ...
         *   the newline 
         */
		if (stream->linebuf[stream->preview] != '\0' || (stream->linecol != 0 && !stream->just_did_nl) || stream->html_pre_level > 0) {
			/* write it out */
...
static void outtab_stream(out_stream_info *stream) {
	...
	if (stream->html_target) {
		...
		do {
			...
			++(stream->linecol);
		} while (((stream->linecol + 1) & 3) != 0 && stream->linecol < maxcol);
	}
}
...
static void outchar_noxlat_stream(out_stream_info *stream, char c) {
	...
	if (stream->capturing) {
		...
		if (stream->capture_obj != MCMONINV) {
			...
			if (mcmobjsiz(stream->capture_ctx, stream->capture_obj) <= stream->capture_ofs) {
	...
	if (stream->linecol + 1 < G_os_linewidth) {
		...
		if (outissp(c) && c != '\t' && stream->linecol == 0 && !qspace && stream->html_pre_level == 0)
			return;
     ...
     */
	for (; i >= 0 && !outissp(stream->linebuf[i]) && !(!stream->os_line_wrap && stream->linebuf[i] == '-'); --i)
		;
...
static void outchar_html_stream(out_stream_info *stream,
     ...
     */
	if (ampptr >= amp_tbl + sizeof(amp_tbl) / sizeof(amp_tbl[0]) || ampptr->expan == 0) {
		char xlat_buf[50];
...
static int outformatlen_stream(out_stream_info *stream,
	...
	while (c != '\0') {
		...
		if (infmt) {
             ...
             *   string 
             */
			if (c == '\\' || f == &fmsbuf[sizeof(fmsbuf)] || (stream->html_mode && (c == '<' || c == '&'))) {
				...
				default:
					...
					if (outisal(c)) {
						...
						*dst++ = '\0';
                         ...
                         */
						if (!scumm_stricmp(attrname, "height") && stream->html_defer_br != HTML_DEFER_BR_NONE) {
							int ht;
						...
						} else if (!scumm_stricmp(attrname, "alt") && !stream->html_in_ignore && stream->html_allow_alt) {
		...
		} else if (c == '\\') /* special escape code? */
		{
		...
		} else if (!stream->html_target && stream->html_mode && (c == '<' || c == '&')) {
			/*
			...
			if (c == '<') {
				...
				} else if (!scumm_stricmp(tagbuf, "b") || !scumm_stricmp(tagbuf, "i") || !scumm_stricmp(tagbuf, "em") || !scumm_stricmp(tagbuf, "strong")) {
					int attr = 0;
...
static char out_parse_entity(char *outbuf, size_t outbuf_size, const char **sp, size_t *slenp) {
     ...
     */
	for (dst = ampbuf;
	     c != '\0' && (outisdg(c) || outisal(c)) && dst < ampbuf + sizeof(ampbuf) - 1;
