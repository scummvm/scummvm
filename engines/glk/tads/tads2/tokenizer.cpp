...
void tok_write_defines(tokcxdef *ctx, osfildef *fp, errcxdef *ec) {
	...
	for (i = TOKDFHSHSIZ, dfp = ctx->tokcxdf; i; ++dfp, --i) {
		...
		for (df = *dfp; df; df = df->nxt) {
			...
			if (osfwb(fp, buf, 4) || osfwb(fp, df->nm, df->len) || (df->explen != 0 && osfwb(fp, df->expan, df->explen)))
...
static void tokelse(tokcxdef *ctx, const char *p, int len) {
	...
	cnt = ctx->tokcxifcnt;
	if (cnt == 0 || ctx->tokcxif[cnt - 1] == TOKIF_ELSE_YES || ctx->tokcxif[cnt - 1] == TOKIF_ELSE_NO) {
...
static void tokpragma(tokcxdef *ctx, const char *p, int len) {
	...
	/* see what we have */
	if (len > 1 && (*p == 'c' || *p == 'C') && (*(p + 1) == '+' || *(p + 1) == '-' || t_isspace(*(p + 1)))) {
...
static void tokinclude(tokcxdef *ctx, const char *p, int len) {
	...
	case '<':
		match = '>';
		if (path && path->tokpnxt)
			path = path->tokpnxt; /* skip 1st path */
	...
	find_matching_delim:
		for (++p, --len; len && *p != match; --len, ++p)
			;
		if (len == 0 || *p != match)
			errlog(ctx->tokcxerr, ERR_INCMTCH);
		break;
	...
	for (lin = ctx->tokcxhdr; lin; lin = (linfdef *)lin->linflin.linnxt) {
		...
		while (p2 > lin->linfnam && *(p2 - 1) != OSPATHCHAR && !strchr(OSPATHALT, *(p2 - 1)))
			--p2;
		if (strlen(p2) == flen2 && !memicmp(p2, q, flen2)) {
	...
	if (!child)
		errsig1(ctx->tokcxerr, ERR_INCSEAR,
...
static int tokgetlin(tokcxdef *ctx, int dopound) {
	for (;;) {
		...
		if (dopound && ctx->tokcxlen != 0 && ctx->tokcxptr[0] == '#' && !(ctx->tokcxlin->linflg & LINFNOINC)) {
			const char *p;
			int len;
			static const struct
			{
			...
			for (dirp = dir, i = sizeof(dir) / sizeof(dir[0]); i; --i, ++dirp) {
				...
				if (len >= dirp->len && !memcmp(p, dirp->nm, (size_t)dirp->len) && (len == dirp->len || t_isspace(*(p + dirp->len)))) {
					int cnt;
					int stat;
                     ...
                     */
					cnt = ctx->tokcxifcnt;
					if (dirp->ok_in_if || cnt == 0 || ((stat = ctx->tokcxifcur) == TOKIF_IF_YES || stat == TOKIF_ELSE_YES)) {
...
int toknext(tokcxdef *ctx) {
	...
	skipblanks:
		...
		while (len && t_isspace(*p))
			++p, --len; /* scan off whitespace */
...
nexttoken:
	if (Common::isAlpha((uchar)*p) || *p == '_' || *p == '$') {
		...
		for (hash = 0, l = 0, tq = tok->toknam;
		     ...
		     (thischar = ((Common::isUpper((uchar)*p) && (ctx->tokcxflg & TOKCXCASEFOLD))
		                      ? Common::isLower((uchar)*p)
		                      : *p)),
		...
		*tq = '\0';
         ...
         */
		if (l == 9 && !memcmp(tok->toknam, ((ctx->tokcxflg & TOKCXCASEFOLD) ? "__defined" : "__DEFINED"), (size_t)9) && len > 2 && *p == '(' && TOKISSYM(*(p + 1)) && !Common::isDigit((uchar) * (p + 1))) {
			int symlen;
			char mysym[TOKNAMMAX];
		...
		for (warned = FALSE;;) {
			if (len >= 2 && *p == '\\') {
				...
				len -= 2;
			           ...
			           (delim == '"' && len >= 2 && *p == '<' && *(p + 1) == '<' && !(ctx->tokcxflg & TOKCXFINMAC))) {
				(*ctx->tokcxsad)(ctx->tokcxscx, strstart,
				                 (ushort)(p - strstart));
				if (len == 0) {
					if (ctx->tokcxmlvl) {
		...
		if (len != 0 && *p == delim) {
             ...
             */
			if ((ctx->tokcxflg & TOKCXF_EMBED_PAREN_AFT) != 0 && !(ctx->tokcxflg & TOKCXFINMAC)) {
				/* clear the flag */
		...
		goto done;
	} else if (len >= 2 && *p == '>' && *(p + 1) == '>' && (ctx->tokcxflg & TOKCXFINMAC) != 0) {
		/* skip the ">>" */
		ctx->tokcxflg &= ~TOKCXFINMAC;
		p += 2;
		len -= 2;
...
void toktleach(toktdef *tab1,
	...
	for (p = (toksdef *)tab->toktlptr, cnt = tab->toktlcnt; cnt; --cnt) {
		(*cb)(ctx, p);
		p = (toksdef *)(((uchar *)p) + osrndsz(p->tokslen + sizeof(toks1def)));
	}
}
...
void toktlset(toktdef *tab1, toksdef *newsym) {
	...
	for (p = (toksdef *)tab->toktlptr, cnt = tab->toktlcnt; cnt; --cnt) {
		if (p->tokslen == newsym->tokslen && !memcmp(p->toksnam, newsym->toksnam, (size_t)newsym->tokslen)) {
...
tokcxdef *tokcxini(errcxdef *errctx, mcmcxdef *mcmctx, tokldef *sctab) {
	...
	siz = sizeof(tokcxdef) + (i * sizeof(tokscdef *)) + ((cnt + 1) * sizeof(tokscdef));
