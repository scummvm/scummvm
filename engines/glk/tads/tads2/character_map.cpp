...
static int cmap_load_internal(char *filename) {
	...
	/* check the signature */
	if (osfrb(fp, buf, sizeof(sig1)) || memcmp(buf, sig1, sizeof(sig1)) != 0) {
		osfcls(fp);
		return 2;
	}
	...
	if (osfrb(fp, lenbuf, 2) || (len = osrp2(lenbuf)) > sizeof(G_cmap_ldesc) || osfrb(fp, G_cmap_ldesc, len)) {
		osfcls(fp);
		return 4;
	}

	/* load the two tables - input, then output */
	if (osfrb(fp, G_cmap_input, sizeof(G_cmap_input)) || osfrb(fp, G_cmap_output, sizeof(G_cmap_output))) {
		osfcls(fp);
		return 5;
	}
	...
	if (!memcmp(buf, "SYSI", 4)) {
		...
		if (osfrb(fp, lenbuf, 2) || (len = osrp2(lenbuf)) > sizeof(buf) || osfrb(fp, buf, len)) {
			osfcls(fp);
			return 7;
		}
	...
	if (!memcmp(buf, "ENTY", 4)) {
		...
		for (;;) {
			...
			/* read the string */
			if (blen > CMAP_MAX_ENTITY_EXPANSION || osfrb(fp, expansion, blen)) {
				osfcls(fp);
				return 10;
			}
