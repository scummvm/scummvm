...
static void fiordrscext(errcxdef *ec, osfildef *fp, appctxdef *appctx,
	...
	for (;;) {
		...
		if (osfrb(fp, buf, 1) || osfrb(fp, buf + 1, (int)(buf[0] + 4)))
			errsig1(ec, ERR_RDRSC, ERRTSTR,
			        errstr(ec, resfilename, strlen(resfilename)));
...
static void fiord1(mcmcxdef *mctx, voccxdef *vctx, tokcxdef *tctx,
	...
	if (memcmp(buf + sizeof(FIOFILHDR), FIOVSNHDR,
	    ...
	    memcmp(buf + sizeof(FIOFILHDR), FIOVSNHDR3,
	           (size_t)sizeof(FIOVSNHDR3)))
		errsig(ec, ERR_BADVSN);
	if (osfrb(fp, vctx->voccxtim, (size_t)26))
		errsig(ec, ERR_RDGAM);
		...
		if (fioisrsc(buf, "OBJ")) {
			...
			while (curpos != endpos) {
				...
				case TOKSTEXTERN:
					...
					if (osfrb(fp, buf + 3, 1) || osfrb(fp, ex->runxnam, (int)buf[3]))
						errsig(ec, ERR_RDGAM);
					ex->runxnam[buf[3]] = '\0';
					curpos += buf[3] + 4;
					break;
			...
			curpos = osfpos(fp) - startofs;
			bsiz = endpos - curpos;
			if (bsiz && bsiz < OSMALMAX && (bufp = p = (uchar *)osmalloc((size_t)bsiz)) != 0) {
				uchar *p1;
				ulong siz2;
				uint sizcur;
 ...
 */            
			...
			if (bsiz && bsiz < OSMALMAX && (bufp = p = (uchar *)osmalloc((size_t)bsiz)) != 0) {
				...
				while (curpos != endpos) {
					if (osfrb(fp, buf, 10) || osfrb(fp, buf + 10,
			...
			if (osfrb(fp, fmts, fmtl))
				errsig(ec, ERR_RDGAM);
			if (*flagp & FIOFCRYPT)
				fioxor(fmts, fmtl, xor_seed, xor_inc);
			...
			*preinit = osrp2(buf);
			...
			if (osfrb(fp, buf, 6) || (len = osrp2(buf + 4)) > CMAP_LDESC_MAX_LEN || osfrb(fp, buf + 6, len))
				errsig(ec, ERR_RDGAM);
...
int fiorso_getgame(char *saved_file, char *fnamebuf, size_t buflen) {
	...
	if (osfrb(fp, buf, (int)(sizeof(FIOSAVHDR_PREFIX) + 2)) || memcmp(buf, FIOSAVHDR_PREFIX, sizeof(FIOSAVHDR_PREFIX)) != 0) {
		/* 
...
int fiorso(voccxdef *vctx, char *fname) {
	...
	if (osfrb(fp, buf, (int)(sizeof(FIOSAVHDR) + sizeof(FIOSAVVSN))) || memcmp(buf, FIOSAVHDR, (size_t)sizeof(FIOSAVHDR))) {
     ...
     */
	if (osfrb(fp, timestamp, 26) || memcmp(timestamp, vctx->voccxtim, (size_t)26)) {
	...
	if (fiorfda(fp, vctx->voccxdmn, vctx->voccxdmc) || fiorfda(fp, vctx->voccxfus, vctx->voccxfuc) || fiorfda(fp, vctx->voccxalm, vctx->voccxalc))
		goto ret_error;
...
int fiosav(voccxdef *vctx, char *fname, char *game_fname) {
	...
	if (game_fname != 0) {
		...
		oswp2(buf, len);
		if (osfwb(fp, FIOSAVHDR_PREFIX, (int)sizeof(FIOSAVHDR_PREFIX)) || osfwb(fp, buf, 2) || osfwb(fp, game_fname, (int)len))
			goto ret_error;
	}

	/* write save game header and timestamp */
	if (osfwb(fp, FIOSAVHDR, (int)sizeof(FIOSAVHDR)) || osfwb(fp, FIOSAVVSN, (int)sizeof(FIOSAVVSN)) || osfwb(fp, vctx->voccxtim, 26))
		goto ret_error;
			...
			if (*v != 0) {
				...
				if (mcmobjdirty(mctx, (mcmon)obj)) {
					...
					if ((*v)->vociflg & VOCIFNEW) {
						...
						mutsiz = objfree(p);
						oswp2(buf + 3, mutsiz);
						if (osfwb(fp, buf, 7) || osfwb(fp, p, mutsiz))
							err = TRUE;
						...
						oswp2(buf + 3, propcnt);
						oswp2(buf + 5, mutsiz);
						if (osfwb(fp, buf, 7) || osfwb(fp, mut, mutsiz))
							err = TRUE;
					}
	...
	if (fiowfda(fp, vctx->voccxdmn, vctx->voccxdmc) || fiowfda(fp, vctx->voccxfus, vctx->voccxfuc) || fiowfda(fp, vctx->voccxalm, vctx->voccxalc))
		goto ret_error;
