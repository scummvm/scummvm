...
startover:
	...
	if (!chunk)
		goto error;      /* can't get any more space from heap */
...
void mcmrsrv(mcmcxdef *cctx, ushort siz, mcmon clinum, mclhd loadhd) {
	...
	if (!o)
		errsig(ctx->mcmcxerr, ERR_NOHDR); /* can't get a new header */
...
uchar *mcmrealo(mcmcxdef *cctx, mcmon cliobj, ushort newsize) {
	...
	if (newsize < o->mcmosiz)
		...
		if (nxto && ((nxto->mcmoflg & MCMOFFREE) && nxto->mcmosiz >= newsize - o->mcmosiz)) {
			...
			if (nxt == MCMONINV)
				errsig(ctx->mcmcxerr, ERR_NOMEM2);
...
void mcmgfre(mcmcx1def *ctx, mcmon obj) {
	...
	if (o->mcmolcnt)
		errsig(ctx->mcmcxerr, ERR_LCKFRE);

	/* take out of LRU chain if it's in the chain */
	if (o->mcmoflg & MCMOFLRU)
...
startover:
     ...
     */
	if (ctx->mcmcxpage == ctx->mcmcxpgmx)
		goto error;             /* no more pages */
...
static void mcmsplt(mcmcx1def *ctx, mcmon n, ushort siz) {
	...
	if (o->mcmosiz < siz + MCMSPLIT)
		return; /* don't split; we're done */

	newp = mcmoal(ctx, &newn);
	if (newn == MCMONINV)
		return; /* ignore error - just skip split */
...
static void mcmgarb(mcmcx1def *ctx) {
	...
	for (h = ctx->mcmcxhpch; h; h = h->mcmhnxt) {
		...
		if (!p)
			continue;         /* can't do anything - no free blocks */
		...
		for (q = p;;) {
			...
			assert(mcmgobje(ctx, *(mcmon *)q)->mcmoptr == q + osrndsz(sizeof(mcmon)));
			...
			if (flags & MCMOFLOCK) {
				...
				q = p;
				if (p)
					continue; /* try again; start with next free block */
				else
					break; /* no more free blocks - done with heap */
			}

			/* if the block is free, we can relocate between p and q */
			if (flags & MCMOFFREE) {
				if (q != nxt)
					p = mcmreloc(ctx, p, q); /* relocate */
...
static int mcmswap(mcmcx1def *ctx, ushort siz) {
	...
	for (pass = 1, tot = 0; pass < 3 && tot < siz; ++pass) {
		for (n = ctx->mcmcxlru; n != MCMONINV && tot < siz; n = nxt) {
			...
			if (!(o->mcmoflg & (MCMOFLOCK | MCMOFNOSWAP | MCMOFPAGE)) && (pass == 2 || o->mcmosiz >= siz)) {
...
void mcmcswf(mcmcx1def *ctx, mcmon objn, mcsseg swapn, mcsseg oldswapn) {
     ...
     *   be replaced).  
     */
	if (((o->mcmoflg & (MCMOFDIRTY | MCMOFNODISC)) && o->mcmoswh == oldswapn) || (o->mcmoflg & MCMOFPRES))
		o->mcmoswh = swapn;
}
