...
linfdef *linfini(mcmcxdef *mctx, errcxdef *ec, const char *filename,
     ...
     */
	if (fp == 0 && (!os_locate(filename, flen, (char *)0, fbuf, sizeof(fbuf)) || (fp = osfoprs(fbuf, OSFTTEXT)) == 0)) {
		/*
	...
	linf = (linfdef *)mchalo(ec, (ushort)(sizeof(linfdef) + flen + len + 1), "linfini");
...
int linfload(osfildef *fp, dbgcxdef *dbgctx, errcxdef *ec, tokpdef *path) {
	...
	if (osfrb(fp, buf, 6) || osfrb(fp, buf + 6, (int)buf[1]))
		return TRUE;
	...
	for (objn = linf->linfpg; pgcnt; ++objn, --pgcnt) {
		...
		if (osfrb(fp, objp, (1024 * DBGLINFSIZ)))
			return (TRUE);
		mcmunlck(linf->linfmem, *objn);
	}
...
#define linf ((linfdef *)lin)
     ...
     *   the file. 
     */
	if (fname[0] != '\0' || dbgu_find_src(linf->linfnam, strlen(linf->linfnam),
	                                      fname, OSFNMAX, TRUE)) {
		/* open the file */
