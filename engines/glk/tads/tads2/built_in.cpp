...
void biffnd(bifcxdef *ctx, int argc) {
	...
	case DAT_LIST:
		...
		for (typ = DAT_NIL, outv = 1; len1; ++outv, p1 += siz, len1 -= siz) {
			...
			switch (val.runstyp) {
			case DAT_NUMBER:
				if (val.runsv.runsvnum != osrp4s(p1 + 1))
					continue;
				break;
			...
			case DAT_PROPNUM:
				if (osrp2(p1 + 1) != val.runsv.runsvprp)
					continue;
				break;

			case DAT_OBJECT:
			case DAT_FNADDR:
				if (osrp2(p1 + 1) != val.runsv.runsvobj)
					continue;
				break;
...
void bifask(bifcxdef *ctx, int argc) {
	...
	if (argc >= 3) {
		...
		if (file_type == OSFTUNK && prompt_type == OS_AFP_SAVE && bif_stristr(pbuf, "script") != 0) {
...
void bifsub(bifcxdef *ctx, int argc) {
	...
	if (ofs < 1)
		runsig1(ctx->bifcxrun, ERR_INVVBIF, ERRTSTR, "substr");
...
int bifinh(voccxdef *voc, vocidef *v, objnum cls) {
	...
	for (i = v->vocinsc, sc = v->vocisc; i; ++sc, --i) {
		if (*sc == cls || bifinh(voc, vocinh(voc, *sc), cls))
			return (TRUE);
	}
	return (FALSE);
}
...
void biffob(bifcxdef *ctx, int argc) {
	...
	for (vpg = voc->voccxinh, i = 0; i < VOCINHMAX; ++vpg, ++i) {
		...
		for (v = *vpg, obj = (i << 8), j = 0; j < 256; ++v, ++obj, ++j) {
			if (!*v || ((*v)->vociflg & VOCIFCLASS) || (cls != MCMONINV && !bifinh(voc, *v, cls)))
				continue;
...
void bifarg(bifcxdef *ctx, int argc) {
	...
	if (argnum < 1)
		runsig1(ctx->bifcxrun, ERR_INVVBIF, ERRTSTR, "getarg");
...
void bifgfu(bifcxdef *ctx, int argc) {
	...
	if (runtostyp(rcx) == DAT_FNADDR) {
		...
		for (slots = vcx->voccxfuc, daem = vcx->voccxfus;
		     slots; ++daem, --slots) {
			if (daem->vocdfn == func && daem->vocdarg.runstyp == val.runstyp && !memcmp(&val.runsv, &daem->vocdarg.runsv, (size_t)datsiz(val.runstyp, &val.runsv)))
				goto ret_num;
		}
	} else {
...
static int get_ext_key_name(char *namebuf, int c, int extc) {
	...
	if (extc >= 1 && extc <= (int)(sizeof(ext_key_names) / sizeof(ext_key_names[0]))) {
		/* use the array name */
...
void biffopen(bifcxdef *ctx, int argc) {
	...
	case 'w':
         ...
         */
		if (ctx->bifcxsafetyw > 2 || (!in_same_dir && ctx->bifcxsafetyw > 0)) {
	...
	case 'r':
         ...
         */
		if (ctx->bifcxsafetyr > 3 || (!in_same_dir && ctx->bifcxsafetyr > 1)) {
			/* this operation is not allowed - return failure */
			runpnil(ctx->bifcxrun);
			return;
		}
...
static objnum bif_get_optional_obj_arg(bifcxdef *ctx, int *rem_argc) {
     ...
     *   arguments 
     */
	if (runtostyp(ctx->bifcxrun) != DAT_OBJECT && runtostyp(ctx->bifcxrun) != DAT_NIL)
		return MCMONINV;
	...
	if (runtostyp(ctx->bifcxrun) == DAT_NUMBER) {
		...
		for (bcnt = 0, dst = lblbuf; len != 0; lstadv(&p, &len)) {
			...
			if (*p == DAT_SSTRING) {
                 ...
                 */
				if (resid != 0 && !os_get_str_rsc(resid, dst,
			...
			if (bcnt >= sizeof(labels) / sizeof(labels[0]) || dst >= lblbuf + sizeof(lblbuf))
				break;
		}
	}
...
void bifresexists(bifcxdef *ctx, int argc) {
     ...
     */
	appctx = ctx->bifcxappctx;
	found = (appctx != 0 && appctx->resfile_exists != 0 && (*appctx->resfile_exists)(appctx->resfile_exists_ctx, resname, strlen(resname)));
