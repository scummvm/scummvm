			...
			if (exprlen > 1) {
				...
				case '?':
                     ...
                     */
					{
                         ...
                         */
						while (exprlen > 1 && (*(expr + 1) == '?' || *(expr + 1) == '+' || *(expr + 1) == '*')) {
							++expr;
							--exprlen;
						}
					}
...
static void re_note_group(re_context *ctx, re_group_register *regs,
     ...
     */
	if (id == RE_STATE_INVALID || !(ctx->tuple_arr[id].flags & (RE_STATE_GROUP_BEGIN | RE_STATE_GROUP_END)) || (group_index = (int)ctx->tuple_arr[id].ch) >= RE_GROUP_REG_CNT)
		return;
...
static int re_match(re_context *ctx, const char *entire_str,
	...
	for (;;) {
		...
		if (!(tuple->flags & (RE_STATE_GROUP_BEGIN | RE_STATE_GROUP_END)) && tuple->ch != RE_EPSILON) {
			/*
			...
			case RE_NON_WORD_BOUNDARY: {
				...
				/*
                     ...
                     *   classification 
                     */
				prev_is_word = (p != entire_str && re_is_word_char(*(p - 1)));

				/* make the same check for the current character */
				next_is_word = (curlen != 0 && re_is_word_char(*p));
				...
				/* 
                     ...
                     *   failure if not 
                     */
				if ((tuple->ch == RE_WORD_BOUNDARY && !boundary) || (tuple->ch == RE_NON_WORD_BOUNDARY && boundary))
					return -1;
			} break;
			...
			case RE_RANGE_EXCL: {
				...
				if ((tuple->ch == RE_RANGE && !match) || (tuple->ch == RE_RANGE_EXCL && match))
					return -1;
