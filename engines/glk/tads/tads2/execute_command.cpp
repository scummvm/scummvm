...
void vocremfd(voccxdef *ctx, vocddef *what, objnum func, prpnum prop,
	...
	if (what == ctx->voccxdmn)
		...
		errsig(ctx->voccxerr, ERR_BADREMF);

	/* find the slot with this same fuse/daemon/notifier, and remove it */
	for (; slots; ++what, --slots) {
		if (what->vocdfn == func && what->vocdprp == prop && (!val || (val->runstyp == what->vocdarg.runstyp && !memcmp(&val->runsv, &what->vocdarg.runsv, (size_t)datsiz(val->runstyp, &val->runsv))))) {
			/* save an undo record for this slot before changing */
			vocdusav(ctx, what);
...
void vocturn(voccxdef *ctx, int turncnt, int do_fuses) {
	...
	while (turncnt--) {
		...
		for (i = ctx->voccxalc, p = ctx->voccxalm; i; ++p, --i) {
			if (p->vocdfn != MCMONINV && p->vocdtim != VOCDTIM_EACH_TURN && p->vocdtim != 0) {
				/* save an undo record for this slot before changing */
				vocdusav(ctx, p);
...
void exedaem(voccxdef *ctx) {
	...
	for (i = ctx->voccxalc, daemon = ctx->voccxalm; i; ++daemon, --i) {
		if (daemon->vocdfn != MCMONINV && daemon->vocdtim == VOCDTIM_EACH_TURN) {
			ERRBEGIN(ctx->voccxerr)
...
static void exe_save_again_obj(vocoldef *againv, const vocoldef *objv,
	...
	if (objv != 0) {
		...
		if (objv->vocolfst != 0 && objv->vocollst != 0) {
             ...
             */
			copylen = objv->vocollst - objv->vocolfst + strlen(objv->vocollst) + 1;
...
static int exe1cmd(voccxdef *ctx, objnum actor, objnum verb, vocoldef *dobjv,
	...
	if (verb == ctx->voccxvag) {
		...
		if (validate_dobj && dobj != MCMONINV && !vocchkaccess(ctx, dobj, PRP_VALIDDO, 0, actor, verb)) {
		...
		if (validate_iobj && iobj != MCMONINV && !vocchkaccess(ctx, iobj, PRP_VALIDIO, 0, actor, verb)) {
     ...
     *   prep) 
     */
	if (iobj != MCMONINV && exegen(ctx, iobj, PRP_IOBJGEN, voctplvi(tpl), voctplio(tpl))) {
		/* reset the run-time context if this is a top-level call */
		if (!recursive)
			runrst(rcx);
     ...
     *   prep) 
     */
	if (dobj != MCMONINV && exegen(ctx, dobj, PRP_DOBJGEN, voctplvd(tpl), voctpldo(tpl))) {
		/* reset the run-time context if this is a top-level call */
		if (!recursive)
			runrst(rcx);
...
static void exesaveit(voccxdef *ctx, vocoldef *dolist) {
	...
	if (cnt == 1) {
         ...
         */
		if (dolist[0].vocolflg == VOCS_STR || dolist[0].vocolflg == VOCS_NUM) {
			/* 
		...
		ctx->voccxit = MCMONINV;
		if (dbg)
			tioputs(ctx->voccxtio, ".. setting \"them\": [");
		for (i = 0; i < cnt; ++i) {
...
static int exeloop(voccxdef *ctx, objnum actor, objnum verb,
     ...
     *   Otherwise, proceed.  
     */
	if (((multi_flags & VOCS_ALL) != 0 || dobj_cnt > 1) && dolist && dolist[0].vocolobj != MCMONINV) {
		int typ;
		...
		rundisc(rcx);
		ERRCATCH(ctx->voccxerr, err)
		if (err == ERR_RUNEXIT || err == ERR_RUNEXITOBJ || err == ERR_RUNABRT)
			return err;
		else
			errrse(ctx->voccxerr);
		ERREND(ctx->voccxerr)
...
int execmd(voccxdef *ctx, objnum actor, objnum prep,
	...
	for (;;) {
		...
		if (voclistlen(dolist) == 0 && voclistlen(iolist) == 0) {
             ...
             */
			if (!(tplflags & VOCTPLFLG_DOBJ_FIRST) && vocdisambig(ctx, dolist1, dolist, PRP_DODEFAULT,
			                                                      PRP_VALIDDO, voctplvd(tpl), cmd, otherobj,
			                                                      actor, verb, prep, cmdbuf, FALSE)) {
				err = -1;
				goto exit_error;
	...
	exit_error:;
		...
		if (askflags) {
			...
			if (runtostyp(rcx) == DAT_LIST && foundtpl) {
				...
				for (objcnt = 0; lstsiz && objcnt < 2; lstadv(&l, &lstsiz)) {
					if (*l == DAT_OBJECT) {
                         ...
                         */
						if (voclistlen(iolist) != 0 && askflags == ERR_RUNASKD && !(tplflags & VOCTPLFLG_DOBJ_FIRST)) {
                         ...
                         */
						if (askflags == ERR_RUNASKI && (tplflags & VOCTPLFLG_DOBJ_FIRST) != 0) {
             ...
             */
			if (ctx->voccxpask3 != MCMONINV && askflags == ERR_RUNASKI && voclistlen(dolist) != 0) {
				/* call parseAskobjIndirect */
             ...
             *   preposition 
             */
			if (askflags == ERR_RUNASKI && prep != MCMONINV && (exenewtype[0] & VOCT_PREP) != 0) {
				vocwdef *vp;
			...
			if (cnt == 0 || (exenewlist[next] && !vocspec(exenewlist[next], VOCW_THEN) && *exenewlist[next] != '\0')) {
				strcpy(cmdbuf, exenewcmd);
				ctx->voccxredo = TRUE;
				VOC_RETVAL(ctx, save_sp, 1);
			}
