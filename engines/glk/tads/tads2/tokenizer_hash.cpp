...
void tokthadd(toktdef *toktab1, char *name, int namel,
	...
	siz = osrndsz(siz);
	toktab->tokthofs += siz;
	if (siz > toktab->tokthsize)
		toktab->tokthsize = 0;
	else
		toktab->tokthsize -= siz;
}
...
static int tokthset1(void *ctx0, toksdef *sym, mcmon objn) {
	...
	if (sym->tokslen == ctx->tokseasym.tokslen && !memcmp(sym->toksnam, ctx->tokseasym.toksnam,
...
static int tokthfind1(void *ctx0, toksdef *sym, mcmon objn) {
	...
	if (sym->toksval == ctx->tokseasym.toksval && sym->tokstyp == ctx->tokseasym.tokstyp) {
		memcpy(ctx->toksearet, sym,
