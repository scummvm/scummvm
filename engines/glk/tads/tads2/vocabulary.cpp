...
uint vochsh(const uchar *t, int len) {
	...
	if (len > 6)
		len = 6;
	for (; len; --len, ++t)
		ret = (ret + (uint)(vocisupper(*t) ? tolower(*t) : *t)) & (VOCHASHSIZ - 1);
	return (ret);
}
...
static void vocwset(voccxdef *ctx, vocdef *v, prpnum p, objnum objn,
	...
	for (inx = v->vocwlst, vw = vocwget(ctx, inx); vw;
		...
		if ((vw->vocwflg & VOCFDEL) && vw->vocwobj == objn && vw->vocwtyp == p) {
			/*
	...
	for (inx = v->vocwlst, vw = vocwget(ctx, inx); vw;
	     inx = vw->vocwnxt, vw = vocwget(ctx, inx)) {
		if (vw->vocwobj == objn && vw->vocwtyp == p && (vw->vocwflg & VOCFCLASS) == (classflg & VOCFCLASS)) {
	...
	if (ctx->voccxwfre != VOCCXW_NONE) {
		...
		if ((ctx->voccxwalocnt % VOCWPGSIZ) == 0) {
			...
			if (pg >= VOCWPGMAX)
				errsig(ctx->voccxerr, ERR_VOCMNPG);
     ...
     */
	if (p == PRP_VERB && (ctx->voccxflg & VOCCXFVWARN) && (vw->vocwflg & VOCFCLASS) == 0) {
		for (vw2 = vocwget(ctx, v->vocwlst); vw2;
             ...
             */
			if (vw2 != vw && (vw2->vocwflg & VOCFCLASS) == 0 && vw2->vocwtyp == PRP_VERB) {
				if (v->vocln2 != 0)
...
void vocadd2(voccxdef *ctx, prpnum p, objnum objn, int classflg,
	...
	for (v = ctx->voccxhsh[hshval]; v; v = v->vocnxt) {
		...
		if (v->voclen == len && !memcmp(wrdtxt, v->voctxt, (size_t)len) && ((!wrd2 && v->vocln2 == 0) || (v->vocln2 == len2 && !memcmp(wrd2, v->voctxt + len, (size_t)len2)))) {
			vocwset(ctx, v, p, objn, classflg);
			return;
		}
	}
...
void vociadd(voccxdef *ctx, objnum obj, objnum loc,
	...
	/* set up the entry */
	if (vocinh(ctx, obj) != (vocidef *)0)
		errsig(ctx->voccxerr, ERR_VOCINUS);
	v->vociloc = loc;
	v->vociilc = MCMONINV;
...
static void voc_count_cb(void *ctx0, vocdef *voc, vocwdef *vocw) {
     ...
     */
	if ((ctx->prp == 0 || ctx->prp == vocw->vocwtyp) && !(vocw->vocwflg & VOCFDEL)) {
		/* count the word */
		ctx->cnt++;
	...
	for (i = VOCHASHSIZ, vp = ctx->voccxhsh; i; ++vp, --i) {
		...
		for (prv = (vocdef *)0, v = *vp; v; v = nxt) {
			...
			if (wrd1) {
				...
				if (v->voclen != len1 || memicmp((char *)v->voctxt, wrd1, (size_t)len1)) {
					prv = v;
					continue;
				}

				/* if there's a second word, compare it as well */
				if (wrd2 && (v->vocln2 != len2 || memicmp((char *)v->voctxt + len1,
				                                          wrd2, (size_t)len2))) {
					prv = v;
					continue;
				}
			...
			for (prvw = 0, idx = v->vocwlst, vw = vocwget(ctx, idx); vw;
				...
				if (revert) {
                     ...
                     *   match) 
                     */
					do_del = (vw->vocwobj == objn && (wrd1 == 0 || vw->vocwtyp == prp));
