...
void dbgbpenum(dbgcxdef *ctx, void (*cbfunc)(void *cbctx, int bpnum, const char *desc, const char *cond, int disabled), void *cbctx) {}
	...
	for (i = 1, argp = argv + 1; i < argc; ++argp, ++i) {
		...
		if (*arg == '-') {
			...
			case 's': {
				...
				safety_read = *p - '0';
				safety_write = (*(p + 1) != '\0' ? *(p + 1) - '0' : safety_read);

				/* range-check the values */
				if (safety_read < 0 || safety_read > 4 || safety_write < 0 || safety_write > 4)
					trdusage_s(ec);

				/* tell the host system about the setting */
				if (appctx != 0 && appctx->set_io_safety_level != 0)
					(*appctx->set_io_safety_level)(appctx->io_safety_level_ctx,
