...
void dbglv(dbgcxdef *ctx, int exittype) {
	...
	if (ctx->dbgcxfcn)
		--(ctx->dbgcxfcn); /* decrement frame pointer */
     ...
     *   calls a method 
     */
	if ((ctx->dbgcxflg & DBGCXFSS) != 0 && (ctx->dbgcxflg & DBGCXFSO) != 0 && ctx->dbgcxsof == 0 && ctx->dbgcxdep == 0) {
		/* 
...
void dbgstktr(dbgcxdef *ctx,
	...
	for (i = ctx->dbgcxfcn, j = ctx->dbgcxdep, f = &ctx->dbgcxfrm[i - 1]; i; --f, --j, --i) {
		p = buf;
		if (toponly) {
			int v = (i < 50 ? i : 50);
