...
namespace TADS {
	namespace TADS2 {
		...
		static int voctype[] = { 0, 0, VOCT_VERB, VOCT_NOUN, VOCT_ADJ, VOCT_PREP, VOCT_ARTICLE };
		...
		int voctok(voccxdef *ctx, char *cmd, char *outbuf, char **wrd,
			...
			for (i = 0;;) {
				...
				if (vocisalpha(*cmd) || *cmd == '-') {
					while (vocisalpha(*cmd) || vocisdigit(*cmd) || *cmd == '\'' || *cmd == '-') {
					...
					if (ctx->voccxspp) {
						...
						for (x = vocsptab; x->vocspin; ++x) {
							if (!strncmp((char *)wrd[i - 1], x->vocspin, (size_t)6)
							    && (cvt_ones || (x->vocspout != VOCW_ONE && x->vocspout != VOCW_ONES))
							    && x->vocspout != VOCW_OF) {
								*wrd[i - 1] = x->vocspout;
								*(wrd[i - 1] + 1) = '\0';
								break;
 ...
 */
         ...
         */
			...
			if (((typelist[cur]
			      & (VOCT_ARTICLE | VOCT_ADJ | VOCT_NOUN | VOCT_UNKNOWN))
			     == 0)
			    && !vocisdigit(*cmd[cur])) {
				VOC_RETVAL(ctx, save_sp, 0);
			}
     ...
     */
			...
			for (i = 0; i < l1; ++i) {
				...
				nounlist[outcnt].vocolfst = cmd[firstwrd];
				nounlist[outcnt].vocollst = cmd[cur - 1];
				nounlist[outcnt].vocolflg = flags1[i] | (trying_count ? VOCS_COUNT : 0) | trim_flags;
				if (trim_flags)
 ...
 */
		...
		static int vocsetit(voccxdef *ctx, objnum obj, int accprop,
			...
			if (obj == MCMONINV || !vocchkaccess(ctx, obj, (prpnum)accprop, 0, actor, verb)) {
				char nambuf[40];
 ...
 */
         ...
         */
				...
				if (inlist[inpos].vocolflg == VOCS_STR) {
					...
					vocsetobj(ctx, ctx->voccxnum, DAT_NUMBER, vbuf,
					          &inlist[inpos], &outlist[outpos]);
					++outpos;
				} else if (inlist[inpos].vocolflg == VOCS_IT || (inlist[inpos].vocolflg == VOCS_THEM && ctx->voccxthc == 0)) {
                     ...
                     */
                         ...
                         */
                             ...
                             */
									...
									list1[i + j] = voc_new_num_obj(ctx, objn,
									                               cmdActor, cmdVerb,
									                               l, FALSE);
                         ...
                         */
								list1[i] = voc_new_num_obj(ctx, list1[i], cmdActor, cmdVerb,
								                           (long)0, TRUE);
								if (list1[i] == MCMONINV) {
									err = VOCERR(40);
									goto done;
             ...
             */
                 ...
                 */
						for (i = 0; i < wrdcnt; ++i) {
							if (!(distypelist[i] & (VOCT_ADJ | VOCT_NOUN | VOCT_ARTICLE)))
								break;
						}
                             ...
                             */
								...
								for (i = cnt3 = 0; i < cnt2; ++i) {
									...
									if (disnounlist[i].vocolfst != 0) {
										...
										for (j = i + 1; disnounlist[i].vocolfst == disnounlist[j].vocolfst; ++j)
											;
										i = j - 1;
									}
								}
     ...
     */
         ...
         */
             ...
             */
					if (swapObj && ((typelist[cur - 1] & VOCT_NOUN) || (typelist[cur - 1] & VOCT_ADJ))) {
						--cur;
						swapObj = FALSE;
						cmdPrep = MCMONINV;
						goto retry_swap;
         ...
         */
             ...
             */
                 ...
                 */
					...
					if (cnt > 0) {
						...
						if (vocready(ctx, cmd, typelist, cur, cmdActor, cmdPrep,
						             ...
						             first_word, &preparse_list, &next_start))
							continue;
						else if ((typelist[cur] & VOCT_PREP) && vocffw(ctx, vverb, vvlen, cmd[cur], (int)strlen(cmd[cur]), PRP_VERB, (vocseadef *)0) && !vprep) {
							vprep = cmd[cur++];
                     ...
                     */
							vocoldef *np1 = (dolist[0].vocolflg & VOCS_TRIMPREP) != 0
							  ? &dolist[0]
                 ...
                 */
					...
					cur = next;
					if (cmd[cur]) {
						if ((typelist[cur] & VOCT_PREP) && vocffw(ctx, vverb, vvlen, cmd[cur], (int)strlen(cmd[cur]), PRP_VERB, (vocseadef *)0) && !vprep) {
							vprep = cmd[cur++];
						}
					}
             ...
             */
					if (cmdPrep == MCMONINV && (v = vocffw(ctx, vverb, vvlen, vprep, (vprep ? (int)strlen(vprep) : 0), PRP_VERB, (vocseadef *)0)) != 0) {
