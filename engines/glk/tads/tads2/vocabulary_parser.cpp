...
static int voceq(const uchar *s1, uint l1, uchar *s2, uint l2) {
	...
	if (l1 == 0 && l2 == 0)
		return (TRUE); /* both NULL - a match */
	if (l1 == 0 || l2 == 0)
		return (FALSE); /* one NULL only - not a match */
	if (l1 >= 6 && l2 >= l1)
		l2 = l1;
	if (l1 != l2)
...
vocwdef *vocfnw(voccxdef *voccx, vocseadef *search_ctx) {
	...
	for (v = c, vf = 0; v != 0 && vf == 0; v = v->vocnxt, first = FALSE) {
		...
		if (first || (voceq(search_ctx->wrd1, search_ctx->len1,
		                    v->voctxt, v->voclen) &&
			...
			for (; vw; vw = vocwget(voccx, vw->vocwnxt)) {
				if (search_ctx->vw->vocwtyp == vw->vocwtyp && !(vw->vocwflg & VOCFCLASS) && !(vw->vocwflg & VOCFDEL)) {
					/*
...
vocwdef *vocffw(voccxdef *ctx, const char *wrd, int len, const char *wrd2, int len2,
	...
	for (v = ctx->voccxhsh[hshval], vf = 0; v != 0 && vf == 0;
	     v = v->vocnxt) {
		/* if this word matches, look at the objects in its list */
		if (voceq((const uchar *)wrd, len, v->voctxt, v->voclen) && voceq((const uchar *)wrd2, len2, v->voctxt + v->voclen, v->vocln2)) {
			/* look for a suitable object in the vocwdef list */
			for (vw = vocwget(ctx, v->vocwlst); vw;
			     vw = vocwget(ctx, vw->vocwnxt)) {
				if (vw->vocwtyp == p && !(vw->vocwflg & VOCFCLASS) && !(vw->vocwflg & VOCFDEL)) {
					/*
                     *   remember the first vocdef that we found, and
                     *   remember this, the first matching vocwdef; then
...
static void vocerr_va_prep(voccxdef *ctx, struct vocerr_va_info *info,
	...
	if (ctx->voccxper != MCMONINV || ctx->voccxperp != MCMONINV) {
		...
		runfn(rcx, (objnum)(ctx->voccxperp == MCMONINV ? ctx->voccxper : ctx->voccxperp), argc);
			...
			case DAT_NUMBER:
				...
				*next_start = runpopnum(ctx->voccxrun);
				...
				for (i = 0; i < *next_start; ++i) {
					...
					if ((typelist[i] & VOCT_UNKNOWN) != 0 && ctx->voccxunknown > 0)
						--(ctx->voccxunknown);
				}
				break;
...
static int voc_check_special(voccxdef *ctx, const char *wrd, int checktyp) {
	...
	if (ctx->voccxspp) {
		...
		for (p = ctx->voccxspp, endp = p + ctx->voccxspl;
			...
			if (typ == checktyp && len == wrdlen && !memcmp(p, wrd, (size_t)len))
				return TRUE;
		...
		for (x = vocsptab; x->vocspin; ++x) {
			...
			if (x->vocspout == checktyp && !strncmp((const char *)wrd, x->vocspin, (size_t)6))
				return TRUE;
		}
	}
		...
		if (vocisalpha(*cmd) || *cmd == '-') {
			...
			*outbuf = '\0';
			...
			for (p = ctx->voccxcpp, l = ctx->voccxcpl; l;) {
				...
				if (i > 1 && len == (l2 - 2) && !memcmp(w, p2 + 2, (size_t)len) && strlen((char *)wrd[i - 2]) == (l1 - 2) && !memcmp(wrd[i - 2], p + 2, (size_t)(l1 - 2))) {
			...
			if (ctx->voccxspp) {
				...
				for (p = ctx->voccxspp, endp = p + ctx->voccxspl;
					...
					typ = *p++;
					len = *p++;
					if (len == wrdlen && !memcmp(p, wrd[i - 1], (size_t)len) && (cvt_ones || (typ != VOCW_ONE && typ != VOCW_ONES)) && typ != VOCW_OF) {
						*wrd[i - 1] = typ;
						*(wrd[i - 1] + 1) = '\0';
						break;
					}
				...
				for (x = vocsptab; x->vocspin; ++x) {
					if (!strncmp((char *)wrd[i - 1], x->vocspin, (size_t)6) && (cvt_ones || (x->vocspout != VOCW_ONE && x->vocspout != VOCW_ONES)) && x->vocspout != VOCW_OF) {
						*wrd[i - 1] = x->vocspout;
						*(wrd[i - 1] + 1) = '\0';
						break;
					}
			...
			while (vocisdigit(*cmd) || vocisalpha(*cmd) || *cmd == '\'' || *cmd == '-')
				*outbuf++ = *cmd++;
		} else
			switch (*cmd) {
			case '.':
...
static char *voc_read_oops(voccxdef *ctx, char *oopsbuf, size_t oopsbuflen,
     ...
     */
	if ((strlen(p) > 5 && memcmp(p, "oops ", 5) == 0) || (strlen(p) > 5 && memcmp(p, "oops,", 5) == 0)) {
		/* we found "OOPS" - move to the next character */
		p += 5;
	} else if ((strlen(p) > 2 && memcmp(p, "o ", 2) == 0) || (strlen(p) > 2 && memcmp(p, "o,", 2) == 0)) {
...
startover:
	...
	for (cur = 0; cmd[cur]; ++cur) {
		...
		if (t == 0 && !voc_check_special(ctx, p, VOCW_OF)) {
             ...
             */
			if (ctx->voccxunknown != 0 && unknown_count >= ctx->voccxlastunk - ctx->voccxunknown) {
...
static int vocgol(voccxdef *ctx, objnum *list, uint *flags, char **wrdlst,
	...
	if ((typ & (VOCT_NOUN | VOCT_PLURAL)) && (typ & VOCT_ADJ)) {
		...
		if (cur + 1 == last || cur == ofword - 1) {
			...
			if ((typ & VOCT_ADJ) != 0)
				try_endadj = TRUE;
		} else if ((cur + 2 == last || cur == ofword - 2) && vocisdigit(wrdlst[cur + 1][0])) {
			/*
	...
	if (ctx->voccxflg & VOCCXFDBG) {
		...
		sprintf(buf, "... %s (treating as %s%s)\\n", wrd,
		        (wrdtyp == PRP_ADJ ? "adjective" : wrdtyp == PRP_NOUN ? "noun" : wrdtyp == PRP_INVALID ? "unknown" : "plural"),
...
add_words:
	for (v = vocffw(ctx, wrd, len, (char *)0, 0, wrdtyp, &search_ctx); v != 0; v = vocfnw(ctx, &search_ctx)) {
		int i;
...
static int vocg1o(voccxdef *ctx, char *cmd[], int typelist[],
	...
	*next = cur;
	...
	if ((vocspec(cmd[cur], VOCW_ALL) || vocspec(cmd[cur], VOCW_BOTH) || vocspec(cmd[cur], VOCW_ANY)) &&
	    cmd[cur + 1] != (char *)0) {
		int nxt;
		int n = cur + 1;
		int has_of;
		...
		nxt = n;
		if (typelist[n] & VOCT_ARTICLE)
			++n; /* skip leading article */
		for (;;) {
			if (!cmd[n])
				break;
         ...
         *   have ANY but not OF).  
         */
		if (n > cur && ((typelist[n - 1] & VOCT_PLURAL) || (has_any && !has_of))) {
			if (ctx->voccxflg & VOCCXFDBG)
				tioputs(ctx->vocxtio,
	...
	if (((typelist[cur] & (VOCT_ARTICLE | VOCT_ADJ | VOCT_NOUN | VOCT_UNKNOWN)) == 0) && !vocisdigit(*cmd[cur])) {
		VOC_RETVAL(ctx, save_sp, 0);
	}

	if (typelist[cur] & VOCT_ARTICLE) {
		++cur;
		if (cmd[cur] == (char *)0 || ((typelist[cur] & (VOCT_ADJ | VOCT_NOUN | VOCT_UNKNOWN)) == 0 && !vocisdigit(*cmd[cur]))) {
	...
	for (found_plural = FALSE, unknown_count = 0, l1 = 0;;) {
		...
		if (typelist[cur] & VOCT_ADJ)
			...
			if (cmd[cur] == (char *)0)
				break;
			if (vocisdigit(*cmd[cur]))
				++cur;
			if (cmd[cur] == (char *)0)
				break;
			if (!voc_check_special(ctx, cmd[cur], VOCW_OF))
				break;
...
try_again:
	...
	*next = cur;
     ...
     */
	retry_with_count = ((vocisdigit(*cmd[firstwrd]) && found_plural) || (vocisdigit(*cmd[firstwrd]) && cur != firstwrd + 1 && atoi(cmd[firstwrd]) == 1));
...
retry_exclude_first:
	for (i = firstwrd + 1; i < cur; ++i) {
		...
		if (voc_check_special(ctx, cmd[i], VOCW_OF) || (typelist[i] & VOCT_ARTICLE))
			continue;
...
int vocgobj(voccxdef *ctx, char *cmd[], int typelist[],
	...
	for (;;) {
		...
		if (cur != -1 && cmd[cur] != 0 && vocspec(cmd[cur], VOCW_AND)) {
			lastcur = cur;
			while (cmd[cur] && vocspec(cmd[cur], VOCW_AND))
				++cur;
			again = TRUE;
			if (complain)
				complain = 2;
...
static objnum vocgetactor(voccxdef *ctx, char *cmd[], int typelist[],
	...
	*next = cur; /* assume no error will occur */
	...
	if (cnt > 0 && *next != -1 && cmd[*next] && vocspec(cmd[*next], VOCW_AND)) {
		...
		if (nounlist[0].vocolobj == MCMONINV || objgetap(ctx->voccxmem, nounlist[0].vocolobj, PRP_VALIDACTOR,
		                                                 (objnum *)0, FALSE))
...
static int voc_remove_objs_with_flag(voccxdef *ctx,
	...
	if (flag_cnt != 0 && flag_cnt < cnt - special_cnt) {
		...
		for (i = 0, dst = 0; i < cnt; ++i) {
             ...
             */
			if ((flags[i] & flag_to_remove) == 0 || list[i] == ctx->voccxnum || list[i] == ctx->voccxstr) {
...
int vocdisambig(voccxdef *ctx, vocoldef *outlist, vocoldef *inlist,
	...
	for (inpos = outpos = 0; inpos < listlen; ++inpos) {
		...
		} else /* we have a (possibly ambiguous) noun */
		{
             ...
             */
			while (inlist[lpos].vocolfst == inlist[inpos].vocolfst && lpos < listlen) {
                 ...
                 */
				flags1[i] = inlist[lpos].vocolflg & (VOCS_PLURAL | VOCS_ANY | VOCS_COUNT | VOCS_ENDADJ | VOCS_TRUNC);
			...
			if (unknown_count > 0) {
				...
				prp = (defprop == PRP_DODEFAULT
				           ? PRP_PARSEUNKNOWNDOBJ
				           : PRP_PARSEUNKNOWNIOBJ);
				...
				if (objgetap(ctx->voccxmem, cmdVerb, prp, (objnum *)0, FALSE)) {
					...
					case DAT_LIST:
						...
						while (lstlen != 0) {
							...
							if (*lstp == DAT_OBJECT && i < VOCMAXAMBIG)
			...
			if (inlist[inpos].vocolfst != 0 && inlist[inpos].vocollst != 0) {
				for (p = inlist[inpos].vocolfst; p <= inlist[inpos].vocollst; p += strlen(p) + 1) {
             ...
             *   the number object.  
             */
			if ((cnt2 == 0 || (cnt2 == 1 && list2[0] == ctx->voccxnum)) && inlist[inpos].vocolfst != 0 && inlist[inpos].vocolfst == inlist[inpos].vocollst && vocisdigit(*inlist[inpos].vocolfst)) {
				long v1;
				char vbuf[4];
				...
				for (; inlist[inpos + 1].vocolobj != MCMONINV && inlist[inpos + 1].vocolfst == inlist[inpos].vocolfst; ++inpos)
					;
				continue;
			}
			...
			for (;;) {
				...
				} else if (stat == VOC_DISAMBIG_PARSE_RESP || stat == VOC_DISAMBIG_PROMPTED) {
					/* 
                 ...
                 */
				if (stat == VOC_DISAMBIG_PARSE_RESP || stat == VOC_DISAMBIG_PROMPTED) {
					/* 
					...
					for (i = 0; i < cnt;) {
						...
						runppr(ctx->voccxrun, list1[i],
						       (prpnum)(eqcnt == 1 ? PRP_THEDESC : PRP_ADESC), 0);
                 ...
                 *   more. 
                 */
				if (stat != VOC_DISAMBIG_PARSE_RESP && vocread(ctx, cmdActor, cmdVerb, disnewbuf,
				...
				if (vocgtyp(ctx, diswordlist, distypelist, cmdbuf) || ctx->voccxunknown != 0) {
					/* 
				...
				if (i && (distypelist[i - 1] & VOCT_ADJ) && (distypelist[i - 1] & VOCT_NOUN)) {
					/*
			...
			try_current_flags:
				...
				if (vocspec(diswordlist[0], VOCW_ALL) || vocspec(diswordlist[0], VOCW_BOTH)) {
					char *nam;
					static char all_name[] = "all";
					...
					if (cnt2 > 0) {
                         ...
                         */
						if (diswordlist[next] != 0 && !vocspec(diswordlist[next], VOCW_ONE) && !vocspec(diswordlist[next], VOCW_ONES) && !vocspec(diswordlist[next], VOCW_THEN)) {
							cnt2 = 0;
						}
					} else if (cnt2 < 0) {
						/* 
					...
					if (cnt2 > 0) {
						...
						for (i = 0, newcnt = 0; i < cnt2; ++i) {
							...
							if (!found) {
								...
								flags2[newcnt] = disnounlist[i].vocolflg & (VOCS_PLURAL | VOCS_ANY | VOCS_COUNT);
                         ...
                         */
						if (newcnt && (newcnt <= cnt3 || (diswordlist[next] && vocspec(diswordlist[next],
						                                                               VOCW_ONES)))) {
							for (i = 0; i < cnt; ++i)
...
static int vocready(voccxdef *ctx, char *cmd[], int *typelist, int cur,
                    ...
                    int *next_start) {
	if (cur != -1 && (cmd[cur] == (char *)0 || vocspec(cmd[cur], VOCW_AND) || vocspec(cmd[cur], VOCW_THEN))) {
		if (ctx->voccxflg & VOCCXFDBG) {
			char buf[128];
...
static int voc1cmd(voccxdef *ctx, char *cmd[], char *cmdbuf,
	...
	skip_leading_stuff:
		...
		while (cmd[cur] && (vocspec(cmd[cur], VOCW_THEN) || vocspec(cmd[cur], VOCW_AND)))
			++cur;
...

		{
			...
			if ((o = vocgetactor(ctx, cmd, typelist, cur, &next, cmdbuf)) != MCMONINV) {
			...
			if (vocffw(ctx, vverb, vvlen, cmd[cur], (int)strlen(cmd[cur]),
				...
				if ((v = vocffw(ctx, cmd[cur], (int)strlen(cmd[cur]),
					...
					if (typelist[cur] & VOCT_NOUN) {
						...
						for (i = cur + 1;
						     cmd[i] && (typelist[i] & VOCT_ADJ) && !(typelist[i] & VOCT_NOUN); ++i)
							;
	...
	retry_swap:
		...
		if ((cnt = vocchknoun2(ctx, cmd, typelist, cur, &next, dolist,
             ...
             */
			if (swapObj &&
			    ((typelist[cur - 1] & VOCT_NOUN) || (typelist[cur - 1] & VOCT_ADJ))) {
				--cur;
				swapObj = FALSE;
				cmdPrep = MCMONINV;
				goto retry_swap;
		...
		if (cmdPrep == MCMONINV && (typelist[cur] & VOCT_PREP)) {
			...
			if (cmd[cur] == (char *)0 || vocspec(cmd[cur], VOCW_AND) || vocspec(cmd[cur], VOCW_THEN)) {
             ...
             */
			if (cmd[cur] && (typelist[cur] & VOCT_PREP) && cmd[cur + 1] && vprep == 0 && vocffw(ctx, vverb, vvlen, p1, (int)strlen(p1), PRP_VERB, (vocseadef *)0)) {
				/* 
...
int voccmd(voccxdef *ctx, char *cmd, uint cmdlen) {
	...
	if ((wrdcnt = voctok(ctx, cmd, buffer, wordlist, TRUE, FALSE, TRUE)) > 0) {
		...
		for (cur = 0; cur < wrdcnt; ++cur) {
			...
			if (!vocspec(wordlist[cur], VOCW_THEN) && !vocspec(wordlist[cur], VOCW_AND))
				break;
		}
	}
	...
	for (first = TRUE, cmdActor = ctx->voccxactor = MCMONINV; cur < wrdcnt;
		...
		for (next = cur; cur < wrdcnt && !vocspec(wordlist[cur], VOCW_THEN); ++cur)
			;
		wordlist[cur] = (char *)0;
		...
		while (cur + 1 < wrdcnt && (vocspec(wordlist[cur + 1], VOCW_THEN) || vocspec(wordlist[cur + 1], VOCW_AND)))
			++cur;
