...
namespace Hugo {
	...
	int Hugo::EvalExpr(int p) {
		...
		do {
			if (eval[p] == 1) {
				if (eval[p + 1] == OPEN_BRACKET_T || eval[p + 1] == OPEN_SQUARE_T) {
					eval[p] = 0;
		...
		*/
			if (eval[p + 4] == 1 && (eval[p + 5] == OPEN_BRACKET_T || eval[p + 5] == OPEN_SQUARE_T)) {
				eval[p + 4] = 0;
	   ...
	   is a ")", "]", or end of line
	*/
		} while ((evalcount > p + 2) && !(eval[p + 2] == 1 && (eval[p + 3] == CLOSE_BRACKET_T || eval[p + 3] == CLOSE_SQUARE_T || eval[p + 3] == 255)));
	...
	int Hugo::GetVal() {
		...
		case CHILDREN_T: {
			codeptr += 2; /* skip the "(" */
			val = GetValue();
			if (MEM(codeptr) == CLOSE_BRACKET_T)
				codeptr++;
			val = Children(val);
			break;
		}
	...
	int Hugo::GetValue() {
	   ...
	   read n
	*/
		if (((n >= MINUS_T && n <= PIPE_T) || n == AMPERSAND_T) && ((!inexpr)) && !inobj)
		/*
#if !defined (DEBUGGER)
		((!inexpr)) && !inobj)
#else
	...
	void Hugo::SetupExpr() {
		...
		do {
			...
			*/
			...
			case PARSE_T:
			case SERIAL_T: {
				if ((t == AMPERSAND_T || t == MINUS_T || t == PLUS_T) && justgotvalue == 1)
