							...
							for (i = 1; i <= words; i++) {
								if (!strcmp(word[i], ".")) {
									...
									if (hugo_fprintf(record, "%s", "\n") < 0)
										FatalError(WRITE_E);
									if (i == words)
										goto RecordedNewline;
								} else if (hugo_fputs(word[i], record) < 0 || hugo_fprintf(record, "%s", " ") < 0) {
									FatalError(WRITE_E);
								}
							}
							if (hugo_fprintf(record, "%s", "\n") < 0)
...
int Hugo::RunRestart() {
	...
	if (!(file = HUGO_FOPEN(gamefile, "rb")))
		goto RestartError;
#else
	file = game;
#endif

	if (hugo_fseek(file, (objtable - gameseg) * 16, SEEK_SET))
		goto RestartError;
	...
	*/
	if (codeptr != addr) {
		...
		/* Checking to see if currentroutine is 0 is a way of seeing
		   ...
		   debugger arrays.
		*/
		if ((old_currentroutine = currentroutine) == 0)
			return;
 ...
 */
	...
	ProcessToken:
		...
		Writevalloop:
			...
			i = GetValue();
			if (ioblock) {
				if ((ioblock == 2) || hugo_fputc(i % 256, io) == EOF || hugo_fputc(i / 256, io) == EOF) {
					ioerror = true;
					retflag = true;
					break;
				}
	...
	if (wascalled) {
		...
		ReturnfromRoutine:
			...
			*/
			...
			if (old_currentroutine != mainaddr && old_currentroutine != initaddr && currentroutine != mainaddr && currentroutine != initaddr) {
...
int Hugo::RunSave() {
	...
	if (debugger_collapsing)
		return 1;
#endif
	if (!strcmp(line, gamefile))
		return 0;
	if (!strcmp(line, ""))
		return 0;
	if (!hugo_overwrite(line))
		return 0;
	if (!(save = HUGO_FOPEN(line, "w+b")))
		return 0;
