...
*/
	...
	*/
		if (obj_match_state == 2 && domain != 0 && Parent(pobj) != domain && pobj != 0 && !speaking) {
	...
	int Hugo::MatchWord(int *wordnum) {
		...
		/* a number */
		case NUMBER_T:
			if ((STARTS_AS_NUMBER(word[*wordnum])) && !strcmp(itoa(atoi(word[*wordnum]), num, 10), word[*wordnum])) {
				if (obj_match_state == 1)
			...
			if (obj_match_state == 0) {
				...
				CheckWordorString:
					...
					if (p == DICTENTRY_T || p == STRING_T) {
						if ((PeekWord(nextsyntax + 1) == wd[*wordnum]) || (p == STRING_T && wd[*wordnum] == UNKNOWN_WORD)) {
					...
					/* or a number */
					else if ((p == NUMBER_T && STARTS_AS_NUMBER(word[*wordnum])) && !strcmp(word[*wordnum], itoa(atoi(word[*wordnum]), num, 10))) {
						grammaraddr = nextsyntax;
						if (*wordnum != objstart)
							return 1;
						else
					...
					*/
					...
					if (word[*wordnum][0] == '~') {
						...
						if (multicheck != MULTI_T && multicheck != MULTIHELD_T && multicheck != MULTINOTHELD_T) {
							strcpy(parseerr, word[1]);
							/* "You can't...multiple objects." */
							ParseError(3, 0);
							return 2;
				...
				*/
				...
				CheckXobjectFinish:
					...
					for (i = *wordnum + 1; i <= words; i++) {
						if (p == DICTENTRY_T || p == STRING_T) {
							if ((PeekWord(nextsyntax + 1) == wd[i]) || (p == STRING_T && wd[i] == UNKNOWN_WORD)) {
								objfinish = i - 1;
								break;
							}
						} else if (Peek(nextsyntax + 3) == FORWARD_SLASH_T) {
	...
	*/
	...
	int Hugo::ValidObj(int obj) {
		...
		if (!Available(obj, 0) && !speaking && (Peek(grammaraddr) != OPEN_BRACKET_T || Peek(grammaraddr + 1) != ROUTINE_T)) {
			if (Peek(grammaraddr) == ANYTHING_T)
