...
int Hugo::Available(int obj, char non_grammar) {
	...
	if (findobjectaddr) {
		...
		if ((Peek(grammaraddr) == ANYTHING_T || (Peek(grammaraddr) == OPEN_BRACKET_T && Peek(grammaraddr + 1) == ROUTINE_T)) && non_grammar == 0) {
			passlocal[1] = 0;
		} else {
			if (domain > 0)
				passlocal[1] = domain;
...
bool Hugo::MatchObject(int *wordnum) {
	...
	NextLoop:
		(*wordnum)++; /* next word */
		if ((*wordnum > words || word[*wordnum][0] == '\0') || *wordnum > objfinish)
			goto Clarify;
...
Clarify:
	...
	if (!strcmp(word[*wordnum], "~all")) /* if "~all" is specified */
	{
		...
		if ((*wordnum > words || word[*wordnum][0] == '\0') || (obj_match_state != 1 && *wordnum >= objfinish)) {
	...
	*/
	if (pobjcount >= 1) {
		...
		for (k = 0; k < pobjcount; k++) /* disqualify if unavailable */
		{
			...
			else if ((Peek(grammaraddr) == NOTHELD_T || Peek(grammaraddr) == MULTINOTHELD_T) && Parent(i) == var[actor]) {
				SubtractPossibleObject(i);
				k--;
	...
	*/
	...
	if ((strcmp(word[*wordnum], "~and") && strcmp(word[*wordnum], "~except")) || obj_match_state == 5) {
		/* At the end yet? */
		if ((*wordnum > words || word[*wordnum][0] == '\0') || *wordnum > objfinish) {
