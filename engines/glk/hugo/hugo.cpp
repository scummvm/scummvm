...
namespace Hugo {
	...
	Hugo::Hugo(OSystem *syst, const GlkGameDescription &gameDesc)
	  ...
	  , physical_windowheight(0)
	  , physical_windowtop(0)
	  , physical_windowleft(0)
	  , physical_windowbottom(0)
	  , physical_windowright(0)
	  , inwindow(0)
	  , charwidth(0)
	  , lineheight(0)
	  , current_text_x(0)
	  ...
	  , glk_fcolor(DEF_FCOLOR)
	  , glk_bgcolor(DEF_BGCOLOR)
	  , mainwin_bgcolor(0)
	  , glk_current_font(0)
	  ...
	  , speaking(0)
	  , oopscount(0)
	  , parse_called_twice(0)
	  , reparse_everything(0)
	  , full_buffer(0)
	  ...
	  , last_window_top(0)
	  , last_window_bottom(0)
	  , last_window_left(0)
	  , last_window_right(0)
	  , lowest_windowbottom(0)
	  , physical_lowest_windowbottom(0)
	...
	Common::Error Hugo::readSaveData(Common::SeekableReadStream *rs) {
		...
		if (hugo_fseek(game, objtable * 16L, SEEK_SET))
			goto RestoreError;
		i = 0;

		while (i < codeend - (long)(objtable * 16L)) {
			if ((hbyte = hugo_fgetc(rs)) == EOF && hugo_ferror(rs))
				goto RestoreError;

			if (hbyte == 0) {
				if ((lbyte = hugo_fgetc(rs)) == EOF && hugo_ferror(rs))
					goto RestoreError;
				SETMEM(objtable * 16L + i, (unsigned char)lbyte);
				i++;

				/* Skip byte in game file */
				if (hugo_fgetc(game) == EOF)
					goto RestoreError;
			} else {
				while (hbyte--) {
					/* Get unchanged game file byte */
					if ((lbyte = hugo_fgetc(game)) == EOF)
						goto RestoreError;
					SETMEM(objtable * 16L + i, (unsigned char)lbyte);
					i++;
				}
	...
	Common::Error Hugo::writeGameData(Common::WriteStream *ws) {
		...
		/* Write ID */
		if (hugo_fputc(id[0], ws) == EOF || hugo_fputc(id[1], ws) == EOF)
			goto SaveError;

		/* Write serial number */
		if (hugo_fputs(serial, ws) == EOF)
			goto SaveError;
		...
		for (i = 0; i <= codeend - (long)(objtable * 16L); i++) {
			...
			/* If memory differs (or samecount exceeds 1 byte) */
				...
				if (samecount)
					if (hugo_fputc(samecount, ws) == EOF)
						goto SaveError;

				if (lbyte != hbyte) {
					if (hugo_fputc(0, ws) == EOF)
						goto SaveError;
					if (hugo_fputc(hbyte, ws) == EOF)
						goto SaveError;
					samecount = 0;
				} else
					samecount = 1;
