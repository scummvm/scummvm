...
Graphics::Font *GraphicsManager::createArialFont(int size, bool bold) const {
	...
	if (stream) {
		font = Graphics::loadTTFFont(stream, DisposeAfterUse::YES, size, Graphics::kTTFSizeModeCharacter, 96, _vm->isTrueColor() ? Graphics::kTTFRenderModeLight : Graphics::kTTFRenderModeMonochrome);
	} else {
		const char *fname;
		if (bold)
...
Graphics::Font *GraphicsManager::createMSGothicFont(int size, bool bold) const {
	...
	if (stream) {
		...
		font = Graphics::loadTTFFont(stream, DisposeAfterUse::YES, size, Graphics::kTTFSizeModeCharacter, 96, Graphics::kTTFRenderModeMonochrome);
	} else {
