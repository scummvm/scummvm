...
void SaveLoad::saveGame(SavegameType type, EntityIndex entity, uint32 value) {
	...
	if (header.count > 0) {
		...
		if (!entry.isValid()) {
			...
			return;
		} else if ((type == kSavegameTypeTime || type == kSavegameTypeEvent) && (entry.type == kSavegameTypeTickInterval && (getState()->time - entry.time) < 450)) {
			_savegame->seek(header.offsetEntry);
			--header.count;
		} else {
			_savegame->seek(header.offset);
