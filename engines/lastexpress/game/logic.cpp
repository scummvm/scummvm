...
void Logic::eventMouse(const Common::Event &ev) {
	...
	if (getInventory()->getSelectedItem() == kItemWhistle
	 ...
	 && !getInventory()->isMagnifierInUse()) {
		...
		if (ev.type == Common::EVENT_LBUTTONUP && !getSoundQueue()->isBuffered("LIB045")) {
			...
			getSound()->playSoundEvent(kCharacterCath, 45);
			...
			if (getEntities()->isPlayerPosition(kCarGreenSleeping, 26) || getEntities()->isPlayerPosition(kCarGreenSleeping, 25) || getEntities()->isPlayerPosition(kCarGreenSleeping, 23)) {
				getSavePoints()->push(kCharacterCath, kCharacterCond1, kCharacterAction226078300);
				...
				getSavePoints()->push(kCharacterCath, kCharacterCond2, kCharacterAction226078300);
			}
	...
	if (getInventory()->getSelectedItem() == kItemMatch
	 ...
	 && !getInventory()->isEggHighlighted()
	 && !getInventory()->isMagnifierInUse()
	 && (getInventory()->get(kItem2)->location == kObjectLocationNone || getEntityData(kCharacterCath)->car != kCarRedSleeping || getEntityData(kCharacterCath)->entityPosition != kPosition_2300)) {
	...
	// Handle character item case
	CharacterIndex entityIndex = getEntities()->canInteractWith(ev.mouse);
	if (entityIndex
	 && !getInventory()->isPortraitHighlighted()
	 && !getInventory()->isOpened()
	 && !getInventory()->isEggHighlighted()
	 && !getInventory()->isMagnifierInUse()) {
		...
		if (getInventory()->hasItem((InventoryItem)(item & kItemToggleHigh))) {
			...
			if (ev.type == Common::EVENT_LBUTTONUP)
				getSavePoints()->push(kCharacterCath, entityIndex, kCharacterAction1, (InventoryItem)(item & kItemToggleHigh));
			...
			if (ev.type == Common::EVENT_LBUTTONUP)
				getSavePoints()->push(kCharacterCath, entityIndex, kCharacterAction1, kCursorNormal);
		}
	}
	...
	_engine->getCursor()->setStyle(getActionOld()->getCursor(*hotspot));
...
void Logic::eventTick(const Common::Event &) {
	...
	if (getFlags()->isGameRunning) {
		...
		if (getState()->timeDelta) {
			...
			if (!_ticksSinceLastSavegame) {
				_ticksSinceLastSavegame = EVENT_TICKS_BEETWEEN_SAVEGAMES;
				getSaveLoad()->saveGame(kSavegameTypeAuto, kCharacterMaster, kEventNone);
			}

			// Save after game ticks interval
			if ((getState()->timeTicks - getSaveLoad()->getLastSavegameTicks()) > GAME_TICKS_BEETWEEN_SAVEGAMES)
				getSaveLoad()->saveGame(kSavegameTypeTickInterval, kCharacterMaster, kEventNone);
		}
	}
	...
	//////////////////////////////////////////////////////////////////////////
	// Update cursor if we can interact with an character
	CharacterIndex entity = getEntities()->canInteractWith(getCoords());
	if (!entity) {
...
void Logic::gameOver(SavegameType type, uint32 value, SceneIndex sceneIndex, bool showScene) const {
	...
	if (showScene) {
		...
		if (sceneIndex && !getFlags()->mouseRightClick) {
			...
			while (getSoundQueue()->isBuffered(kCharacterTableE)) {
	...
	// Show Menu
	getMenuOld()->show(false, type, value);
}
...
void Logic::switchChapter() const {
	...
	case kChapter1:
		...
		RESET_ENTITY_STATE(kCharacterMaster, Chapters, setup_chapter2);
		break;

	case kChapter2:
		getInventory()->addItem(kItemScarf);

		RESET_ENTITY_STATE(kCharacterMaster, Chapters, setup_chapter3);
		break;
	...
	case kChapter3:
		...
		RESET_ENTITY_STATE(kCharacterMaster, Chapters, setup_chapter4);
		break;

	case kChapter4:
		RESET_ENTITY_STATE(kCharacterMaster, Chapters, setup_chapter5);
		break;
...
void Logic::playFinalSequence() const {
	...
	getMenuOld()->show(false, kSavegameTypeIndex, 0);
}
	 ...
	 || getInventory()->isMagnifierInUse()) {
		...
		if (getInventory()->getSelectedItem() != kItemMatch
		 ...
		 || (getInventory()->get(kItem2)->location
		  && getEntityData(kCharacterCath)->car == kCarRedSleeping
		  && getEntityData(kCharacterCath)->entityPosition == kPosition_2300)) {
			...
			if (!interact
			 ...
			 && !getInventory()->isMagnifierInUse()) {
				...
				style = (hotspot) ? getActionOld()->getCursor(*hotspot) : kCursorNormal;
			}
		} else {
