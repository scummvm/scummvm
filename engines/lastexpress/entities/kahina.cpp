...
case kActionNone:
	if (getEntities()->updateEntity(_entityIndex, (CarIndex)params->param1, (EntityPosition)params->param2)) {
		callbackAction();
	} else if (getEntities()->isDistanceBetweenEntities(kEntityKahina, kEntityPlayer, 1000) && !getEntities()->isInGreenCarEntrance(kEntityPlayer) && !getEntities()->isInsideCompartments(kEntityPlayer) && !getEntities()->checkFields10(kEntityPlayer)) {
...
case kActionCallback:
	...
	case 4:
		if (getEntities()->isInsideCompartment(kEntityPlayer, kCarGreenSleeping, kPosition_8200) || getEntities()->isOutsideAlexeiWindow() || getEntities()->isDistanceBetweenEntities(kEntityKahina, kEntityPlayer, 2000)) {
...
label_callback_2:
	...
	if (!getProgress().field_44 && getState()->time > kTime2214000) {
