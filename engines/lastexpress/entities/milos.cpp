...
case kActionNone:
	if (params->param1) {
		if (getEntities()->isDistanceBetweenEntities(kEntityMilos, kEntityVesna, 750) || getEntities()->checkDistanceFromPosition(kEntityVesna, kPosition_3050, 500)) {
...
case kActionDrawScene:
	if (!getEvent(kEventMilosCompartmentVisitAugust) && !getEntities()->isInsideTrainCar(kEntityPlayer, kCarRedSleeping) && params->param1)
		setup_chapter2Handler();
	break;
...
case kActionDrawScene:
	if (getEvent(kEventMilosCompartmentVisitAugust) || getEntities()->isInsideTrainCar(kEntityPlayer, kCarRedSleeping) || !params->param3 || getState()->time >= kTime2106000) {
...
case kActionNone:
	...
	if (getEntities()->isDistanceBetweenEntities(kEntityMilos, kEntityPlayer, 1000) && !getEntities()->isInGreenCarEntrance(kEntityPlayer) && !getEntities()->isInsideCompartments(kEntityPlayer) && !getEntities()->checkFields10(kEntityPlayer)) {
