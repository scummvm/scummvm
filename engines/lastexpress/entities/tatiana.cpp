...
case kActionDrawScene:
	...
	params->param4 = getEntities()->isPlayerPosition(kCarRestaurant, 69) || getEntities()->isPlayerPosition(kCarRestaurant, 70) || getEntities()->isPlayerPosition(kCarRestaurant, 71);
	break;
...
case kActionNone:
	...
	label_update:
		if (!getEvent(kEventTatianaAskMatchSpeakRussian) && !getEvent(kEventTatianaAskMatch) && getInventory()->hasItem(kItemMatchBox) && getEntities()->isInGreenCarEntrance(kEntityPlayer)) {
...
case kActionNone:
	if (!params->param1 && getInventory()->hasItem(kItemFirebird) && getEntities()->checkFields19(kEntityPlayer, kCarRedSleeping, kPosition_7850) && (getState()->time < kTime2133000 || getProgress().field_40)) {
		setCallback(1);
		setup_trapCath();
		break;
	}
...
case kActionNone:
	if (getEntities()->isInsideTrainCar(kEntityPlayer, kCarKronos) || getData()->car != getEntityData(kEntityPlayer)->car || getEntities()->updateEntity(kEntityTatiana, kCarKronos, kPosition_9270))
		callbackAction();
	break;
...
case kActionNone:
	...
	if (getEntities()->checkFields19(kEntityPlayer, kCarRedSleeping, kPosition_7850) && !getEvent(kEventVassiliCompartmentStealEgg) && (getState()->time <= kTime2133000 || getProgress().field_40)) {
