...
Mertens::Mertens(LastExpressEngine *engine)
  : Entity(engine, kEntityMertens) {
...
case kActionNone:
	...
	getData()->inventoryItem = (getProgress().chapter == kChapter1
	                            ...
	                            && !getEvent(kEventMertensAskTylerCompartmentD))
	  ? kItemMatchBox
	  : kItemNone;
	break;
...
case kActionDefault:
	...
	if (!getSoundQueue()->isBuffered(kEntityMertens)) {
		if (getProgress().chapter == kChapter3 && !params->param1 && getState()->time < kTime2173500 && (getState()->time > kTime2106000 || (params->param2 != kEntityPlayer && getState()->time > kTime2079000))) {
