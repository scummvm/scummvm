...
case kActionCallback:
	...
	case 1:
		...
		if (!getEntities()->isInKronosSalon(kEntityPlayer)) {
			...
			if (getEntityData(kEntityPlayer)->car > kCarRedSleeping || (getEntityData(kEntityPlayer)->car == kCarRedSleeping && getEntityData(kEntityPlayer)->entityPosition > kPosition_9270)) {
				getSound()->playSound(kEntityPlayer, "BUMP");
				...
				if (getEntityData(kEntityPlayer)->car < kCarGreenSleeping || (getEntityData(kEntityPlayer)->car == kCarGreenSleeping && getEntityData(kEntityPlayer)->entityPosition < kPosition_4840)) {
					getSound()->playSound(kEntityPlayer, "BUMP");
			...
			if (getEntities()->isInsideCompartment(kEntityPlayer, kCarRedSleeping, kPosition_4840) || getEntities()->isInsideCompartment(kEntityPlayer, kCarRedSleeping, kPosition_4455)) {
