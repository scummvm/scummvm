...
case kActionNone:
	if (ENTITY_PARAM(0, 1) && params->param2 != kTimeInvalid) {
		...
		if (getState()->time <= kTime2038500) {
			if (!getEntities()->isPlayerInCar(kCarRedSleeping) || !params->param1 || getSoundQueue()->isBuffered("FRA2012") // the original game tests this sound twice. Maybe a bug?
			    || getSoundQueue()->isBuffered("FRA2010") || !params->param2)
