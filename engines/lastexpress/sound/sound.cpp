...
const char *SoundManager::getDialogName(EntityIndex entity) const {
	...
	case kEntityAugust:
		...
		if (getEvent(kEventDinerAugust) || getEvent(kEventDinerAugustAlexeiBackground) || getEvent(kEventMeetAugustTylerCompartment) || getEvent(kEventMeetAugustTylerCompartmentBed) || getEvent(kEventMeetAugustHisCompartment) || getEvent(kEventMeetAugustHisCompartmentBed))
			return "XAUG3";
	...
	case kEntityFrancois:
		...
		if (getProgress().field_A0 || getEvent(kEventFrancoisWhistle) || getEvent(kEventFrancoisWhistleD) || getEvent(kEventFrancoisWhistleNight) || getEvent(kEventFrancoisWhistleNightD))
			return "XFRA2";
...
SoundFlag SoundManager::getSoundFlag(EntityIndex entity) const {
	...
	if (getEntityData(kEntityPlayer)->location == kLocationOutsideTrain) {
		if (getEntityData(kEntityPlayer)->car != kCarKronos && !getEntities()->isOutsideAlexeiWindow() && !getEntities()->isOutsideAnnaWindow())
			return kVolumeNone;
	...
	case kCarRedSleeping:
		...
		if (getEntityData(kEntityPlayer)->location == kLocationInsideCompartment && (getEntityData(entity)->location != kLocationInsideCompartment || !getEntities()->isDistanceBetweenEntities(kEntityPlayer, entity, 400)))
			ret = (SoundFlag)(ret / 2);
		break;
