...
Scheduler::Scheduler(HugoEngine *vm)
  : _vm(vm) {
	_actListArr = nullptr;
	_curTick = 0;
	_oldTime = 0;
...
void Scheduler::newScreen(const int screenIndex) {
	...
	if (!_vm->isPacked()) {
		...
		if (!Common::File::exists(_vm->_picDir + filename + ".PCX") && !Common::File::exists(filename + ".ART")) {
...
Event *Scheduler::doAction(Event *curEvent) {
	...
	case COND_BOX: // act25: Conditional on bounding box
		...
		if ((dx >= action->_a25._x1) && (dx <= action->_a25._x2) && (dy >= action->_a25._y1) && (dy <= action->_a25._y2))
...
Scheduler_v1d::Scheduler_v1d(HugoEngine *vm)
  : Scheduler(vm) {
}
...
Scheduler_v2d::Scheduler_v2d(HugoEngine *vm)
  : Scheduler_v1d(vm) {
}
...
Scheduler_v3d::Scheduler_v3d(HugoEngine *vm)
  : Scheduler_v2d(vm) {
}
...
Scheduler_v1w::Scheduler_v1w(HugoEngine *vm)
  : Scheduler_v3d(vm) {
}
