...
void FileManager_v3d::readBackground(const int screenIndex) {
	...
	_sceneryArchive1.seek((uint32) screenIndex * sizeof(SceneBlock), Seek::SET);
...
void FileManager_v3d::readOverlay(const int screenNum, ImagePtr image, OvlType overlayType) {
	...
	if (screenNum < 20) {
		...
		switch (overlayType) {
		case kOvlBoundary:
			_sceneryArchive2.seek(sceneBlock._boundaryOffset, Seek::SET);
			i = sceneBlock._boundaryLength;
			break;
		case kOvlOverlay:
			_sceneryArchive2.seek(sceneBlock._overlayOffset, Seek::SET);
			i = sceneBlock._overlayLength;
			break;
		case kOvlBase:
			_sceneryArchive2.seek(sceneBlock._baseOffset, Seek::SET);
			i = sceneBlock._baseLength;
			break;
		default:
			error("Bad overlayType: %d", overlayType);
