...
// The queries below take the file or folder name as their only argument.
static Common::String dosFileNameArg(int nargs) {
	Common::String name;
...
void DirectOSXtra::m_dosGetVersion(int nargs) {
	g_lingo->dropStack(nargs);
	g_lingo->push(Datum(Common::String("2.01")));
}
...
void DirectOSXtra::m_dosIsFileExist(int nargs) {
	Common::String name = dosFileNameArg(nargs);
...
void DirectOSXtra::m_dosIsFolderExist(int nargs) {
	Common::String name = dosFileNameArg(nargs);
