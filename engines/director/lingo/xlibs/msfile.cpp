...
bool MSFile::copyStream(Common::SeekableReadStream* srcStream, Common::SeekableWriteStream* destStream) {
	...
	if (srcStream && destStream) {
		...
		// Allocate a buffer to hold the read file contents
		buffer = malloc(size);
		assert(buffer);

		// Read the full file contents into the buffer
		srcStream->read(buffer, size);
		bool error = srcStream->err();
		delete srcStream;
		...
		// Write the source file buffer to the destination file. This is effectively a file copy.
