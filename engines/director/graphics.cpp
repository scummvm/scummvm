...
void DirectorEngine::testFontScaling() {
	...
	Graphics::MacFONTFont::testBlit(font1, &surface, 255, x, y + 200, 500);

	Graphics::MacFont bigFont(Graphics::kMacFontNewYork, 15);

	font1 = (const Graphics::MacFONTFont *)_wm->_fontMan->getFont(bigFont);

	Graphics::MacFONTFont::testBlit(font1, &surface, 255, x, y + 50 + 200, 500);
	...
	for (int i = 9; i <= 20; i++) {
		...
		font->drawString(&surface, text, x, y, width, 255);
	...
	for (int i = 0; i < 16; i++) {
		for (int j = 0; j < 16; j++) {
			...
			for (x = x1; x < x1 + 6; x++)
				for (y = y1; y < y1 + 6; y++)
					*((byte *)surface.getBasePtr(x, y)) = (i * 16 + j);
		}
	}
