...
int16 ScriptFunctions::sfDrawText(int16 argc, int16 *argv) {
	...
	if (text) {
		...
		// 1946 is the text for the save/load screen, which we don't want to voice here (since it's
		...
		if (argv[argc - 1] != 1946 && argv[argc - 1] != 1828 && argv[argc - 1] != 1831 && argv[argc - 1] != 1837 &&
				argv[argc - 1] != 1843 && _vm->getGameID() == GID_RTZ) {
