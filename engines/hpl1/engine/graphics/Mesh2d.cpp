...
namespace hpl {
	...
	tVertexVec* cMesh2D::GetVertexVec(const cRect2f& aImageRect,cVector2f avSize,eTileRotation aRotation)
	{
		...
		for(int j=0;j<(int)mvVtx[aRotation].size();j++)
		{
			...
			switch(aRotation){
				case eTileRotation_0: mvVtx[aRotation][j].norm = cVector3f(1,0,3); break;
				case eTileRotation_90: mvVtx[aRotation][j].norm = cVector3f(0,1,3); break;
				case eTileRotation_180: mvVtx[aRotation][j].norm = cVector3f(-1,0,3); break;
				case eTileRotation_270: mvVtx[aRotation][j].norm = cVector3f(0,-1,3); break;
			}
		}
	...
	void cMesh2D::CalculateEdges(eTileRotation aRotation,tVertexVec &aVtx, tUIntVec &aIdx)
	{
		...
		for(int i=0;i<(int)aIdx.size();i++)
		{
			...
			//Y:
			if(aVtx[aIdx[i]].pos.y>vLargest.y)vLargest.y = aVtx[aIdx[i]].pos.y;
			else if(aVtx[aIdx[i]].pos.y<vSmallest.y)vSmallest.y = aVtx[aIdx[i]].pos.y;
