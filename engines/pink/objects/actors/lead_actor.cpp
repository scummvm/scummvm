...
void LeadActor::start(bool isHandler) {
	...
	case kPDA:
		if (_stateBeforePDA == kInventory)
			startInventory(1);
		_page->getGame()->getSurface()->saveStage();
...
void LeadActor::onPDAClose() {
	_page->initPalette();
	_page->getGame()->getSurface()->loadStage();
...
void LeadActor::forceUpdateCursor() {
	...
	vm->getSurface()->update(); // we have actions, that should be drawn to properly update cursor
...
Actor *LeadActor::getActorByPoint(Common::Point point) {
	return _page->getGame()->getSurface()->getActorByPoint(point);
}
