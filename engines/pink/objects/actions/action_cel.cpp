...
bool ActionCEL::initPalette(Surface *surface) {
	loadDecoder();
	if (_decoder.getCurFrame() == -1) {
		_decoder.decodeNextFrame();
		_decoder.rewind();
	}
	surface->setPalette(_decoder.getPalette());
	return true;
}
...
void ActionCEL::start() {
	...
	_decoder.start();
	this->onStart();
	_actor->getPage()->getGame()->getSurface()->addSprite(this);
}

void ActionCEL::end() {
	_actor->getPage()->getGame()->getSurface()->removeSprite(this);
	_decoder.close();
}
...
void ActionCEL::setFrame(uint frame) {
	...
	_actor->getPage()->getGame()->getSurface()->addDirtyRect(_bounds);
}

void ActionCEL::decodeNext() {
	_decoder.decodeNextFrame();
	_actor->getPage()->getGame()->getSurface()->addDirtyRects(this);
}
