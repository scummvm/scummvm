...
#define SCI_WORKAROUNDENTRY_TERMINATOR                                      \
	{                                                                         \
		(SciGameId)0, -1, -1, 0, NULL, NULL, NULL, 0, 0, { WORKAROUND_NONE, 0 } \
	}
#define SCI_MESSAGEWORKAROUNDENTRY_TERMINATOR                                                                           \
	{                                                                                                                     \
		(SciGameId)0, (SciMedia)0, (kLanguage)0, -1, -1, 0, 0, 0, 0, { MSG_WORKAROUND_NONE, -1, 0, 0, 0, 0, 0, 0, 0, NULL } \
	}
	...
	if (workaroundList) {
		...
		do {
			...
			while (workaround->methodName) {
				bool objectNameMatches = (workaround->objectName == NULL) || (workaround->objectName == searchObjectName);
...
static SciMessageWorkaroundSolution findMessageWorkaround(int module, byte noun, byte verb, byte cond, byte seq, const SciMessageWorkaroundEntry *workaroundList) {
	...
	while (workaround->solution.type != MSG_WORKAROUND_NONE) {
		if (workaround->gameId == g_sci->getGameId() && (workaround->media == SCI_MEDIA_ALL || (workaround->media == SCI_MEDIA_FLOPPY && !g_sci->isCD()) || (workaround->media == SCI_MEDIA_CD && g_sci->isCD())) && (workaround->language == K_LANG_NONE || workaround->language == g_sci->getSciLanguage()) && (workaround->roomNumber == -1 || workaround->roomNumber == g_sci->getEngineState()->currentRoomNumber()) && workaround->module == module && workaround->noun == noun && workaround->verb == verb && workaround->cond == cond && workaround->seq == seq) {
			break;
		}
		workaround++;
	}
