...
static void fix_vertex_order(Polygon *polygon) {
	...
	// clockwise
	if (((area > 0) && (polygon->type == POLY_CONTAINED_ACCESS)) || ((area < 0) && (polygon->type != POLY_CONTAINED_ACCESS))) {
...
bool PathfindingState::edgeOnScreenBorder(const Common::Point &p, const Common::Point &q) {
	return ((p.x == 0 && q.x == 0) || (p.y == 0 && q.y == 0) || ((p.x == _width - 1) && (q.x == _width - 1)) || ((p.y == _height - 1) && (q.y == _height - 1)));
}
...
static bool nearbyPolygon(const Common::Point &point, Polygon *polygon) {
	...
	return ((contained(Common::Point(point.x, point.y + 1), polygon) != CONT_INSIDE) || (contained(Common::Point(point.x, point.y - 1), polygon) != CONT_INSIDE) || (contained(Common::Point(point.x + 1, point.y), polygon) != CONT_INSIDE) || (contained(Common::Point(point.x - 1, point.y), polygon) != CONT_INSIDE));
}
...
static Polygon *convert_polygon(EngineState *s, reg_t polygon) {
	...
	if ((size == 19) && g_sci->getGameId() == GID_LSL1) {
		if ((s->currentRoomNumber() == 350) && (readPoint(pointList, 18) == Common::Point(108, 137))) {
...
reg_t kIntersections(EngineState *s, int argc, reg_t *argv) {
	...
	while (1) {
		...
		if (foundIntersection) {
			...
			if (PointInRect(Common::Point(intersectionX, intersectionY), pSourceX, pSourceY, pDestX, pDestY) && PointInRect(Common::Point(intersectionX, intersectionY), qSourceX, qSourceY, qDestX, qDestY)) {
