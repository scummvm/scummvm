...
struct SCALER_NoScale {
	...
	SCALER_NoScale(const CelObj &celObj, const int16 maxWidth, const Common::Point &scaledPosition) : _row(nullptr),
...
struct SCALER_Scale {
	...
	SCALER_Scale(const CelObj &celObj, const Common::Rect &targetRect, const Common::Point &scaledPosition, const Ratio scaleX, const Ratio scaleY) : _row(nullptr),
#ifndef NDEBUG
		...
		if (useLarryScale) {
			...
			public:
				Copier(READER &souceReader, Buffer &targetBuffer) : _souceReader(souceReader),
...
public:
	READER_Compressed(const CelObj &celObj, const int16 maxWidth) : _resource(celObj.getResPointer()),
	                                                                _y(-1),
	                                                                _sourceHeight(celObj._height),
...
struct RENDERER {
	...
	RENDERER(MAPPER &mapper, SCALER &scaler, const uint8 skipColor) : _mapper(mapper),
	                                                                  _scaler(scaler),
