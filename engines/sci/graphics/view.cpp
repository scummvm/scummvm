...
void GfxView::initData(GuiResourceId resourceId) {
	...
	case GID_LAURABOW2:
		...
		if (g_sci->isCD() && resourceId == 995) {
			// security checks
			if (_loop.size() >= 14 && _loop[13].cel.size() == 2 && _loop[13].cel[0].width == 46 && _loop[13].cel[0].height == 11) {
	...
	case GID_KQ6:
		...
		if (g_sci->isCD() && resourceId == 947) {
			// security checks
			if (_loop.size() == 12 && _loop[8].cel.size() == 2 && _loop[8].cel[0].width == 50 && _loop[8].cel[0].height == 15) {
...
void GfxView::unditherBitmap(SciSpan<byte> &bitmapPtr, int16 width, int16 height, byte clearKey) {
	...
	for (y = 0; y < checkHeight; y++) {
		color1 = curPtr[0];
		color2 = (curPtr[1] << 4) | curPtr[2];
		nextColor1 = nextPtr[0] << 4;
		nextColor2 = (nextPtr[2] << 4) | nextPtr[1];
		curPtr += 3;
		nextPtr += 3;
		for (x = 3; x < width; x++) {
