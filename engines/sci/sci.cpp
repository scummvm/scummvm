	...
	if (isCD()) {
		...
		if (useGlobal90) {
			if (subtitlesOn && !speechOn) {
				_gamestate->variables[VAR_GLOBAL][kGlobalMessageType] = make_reg(0, 1);	// subtitles
			} else if (!subtitlesOn && speechOn) {
				_gamestate->variables[VAR_GLOBAL][kGlobalMessageType] = make_reg(0, 2);	// speech
			} else if (subtitlesOn && speechOn) {
...
#endif // ENABLE_SCI32
					_gamestate->variables[VAR_GLOBAL][kGlobalMessageType] = make_reg(0, 3);	// speech + subtitles
					break;
				default:
...
void SciEngine::updateScummVMAudioOptions() {
	...
	if (isCD() && getSciVersion() == SCI_VERSION_1_1) {
		uint16 ingameSetting = _gamestate->variables[VAR_GLOBAL][kGlobalMessageType].getOffset();
