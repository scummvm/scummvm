...
int Audio32::readBuffer(Audio::st_sample_t *const buffer, const int numSamples) {
	...
	// engine, which halves the content of the target
	...
	// the earliest channel having the highest amount of
	// attenuation (lowest volume) and the final channel
	// having no attenuation.
	uint8 attenuationAmount = _numActiveChannels;
	...
	for (int16 channelIndex = 0; channelIndex < _numActiveChannels; ++channelIndex) {
		...
		if (channelIndex == _monitoredChannelIndex) {
			...
			int channelSamplesWritten = writeAudioInternal(channel.stream, channel.converter, buffer, numSamples, leftVolume, rightVolume, channel.loop);
...
int16 Audio32::findChannelByArgs(int argc, const reg_t *argv, const int startIndex, const reg_t soundNode) const {
	...
	// This is a plain Audio resource
	if (argc < 5) {
		const ResourceId searchId(kResourceTypeAudio, argv[startIndex].toUint16());

		for (int16 i = 0; i < _numActiveChannels; ++i) {
			const AudioChannel &candidate = getChannel(i);

			if (
				candidate.id == searchId &&
				(soundNode.isNull() || candidate.soundNode == soundNode)
			) {
				return i;
			}
		}
	// This is an Audio36 resource
	} else {
		const ResourceId searchId(kResourceTypeAudio36, argv[startIndex].toUint16(), argv[startIndex + 1].toUint16(), argv[startIndex + 2].toUint16(), argv[startIndex + 3].toUint16(), argv[startIndex + 4].toUint16());

		for (int16 i = 0; i < _numActiveChannels; ++i) {
			const AudioChannel &candidate = getChannel(i);

			if (candidate.robot == false && candidate.id == searchId) {
				return i;
			}
		}
	}
...
reg_t Audio32::kernelPlay(const bool autoPlay, const int argc, const reg_t *const argv) {
	if (argc == 0) {
		return make_reg(0, getNumActiveChannels());
	}
