...
void Gfx::loadECSExtraPalettes() {
	...
	static const PalEntry ecsBottomPal[32] = {
		{ 0x00, 0x00, 0x00 },
		{ 0x00, 0x00, 0x00 },
		{ 0x33, 0x11, 0x11 },
		{ 0x00, 0x44, 0x33 },
	...
	for (int i = 0; i < 32; i++) {
		_currentPal[(i + 32) * 3] = _globalPalette[i + 32].red = ecsBottomPal[i].red;
		_currentPal[(i + 32) * 3 + 1] = _globalPalette[i + 32].green = ecsBottomPal[i].green;
		_currentPal[(i + 32) * 3 + 2] = _globalPalette[i + 32].blue = ecsBottomPal[i].blue;
	}
}
