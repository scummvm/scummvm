...
Interface::Interface(SagaEngine *vm) : _vm(vm) {
	...
	if (_vm->getGameId() == GID_ITE && _vm->getLanguage() == Common::ZH_TWN) {
		...
		for (unsigned y = 0; y < 43; y++)
			memset(n.getBuffer() + y * _mainPanel.imageWidth + 53, _vm->isECS() ? kITEECSColorBlue : kITEDOSColorBlue89, 114);
		_mainPanel.image = n;
	}
...
void Interface::drawOption() {
	...
	if (_optionSaveRectTop.height() > 0) {
		if (_vm->getGameId() == GID_ITE)
			_vm->_gfx->drawRect(_optionSaveRectTop, _vm->iteColorDarkGrey());
	}
	...
	if (_optionSaveRectBottom.height() > 0) {
		_vm->_gfx->drawRect(_optionSaveRectBottom, _vm->iteColorDarkGrey());
	}
	...
	for (uint j = 0; j < _vm->getDisplayInfo().optionSaveFileVisible; j++) {
		if (_vm->getGameId() == GID_ITE)
			bgColor = _vm->iteColorDarkGrey0C();
		else
			bgColor = _vm->KnownColor2ColorId(kKnownColorBlack);
		fgColor = _vm->iteColorBrightWhite();
...
void Interface::drawInventory() {
	...
	for (int i = 0; i < _mainPanel.buttonsCount; i++) {
		...
		if (_vm->getGameId() == GID_ITE)
			_vm->_gfx->drawRect(rect, _vm->iteColorDarkGrey());
		else
...
void Interface::drawButtonBox(const Rect& rect, ButtonKind kind, bool down) {
	...
	case kSlider:
		...
		fillColor = _vm->iteColorLightBlue96();
		odl = _vm->iteColorDarkBlue8a();
		our = _vm->iteColorLightBlue92();
		idl = 0x89;
		iur = 0x94;
		solidColor = down ? _vm->iteColorLightBlue94() : _vm->iteColorLightBlue96();
		break;
	case kEdit:
		if (_vm->getGameId() == GID_ITE) {
			cornerColor = frameColor = fillColor = _vm->iteColorLightBlue96();
			our = _vm->iteColorDarkBlue8a();
			odl = _vm->iteColorLightBlue94();
			solidColor = down ? _vm->iteColorBlue() : _vm->iteColorDarkGrey0C();
		} else {
	...
	default:
		cornerColor = 0x8b;
		frameColor = _vm->KnownColor2ColorId(kKnownColorBlack);
		solidColor = fillColor = _vm->iteColorLightBlue96();
		odl = _vm->iteColorDarkBlue8a();
		our = _vm->iteColorLightBlue94();
		idl = 0x97;
		iur = 0x95;
		if (down) {
			SWAP(odl, our);
...
void Interface::converseDisplayTextLines() {
	...
	if (_vm->getGameId() == GID_ITE) {
		bulletForegnd = _vm->iteColorGreen();
	} else {
	...
	if (_vm->getGameId() == GID_ITE)
		_vm->_gfx->drawRect(rect, _vm->iteColorDarkGrey());	// fill bullet place
	...
	for (int i = 0; i < _vm->getDisplayInfo().converseTextLines; i++) {
		...
		if (_conversePos >= 0 && _converseText[_conversePos].stringNum == _converseText[relPos].stringNum) {
			if (_vm->getGameId() == GID_ITE) {
				foregnd = _vm->iteColorBrightWhite();
				backgnd = (!_vm->leftMouseButtonPressed()) ? _vm->iteColorDarkGrey() : _vm->iteColorGrey();
			} else {
			...
			if (_vm->getGameId() == GID_ITE) {
				foregnd = _vm->iteColorBlue();
				backgnd = _vm->iteColorDarkGrey();
			} else {
