...
protected:
	virtual bool loadMacMIDI() override;
	virtual bool loadRes(uint32 contextOffset, uint32 contextSize) override {
		return loadResV1(contextOffset, contextSize);
	}
	virtual void processPatches(Resource *resource, const GamePatchDescription *patchFiles) override;
};

class Resource_RSC : public Resource {
public:
	Resource_RSC(SagaEngine *vm) : Resource(vm) {}
	virtual uint32 convertResourceId(uint32 resourceId) override {
		return _vm->isMacResources() ? resourceId - 2 : resourceId;
	}
	virtual void loadGlobalResources(int chapter, int actorsEntrance) override {}
	virtual MetaResource* getMetaResource() override {
		MetaResource *dummy = 0;
		return dummy;
	}
protected:
	virtual ResourceContext *createContext() override {
...
public:
	...
	virtual uint32 convertResourceId(uint32 resourceId) override { return resourceId; }
	virtual void loadGlobalResources(int chapter, int actorsEntrance) override;
	virtual MetaResource* getMetaResource() override { return &_metaResource; }
protected:
	virtual ResourceContext *createContext() override {
