...
static const int kamenickyEncodingTable[] = {
...
static const int polishEncodingTable[] = {
...
static const int czechCharacterConversionTable[] = {
...
void DraciEngine::sayText(const Common::String &text, bool isSubtitle, bool voiceSampleExists) {
	Common::TextToSpeechManager *ttsMan = g_system->getTextToSpeechManager();
	bool speak = (!isSubtitle && ConfMan.getBool("tts_enabled_objects")) || 
				 (isSubtitle && (ConfMan.getBool("tts_enabled_speech") || 
				 (ConfMan.getBool("tts_enabled_missing_voice") && !voiceSampleExists)));
	...
	if (ttsMan != nullptr && speak && _previousSaid != text) {
