...
#define ARG_OBJECT_FROM_PTR(x)                         \
	ARG_UC_PTR(ucptr_##x);                             \
	uint16 id_##x = UCMachine::ptrToObject(ucptr_##x); \
	Object *x = getObject(id_##x);
#define ARG_OBJECT_FROM_ID(x) \
	ARG_OBJID(id_##x);        \
	Object *x = getObject(id_##x);

#define ARG_ITEM_FROM_PTR(x)                           \
	ARG_UC_PTR(ucptr_##x);                             \
	uint16 id_##x = UCMachine::ptrToObject(ucptr_##x); \
	Item *x = getItem(id_##x);
#define ARG_ITEM_FROM_ID(x) \
	ARG_OBJID(id_##x);      \
	Item *x = getItem(id_##x);

#define ARG_CONTAINER_FROM_PTR(x)                      \
	ARG_UC_PTR(ucptr_##x);                             \
	uint16 id_##x = UCMachine::ptrToObject(ucptr_##x); \
	Container *x = getContainer(id_##x);
#define ARG_CONTAINER_FROM_ID(x) \
	ARG_OBJID(id_##x);           \
	Container *x = getContainer(id_##x);

#define ARG_ACTOR_FROM_PTR(x)                          \
	ARG_UC_PTR(ucptr_##x);                             \
	uint16 id_##x = UCMachine::ptrToObject(ucptr_##x); \
	Actor *x = getActor(id_##x);
#define ARG_ACTOR_FROM_ID(x) \
	ARG_OBJID(id_##x);       \
	Actor *x = getActor(id_##x);

#define ARG_EGG_FROM_PTR(x)                            \
	ARG_UC_PTR(ucptr_##x);                             \
	uint16 id_##x = UCMachine::ptrToObject(ucptr_##x); \
	Egg *x = p_dynamic_cast<Egg *>(getObject(id_##x));
#define ARG_EGG_FROM_ID(x) \
	ARG_OBJID(id_##x);     \
	Egg *x = p_dynamic_cast<Egg *>(getObject(id_##x));
