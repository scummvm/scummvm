...
inline bool SortItem::operator<(const SortItem &si2) const {
	...
	if (si1._x <= si2._xLeft)
		return true;
	else if (si1._xLeft >= si2._x)
		return false;

	// Clearly in y?
	if (si1._y <= si2._yFar)
		return true;
	else if (si1._yFar >= si2._y)
		return false;
	...
	if (si1._z < si2._z)
		return true;
	else if (si1._z > si2._z)
		return false;

	// Biased Clearly in z
	if ((si1._zTop + si1._z) / 2 <= si2._z)
		return true;
	else if (si1._z >= (si2._zTop + si2._z) / 2)
		return false;

	// Biased Clearly X
	if ((si1._x + si1._xLeft) / 2 <= si2._xLeft)
		return true;
	else if (si1._xLeft >= (si2._x + si2._xLeft) / 2)
		return false;

	// Biased Clearly Y
	if ((si1._y + si1._yFar) / 2 <= si2._yFar)
		return true;
	else if (si1._yFar >= (si2._y + si2._yFar) / 2)
		return false;
...
ItemSorter::ItemSorter() : _shapes(nullptr), _surf(nullptr), _items(nullptr), _itemsTail(nullptr),
                           _itemsUnused(nullptr), _sortLimit(0), _camSx(0), _camSy(0), _orderCounter(0) {
	int i = 2048;
	while (i--)
		_itemsUnused = new SortItem(_itemsUnused);
}
...
void ItemSorter::BeginDisplayList(RenderSurface *rs,
	...
	if (!_shapes)
		_shapes = GameData::get_instance()->getMainShapes();
...
uint16 ItemSorter::Trace(int32 x, int32 y, HitFace *face, bool item_highlight) {
	...
	if (item_highlight) {
		...
		for (it = _itemsTail; it != nullptr; it = it->_prev) {
			if (!(it->_flags & (Item::FLG_DISPOSABLE | Item::FLG_FAST_ONLY)) && !it->_fixed) {
				...
				// Doesn't Overlap
				if (x < it->_sx || x >= it->_sx2 || y < it->_sy || y >= it->_sy2)
					continue;
				...
				if (it->_flags & Item::FLG_FLIPPED) {
					if (!_frame->hasPoint(it->_sxBot - x, y - it->_syBot))
						continue;
				} else {
					if (!_frame->hasPoint(x - it->_sxBot, y - it->_syBot))
						continue;
				}
	...
	if (!selected)
		for (it = _items; it != nullptr; it = it->_next) {
			...
			// Ok now check against selected
			if (!selected || (it->_order > selected->_order))
				selected = it;
		}
