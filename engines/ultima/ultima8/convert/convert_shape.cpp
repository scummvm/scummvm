...
void ConvertShape::Read(IDataSource *source, const ConvertShapeFormat *csf, uint32 real_len) {
	...
	if (csf->_bytes_special) {
		...
		for (uint32 i = 0; i < csf->_bytes_special; i++)
			special[source->readByte() & 0xFF] = i + 2;
	}

	// Read the header unknown
	if (csf->_bytes_header_unk)
		source->read(_header_unknown, csf->_bytes_header_unk);

#ifdef COMP_SHAPENUM
	if (shapenum == COMP_SHAPENUM)
		pout << Std::hex;
#endif

	// Now read _num_frames
	_num_frames = 1;
	if (csf->_bytes_num_frames)
		_num_frames = source->readX(csf->_bytes_num_frames);
	if (_num_frames == 0)
		_num_frames = CalcNumFrames(source, csf, real_len, start_pos);

#ifdef COMP_SHAPENUM
	if (shapenum == COMP_SHAPENUM)
		pout << "_num_frames " << _num_frames << Std::endl;
#endif
	...
	for (uint32 f = 0; f < _num_frames; ++f) {
...
#ifdef COMP_SHAPENUM
		if (shapenum == COMP_SHAPENUM)
			pout << "Seeking to " << (csf->_len_header + (csf->_len_frameheader * f)) << Std::endl;
		if (shapenum == COMP_SHAPENUM)
			pout << "Real " << (start_pos + csf->_len_header + (csf->_len_frameheader * f)) << Std::endl;
#endif
		// Seek to initial pos
...
#ifdef COMP_SHAPENUM
		if (shapenum == COMP_SHAPENUM)
			pout << "seeked to " << source->pos() << Std::endl;
#endif

		// Read the offset
		uint32 frame_offset = csf->_len_header + (csf->_len_frameheader * f);
		if (csf->_bytes_frame_offset)
			frame_offset = source->readX(csf->_bytes_frame_offset);
#ifdef COMP_SHAPENUM
		if (shapenum == COMP_SHAPENUM)
		...
		// Read the unknown
		if (csf->_bytes_frameheader_unk)
			source->read(frame->_header_unknown, csf->_bytes_frameheader_unk);
...
void ConvertShapeFrame::Read(IDataSource *source, const ConvertShapeFormat *csf, uint32 frame_length) {
	// Read unknown
	if (csf->_bytes_frame_unknown)
		source->read(_unknown, csf->_bytes_frame_unknown);
...
void ConvertShapeFrame::ReadCmpFrame(IDataSource *source, const ConvertShapeFormat *csf, const uint8 special[256], ConvertShapeFrame *prev) {
	...
	if (!rlebuf)
		rlebuf = new OAutoBufferDataSource(1024);
	rlebuf->clear();
...
int ConvertShape::CalcNumFrames(IDataSource *source, const ConvertShapeFormat *csf, uint32 real_len, uint32 start_pos) {
	...
	for (f = 0;; f++) {
		...
		if ((source->pos() - start_pos) >= first_offset)
			break;

		// Read the offset
		uint32 frame_offset = csf->_len_header + (csf->_len_frameheader * f);
		if (csf->_bytes_frame_offset)
			frame_offset = source->readX(csf->_bytes_frame_offset) + csf->_bytes_special;

		if (frame_offset < first_offset)
			first_offset = frame_offset;

		// Read the unknown
		if (csf->_bytes_frameheader_unk)
			source->skip(csf->_bytes_frameheader_unk);
...
bool ConvertShape::Check(IDataSource *source, const ConvertShapeFormat *csf, uint32 real_len) {
	...
	if (csf->_bytes_special)
		source->skip(csf->_bytes_special);

	// Read the header unknown
	if (csf->_bytes_header_unk)
		source->skip(csf->_bytes_header_unk);

	// Now read _num_frames
	int numFrames = 1;
	if (csf->_bytes_num_frames)
		numFrames = source->readX(csf->_bytes_num_frames);
	if (numFrames == 0)
		numFrames = CalcNumFrames(source, csf, real_len, start_pos);
	...
	for (int f = 0; f < numFrames; f++) {
		...
		// Read unknown
		if (csf->_bytes_frame_unknown)
			source->read(frame->_unknown, csf->_bytes_frame_unknown);
		...
		if (frame->_height) {
			...
			if (!csf->_bytes_special) {
				...
				for (int i = 0; i < frame->_height; i++) {
					...
					if (line_offset > highest_offset_byte)
						highest_offset_byte = line_offset;
				};
...
void ConvertShape::Write(ODataSource *dest, const ConvertShapeFormat *csf, uint32 &write_len) {
	...
	// Write the ident
	if (csf->_bytes_ident)
		dest->write(csf->_ident, csf->_bytes_ident);

	// Write the header unknown
	if (csf->_bytes_header_unk)
		dest->write(_header_unknown, csf->_bytes_header_unk);

	// Now write _num_frames
	if (csf->_bytes_num_frames)
		dest->writeX(_num_frames, csf->_bytes_num_frames);
	...
	for (uint32 i = 0; i < _num_frames * csf->_len_frameheader; i++)
		dest->writeByte(0);
	...
	for (uint32 f = 0; f < _num_frames; f++) {
		...
		// Write the offset
		if (csf->_bytes_frame_offset)
			dest->writeX(frame_offset, csf->_bytes_frame_offset);

		// Write the unknown
		if (csf->_bytes_frameheader_unk)
			dest->write(frame->_header_unknown, csf->_bytes_frameheader_unk);
		...
		// Write unknown
		if (csf->_bytes_frame_unknown)
			dest->write(frame->_unknown, csf->_bytes_frame_unknown);
