...
ScalerGump::ScalerGump(int32 x, int32 y, int32 width, int32 height) : DesktopGump(x, y, width, height),
...
void ScalerGump::ParentToGump(int32 &px, int32 &py, PointRoundDir r) {
	px -= _x;
	px *= _dims.w;
	if (px < 0 && r == ROUND_TOPLEFT)
		px -= (_width - 1);
	if (px > 0 && r == ROUND_BOTTOMRIGHT)
		px += (_width - 1);
	px /= _width;

	py -= _y;
	py *= _dims.h;
	if (py < 0 && r == ROUND_TOPLEFT)
		py -= (_height - 1);
	if (py > 0 && r == ROUND_BOTTOMRIGHT)
		py += (_height - 1);
	py /= _height;
}
...
void ScalerGump::GumpToParent(int32 &gx, int32 &gy, PointRoundDir r) {
	gx *= _width;
	if (gx < 0 && r == ROUND_TOPLEFT)
		gx -= (_dims.w - 1);
	if (gx > 0 && r == ROUND_BOTTOMRIGHT)
		gx += (_dims.w - 1);
	gx /= _dims.w;
	gx += _x;

	gy *= _height;
	if (gy < 0 && r == ROUND_TOPLEFT)
		gy -= (_dims.h - 1);
	if (gy > 0 && r == ROUND_BOTTOMRIGHT)
		gy += (_dims.h - 1);
	gy /= _dims.h;
	gy += _y;
}
...
void ScalerGump::setupScaling() {
	...
	if (_swidth1 < 0)
		...
		_swidth1 = _width;
	else if (_swidth1 < 100)
		_swidth1 = _width / _swidth1;

	if (_sheight1 < 0)
		_sheight1 = -_sheight1;
	else if (_sheight1 == 0)
		_sheight1 = _height;
	else if (_sheight1 < 100)
		_sheight1 = _height / _sheight1;

	if (_swidth2 < 0)
		_swidth2 = -_swidth2;
	else if (_swidth2 != 0 && _swidth2 < 100)
		_swidth2 = _width / _swidth2;

	if (_sheight2 < 0)
		_sheight2 = -_sheight2;
	else if (_sheight2 != 0 && _sheight2 < 100)
		_sheight2 = _height / _sheight2;

	_dims.w = _swidth1;
	_dims.h = _sheight1;

	// We don't care, we are not going to support filters, at least not at the moment
	if (_swidth1 == _width && _sheight1 == _height)
		return;
