...
protected:
	 ...
	 * The only reason this is an int and not an uint32 is to
...
public:
	 ...
	 */
	Timestamp(uint32 msecs, uint32 framerate);
	 ...
	 * @param framerate number of frames per second (must be > 0)
	 */
	Timestamp(uint32 secs, uint32 frames, uint32 framerate);
	 ...
	 */
	Timestamp convertToFramerate(uint32 newFramerate) const;
...
//	Timestamp &operator-=(const Timestamp &ts);
	...
	inline uint32 framerate() const {
		return _framerate / _framerateFactor;
	}
