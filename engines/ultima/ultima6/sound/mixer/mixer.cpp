...
public:
	 ...
	 */
	void mix(sint16 *data, uint32 len);
	 ...
	 */
	void setBalance(const sint8 balance);
...
MixerImpl::MixerImpl(uint32 sampleRate)
	: _sampleRate(sampleRate), _mixerReady(false), _handleSeed(0) {
...
uint32 MixerImpl::getOutputRate() const {
	return _sampleRate;
}
...
void MixerImpl::insertChannel(SoundHandle *handle, Channel *chan) {
	...
	if (index == -1) {
		//FIXME warning("MixerImpl::out of mixer slots");
		delete chan;
		return;
	}
...
void MixerImpl::mixCallback(uint8 *samples, uint32 len) {
	assert(samples);

	Common::StackLock lock(_mutex);

	sint16 *buf = (sint16 *)samples;
	len >>= 2;
	...
	//  zero the buf
	memset(buf, 0, 2 * len * sizeof(sint16));
...
void MixerImpl::setChannelVolume(SoundHandle handle, uint8 volume) {
	Common::StackLock lock(_mutex);
...
void MixerImpl::setChannelBalance(SoundHandle handle, sint8 balance) {
	Common::StackLock lock(_mutex);
...
void Channel::setVolume(const uint8 volume) {
	_volume = volume;
	updateChannelVolumes();
}

void Channel::setBalance(const sint8 balance) {
	_balance = balance;
	updateChannelVolumes();
}
...
void Channel::pause(bool paused) {
	...
	if (paused) {
		...
		if (_pauseLevel == 1)
			_pauseStartTime = getMillis();
	} else if (_pauseLevel > 0) {
		_pauseLevel--;

		if (!_pauseLevel) {
			_pauseTime = (getMillis() - _pauseStartTime);
			_pauseStartTime = 0;
		}
	}
}
...
Timestamp Channel::getElapsedTime() {
	...
	if (isPaused())
		...
		delta = getMillis() - _mixerTimeStamp - _pauseTime;
...
void Channel::mix(sint16 *data, uint32 len) {
	assert(_stream);
