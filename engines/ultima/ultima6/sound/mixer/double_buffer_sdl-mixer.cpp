...
void DoubleBufferSDLMixerManager::startAudio() {
	...
	_activeSoundBuf = 0;
	uint32 bufSize = _obtainedRate.samples * 4;
	_soundBufSize = bufSize;
	_soundBuffers[0] = (uint8 *)calloc(1, bufSize);
	_soundBuffers[1] = (uint8 *)calloc(1, bufSize);
...
void DoubleBufferSDLMixerManager::callbackHandler(uint8 *samples, int len) {
	assert(_mixer);
