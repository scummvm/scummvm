...
createarray:
		...
		case MINUSOP:
			...
			break;
		case NOTOP:
			ttype(S->top - 1) = (ttype(S->top - 1) == LUA_T_NIL) ? LUA_T_NUMBER : LUA_T_NIL;
			nvalue(S->top - 1) = 1;
			break;
		case ONTJMPW:
			aux = next_word(pc);
...
callfunc:
			{
				...
				if (ttype(func) == LUA_T_PROTO || (ttype(func) == LUA_T_CLOSURE &&
					...
					ttype(func) = (ttype(func) == LUA_T_PROTO) ? LUA_T_PMARK : LUA_T_CLMARK;
					goto newfunc;
				}
