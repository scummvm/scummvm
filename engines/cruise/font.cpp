...
int32 getLineHeight(int16 charCount, const FontEntry *fontPtr) {
	...
	for (int i = 0; i < charCount; ++i) {
		...
		if (charHeight > highestChar)
			highestChar = charHeight;
	}
...
int32 prepareWordRender(int32 inRightBorder_X, int16 wordSpacingWidth,
	...
	do {
		...
		if (character == ' ') {
			...
			if (pixelCount + wordSpacingWidth + SPACE_WIDTH >= inRightBorder_X) {
				finish = 1;
			} else {
			...
			if (character == '|' || !character) {
				...
				if (charData >= 0) {
					if (pixelCount + wordSpacingWidth + (int16)fontData[charData].charWidth >= inRightBorder_X) {
						finish = 1;
						if (temp_pc) {
							pixelCount = temp_pc;
							counter = temp_cc;
						...
						pixelCount += wordSpacingWidth + (int16)fontData[charData].charWidth;
					}
				}
			}
		}
				...
				if (character == ' ' || character == 0x7C) {
					...
					if (charData >= 0) {
						...
						renderWord((const uint8 *)fontPtr_Data + fe.offset,
						           ...
						           drawPosPixel_X,
						           fe.height2 - fe.charHeight + lineHeight + heightOffset,
						           fe.charHeight,
						           fe.v1,
						...
						drawPosPixel_X += wordSpacingWidth + (int16)fe.charWidth;
					}
				}
			} else {
