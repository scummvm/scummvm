...
TextDialog::TextDialog(MADSEngine *vm, const Common::String &fontName,
                       const Common::Point &pos, MSurface *icon, int maxTextChars)
  : Dialog(vm) {
	_font = _vm->_font->getFont(fontName);
	_position = pos;
	_icon = icon;
...
void TextDialog::wordWrap(const Common::String &line) {
	...
	if (!line.empty()) {
		...
		do {
			...
			if (((_currentX + (int)tempLine2.size()) > _lineSize) || ((_lineWidth + lineWidth) > _innerWidth)) {
				incNumLines();
				appendLine(tempLine);
			} else {
...
FullScreenDialog::FullScreenDialog(MADSEngine *vm)
  : _vm(vm) {
