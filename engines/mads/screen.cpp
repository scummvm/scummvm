...
void ScreenObjects::check(bool scanFlag) {
	...
	if ((_vm->_events->_mouseMoved || userInterface._scrollbarActive || _v8332A || _forceRescan) && scanFlag) {
		_category = CAT_NONE;
		...
		if (_vm->_easyMouse && _vm->_events->_mouseButtons && category != _category && scene._userInterface._category != CAT_NONE) {
			_released = true;
		...
		if (_vm->_events->_mouseButtons || (_vm->_easyMouse && scene._action._interAwaiting > AWAITING_COMMAND && scene._userInterface._category == CAT_INV_LIST) ||
	...
	if (currentTicks >= _baseTime) {
		...
		for (uint idx = 0; idx < uiSlots.size(); ++idx) {
			...
			if (slot._flags != IMG_REFRESH && slot._flags > IMG_UPDATE_ONLY && slot._segmentId != IMG_SPINNING_OBJECT)
