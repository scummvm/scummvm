...
namespace Nebular {
	...
	Scene201::Scene201(MADSEngine *vm)
	  : Scene2xx(vm) {
	...
	Scene202::Scene202(MADSEngine *vm)
	  : Scene2xx(vm) {
		_activeMsgFl = false;
		_ladderTopFl = false;
	...
	Scene203::Scene203(MADSEngine *vm)
	  : Scene2xx(vm) {
	...
	Scene205::Scene205(MADSEngine *vm)
	  : Scene2xx(vm) {
	...
	Scene207::Scene207(MADSEngine *vm)
	  : Scene2xx(vm) {
		_vultureFl = false;
		_spiderFl = false;
		_eyeFl = false;
	...
	void Scene207::actions() {
		if (_action._savedFields._lookFlag)
			...
			if ((_game._player._playerPos.x > 150) && (_game._player._playerPos.x < 189) && (_game._player._playerPos.y > 111) && (_game._player._playerPos.y < 130)) {
				if ((_game._player._playerPos.x <= 162) || (_game._player._playerPos.x >= 181) || (_game._player._playerPos.y <= 115) || (_game._player._playerPos.y >= 126)) {
	...
	Scene208::Scene208(MADSEngine *vm)
	  : Scene2xx(vm) {
	...
	void Scene208::step() {
		if (_boundingFl && _scene->_animation[0] && (_rhotundaTime <= _scene->_animation[0]->getCurrentFrame())) {
	...
	Scene209::Scene209(MADSEngine *vm)
	  : Scene2xx(vm) {
		_dodgeFl = false;
		_forceDodgeFl = false;
		_pitchFl = false;
	...
	Scene210::Scene210(MADSEngine *vm)
	  : Scene2xx(vm) {
		_curDialogNode = -1;
		_nextHandsPlace = 0;
	...
	void Scene210::actions() {
		if (_action.isAction(VERB_LOOK, NOUN_BINOCULARS, NOUN_HUT_TO_NORTH)) {
			...
			handleConversations();
		} else if (_action.isAction(VERB_TALKTO, NOUN_NATIVE_WOMAN) || ((_game._player._playerPos == Common::Point(214, 150)) && (_game._player._facing == FACING_NORTHWEST) && (_twinkleAnimationType == 1) && _stopWalking)) {
			switch (_game._trigger) {
			case 0: {
				_game._player._stepEnabled = false;
				int quote;
	...
	Scene211::Scene211(MADSEngine *vm)
	  : Scene2xx(vm) {
		_ambushFl = false;
		_wakeFl = false;
		_monkeyFrame = 0;
	...
	void Scene211::step() {
		if (_globals[kMonkeyStatus] == MONKEY_AMBUSH_READY) {
			...
			if ((_game._player._playerPos == Common::Point(52, 132)) && (_game._player._facing == FACING_WEST) && !_game._player._moving && (_game._trigger || !_ambushFl)) {
				switch (_game._trigger) {
				case 0:
	...
	Scene214::Scene214(MADSEngine *vm)
	  : Scene2xx(vm) {
		_devilTime = 0;
