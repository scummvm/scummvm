...
const Common::Rect BonusTextUI::kTitleRect(1, 1, 25, 6);
const Common::Rect BonusTextUI::kMainRect(24, 9, 54, 20);
const Common::Rect BonusTextUI::kMusicRect(40, 1, 70, 23);
const Common::Rect BonusTextUI::kViewerRect(5, 4, 76, 19);
const Common::Rect BonusTextUI::kNowPlayingRect(24, 10, 56, 15);
...
void BonusTextUI::drawTitlePanel() {
	_cells.drawShadow(kTitleRect, kShadowAttribute);
	_cells.fill(Common::Rect(kTitleRect.left + 1, kTitleRect.top + 1,
			kTitleRect.right - 1, kTitleRect.bottom - 1), 0x20, kTitleAttribute);
	_cells.drawBox(kTitleRect, kTitleAttribute, true);

	for (int line = 0; line < 3; ++line) {
		_cells.drawCenteredText(kTitleRect.top + 1 + line,
				kTitleRect.left + 1, kTitleRect.right - 1,
...
BonusTextUI::MainChoice BonusTextUI::runMainMenu(int &selected) {
	const int result = runMenu(kMainRect, _text.mainMenuTitle,
...
int BonusTextUI::runMusicMenu(int &selected) {
	...
	return runMenu(kMusicRect, _text.musicMenuTitle,
...
bool BonusTextUI::showBonusText(const Common::Path &filename) {
	...
	Common::Array<Common::String> lines;
	const int textWidth = kViewerRect.width() - 2;
	...
	const int firstTextRow = kViewerRect.top + 1;
	const int linesPerPage = kViewerRect.height() - 2;
	int firstLine = 0;
	bool dirty = true;
	...
	while (!g_engine->shouldQuit()) {
		...
		if (dirty) {
			drawDesktop();
			drawTitlePanel();
			_cells.drawShadow(kViewerRect, kShadowAttribute);
			_cells.fill(Common::Rect(kViewerRect.left + 1, kViewerRect.top + 1,
					kViewerRect.right - 1, kViewerRect.bottom - 1),
					0x20, kNormalAttribute);
			_cells.drawBox(kViewerRect, kNormalAttribute, true);

			Common::String viewerTitle = _text.bonusTextFilename;
			viewerTitle.toUppercase();
			_cells.drawTitle(kViewerRect, viewerTitle,
					kNormalAttribute, kHotkeyAttribute);
			for (int row = 0; row < linesPerPage &&
					firstLine + row < (int)lines.size(); ++row) {
				_cells.drawText(kViewerRect.left + 1, firstTextRow + row,
						lines[firstLine + row], kNormalAttribute,
						kHotkeyAttribute, false, textWidth);
			}
			_cells.drawFooter(kViewerRect, _text.continuePrompt,
					kNormalAttribute, kHotkeyAttribute);
			drawMouseCursor();
			present(true);
			dirty = false;
...
void BonusTextUI::showNowPlaying(const Common::String &trackTitle,
	...
	while (!g_engine->shouldQuit()) {
		...
		if (dirty) {
			drawDesktop();
			drawTitlePanel();
			_cells.drawShadow(kNowPlayingRect, kShadowAttribute);
			_cells.fill(Common::Rect(kNowPlayingRect.left + 1,
					kNowPlayingRect.top + 1, kNowPlayingRect.right - 1,
					kNowPlayingRect.bottom - 1), 0x20, kNormalAttribute);
			_cells.drawBox(kNowPlayingRect, kNormalAttribute, true);
			_cells.drawTitle(kNowPlayingRect, _text.nowPlaying,
					kNormalAttribute, kHotkeyAttribute);
			_cells.drawCenteredText(kNowPlayingRect.top + 2,
					kNowPlayingRect.left + 1, kNowPlayingRect.right - 1,
