...
namespace Nebular {
	...
	Scene701::Scene701(MADSEngine *vm)
	  : Scene7xx(vm) {
		_fishingLineId = -1;
	}
	...
	void Scene701::actions() {
		...
		} else if ((_action.isAction(VERB_PULL, NOUN_BOAT) || _action.isAction(VERB_TAKE, NOUN_BOAT) || _action.isAction(VERB_PULL, NOUN_FISHING_LINE) || _action.isAction(VERB_TAKE, NOUN_FISHING_LINE)) && !_game._objects.isInInventory(OBJ_FISHING_LINE)) {
	...
	Scene703::Scene703(MADSEngine *vm)
	  : Scene7xx(vm) {
		_monsterMode = -1;
		_boatFrame = -1;
		_curSequence = -1;
	...
	Scene704::Scene704(MADSEngine *vm)
	  : Scene7xx(vm) {
		_bottleHotspotId = -1;
		_boatCurrentFrame = -1;
		_animationMode = -1;
	...
	Scene706::Scene706(MADSEngine *vm)
	  : Scene7xx(vm) {
		_vaseHotspotId = -1;
		_vaseMode = -1;
		_animationMode = -1;
	...
	void Scene706::actions() {
		...
		if (_action.isAction(VERB_PUT, NOUN_BOTTLE, NOUN_PEDESTAL)) {
			if ((_globals[kBottleStatus] == 2 && _game._difficulty == DIFFICULTY_HARD) || (_globals[kBottleStatus] != 0 && _game._difficulty != DIFFICULTY_HARD)) {
	...
	Scene751::Scene751(MADSEngine *vm)
	  : Scene7xx(vm) {
	...
	Scene752::Scene752(MADSEngine *vm)
	  : Scene7xx(vm) {
		_cardId = -1;
	}
	...
	void Scene752::actions() {
		...
		} else if (_action.isAction(VERB_TAKE, NOUN_BONES) && (_action._savedFields._mainObjectSource == CAT_HOTSPOT) && (!_game._objects.isInInventory(OBJ_BONES) || _game._trigger)) {
			switch (_game._trigger) {
			case 0:
				_game._player._stepEnabled = false;
				_game._player._visible = false;
