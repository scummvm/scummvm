...
namespace Nebular {
	...
	Scene501::Scene501(MADSEngine *vm)
	  : Scene5xx(vm) {
		_mainSequenceId = -1;
		_mainSpriteId = -1;
		_doorHotspotid = -1;
	...
	Scene503::Scene503(MADSEngine *vm)
	  : Scene5xx(vm) {
	...
	Scene504::Scene504(MADSEngine *vm)
	  : Scene5xx(vm) {
		_carAnimationMode = -1;
		_carFrame = -1;
	}
		...
		if (_game._trigger >= 70) {
			...
			case 70:
				if (_globals[kHoverCarDestination] != -1) {
					...
					_carAnimationMode = 2;
					if (((_globals[kHoverCarLocation] >= 500 && _globals[kHoverCarLocation] <= 599) && (_globals[kHoverCarDestination] >= 500 && _globals[kHoverCarDestination] <= 599)) || ((_globals[kHoverCarLocation] >= 600 && _globals[kHoverCarLocation] <= 699) && (_globals[kHoverCarDestination] >= 600 && _globals[kHoverCarDestination] <= 699))) {
	...
	Scene505::Scene505(MADSEngine *vm)
	  : Scene5xx(vm) {
		_frame = -1;
		_nextButtonId = -1;
		_homeSelectedId = -1;
	...
	Scene506::Scene506(MADSEngine *vm)
	  : Scene5xx(vm)
	  , _doorPos(0, 0) {
	...
	Scene507::Scene507(MADSEngine *vm)
	  : Scene5xx(vm) {
	...
	Scene508::Scene508(MADSEngine *vm)
	  : Scene5xx(vm) {
		_chosenObject = -1;
	}
	...
	Scene511::Scene511(MADSEngine *vm)
	  : Scene5xx(vm) {
		_handingLine = false;
		_lineMoving = false;
	...
	Scene512::Scene512(MADSEngine *vm)
	  : Scene5xx(vm) {
	...
	void Scene512::actions() {
		...
		else if (_action.isAction(VERB_LOOK, NOUN_FISHING_ROD) && (!_scene->_animation[0] || _scene->_animation[0]->getCurrentFrame() == 4))
