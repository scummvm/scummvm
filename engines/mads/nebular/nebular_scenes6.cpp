...
namespace Nebular {
	...
	Scene602::Scene602(MADSEngine *vm)
	  : Scene6xx(vm) {
		_lastSpriteIdx = -1;
		_lastSequenceIdx = -1;
		_cycleIndex = -1;
		...
		if (_action.isAction(VERB_WALK_THROUGH, NOUN_HALLWAY))
			...
			if ((_globals[kSafeStatus] == 0) && (_game._difficulty != DIFFICULTY_HARD)) {
				...
				handleSafeActions();
		            ...
		            || _action.isAction(VERB_REFLECT, NOUN_COMPACT_CASE, NOUN_LASER_BEAM) || _action.isAction(VERB_REFLECT, NOUN_REARVIEW_MIRROR, NOUN_LASER_BEAM))
		           && (_globals[kSafeStatus] == 0)) {
			switch (_game._trigger) {
			case 0:
	...
	Scene603::Scene603(MADSEngine *vm)
	  : Scene6xx(vm) {
	...
	Scene604::Scene604(MADSEngine *vm)
	  : Scene6xx(vm) {
	...
	Scene607::Scene607(MADSEngine *vm)
	  : Scene6xx(vm) {
		_dogTimer = 0;
		_lastFrameTime = 0;
	...
	Scene608::Scene608(MADSEngine *vm)
	  : Scene6xx(vm) {
		_carMode = -1;
		_carFrame = -1;
		_carMoveMode = -1;
	...
	Scene609::Scene609(MADSEngine *vm)
	  : Scene6xx(vm) {
		_videoDoorMode = -1;
	}
	...
	Scene610::Scene610(MADSEngine *vm)
	  : Scene6xx(vm) {
		_handsetHotspotId = -1;
		_checkVal = -1;
	...
	Scene611::Scene611(MADSEngine *vm)
	  : Scene6xx(vm)
	  , _defaultDialogPos(0, 0) {
		_seenRatFl = false;
	...
	Scene612::Scene612(MADSEngine *vm)
	  : Scene6xx(vm) {
		_actionMode = -1;
		_cycleIndex = -1;
	}
