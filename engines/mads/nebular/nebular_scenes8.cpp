...
namespace Nebular {
	...
	void Scene8xx::setPlayerSpritesPrefix() {
		...
		if ((_globals[kFromCockpit] && !_globals[kExitShip]) || _scene->_nextSceneId == 804 || _scene->_nextSceneId == 805 || _scene->_nextSceneId == 808 || _scene->_nextSceneId == 810) {
	...
	Scene801::Scene801(MADSEngine *vm)
	  : Scene8xx(vm) {
	...
	Scene804::Scene804(MADSEngine *vm)
	  : Scene8xx(vm) {
	...
	void Scene804::actions() {
		if (_action.isAction(VERB_LOOK, NOUN_SERVICE_PANEL) || _action.isAction(VERB_OPEN, NOUN_SERVICE_PANEL)) {
		...
		} else if (_action.isAction(VERB_APPLY, NOUN_POLYCEMENT, NOUN_CRACK) || _action.isAction(VERB_PUT, NOUN_POLYCEMENT, NOUN_CRACK)) {
			if (!_globals[kWindowFixed]) {
				_resetFrame = 2;
				_game._player._stepEnabled = false;
			}
		...
		} else if ((_action.isAction(VERB_LOOK, NOUN_WINDOW)) || (_action.isAction(VERB_LOOK_OUT, NOUN_WINDOW))) {
			if (_globals[kBeamIsUp]) {
	...
	Scene808::Scene808(MADSEngine *vm)
	  : Scene8xx(vm) {
		_goingTo803 = false;
	}
	...
	Scene810::Scene810(MADSEngine *vm)
	  : Scene8xx(vm) {
		_moveAllowed = false;
	}
