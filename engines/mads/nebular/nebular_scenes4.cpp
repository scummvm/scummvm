...
namespace Nebular {
	...
	Scene401::Scene401(MADSEngine *vm)
	  : Scene4xx(vm)
	  , _destPos(0, 0) {
		_northFl = false;
		_timer = 0;
	...
	Scene402::Scene402(MADSEngine *vm)
	  : Scene4xx(vm) {
		_lightOn = false;
		_blowingSmoke = false;
	...
	Scene406::Scene406(MADSEngine *vm)
	  : Scene4xx(vm) {
	...
	Scene407::Scene407(MADSEngine *vm)
	  : Scene4xx(vm)
	  , _destPos(0, 0) {
		_fromNorth = false;
	}
	...
	Scene411::Scene411(MADSEngine *vm)
	  : Scene4xx(vm) {
	...
	void Scene411::preActions() {
		...
		if (_action.isAction(VERB_LOOK, NOUN_EXPLOSIVES) || _action.isAction(VERB_LOOK, NOUN_KETTLE) || _action.isAction(VERB_LOOK, NOUN_MISHAP) || _action.isAction(VERB_LOOK, NOUN_ALCOVE) || _action.isAction(VERB_LOOK, NOUN_SINK) || _action.isAction(VERB_PUT, NOUN_SINK) || _action.isAction(VERB_LOOK, NOUN_EXPERIMENT) || _action.isAction(VERB_LOOK, NOUN_DRAWING_BOARD))
			_game._player._needToWalk = true;
	...
	Scene413::Scene413(MADSEngine *vm)
	  : Scene4xx(vm) {
		_rexDeath = -1;
		_canMove = -1;
	}
