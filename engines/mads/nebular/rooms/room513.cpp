...
static void room_513_init() {
	...
	g_sprite_ids[3] = _scene->_sprites.addSprites("*RXCD_9");
...
static void room_513_daemon() {
	...
	case 82:
		...
		_scene->_sequences.addSubEntry(g_sequence_ids[2], SEQUENCE_TRIGGER_EXPIRE, 0, 83);
		break;
...
static void room_513_parser() {
	if (player_said_2(get_into, car)) {
		...
		case 0:
			player.commands_allowed = false;
			player.walker_visible = false;
			g_sequence_ids[4] = _scene->_sequences.startPingPongCycle(g_sprite_ids[4], false, 7, 1, 0, 0);
