...
static void handlePeek() {
	...
	case 133:
		...
		g_sequence_ids[3] = _scene->_sequences.addReverseSpriteCycle(g_sprite_ids[3], false, 8, 1, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 51, 52);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 134);
		break;
	...
	case 137:
		...
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 51, 52);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 138);
		break;
...
static void handleVerticalMove() {
	switch (kernel.trigger) {
	case 140:
		g_sequence_ids[3] = _scene->_sequences.addSpriteCycle(g_sprite_ids[3], false, 8, 8, 0, 1);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 1, 5);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 141);
		break;
	...
	case 142:
		...
		g_sequence_ids[3] = _scene->_sequences.addReverseSpriteCycle(g_sprite_ids[3], false, 8, 1, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 1, 5);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 143);
		break;
...
static void handleLookStay() {
	...
	case 145:
		...
		g_sequence_ids[3] = _scene->_sequences.addReverseSpriteCycle(g_sprite_ids[3], false, 8, 1, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 51, 52);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 146);
		break;
...
static void handleLookRight() {
	...
	case 151:
		_scene->_sequences.remove(g_sequence_ids[3]);
		g_sequence_ids[3] = _scene->_sequences.startPingPongCycle(g_sprite_ids[3], false, 8, 2, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 8, 14);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 152);
		break;
...
static void handleGetBinoculars() {
	...
	case 161:
		...
		_scene->_sequences.remove(g_sequence_ids[3]);
		g_sequence_ids[3] = _scene->_sequences.addSpriteCycle(g_sprite_ids[3], false, 8, 1, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 8, 24);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_SPRITE, 20, 165);
		if (!local._fallFl && !local._dodgeFl) {
			_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 162);
		} else {
			_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 163);
		}
		break;
	...
	{
		int oldIdx = g_sequence_ids[3];
		g_sequence_ids[3] = _scene->_sequences.startPingPongCycle(g_sprite_ids[3], false, 8, 6, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 23, 25);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 163);
	}
	break;
	...
	{
		int oldIdx = g_sequence_ids[3];
		g_sequence_ids[3] = _scene->_sequences.startPingPongCycle(g_sprite_ids[3], false, 8, 0, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 23, 24);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
...
static void handleBinocularBlink() {
	...
	case 167:
	{
		...
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 23, 25);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 168);
	}
	break;
...
static void handleBinocularScan() {
	...
	case 171:
	{
		...
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 43, 45);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 172);
	}
	break;
	...
	case 172:
	{
		...
		case 1:
			...
			break;
		case 2:
			g_sequence_ids[3] = _scene->_sequences.startPingPongCycle(g_sprite_ids[3], false, 12, 4, 0, 0);
			break;
		default:
			break;
		}
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 23, 25);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 173);
	}
	break;
	...
	case 173:
	{
		int oldIdx = g_sequence_ids[3];
		g_sequence_ids[3] = _scene->_sequences.startPingPongCycle(g_sprite_ids[3], false, 12, 2, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 26, 30);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 174);
	}
	break;
	...
	{
		int oldIdx = g_sequence_ids[3];
		g_sequence_ids[3] = _scene->_sequences.startPingPongCycle(g_sprite_ids[3], false, 12, 0, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 23, 24);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
...
static void handleJumpInTree() {
	...
	case 178:
	{
		...
		g_sequence_ids[3] = _scene->_sequences.addSpriteCycle(g_sprite_ids[3], false, 8, 1, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 46, 49);
		if (!local._removeMonkeyFl)
			_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);

		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 179);
	}
	break;
	...
	case 179:
	{
		...
		int oldIdx = g_sequence_ids[3];
		g_sequence_ids[3] = _scene->_sequences.addSpriteCycle(g_sprite_ids[3], false, 8, 1, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 53, 61);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 180);
	}
	break;
...
static void handleTongue() {
	...
	case 182:
	{
		...
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 26, 30);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 183);
	}
	break;
	...
	case 183:
	{
		int oldIdx = g_sequence_ids[3];
		g_sequence_ids[3] = _scene->_sequences.addSpriteCycle(g_sprite_ids[3], false, 12, 1, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 31, 33);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 184);
	}
	break;
	...
	case 184:
	{
		int oldIdx = g_sequence_ids[3];
		g_sequence_ids[3] = _scene->_sequences.addSpriteCycle(g_sprite_ids[3], false, 12, 1, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 36, 37);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 185);
	}
	break;
	...
	case 185:
	{
		g_engine->_soundManager->command(18, 0);
		int oldIdx = g_sequence_ids[3];
		g_sequence_ids[3] = _scene->_sequences.startPingPongCycle(g_sprite_ids[3], false, 6, 20, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 38, 39);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 186);
	}
	break;
	...
	case 186:
	{
		int oldIdx = g_sequence_ids[3];
		g_sequence_ids[3] = _scene->_sequences.addSpriteCycle(g_sprite_ids[3], false, 12, 1, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[3], 40, 41);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[3], SEQUENCE_TRIGGER_EXPIRE, 0, 178);
...
static void handleStandFromPeek() {
	...
	case 189:
		...
		_scene->_sequences.remove(g_sequence_ids[3]);
		g_sequence_ids[3] = _scene->_sequences.startCycle(g_sprite_ids[3], false, 50);
	...
	case 191:
		...
		_scene->_sequences.setAnimRange(g_sequence_ids[6], 1, 5);
		_scene->_sequences.addSubEntry(g_sequence_ids[6], SEQUENCE_TRIGGER_EXPIRE, 0, 192);
		break;
	...
	case 193:
		...
		g_sequence_ids[4] = _scene->_sequences.startCycle(g_sprite_ids[4], false, 22);
...
static void handleStandBlink() {
	...
	case 247:
		_scene->_sequences.remove(g_sequence_ids[4]);
		g_sequence_ids[4] = _scene->_sequences.startCycle(g_sprite_ids[4], false, 23);
...
static void handleJumpAndHide() {
	...
	case 196:
		...
		_scene->_sequences.setAnimRange(g_sequence_ids[5], 1, 16);
		_scene->_sequences.addSubEntry(g_sequence_ids[5], SEQUENCE_TRIGGER_EXPIRE, 0, 197);
		break;
...
static void handleMonkeyEating() {
	...
	case 199:
		...
		_scene->_sequences.setAnimRange(g_sequence_ids[4], 1, 14);
		_scene->_sequences.addSubEntry(g_sequence_ids[4], SEQUENCE_TRIGGER_EXPIRE, 0, 200);
		break;

	case 200:
	{
		int oldIdx = g_sequence_ids[4];
		g_sequence_ids[4] = _scene->_sequences.startPingPongCycle(g_sprite_ids[4], false, 10, 10, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[4], 15, 16);
		_scene->_sequences.updateTimeout(g_sequence_ids[4], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[4], SEQUENCE_TRIGGER_EXPIRE, 0, 201);
	}
	break;
	...
	{
		int oldIdx = g_sequence_ids[4];
		g_sequence_ids[4] = _scene->_sequences.startCycle(g_sprite_ids[4], false, 12);
		_scene->_sequences.updateTimeout(g_sequence_ids[4], oldIdx);
	...
	case 202:
		_scene->_sequences.remove(g_sequence_ids[4]);
		g_sequence_ids[4] = _scene->_sequences.startCycle(g_sprite_ids[4], false, 19);
		_scene->_sequences.addTimer(20, 203);
		break;

	case 203:
		_scene->_sequences.remove(g_sequence_ids[4]);
		g_sequence_ids[4] = _scene->_sequences.startCycle(g_sprite_ids[4], false, 17);
		_scene->_sequences.addTimer(20, 204);
		break;

	case 204:
		_scene->_sequences.remove(g_sequence_ids[4]);
		g_sequence_ids[4] = _scene->_sequences.startPingPongCycle(g_sprite_ids[4], false, 10, 8, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[4], 18, 19);
		_scene->_sequences.addSubEntry(g_sequence_ids[4], SEQUENCE_TRIGGER_EXPIRE, 0, 205);
		break;
	...
	case 205:
	{
		int oldIdx = g_sequence_ids[4];
		g_sequence_ids[4] = _scene->_sequences.startPingPongCycle(g_sprite_ids[4], false, 10, 8, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[4], 20, 21);
		_scene->_sequences.updateTimeout(g_sequence_ids[4], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[4], SEQUENCE_TRIGGER_EXPIRE, 0, 206);
	}
	break;
	...
	case 206:
	{
		int oldIdx = g_sequence_ids[4];
		g_sequence_ids[4] = _scene->_sequences.addSpriteCycle(g_sprite_ids[4], false, 10, 1, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[4], 22, 25);
		_scene->_sequences.updateTimeout(g_sequence_ids[4], oldIdx);
		if (!local._dodgeFl && !local._fallFl)
			_scene->_sequences.addSubEntry(g_sequence_ids[4], SEQUENCE_TRIGGER_EXPIRE, 0, 207);
		else
			_scene->_sequences.addSubEntry(g_sequence_ids[4], SEQUENCE_TRIGGER_EXPIRE, 0, 209);
	}
	break;
	...
	case 207:
	{
		...
		_scene->_sequences.setAnimRange(g_sequence_ids[4], 26, 27);
		_scene->_sequences.updateTimeout(g_sequence_ids[4], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[4], SEQUENCE_TRIGGER_EXPIRE, 0, 208);
	}
	break;
...
static void handleMonkeyFall() {
	...
	case 219:
	{
		...
		_scene->_sequences.setAnimRange(g_sequence_ids[8], 1, 35);
		_scene->_sequences.setDepth(g_sequence_ids[8], 6);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[8], SEQUENCE_TRIGGER_EXPIRE, 0, 220);
	}
	break;
	...
	case 220:
	{
		...
		_scene->_sequences.remove(g_sequence_ids[3]);
		g_sequence_ids[8] = _scene->_sequences.addSpriteCycle(g_sprite_ids[8], false, 8, 1, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[8], 36, 42);
		_scene->_sequences.setDepth(g_sequence_ids[8], 6);
		_scene->_sequences.updateTimeout(g_sequence_ids[3], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[8], SEQUENCE_TRIGGER_EXPIRE, 0, 221);
	}
	break;
	...
	case 221:
	{
		...
		_scene->_sequences.updateTimeout(g_sequence_ids[9], oldIdx);
		_scene->_sequences.setPosition(g_sequence_ids[9], Common::Point(201, 131));
		_scene->_sequences.setDepth(g_sequence_ids[9], 8);
		_scene->_sequences.updateTimeout(g_sequence_ids[8], oldIdx);
		_scene->_sequences.setAnimRange(g_sequence_ids[8], 43, 72);
		_scene->_sequences.addSubEntry(g_sequence_ids[8], SEQUENCE_TRIGGER_EXPIRE, 0, 222);
	...
	case 222:
	{
		...
		_scene->_sequences.updateTimeout(g_sequence_ids[8], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[8], SEQUENCE_TRIGGER_EXPIRE, 0, 223);
	}
	break;
...
static void handleMonkey1() {
	...
	case 212:
		_scene->_sequences.remove(g_sequence_ids[4]);
		g_sequence_ids[4] = _scene->_sequences.addSpriteCycle(g_sprite_ids[4], false, 9, 1, 0, 0);
		_scene->_sequences.setAnimRange(g_sequence_ids[4], 1, 13);
		_scene->_sequences.addSubEntry(g_sequence_ids[4], SEQUENCE_TRIGGER_EXPIRE, 0, 213);
		break;
	...
	case 213:
	{
		...
		_scene->_sequences.setAnimRange(g_sequence_ids[7], 1, 22);
		_scene->_sequences.updateTimeout(g_sequence_ids[7], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[7], SEQUENCE_TRIGGER_EXPIRE, 0, 214);
	}
	break;
	...
	case 215:
	{
		...
		_scene->_sequences.updateTimeout(g_sequence_ids[7], oldIdx);
		_scene->_sequences.addSubEntry(g_sequence_ids[7], SEQUENCE_TRIGGER_EXPIRE, 0, 216);
	}
	break;
...
static void handleMonkey2() {
	...
	case 252:
	{
		...
		_scene->_sequences.setPosition(g_sequence_ids[12], Common::Point(111, 133));
		_scene->_sequences.setScale(g_sequence_ids[12], 79);
		_scene->_sequences.updateTimeout(g_sequence_ids[12], oldIdx);
...
static void room_209_daemon() {
	...
	if (local._dodgeFl || local._fallFl) { /* if want to dodge or fall */
		...
		case 228:
			...
			_scene->_sequences.setAnimRange(g_sequence_ids[2], 1, 7);
			_scene->_sequences.setPosition(g_sequence_ids[2], Common::Point(116, 131));
			_scene->_sequences.setDepth(g_sequence_ids[2], 4);
			_scene->_sequences.addSubEntry(g_sequence_ids[2], SEQUENCE_TRIGGER_EXPIRE, 0, 229);
			break;
		...
		case 230:
			...
			_scene->_sequences.setPosition(g_sequence_ids[2], Common::Point(116, 131));
			_scene->_sequences.setDepth(g_sequence_ids[2], 4);
			_scene->_sequences.addSubEntry(g_sequence_ids[2], SEQUENCE_TRIGGER_EXPIRE, 0, 231);
			break;
		...
		case 231:
		{
			int oldIdx = g_sequence_ids[2];
			g_sequence_ids[2] = _scene->_sequences.addSpriteCycle(g_sprite_ids[2], false, 8, 1, 0, 0);
			_scene->_sequences.setAnimRange(g_sequence_ids[2], 8, 10);
			_scene->_sequences.setPosition(g_sequence_ids[2], Common::Point(116, 131));
			_scene->_sequences.setDepth(g_sequence_ids[2], 4);
			_scene->_sequences.updateTimeout(g_sequence_ids[2], oldIdx);
		...
		case 234:
			...
			if (player_said_3(hose_down, blowgun, monkey)) {
				...
				_scene->_sequences.setPosition(g_sequence_ids[2], Common::Point(116, 131));
				_scene->_sequences.setDepth(g_sequence_ids[2], 4);
				_scene->_sequences.addSubEntry(g_sequence_ids[2], SEQUENCE_TRIGGER_SPRITE, 12, 239);
				_scene->_sequences.addSubEntry(g_sequence_ids[2], SEQUENCE_TRIGGER_EXPIRE, 0, 235);
		...
		{
			int oldIdx = g_sequence_ids[2];
			g_sequence_ids[2] = _scene->_sequences.startCycle(g_sprite_ids[2], false, 13);
			_scene->_sequences.setPosition(g_sequence_ids[2], Common::Point(116, 131));
			_scene->_sequences.setDepth(g_sequence_ids[2], 4);
			_scene->_sequences.updateTimeout(g_sequence_ids[2], oldIdx);
		...
		case 236:
			_scene->_sequences.remove(g_sequence_ids[2]);
			g_sequence_ids[2] = _scene->_sequences.addSpriteCycle(g_sprite_ids[2], false, 8, 1, 0, 0);
			_scene->_sequences.setAnimRange(g_sequence_ids[2], 14, 16);
			_scene->_sequences.setPosition(g_sequence_ids[2], Common::Point(116, 131));
			_scene->_sequences.setDepth(g_sequence_ids[2], 4);
			_scene->_sequences.addSubEntry(g_sequence_ids[2], SEQUENCE_TRIGGER_EXPIRE, 0, 237);
			break;
...
static void room_209_parser() {
	...
	if (player_said_2(take, plant_stalk) && (kernel.trigger || object_is_here(OBJ_PLANT_STALK))) {
		...
		case 0:
			player.commands_allowed = false;
			player.walker_visible = false;
			g_sequence_ids[11] = _scene->_sequences.startPingPongCycle(g_sprite_ids[11], false, 3, 2, 0, 0);
			_scene->_sequences.setMsgLayout(g_sequence_ids[11]);
			_scene->_sequences.addSubEntry(g_sequence_ids[11], SEQUENCE_TRIGGER_SPRITE, 4, 1);
	...
	if (player_said_2(take, binoculars) && (kernel.trigger || object_is_here(OBJ_BINOCULARS))) {
		...
		case 0:
			...
			_scene->_sequences.setMsgLayout(g_sequence_ids[10]);
			_scene->_sequences.addSubEntry(g_sequence_ids[10], SEQUENCE_TRIGGER_SPRITE, 4, 1);
			_scene->_sequences.addSubEntry(g_sequence_ids[10], SEQUENCE_TRIGGER_EXPIRE, 0, 2);
			break;
