...
void Sound::playSound(int soundID) {
	...
	// Support for sampled sound effects in Monkey Island 1 and 2
	         ...
	         && !_vm->isMacM68kIMuse() && READ_BE_UINT32(ptr) == MKTAG('S', 'B', 'L', ' ')) {
...
int ScummEngine::readSoundResource(ResId idx) {
	...
	case MKTAG('S', 'O', 'U', ' '):
		...
		while (pos < total_size) {
			...
			// these.
			if ((_sound->_musicType == MDT_ADLIB || _sound->_musicType == MDT_TOWNS) && pri != 16 && pri != 15 && pri != 10 && pri != 2 && _game.platform != Common::kPlatformAmiga)
				pri = -1;
...
static int convert_extraflags(byte *ptr, byte *src_ptr) {
	...
	time = num_steps_table[t1] + num_steps_table[t2] + num_steps_table[t3 & 0x7f] + num_steps_table[t4];
	if (flags & 0x20) {
...
static byte *writeMIDIHeader(byte *ptr, const char *type, int ppqn, int total_size) {
	...
	ptr[1] = 0;
	ptr[2] = 0;
	ptr[3] = 1; // MIDI format 0 with 1 track
	ptr += 4;
...
static void convertADResource(ResourceManager *res, const GameSettings &game, ResId idx, byte *src_ptr, int size) {
		...
		*ptr++ = (byte)(dw & 0xFF);
		...
		for (;;) {
			...
			case 2:
				...
				{
					int freq = ((current_instr[ch][1] & 3) << 8) | current_instr[ch][0];
					if (!freq)
						freq = 0x80;
	...
	memcpy(ptr, "\x00\xff\x2f\x00\x00", 5);
	ptr += 5;
}
