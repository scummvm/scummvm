...
public:
	virtual void init(Player_AppleII *player, const byte *params) override {
		_player = player;
		_delta = params[0];
		_count = params[1];
		_interval = params[2];
	...
	virtual bool update() override { // D085
		if (_decInterval) {
			do {
...
public:
	...
	virtual bool update() override { // D0D6
...
public:
	...
	virtual bool update() override { // D132
		// while (pos = 1; pos < 256; ++pos)
		if (_pos < 256) {
			byte interval = _params[_pos];
			if (interval == 0xFF)
...
public:
	...
	virtual bool update() override { // D170
...
public:
	...
	virtual bool update() override { // D222
