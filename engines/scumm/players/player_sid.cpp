...
void Player_SID::handleMusicBuffer() { // $33cd
	int channel = 2;
	while (channel >= 0) {
		if ((statusBits1A & BITMASK[channel]) == 0 || (busyChannelBits & BITMASK[channel]) != 0) {
			--channel;
			continue;
		}
...
void Player_SID::func_4F45(int channel) { // $4F45
	if (swapVarLoaded) {
		...
		} else if (channel < 3 && bgSoundActive && swapPrepared && !(filterSwapped && filterUsed)) {
			busyChannelBits |= BITMASK[channel];
			useSwapVars(channel);
...
void Player_SID::releaseChannel(int channel) {
	...
	if ((SIDReg23 != SIDReg23Stuff) && (SIDReg23 & 0x07) == 0) {
		if (filterUsed) {
			func_4F45(3);
			stopChannel(3);
		}
...
int Player_SID::initSound(int soundResID) { // $4D0A
	...
	if (filterBlocked || (freeChannelCount < requestedChannels)) {
		...
		if ((freeChannelCount + chansWithLowerPrioCount < requestedChannels) || (filterBlocked && !actFilterHasLowerPrio)) {
			initializing = false;
			return -1;
		}
