...
void Player_V2::startSound(int nr) {
	...
	if (nr != _current_nr && restartable && (!_next_nr || nprio <= prio)) {
...
void Player_V2::lowPassFilter(int16 *sample, uint len) {
	for (uint i = 0; i < len; i++) {
		_level = (int)(_level * _decay + sample[0] * (0x10000 - _decay)) >> 16;
...
void Player_V2::squareGenerator(int channel, int freq, int vol,
	...
	for (uint i = 0; i < len; i++) {
		...
		nsample = *sample +
		          (((int32)(duration - (1 << (FIXP_SHIFT - 1))) * (int32)_volumetable[vol]) >> FIXP_SHIFT);
		/* overflow: clip value */
		if (nsample > 0x7fff)
			nsample = 0x7fff;
		if (nsample < -0x8000)
...
void Player_V2::generateSpkSamples(int16 *data, uint len) {
	...
	for (int i = 0; i < 4; i++) {
		if (winning_channel == -1 && _channels[i].d.volume && _channels[i].d.time_left) {
			winning_channel = i;
		}
	}
...
void Player_V2::generatePCjrSamples(int16 *data, uint len) {
	...
	for (i = 1; i < 3; i++) {
		...
		if (_channels[i].d.volume && _channels[i].d.time_left) {
			for (j = 0; j < i; j++) {
				if (_channels[j].d.volume && _channels[j].d.time_left && freq == (_channels[j].d.freq >> 6)) {
