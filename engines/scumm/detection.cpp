...
static Common::Language detectLanguage(const Common::FSList &fslist, byte id) {
	...
	if (!tmp.isOpen()) {
		...
		if (searchFSNode(fslist, "RESOURCE", resDir) && resDir.isDirectory() && resDir.getChildren(tmpList, Common::FSNode::kListFilesOnly) && searchFSNode(tmpList, filename, langFile)) {
			tmp.open(langFile);
		}
		// The Steam version of Dig has the LANGUAGE.BND in the DIG sub dir...
		if (!tmp.isOpen() && id == GID_DIG && searchFSNode(fslist, "DIG", resDir) && resDir.isDirectory() && resDir.getChildren(tmpList, Common::FSNode::kListFilesOnly) && searchFSNode(tmpList, filename, langFile)) {
			tmp.open(langFile);
		}
	}
	if (tmp.isOpen()) {
