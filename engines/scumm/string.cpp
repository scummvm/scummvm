	...
	for (i = 0; i < _blastTextQueuePos; i++) {
		...
		do {
			...
			do {
				...
				if (_game.id == GID_CMI && _language == Common::ZH_TWN && c == '^' && (buf == _blastTextQueue[i].text + 1)) {
					if (*buf == 'c') {
...
void ScummEngine::CHARSET_1() {
	...
	if ((_game.version <= 6 && _haveMsg == 1) || (_game.version == 7 && _haveMsg != 1)) {
...
int ScummEngine::convertMessageToString(const byte *msg, byte *dst, int dstSize) {
	...
	while (1) {
		...
		if (chr == 0xFF) {
			...
			if ((chr != '@') || (_game.id == GID_CMI && _language == Common::ZH_TWN) || (_game.id == GID_LOOM && _game.platform == Common::kPlatformPCEngine && _language == Common::JA_JPN) || (_game.platform == Common::kPlatformFMTowns && _language == Common::JA_JPN && checkSJISCode(lastChr))) {
				*dst++ = chr;
			}
			lastChr = chr;
		}
