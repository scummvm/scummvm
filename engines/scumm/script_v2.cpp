	...
	case 1:
		...
		if (st->verb == 254) {
			...
			for (i = 0; i < NUM_SCRIPT_SLOT; i++, ss++) {
				if (st->objectA == ss->number && ss->freezeResistant == isBackgroundScript && ss->recursive == isSpecialVerb && (ss->where == WIO_ROOM || ss->where == WIO_INVENTORY || ss->where == WIO_FLOBJECT)) {
					slot = i;
					break;
				}
			}
...
void ScummEngine_v2::o2_drawSentence() {
	...
	if (!((_userState & USERSTATE_IFACE_SENTENCE) || (_game.platform == Common::kPlatformNES && (_userState & USERSTATE_IFACE_ALL))))
		return;
...
void ScummEngine_v2::o2_cutscene() {
	...
	setUserState(USERSTATE_SET_IFACE | USERSTATE_SET_CURSOR | USERSTATE_SET_FREEZE | USERSTATE_FREEZE_ON);
