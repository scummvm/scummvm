	...
	do {
		...
		if (_ctx->speed == -1) {
			...
			for (;;) {
				...
				// OH YES THEY DO
				else if (FROM_32(pAni[_ctx->frameNumber].op) == ANI_ADJUSTX || FROM_32(pAni[_ctx->frameNumber].op) == ANI_ADJUSTY) {
					_ctx->frameNumber += 2;
...
static void t2PlayReel(CORO_PARAM, int x, int y, bool bRestore, int speed, SCNHANDLE hFilm,
	...
	*/
	 ...
	 * Sort out x and y
	 */
	assert(((FROM_32(_ctx->pmi->otherFlags) & OTH_RELATIVE) && !(FROM_32(_ctx->pmi->otherFlags) & OTH_ABSOLUTE)) || ((FROM_32(_ctx->pmi->otherFlags) & OTH_ABSOLUTE) && !(FROM_32(_ctx->pmi->otherFlags) & OTH_RELATIVE)));
	...
	if (bRestore) {
		...
		if ((int32)FROM_32(_ctx->pmi->mulZ) != -1) {
			...
			if (bTop)
				baseZposn += Z_TOPPLAY;
		} else if (column == 0 || GetZfactor(_ctx->reelActor, _ctx->pMover, _ctx->bNewMover) > baseZfact) {
