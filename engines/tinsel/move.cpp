	...
	*ptgtX = *ptgtY = -1;
	...
	case REF_DOWN: // Search downwards
		...
		for (i = clickY + 1; i < end; i++)
			if (InPolygon(clickX, i, PATH) != NOPOLY && InPolygon(clickX, i, BLOCK) == NOPOLY) {
				*ptgtX = clickX;
				*ptgtY = i;
				break;
			}
	...
	case REF_RIGHT: // Search to the right
		...
		for (i = clickX + 1; i < end; i++)
			if (InPolygon(i, clickY, PATH) != NOPOLY && InPolygon(i, clickY, BLOCK) == NOPOLY) {
				*ptgtX = i;
				*ptgtY = clickY;
				break;
			}
...
static void GotThere(PMOVER pMover) {
	...
	if (TinselV2 && pMover->bIgPath && pMover->zOverride != -1 && InPolygon(pMover->objX, pMover->objY, PATH) == NOPOLY)
...
static void SetMoverIntDest(PMOVER pMover, int x, int y) {
	...
	if (pMover->hCpath == hTpath || pMover->bIgPath || IsInPolygon(pMover->objX, pMover->objY, hTpath)) {
...
static void SetNextDest(PMOVER pMover) {
	...
	while (targetX != -1 || targetY != -1) {
		...
		if (s1 == (XTHERE | YTHERE)) {
			...
			pMover->over = false;
			break;
		} else if ((s1 & STUCK) || s1 == (XRESTRICT + YRESTRICT) || s1 == (XTHERE | YRESTRICT) || s1 == (YTHERE | XRESTRICT)) {
			/*-------------------------------------------------
			...
			break;
		} else if (((lstatus & YRESTRICT) && !(s1 & YRESTRICT)) || ((lstatus & XRESTRICT) && !(s1 & XRESTRICT))) {
			/*-----------------------------------------------
		...
		if (ma != NULL) {
			...
			if (!InMoverBlock(pMover, pMover->objX, pMover->objY)) {
				...
				*hS2p = -1;
				...
				if (*newx != fromx && InMoverBlock(pMover, *newx, fromy) == NULL && InPolygon(*newx, fromy, BLOCK) == NOPOLY && InPolygon(*newx, fromy, PATH) != NOPOLY) {
					*newy = fromy;
					*s1 |= YRESTRICT;
				} else if (*newy != fromy && InMoverBlock(pMover, fromx, *newy) == NULL && InPolygon(fromx, *newy, BLOCK) == NOPOLY && InPolygon(fromx, *newy, PATH) != NOPOLY) {
					*newx = fromx;
					*s1 |= XRESTRICT;
				} else {
					*newx = fromx;
	...
	if (PolySubtype(hPath) == NODE) {
		...
		if (lastnode == firstnode) {
			...
			if (pMover->line == pMover->Tline && hPath == pMover->hUpath) {
				...
				getNpathNode(hPath, firstnode, &nx, &ny);
				if (ABS(pMover->objX - nx) < XMDIST && ABS(pMover->objY - ny) < YMDIST) {
					pMover->npstatus = ENTERING;
					pMover->hFnpath = hPath;
					SetNextDest(pMover);
				} else {
...
#define NUMBER 1
	...
	if ((pActor->targetX != -1 || pActor->targetY != -1) && (wasx == pActor->objX && wasy == pActor->objY)) {
		for (i = 0; i < NUMBER; i++) {
			MoveActor(pActor);
