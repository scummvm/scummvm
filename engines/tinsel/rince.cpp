...
static void InitMover(PMOVER pMover) {
	...
	if (pMover->direction != FORWARD && pMover->direction != AWAY && pMover->direction != LEFTREEL && pMover->direction != RIGHTREEL)
...
void SetMoverWalkReel(PMOVER pMover, DIRECTION reel, int scale, bool force) {
	...
	if (force || pMover->scale != scale || pMover->direction != reel) {
		...
		if (pMover->scale != scale && scale <= NUM_MAINSCALES && pMover->scale <= NUM_MAINSCALES && (whichReel = ScalingReel(pMover->actorID, pMover->scale, scale, reel)) != 0) {
			//			error("Cripes");
