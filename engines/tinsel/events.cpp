...
void PlayerEvent(PLR_EVENT pEvent, const Common::Point &coOrds) {
	...
	} else if ((pEvent == PLR_PROV_WALKTO) || (pEvent == PLR_WALKTO) || (pEvent == PLR_LOOK) || (pEvent == PLR_ACTION)) {
		++g_leftEvents;
	}
...
void PolyTinselProcess(CORO_PARAM, const void *param) {
	...
	if (TinselV2) {
		...
		if (!TestToken(TOKEN_CONTROL) && (to->event == WALKTO || to->event == ACTION || to->event == LOOK))
			CORO_KILL_SELF();
