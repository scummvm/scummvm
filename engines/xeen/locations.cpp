...
namespace Locations {
	...
	BaseLocation::BaseLocation(LocationAction action)
	  : ButtonContainer(g_vm)
	  , _locationActionId(action)
	  , _ccNum(g_vm->_files->_ccNum)
	  , _vocName("hello1.voc")
	  , _exitToUi(false) {
	...
	void BankLocation::depositWithdrawl(PartyBank whereId) {
		...
		do {
			...
			if ((whereId == WHERE_BANK && !party._bankGems && consType == CONS_GEMS) || (whereId == WHERE_BANK && !party._bankGold && consType == CONS_GOLD) || (whereId == WHERE_PARTY && !party._gems && consType == CONS_GEMS) || (whereId == WHERE_PARTY && !party._gold && consType == CONS_GOLD)) {
	...
	BlacksmithLocation::BlacksmithLocation()
	  : BaseLocation(BLACKSMITH) {
			...
			if (!c->noActions()) {
				if (!_v21) {
					...
					if (c->_conditions[DRUNK]) {
						...
						map.mazeData()._tavernTips = (map.mazeData()._tavernTips + 1) / (_ccNum ? 10 : 15);
		...
		case Common::KEYCODE_t:
			if (_experienceToNextLevel) {
				...
				if (party.subtract(CONS_GOLD, (c->_level._permanent * c->_level._permanent) * 10, WHERE_PARTY, WT_LOC_WAIT)) {
					...
					c->_experience -= c->nextExperienceLevel() - (c->getCurrentExperience() - c->_experience);
					c->_level._permanent++;
	...
	int ArenaLocation::show() {
		...
		for (uint idx = 0; idx < monsters.size(); ++idx) {
			...
			if ((mon._position.x == 5 || mon._position.x == 10) && (mon._position.y == 8 || mon._position.y == 4))
				mon._position.y = 5;
	...
	CutsceneLocation::CutsceneLocation(LocationAction action)
	  : BaseLocation(action)
	  , _keyFound(false) {
		Party &party = *g_vm->_party;
	...
	ReaperCutscene::ReaperCutscene()
	  : CutsceneLocation(REAPER) {
	}
	...
	GolemCutscene::GolemCutscene()
	  : CutsceneLocation(GOLEM) {
	}
	...
	DwarfCutscene::DwarfCutscene()
	  : CutsceneLocation(DWARF_MINE) {}
	...
	SphinxCutscene::SphinxCutscene()
	  : CutsceneLocation(SPHINX) {
	}
...
bool LocationMessage::execute(int portrait, const Common::String &name, const Common::String &text,
	...
	do {
		...
		do {
			...
			if (confirm || _buttonValue == Common::KEYCODE_ESCAPE || _buttonValue == Common::KEYCODE_n)
				result = 0;
