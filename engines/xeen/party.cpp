...
void Party::handleLight() {
	...
	intf._obscurity = _lightCount ||
	                          (map.mazeData()._mazeFlags2 & FLAG_IS_DARK) == 0
	                      ? OBSCURITY_NONE
	                      : OBSCURITY_BLACK;
}
...
bool Party::arePacksFull() const {
	...
	for (uint idx = 0; idx < _activeParty.size(); ++idx) {
		...
		total += (c._weapons[INV_ITEMS_TOTAL - 1].empty() ? 0 : 1) + (c._armor[INV_ITEMS_TOTAL - 1].empty() ? 0 : 1) + (c._accessories[INV_ITEMS_TOTAL - 1].empty() ? 0 : 1) + (c._misc[INV_ITEMS_TOTAL - 1].empty() ? 0 : 1);
	}
