...
Character *ItemsDialog::execute(Character *c, ItemsMode mode) {
	...
	enum { REDRAW_NONE,
		   REDRAW_TEXT,
		   REDRAW_FULL } redrawFlag = REDRAW_FULL;
	for (;;) {
		if (redrawFlag == REDRAW_FULL) {
			// Write text for the dialog
			Common::String msg;
			if (mode != ITEMMODE_CHAR_INFO && mode != ITEMMODE_8 && mode != ITEMMODE_ENCHANT && mode != ITEMMODE_RECHARGE && mode != ITEMMODE_TO_GOLD) {
		...
		if (redrawFlag == REDRAW_TEXT || redrawFlag == REDRAW_FULL) {
			...
			for (int idx = 0; idx < INV_ITEMS_TOTAL; ++idx) {
				...
				case CATEGORY_ACCESSORY:
					if (i._id) {
						if ((mode == ITEMMODE_CHAR_INFO && !g_vm->_extOptions._showItemCosts) || mode == ITEMMODE_8 || mode == ITEMMODE_ENCHANT || mode == ITEMMODE_RECHARGE) {
				...
				case CATEGORY_MISC:
					if (i._material == 0) {
						...
						if (mode == ITEMMODE_CHAR_INFO || mode == ITEMMODE_8 || mode == ITEMMODE_ENCHANT || mode == ITEMMODE_RECHARGE) {
...
void ItemsDialog::loadButtons(ItemsMode mode, Character *&c, ItemCategory category) {
	...
	if (mode == ITEMMODE_ENCHANT || mode == ITEMMODE_RECHARGE || mode == ITEMMODE_TO_GOLD) {
		...
		bool flag = mode == ITEMMODE_BUY || mode == ITEMMODE_SELL || mode == ITEMMODE_IDENTIFY || mode == ITEMMODE_REPAIR;
		addButton(Common::Rect(12, 109, 36, 129), Common::KEYCODE_w, &_iconSprites);
		addButton(Common::Rect(46, 109, 70, 129), Common::KEYCODE_a, &_iconSprites);
		addButton(Common::Rect(80, 109, 104, 129), Common::KEYCODE_c, &_iconSprites);
		addButton(Common::Rect(114, 109, 138, 129), Common::KEYCODE_m, &_iconSprites);
	...
	if ((mode != ITEMMODE_CHAR_INFO || category != CATEGORY_MISC) && mode != ITEMMODE_ENCHANT && mode != ITEMMODE_RECHARGE && mode != ITEMMODE_TO_GOLD) {
