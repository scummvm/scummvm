...
Character *ItemsDialog::execute(Character *c, ItemsMode mode) {
	...
	ItemCategory category = mode == ITEMMODE_RECHARGE || mode == ITEMMODE_COMBAT ? CATEGORY_MISC : CATEGORY_WEAPON;
	...
	for (;;) {
				...
				if (_buttonValue < (int)(_vm->_mode == MODE_COMBAT ? combat._combatParty.size() : party._activeParty.size())) {
					...
					if (mode == ITEMMODE_BUY) {
						...
						if (srcItem._state._cursed)
							...
							ErrorScroll::show(_vm, Common::String::format(Res.CATEGORY_BACKPACK_IS_FULL[category], c->_name.c_str()));
						else {
		...
		case Common::KEYCODE_b:
			// Buy
			if (mode != ITEMMODE_CHAR_INFO && mode != ITEMMODE_RECHARGE && mode != ITEMMODE_ENCHANT && mode != ITEMMODE_TO_GOLD) {
				mode = ITEMMODE_BUY;
				c = &_itemsCharacter;
				redrawFlag = REDRAW_FULL;
			}
		...
		case Common::KEYCODE_e:
			if (mode == ITEMMODE_CHAR_INFO || mode == ITEMMODE_ENCHANT || mode == ITEMMODE_TO_GOLD) {
	...
	if (itemIndex != -1) {
				...
				if (!party._mazeId) {
					...
					default:
						if (combat._itemFlag) {
							...
							ErrorScroll::show(_vm, Common::String::format(Res.NO_SPECIAL_ABILITIES, c._items[category].getFullDescription(itemIndex).c_str()));
						}
					}
				}
				break;
		...
		case ITEMMODE_BUY: {
			...
			if (invItems.isFull()) {
				...
				ErrorScroll::show(_vm, Common::String::format(Res.BACKPACK_IS_FULL, _oldCharacter->_name.c_str()));
			} else {
				int cost = calcItemCost(&c, itemIndex, mode, 0, category);
				Common::String desc = c._items[category].getFullDescription(itemIndex);
				if (Confirm::show(_vm, Common::String::format(Res.BUY_X_FOR_Y_GOLD, desc.c_str(), cost))) {
		...
		case ITEMMODE_SELL: {
			...
			if (noNeed) {
				ErrorScroll::show(_vm, Common::String::format(Res.NO_NEED_OF_THIS, c._items[category].getFullDescription(itemIndex).c_str()));
			} else {
...
void ItemsDialog::itemToGold(Character &c, int itemIndex, ItemCategory category,
	...
	if (category == CATEGORY_WEAPON && item._id >= XEEN_SLAYER_SWORD) {
		sound.playFX(21);
		ErrorScroll::show(_vm, Common::String::format("\v012\t000\x03"
		                                              "c%s",
		                                              Res.SPELL_FAILED));
	} else if (!item.empty()) {
