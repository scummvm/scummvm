...
void CharacterInfo::addButtons() {
	...
	if (Common::RU_RUS == Common::parseLanguage(ConfMan.get("language"))) {
		addButton(Common::Rect(285, 11, 309, 31), Common::KEYCODE_d, &_iconSprites);		// russian key В
		addButton(Common::Rect(285, 43, 309, 63), Common::KEYCODE_COMMA, &_iconSprites);	// russian key Б
		addButton(Common::Rect(285, 75, 309, 95), Common::KEYCODE_c, &_iconSprites);		// russian key С
	} else {
