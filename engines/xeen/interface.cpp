...
void Interface::perform() {
	...
	case Common::KEYCODE_b:
		...
		if (map.getCell(2) < map.mazeData()._difficulties._wallNoPass && !map._isOutdoors) {
			switch (party._mazeDirection) {
			case DIR_NORTH:
				++party._mazePosition.y;
				break;
	...
	case Common::KEYCODE_s:
		...
		if (!party.canShoot()) {
			...
			if (combat._attackMonsters[0] != -1 || combat._attackMonsters[1] != -1 || combat._attackMonsters[2] != -1) {
				if ((_vm->_mode == MODE_INTERACTIVE || _vm->_mode == MODE_SLEEPING) && !combat._monstersAttacking && !_charsShooting) {
					doCombat();
				}
			}
...
void Interface::rest() {
	...
	if ((map._currentCantRest || (map.mazeData()._mazeFlags & RESTRICTION_REST)) && _vm->_mode != MODE_INTERACTIVE2) {
...
void Interface::assembleBorder() {
	...
	if (!windows[10]._enabled && !windows[2]._enabled && !windows[38]._enabled) {
