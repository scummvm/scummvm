...
CBmpDialog::CBmpDialog(CWnd *pParent, CPalette *pPalette, int nID,
		const char *pFileName, const int dx, const int dy, BOOL bSaveBackground) :
		CDialog(nID, pParent) {
...
CBmpDialog::CBmpDialog(CWnd *pParent, CPalette *pPalette, int nID,
		int nBmpID, const int dx, const int dy, BOOL bSaveBackground)
	: CDialog(nID, pParent) {
	// can't access null pointers
	//
	assert(pParent != NULL);
