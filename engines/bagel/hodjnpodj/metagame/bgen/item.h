...
#ifndef HODJNPODJ_METAGAME_BGEN_ITEM_H
#define HODJNPODJ_METAGAME_BGEN_ITEM_H
...
#ifndef FRAME_EXE
	char * GetArtSpec(void)
		{ return(m_pItemBitmapPath[m_nID - MG_OBJ_BASE]); }
	char * GetSoundSpec(void)
...
#ifndef FRAME_EXE
static char	*m_pItemText[MG_OBJ_COUNT];	// descriptive text for each item
static char	*m_pItemPluralText[MG_OBJ_COUNT];	// pluralized descriptive text for each item
static char	*m_pItemBitmapPath[MG_OBJ_COUNT];	// disk files specification for each item
static char	*m_pItemSoundPath[MG_OBJ_COUNT];	// disk files specification for each item
