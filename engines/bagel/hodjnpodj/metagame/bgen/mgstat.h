...
#ifndef HODJNPODJ_METAGAME_BGEN_MGSTAT_H
#define HODJNPODJ_METAGAME_BGEN_MGSTAT_H
...
public:
	...
	const char *m_lpszWavFile;	// Sound file
	const char *m_lpszText;		// Text of message
};
...
#define FINDS(tab, idx) { C##tab * lpTab ; \
...
public:
#ifndef FRAME_EXE
	static CLocTable cLocTable[];		// Location to game/action
	static CSectorTable cSectorTable[]; // Sector table
	static CLengthTable cLengthTable[]; // Level to # objects
#endif
	static CGameTable cGameTable[];		// Game to win result
	static CClueTable cHodjClueTable[]; // Hodj clue booklet
	static CClueTable cPodjClueTable[]; // Podj clue booklet
	...
	// Meta game static tables
	/**
	 * Find Hodj/Podj clue
	 * @param bPodj		Hodj if FALSE, Podj if TRUE
	 * @returns			pointer to clue table entry
	 */
	static CClueTable *FindClue(bool bPodj, int iClue);
};
