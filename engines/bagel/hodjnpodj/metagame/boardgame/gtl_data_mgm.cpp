...
static SPECIAL_TRAVEL aTravelArray[MG_SPECIAL_VISIT_COUNT] = {
...
bool CGtlData::SetMetaGame(bool bOn) {
	...
	if (bOn && (!m_bMetaGame || !m_bInitMetaGame || m_bStartMetaGame)) {
		...
		m_xpcGtlDoc->UpdateAllViews(nullptr, HINT_INIT_METAGAME, nullptr);
	}
...
bool CGtlData::ProcessMove(CNode *lpTargetNode)
...
bool CGtlData::MoveCharToNode(CNode *lpTargetNode)
// lpTargetNode -- node to be moved to
// returns: true if error, false otherwise
{
	int iError = 0;            // error code
	CNode *lpNode, *lpNextNode;
	CBgbObject *lpChar;
	CMap *lpMap;
...
int CGtlData::EstimatePathDistance(CNode *lpNode1, CNode *lpNode2)
...
LPINT CGtlData::FindShortestPath(CNode *lpNode1,
	CNode *lpNode2)
	...
	//      remaining elements are path nodes; delete ptr when done
{
	...
	CNode *lpNode;        // current node
	CNode *lpTestNode;    // current node
	...
	struct DIST *lpDist = new struct DIST[m_iNodes];
