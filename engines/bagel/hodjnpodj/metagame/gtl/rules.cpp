 ...
 * HISTORY
 *
 *	  2.0	09/13/94	EDS		merged dll/nondll under compile flag;
 *								paints directly to screen if insufficient memory
 *	  1.11  07/28/94	EDS		don't clear all sounds, use color button
 *	  1.10	07/18/94	EDS		revised repainting logic
 *	  1.9	07/02/94	EDS		add color OKAY button, fix leaks
 *	  1.8	06/30/94	EDS		added optional narrative sound file
 *	  1.7	06/27/94	EDS		do proper centering on parent window
 *	  1.6   06/10/94	EDS		release scroll head / tail resources when done
 *	  1.5	05/11/94	EDS		properly relinquish resources on ESC and ALT-F4
 *	  1.4	05/10/94	EDS		added keyboard input for scrolling
 *	  1.2	05/04/94	EDS		update commentary for routines
 *	  1.1	04/30/94	EDS		crlf does not generate a space for lines
 *								ending with space or dash
 *	  1.0	04/19/94	EDS		first release
 *
 * MODULE DESCRIPTION:
 *
 *     tbs
 ...
 * CONSTRUCTORS:
 *
 *	CRules				create a rules object and initialize elements
 *
 * DESTRUCTORS:
 *
 *	~CRules				destroy a rules object and release its resources
 *
 * PUBLIC:
 *
 *      n/a
 ...
 * PRIVATE:
 *
 *	UpdateScroll		repaint display with specified text page
 *
 * PRIVATE GLOBAL:
 *
 *	LoadScrollDIB		fetch a DIB scroll bitmap section from disk
 *	CreateScrollBitmap	create a DDB bitmap equivalent to a DIB section
 *	SetupMask			setup a mask device context with a given bitmap
 *	SetupCompatibleContext		setup a bitmap in a compatible device context
 *	ReleaseCompatibleContext	release a compatible device context
 *	CreateWorkAreas		create the scroll bitmaps, masks, and work areas		
 *	UnfurlScroll		show animation of scroll opening with initial text
 *	UpdateMore          update the "more" indicator on the scroll
 *	WritePage			build a new page of text			
 *	DoWaitCursor		set the cursor to an hourglass
 *	DoArrowCursor		set the cursor to an arrow
 *	Sleep 				wait for the given number of clock ticks
 *
 * MEMBERS:
 *
 *      refer to rules.h
