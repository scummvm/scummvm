...
// char * argv[];		/* argument list */
{
    ...
    char * p, *q, *pp, *qq ;		/* argument string pointers */
    char argstr[100] ;		/* argument string */
    int argval ;		/* argument value */
    int argspec ;		/* argument was explicitly specified */
    static char * args[] = {"%trace", "%track", "%msg", "%memtrk",
		    "dump1", "%dump2", "%filoc", "%filrw", "%memtrc",
			"%mfc", "%swi", "%cknull", "%trcerr", ""} ;
    static int * ptrs[15] ;		/* ptrs to arg values */
    static int flag = 0 ;	/* initialization flag */

    if (!flag)
    {
	ptrs[0] = &dbgtrc, ptrs[1] = &dbgtrk, ptrs[2] = &dbgmsg,
		ptrs[3] = &dbgmtk, ptrs[4] = &dbgdp1, ptrs[5] = &dbgdp2,
		ptrs[6] = &dbgfoc, ptrs[7] = &dbgfrw, ptrs[8] = &dbgmtc,
		ptrs[9] = &dbgmfc, ptrs[10] = &dbgswi,
		ptrs[11] = &dbgckn, ptrs[12] = &dbgtrr ;
	flag = 1 ;
    }
    for (i=1; i<argc; ++i) /* loop through parms */
	if ( *(p = argv[i]) == '%')	/* if it begins with percent */
	{
	    for (pp = argstr, qq = p ; *qq && *qq != CCHCOL ; )
		*pp++ = *qq++ ;	/* copy chars up to null or colon */
	    *pp = 0 ;		/* end arg string with null */
	    if ( (argspec = (*qq == CCHCOL)) )
	    			/* if arg contains a colon */
		argval = atoi(qq+1) ;	/* convert number after colon
						to an integer */
	    else		/* no specified argument */
		argval = 1 ;	/* default value is "true" */

	    for (j = 0 ;  *(q = args[j]) != 0 &&
				strcmp(argstr,q) != 0 ; ++j)
		;		/* search thru list of argument names */
	    if (*q)		/* if argument match found */
	    {
		*ptrs[j] = argval ;	/* set argument value */
		if (ptrs[j] == &dbgckn && argspec)	/* if explicit
					arg specified for cknull */
		    dbgckf = argval ;	/* also set follow on byte size */
	    }
	    else		/* argument match not found */
		ifcprf("Invalid argument %s.\n", p) ;
	}
}
...
// #define CWCTRON 1104	/* trace on/off toggle character */
...
//extern int dbgtrc ;	/* trace flag */
//extern int dbgtrk ;	/* track flag */
//extern int dbgckn, dbgckf ;	/* cknull:arg flags */
...
/** dbggnm -- get name from debug stack **/
char * dbggnm(int num)
// int num ;		/* 0=top of stack, 1=next-to-top, etc. */
{
    if (num > dbgdep) return ("");
    else return((char *)(dbglst + (dbgdep - num - 1)));
				/* return ptr to entry */
}
...
int dbggsd(void)
{
    return(dbgdep) ;	/* give caller the stack depth */
}

//* dbgssd -- set stack depth
void dbgssd(int depth)
// int depth ;	/* stack depth -- should have been saved from dbggsd */
{
    dbgdep = depth - 1;	/* reset stack depth */
    if (dbgdep < 0)		/* if result is negative */
	dbgdep = 0 ;	/* reset to zero */
    if (dbgtrc)		/* if tracing enabled */
    {
//	dbganc = -1 ;	/* force dump of whole list */
//	ifcprf("\n<<Reset debug trace stack:") ;
	dbgtyp("\n<<Reset debug trace stack:", TRUE) ;
				/* type list of names */
    }
}
...
#ifndef JXKEYBOARD
int kybtst(void) {return(0);}
int kybrd(void) {return(0);}
...
//    IF ( !(hWnd = hStartWnd))	// if no starting window
//	hWnd = stWddb.hFocusWnd ;	// start with current focus
//    ENDIF

    if (!IsWindow( (hWnd = hStartWnd) )) // is window arg valid?
    {
	hWnd = (HWND)NULL ;		// if not, do all windows
    }

//  IF (IsWindow(hWnd))		// if we have a starting window
//	WHILE ( (hParentWnd = GetParent(hWnd)) )
//			// loop up to the top of the tree
//	    hWnd = hParentWnd ;
//	ENDWHILE
    ...
    if (hWnd)		// if we're doing children of one window
    {
	dbgDumpWindowInfo(hWnd, (DWORD)(LPWDDB)&stWddb) ;
			// dump info on top window
	EnumChildWindows(hWnd, (FARPROC)dbgDumpEnumProc,
			(DWORD)(LPWDDB)&stWddb) ; // do children
    }
    else		// doing all windows on screen
    {
	EnumWindows((FARPROC)dbgDumpEnumProc,
			(DWORD)(LPWDDB)&stWddb) ; // do parents
    }
//    ENDIF
    dbgtrc = iSaveDbgtrc ;
