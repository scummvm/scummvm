...
const int kScene4090FirstState = 0x0ffa;
const int kScene4090LastState = 0x1003;
const int kScene4100FirstState = 0x1004;
const int kScene4100LastState = 0x100d;
const int kScene4110FirstState = 0x100e;
const int kScene4110LastState = 0x1017;
...
const int kScene5130FirstState = 0x140a;
const int kScene5130LastState = 0x1413;
...
const int kScene6060FirstState = 0x17ac;
const int kScene6060LastState = 0x17ad;
const int kScene6070FirstState = 0x17b6;
const int kScene6070LastState = 0x17bf;
const int kScene6080FirstState = 0x17c0;
const int kScene6080LastState = 0x17c9;
const int kScene6090State = 0x17ca;
const int kScene6100State = 0x17d4;
...
bool isImplementedGameplayState(int stateId) {
	return stateId == kTitleFrontEndState ||
		...
		(stateId >= kScene4090FirstState && stateId <= kScene4090LastState) ||
		(stateId >= kScene4100FirstState && stateId <= kScene4100LastState) ||
		...
		(stateId >= kScene5130FirstState && stateId <= kScene5130LastState) ||
		...
		(stateId >= kScene6060FirstState && stateId <= kScene6060LastState) ||
		(stateId >= kScene6070FirstState && stateId <= kScene6070LastState) ||
		(stateId >= kScene6080FirstState && stateId <= kScene6080LastState) ||
		stateId == kScene6090State ||
		stateId == kScene6100State ||
...
int gameplayStateForBootParam(int bootParam) {
	...
	if (bootParam == 6060)
		return kScene6060FirstState;
	if (bootParam == 6070)
		return kScene6070FirstState;
	if (bootParam == 6080)
		return kScene6080FirstState;
	if (bootParam == 6090)
		return kScene6090State;
	if (bootParam == 6100)
		return kScene6100State;
	if (bootParam == 3040)
	...
	if (bootParam == 4090)
		return kScene4090FirstState;
	if (bootParam == 4100)
		return kScene4100FirstState;
	if (bootParam == 4110)
		return kScene4110FirstState;
	if (bootParam == 8010)
...
Common::Error HollywoodEngine::run() {
	...
	while (!Engine::shouldQuit() && handledState) {
		...
		if (stateId >= kScene2090FirstState && stateId <= kScene2090LastState) {
			handledState = true;
		...
		if (stateId >= kScene4090FirstState && stateId <= kScene4090LastState) {
			handledState = true;
			Scene4090 scene4090(this);
			if (!scene4090.play())
				return Common::kReadingFailed;
			continue;
		}

		if (stateId >= kScene4100FirstState && stateId <= kScene4100LastState) {
			handledState = true;
			Scene4100 scene4100(this);
			if (!scene4100.play())
				return Common::kReadingFailed;
			continue;
		}

		if (stateId >= kScene4110FirstState && stateId <= kScene4110LastState) {
			handledState = true;
			Scene4110 scene4110(this);
			if (!scene4110.play())
				return Common::kReadingFailed;
			continue;
		}
		...
		if (stateId == kScene5080State) {
			handledState = true;
			Scene5080 scene5080(this);
			if (!scene5080.play())
				return Common::kReadingFailed;
			continue;
		}

		if (stateId >= kScene5090FirstState && stateId <= kScene5090LastState) {
			handledState = true;
		...
		if (stateId >= kScene5130FirstState && stateId <= kScene5130LastState) {
			handledState = true;
			Scene5130 scene5130(this);
			if (!scene5130.play())
				return Common::kReadingFailed;
			continue;
		}
		...
		if (stateId >= kScene6060FirstState && stateId <= kScene6060LastState) {
			handledState = true;
			Scene6060 scene6060(this);
			if (!scene6060.play())
				return Common::kReadingFailed;
			continue;
		}

		if (stateId >= kScene6070FirstState && stateId <= kScene6070LastState) {
			handledState = true;
			Scene6070 scene6070(this);
			if (!scene6070.play())
				return Common::kReadingFailed;
			continue;
		}

		if (stateId >= kScene6080FirstState && stateId <= kScene6080LastState) {
			handledState = true;
			Scene6080 scene6080(this);
			if (!scene6080.play())
				return Common::kReadingFailed;
			continue;
		}

		if (stateId == kScene6090State) {
			handledState = true;
			Scene6090 scene6090(this);
			if (!scene6090.play())
				return Common::kReadingFailed;
			continue;
		}

		if (stateId == kScene6100State) {
			handledState = true;
			Scene6100 scene6100(this);
			if (!scene6100.play())
				return Common::kReadingFailed;
			continue;
		}
		...
		if (stateId == kScene9090State) {
			handledState = true;
			Scene9090 scene9090(this);
			if (!scene9090.play())
				return Common::kReadingFailed;
			continue;
		}

		if (stateId == kScene9130State) {
			handledState = true;
			Scene9130 scene9130(this);
			if (!scene9130.play())
				return Common::kReadingFailed;
			continue;
		}
