...
void MickeyEngine::readExe(int ofs, uint8 *buffer, long buflen) {
	...
	if (!infile.open("mickey.exe"))
		return;
	infile.seek(ofs, Common::kSeekSet);
...
void MickeyEngine::printDatString(int iStr) {
	...
	infile.seek(hdr.ofsStr[iStr] + IDI_MSA_OFS_DAT, Common::kSeekSet);
...
void MickeyEngine::printDesc(int iRoom) {
	...
	infile.seek(hdr.ofsDesc[iRoom - 1] + IDI_MSA_OFS_DAT, Common::kSeekSet);
...
bool MickeyEngine::checkMenu() {
	...
	infile.seek(hdr.ofsRoom[_gameStateMickey.iRoom - 1] + IDI_MSA_OFS_DAT, Common::kSeekSet);
