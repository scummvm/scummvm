...
namespace OnceUpon {
	...
	Stork::Stork(GobEngine *vm, const ANIFile &ani)
	  : ANIObject(ani)
	  , _shouldDrop(false) {
	...
	void Stork::advance() {
		...
		case kStateFlyNearWithBundle:
			...
			if ((curRight <= _bundleDrop.dropX) && (nextRight >= _bundleDrop.dropX) && _shouldDrop && !_bundleDrop.dropWhileFar)
		...
		case kStateFlyFarWithBundle:
			...
			if ((curX >= _bundleDrop.dropX) && (nextX <= _bundleDrop.dropX) && _shouldDrop && _bundleDrop.dropWhileFar)
