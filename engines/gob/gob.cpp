...
GobEngine::GobEngine(OSystem *syst)
  : Engine(syst)
  , _rnd("gob") {
...
void GobEngine::validateLanguage() {
	if (_global->_languageWanted != _global->_language) {
		...
		if (((_global->_languageWanted == 2) && (_global->_language == 5)) || ((_global->_languageWanted == 5) && (_global->_language == 2)))
...
void GobEngine::validateVideoMode(int16 videoMode) {
	if ((videoMode != 0x10) && (videoMode != 0x13) && (videoMode != 0x14) && (videoMode != 0x18))
...
Endianness GobEngine::getEndianness() const {
	if ((getPlatform() == Common::kPlatformAmiga) || (getPlatform() == Common::kPlatformMacintosh) || (getPlatform() == Common::kPlatformAtariST))
