...
void Scenery::renderStatic(int16 scenery, int16 layer) {
	...
	for (order = 0; order < 100; order++) {
		for (plane = 0, planePtr = layerPtr->planes; plane < planeCount; plane++, planePtr++) {
			...
			_vm->_draw->_sourceSurface = _staticPictToSprite[scenery * 7 + pictIndex];
			_vm->_draw->_destSurface = Draw::kBackSurface;
...
void Scenery::updateStatic(int16 orderFrom, byte index, byte layer) {
	...
	for (order = orderFrom; order < 100; order++) {
		for (planePtr = layerPtr->planes, plane = 0;
			...
			if ((_vm->_draw->_destSpriteX + _vm->_draw->_spriteRight - 1) > _toRedrawRight)
				_vm->_draw->_spriteRight = _toRedrawRight - _vm->_draw->_destSpriteX + 1;

			if ((_vm->_draw->_destSpriteY + _vm->_draw->_spriteBottom - 1) > _toRedrawBottom)
				_vm->_draw->_spriteBottom = _toRedrawBottom - _vm->_draw->_destSpriteY + 1;

			_vm->_draw->_sourceSurface = _staticPictToSprite[index * 7 + pictIndex];
			_vm->_draw->_destSurface = Draw::kBackSurface;
			_vm->_draw->_transparency = planePtr->transp ? 3 : 0;
			_vm->_draw->spriteOperation(DRAW_BLITSURF);
		}
...
void Scenery::updateAnim(int16 layer, int16 frame, int16 animation, int16 flags,
	...
	if ((animation < 0) && ((_vm->getGameType() == kGameTypeWoodruff) || (_vm->getGameType() == kGameTypeAdibou2))) {
		// Object video
		...
		if (doDraw) {
			...
			if (layer & 0x80)
				_vm->_draw->_spriteLeft = _vm->_vidPlayer->getWidth(obj.videoSlot - 1) - (destX + _vm->_draw->_spriteRight);
	...
	do {
		...
		if (flags & 2) {
			...
			if ((left <= right) && ((destX + right - left) >= (_vm->_mult->_animLeft + _vm->_mult->_animWidth)))
				right -= (destX + right - left) - (_vm->_mult->_animLeft + _vm->_mult->_animWidth) + 1;
			...
			if ((top <= bottom) && ((destY + bottom - top) >= (_vm->_mult->_animTop + _vm->_mult->_animHeight)))
				bottom -= (destY + bottom - top) - (_vm->_mult->_animTop + _vm->_mult->_animHeight) + 1;
		...
		if (doDraw) {
			_vm->_draw->_sourceSurface = _animPictToSprite[animation * 7 + pictIndex];
			_vm->_draw->_destSurface = Draw::kBackSurface;
		...
		if (!(flags & 4)) {
			if (_toRedrawLeft == -12345) {
				...
				_toRedrawRight = MAX(_toRedrawRight, (int16)(destX + right - left));
				_toRedrawBottom = MAX(_toRedrawBottom, (int16)(destY + bottom - top));
			}
		}
