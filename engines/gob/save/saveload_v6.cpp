...
SaveLoad_v6::SpriteHandler::SpriteHandler(GobEngine *vm)
...
SaveLoad_v6::TempHandler::TempHandler(GobEngine *vm)
  : SaveHandler(vm)
  , _empty(true)
  , _size(0)
  , _data(0) {
...
SaveLoad_v6::SaveLoad_v6(GobEngine *vm, const char *targetName)
  : SaveLoad(vm) {
	...
	for (int i = 0; i < 60; i++)
		_saveFiles[9 + i].handler = _extraHandler[i] = new ExtraHandler(_vm, *_gameHandler, 2, i);
	for (int i = 0; i < 60; i++)
		_saveFiles[69 + i].handler = _extraHandler[60 + i] = new ExtraHandler(_vm, *_gameHandler, 3, i);
