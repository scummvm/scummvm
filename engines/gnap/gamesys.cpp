...
void GameSys::drawBitmap(int resourceId) {
	...
	if (bmpSurface->format != _backgroundSurface->format || bmpSurface->w != _backgroundSurface->w || bmpSurface->h != _backgroundSurface->h)
...
void GameSys::handleReqRemoveSpriteDrawItems() {
	if (_removeSpriteDrawItemsCount > 0) {
		for (int j = 0; j < _removeSpriteDrawItemsCount; ++j) {
			for (int i = 0; i < _gfxItemsCount; ++i) {
				GfxItem *gfxItem = &_gfxItems[i];
				if (gfxItem->_sequenceId == -1 && !gfxItem->_animation && gfxItem->_flags && gfxItem->_id == _removeSpriteDrawItems[j]._id && _removeSpriteDrawItems[j]._surface == gfxItem->_surface) {
					gfxItem->_flags = 0;
					gfxItem->_currFrame._duration = 0;
					gfxItem->_currFrame._spriteId = -1;
					gfxItem->_currFrame._soundId = -1;
...
void GameSys::fatUpdateFrame() {
	...
	if (_grabSpriteChanged) {
		for (int i = 0; i < _gfxItemsCount; ++i) {
			GfxItem *gfxItem = &_gfxItems[i];
			if (gfxItem->_sequenceId == -1 && !gfxItem->_animation && gfxItem->_flags && gfxItem->_id == _grabSpriteId && gfxItem->_surface == _grabSpriteSurface1) {
				gfxItem->_currFrame._duration = 0;
				gfxItem->_currFrame._isScaled = false;
