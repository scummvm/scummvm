...
/*
  ...
  support this I use the face_index argument of FT2_1_3_(Open|New)_Face()
...
open_face_from_buffer( FT_Library  library,
	...
	args.flags = FT2_1_3_OPEN_STREAM;
	args.stream = stream;
	if ( driver_name ) {
...
FT2_1_3_New_Face_From_Suitcase( FT_Library  library,
	...
	if ( ResError() ) {
		error = FT2_1_3_Err_Cannot_Open_Resource;
		goto Error;
	}
...
FT_New_Face_From_FOND( FT_Library  library,
	...
	if ( lwfn_file_name[0] ) {
		if ( make_lwfn_spec( fond, lwfn_file_name, &lwfn_spec ) == FT2_1_3_Err_Ok )
	...
	if ( have_lwfn && ( !have_sfnt || PREFER_LWFN ) )
		return FT2_1_3_New_Face_From_LWFN( library,
		                              &lwfn_spec,
		                              face_index,
		                              aface );
	else if ( have_sfnt )
		return FT2_1_3_New_Face_From_SFNT( library,
		                              sfnt_id,
		                              face_index,
		                              aface );
...
/*                                                                       */
/* <Description>                                                         */
/*    This is the Mac-specific implementation of FT2_1_3_New_Face.  In        */
/*    addition to the standard FT2_1_3_New_Face() functionality, it also      */
...
FT2_1_3_New_Face( FT_Library   library,
	...
	if ( ResourceForkSize( &spec ) != 0 ) {
		...
		if ( file_type == 'FFIL' || file_type == 'tfil' )
			return FT2_1_3_New_Face_From_Suitcase( library, &spec, face_index, aface );

		if ( file_type == 'LWFN' )
			return FT2_1_3_New_Face_From_LWFN( library, &spec, face_index, aface );
	}

#if TARGET_API_MAC_CARBON

	if ( is_dfont( &spec ) )
		return FT2_1_3_New_Face_From_dfont( library, &spec, face_index, aface );
