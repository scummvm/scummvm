...
tt_face_lookup_table(TT_Face face, FT_ULong tag) {
	...
	FT_TRACE3(("tt_face_lookup_table: %08p, `%c%c%c%c' -- ", face, (FT_Char)(tag >> 24), (FT_Char)(tag >> 16), (FT_Char)(tag >> 8), (FT_Char)(tag)));
...
static FT_Error sfnt_dir_check(FT_Stream stream, FT_ULong offset, FT_UInt num_tables) {
	FT_Error error;
...
#define FT_STRUCTURE TT_TableRec
	...
	if (num_tables == 0) {
		...
		if (FT_STREAM_SEEK(offset) || FT_READ_ULONG(format_tag) || FT_READ_USHORT(num_tables) || FT_STREAM_SKIP(6))
			goto Bad_Format;
	...
	for (nn = 0; nn < num_tables; nn++) {
		...
		if (table.Offset + table.Length > stream->size && table.Tag != glyx_tag && table.Tag != locx_tag)
			goto Bad_Format;
		...
		if (table.Tag == TTAG_head) {
			...
			if (table.Length != 0x36 || FT_STREAM_SEEK(table.Offset + 12) || FT_READ_ULONG(magic) || magic != 0x5F0F3CF5UL)
				goto Bad_Format;
...
#define FT_STRUCTURE TTC_HeaderRec
	...
	if (format_tag == TTAG_ttcf) {
		...
		if (FT_NEW_ARRAY(face->ttc_header.offsets, face->ttc_header.count) || FT_FRAME_ENTER(face->ttc_header.count * 4L))
			goto Exit;
		...
		if (FT_STREAM_SEEK(offset) || FT_READ_LONG(format_tag))
			goto Exit;
	}
...
tt_face_load_directory(TT_Face face, FT_Stream stream, SFNT_Header sfnt) {
	...
	if (FT_STREAM_SEEK(sfnt->offset + 12) || FT_FRAME_ENTER(face->num_tables * 16L))
		goto Exit;
	...
	for (; entry < limit; entry++) {
		...
		FT_TRACE2(("  %c%c%c%c  -  %08lx  -  %08lx\n", (FT_Char)(entry->Tag >> 24), (FT_Char)(entry->Tag >> 16),
						(FT_Char)(entry->Tag >> 8), (FT_Char)(entry->Tag), entry->Offset, entry->Length));
	}
...
#define FT_STRUCTURE  TT_Header
	...
	FT_TRACE2(("tt_face_load_generic_header: %08p, looking up font table `%c%c%c%c'.\n", face,
					(FT_Char)(tag >> 24), (FT_Char)(tag >> 16), (FT_Char)(tag >> 8), (FT_Char)(tag)));
...
#define FT_STRUCTURE TT_MaxProfile
	...
	if (maxProfile->version >= 0x10000L) {
		...
		face->root.internal->max_points = (FT_UShort)MAX(maxProfile->maxCompositePoints, maxProfile->maxPoints);

		face->root.internal->max_contours = (FT_Short)MAX(maxProfile->maxCompositeContours, maxProfile->maxContours);

		face->max_components = (FT_ULong)maxProfile->maxComponentElements + maxProfile->maxComponentDepth;
...
static FT_Error tt_face_load_metrics(TT_Face face, FT_Stream stream, FT_Bool vertical) {
	FT_Error  error;
	FT_Memory memory = stream->memory;
	...
	if (vertical) {
		...
		if (error) {
			...
			error = FT_Err_Hmtx_Table_Missing;
			goto Exit;
		}
	...
	if (num_shorts < 0) {
		...
		error = vertical ? FT_Err_Invalid_Vert_Metrics : FT_Err_Invalid_Horiz_Metrics;
		goto Exit;
	}

	if (FT_NEW_ARRAY(*longs, num_longs) || FT_NEW_ARRAY(*shorts, num_shorts))
		goto Exit;
...
#define FT_STRUCTURE TT_HoriHeader
	...
	if (vertical) {
		...
		if (error) {
			error = FT_Err_Horiz_Header_Missing;
			goto Exit;
		}
...
#define FT_STRUCTURE TT_NameEntryRec
	...
	if (error) {
		...
		error = FT_Err_Name_Table_Missing;
		goto Exit;
	}
	...
	if (FT_NEW_ARRAY(table->names, count) || FT_FRAME_ENTER(count * 12))
		goto Exit;
...
tt_face_load_cmap(TT_Face face, FT_Stream stream) {
	...
	if (error) {
		...
		error = FT_Err_CMap_Table_Missing;
		goto Exit;
	}
...
#define FT_STRUCTURE TT_Postscript
	...
	if (error)
		return FT_Err_Post_Table_Missing;
...
tt_face_load_gasp(TT_Face face, FT_Stream stream) {
	...
	if (FT_NEW_ARRAY(gaspranges, num_ranges) || FT_FRAME_ENTER(num_ranges * 4L))
		goto Exit;
	...
	for (j = 0; j < num_ranges; j++) {
		...
		FT_TRACE3((" [max:%d flag:%d]", gaspranges[j].maxPPEM, gaspranges[j].gaspFlag));
	}
	FT_TRACE3(("\n"));
...
tt_face_load_kern(TT_Face face, FT_Stream stream) {
	...
	for (n = 0; n < num_tables; n++) {
		...
		if (coverage == 0x0001) {
			...
			if (FT_NEW_ARRAY(face->kern_pairs, num_pairs) || FT_FRAME_ENTER(6L * num_pairs))
				goto Exit;
			...
			/* fonts have unsorted tables!)                            */
			{
				...
				for (i = 1; i < num_pairs; i++, pair0++) {
					if (tt_kern_pair_compare(pair0, pair0 + 1) != -1) {
						ft_qsort((void *)face->kern_pairs, (int)num_pairs, sizeof(TT_Kern0_PairRec), tt_kern_pair_compare);
						break;
					}
				}
			}
