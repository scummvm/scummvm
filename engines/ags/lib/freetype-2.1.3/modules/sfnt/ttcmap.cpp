...
code_to_next2(TT_CMapTable cmap, FT_ULong charCode) {
	...
	while (charCode < 0x10000L) {
		...
		offset = sh2->idRangeOffset / 2 + char_lo;
		if (offset >= (FT_UInt)cmap2->numGlyphId || cmap2->glyphIdArray[offset] == 0) {
			charCode++;
			continue;
		}
...
Found1:
	...
	if (seg4->idRangeOffset == 0)
		...
		index1 = (FT_UInt)(seg4->idRangeOffset / 2 + (charCode - seg4->startCount) + (seg4 - cmap4->segments) - segCount);

		if (index1 < (FT_UInt)cmap4->numGlyphId && cmap4->glyphIdArray[index1] != 0)
...
code_to_index8_12(TT_CMapTable cmap, FT_ULong charCode) {
	...
	if ((FT_ULong)(charCode - group->startCharCode) < (FT_ULong)(group->endCharCode - group->startCharCode))
		goto Found1;
...
Found1:
	return (FT_UInt)(group->startGlyphID + (charCode - group->startCharCode));
}
