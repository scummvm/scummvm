...
// TODO: Remove bytecode interpreter stuff
...
tt_face_init(FT_Stream stream, TT_Face face, FT_Int face_index, FT_Int num_params, FT_Parameter *params) {
	...
	if (face->root.face_flags & FT_FACE_FLAG_SCALABLE) {
		...
		if (!error)
			error = tt_face_load_cvt(face, stream) || tt_face_load_fpgm(face, stream);

#else

		if (!error)
			error = tt_face_load_loca(face, stream) || tt_face_load_cvt(face, stream) || tt_face_load_fpgm(face, stream);
...
tt_size_init(TT_Size size) {
	...
	TT_Set_CodeRange(exec, tt_coderange_font, face->font_program, face->font_program_size);
...
static FT_Error Reset_Outline_Size(TT_Size size) {
	...
	metrics->ascender = (FT2_1_3_MulFix(face->root.ascender, metrics->y_scale) + 32) & -64;
	metrics->descender = (FT2_1_3_MulFix(face->root.descender, metrics->y_scale) + 32) & -64;
	metrics->height = (FT2_1_3_MulFix(face->root.height, metrics->y_scale) + 32) & -64;
	metrics->max_advance = (FT2_1_3_MulFix(face->root.max_advance_width, metrics->x_scale) + 32) & -64;
...

	{
		...
		TT_Set_CodeRange(exec, tt_coderange_cvt, face->cvt_program, face->cvt_program_size);
...
static FT_Error Reset_SBit_Size(TT_Size size) {
	...
	if (!error) {
		...
		/* XXX: Is this correct? */
		sbit_metrics->max_advance = (strike->hori.min_origin_SB + strike->hori.max_width + strike->hori.min_advance_SB) << 6;
