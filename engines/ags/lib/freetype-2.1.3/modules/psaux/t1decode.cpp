...
#include "engines/ags/lib/freetype-2.1.3/modules/psaux/psauxerr.h"
...
static FT_Int t1_lookup_glyph_by_stdcharcode(T1_Decoder decoder, FT_Int charcode) {
	...
	for (n = 0; n < decoder->num_glyphs; n++) {
		...
		if (name && name[0] == glyph_name[0] && ft_strcmp(name, glyph_name) == 0)
			return n;
	}
...
#define close_contour t1_builder_close_contour
	...
	while (ip < limit) {
		...
		case 255: /* four bytes integer */
			...
			value = (FT_Int32)(((FT_Long)ip[0] << 24) | ((FT_Long)ip[1] << 16) | ((FT_Long)ip[2] << 8) | ip[3]);
			ip += 4;
			break;
		...
		default:
			if (ip[-1] >= 32) {
				if (ip[-1] < 247)
					...
					if (++ip > limit) {
						FT2_1_3_ERROR(("t1_decoder_parse_charstrings: "));
						FT2_1_3_ERROR(("unexpected EOF in integer\n"));
						goto Syntax_Error;
					}
		...
		if (op == op_none) {
			if (top - decoder->stack >= T1_MAX_CHARSTRINGS_OPERANDS) {
				FT2_1_3_ERROR(("t1_decoder_parse_charstrings: stack overflow!\n"));
				goto Syntax_Error;
			}

			FT2_1_3_TRACE4((" %ld", value));

			*top++ = value;
			decoder->top = top;
		} else if (op == op_callothersubr) { /* callothersubr */
			FT2_1_3_TRACE4((" callothersubr"));
			...
			case 1: /* start flex feature */
				...
				if (start_point(builder, x, y) || check_points(builder, 6))
					goto Memory_Error;
				break;
			...
			case 2: { /* add flex vectors */
				...
				if (idx > 0 && idx < 7)
					add_point(builder, x, y, (FT_Byte)(idx == 3 || idx == 6));
			} break;
			...
			case 3: /* change hints */
				...
				if (ip[0] != 12 || ip[1] != 17) {
					FT2_1_3_ERROR(("t1_decoder_parse_charstrings: "));
					FT2_1_3_ERROR(("`pop' expected, found (%d %d)\n", ip[0], ip[1]));
					goto Syntax_Error;
				}
				ip += 2;
			...
			case 18: { /* multiple masters */
				...
				if (!blend) {
					FT2_1_3_ERROR(("t1_decoder_parse_charstrings: "));
					FT2_1_3_ERROR(("unexpected multiple masters operator!\n"));
					goto Syntax_Error;
				}
			...
			case op_endchar:
				...
				if (hinter) {
					...
					hinter->apply(hinter->hints, builder->current, (PSH_Globals)builder->hints_globals, decoder->hint_mode);
				}
			...
			case op_seac:
				...
				return t1operator_seac(decoder, top[0], top[1], top[2], top[3], top[4]);
			...
			case op_vhcurveto:
				...
				if (start_point(builder, x, y) || check_points(builder, 3))
					goto Memory_Error;
			...
			case op_div:
				...
				if (top[1]) {
					*top = top[0] / top[1];
					...
					FT2_1_3_ERROR(("t1_decoder_parse_charstrings: division by 0\n"));
					goto Syntax_Error;
				}
				break;
			...
			case op_return:
				...
				if (zone <= decoder->zones) {
					FT2_1_3_ERROR(("t1_decoder_parse_charstrings: unexpected return\n"));
					goto Syntax_Error;
				}
			...
			case op_setcurrentpoint:
				FT2_1_3_TRACE4((" setcurrentpoint"));

				FT2_1_3_ERROR(("t1_decoder_parse_charstrings: "));
				FT2_1_3_ERROR(("unexpected `setcurrentpoint'\n"));
				goto Syntax_Error;
