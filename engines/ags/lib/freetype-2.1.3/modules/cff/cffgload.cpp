...
static FT_Int cff_compute_bias(FT_UInt num_subrs) {
	FT_Int result;
...
static FT_Error cff_get_glyph_data(TT_Face face, FT_UInt glyph_index, FT_Byte **pointer, FT_ULong *length) {
	...
	if (face->root.internal->incremental_interface) {
		FT_Data data;
		FT_Error error =
			face->root.internal->incremental_interface->funcs->get_glyph_data(face->root.internal->incremental_interface->object,
																			  glyph_index, &data);
...

	{
		...
		return cff_index_access_element(&cff->charstrings_index, glyph_index, pointer, length);
	}
}
...
static void cff_free_glyph_data(TT_Face face, FT_Byte **pointer, FT_ULong length) {
	...
	if (face->root.internal->incremental_interface) {
		...
		face->root.internal->incremental_interface->funcs->free_glyph_data(face->root.internal->incremental_interface->object, &data);
	} else
#endif /* FT_CONFIG_OPTION_INCREMENTAL */
...
static FT_Error cff_operator_seac(CFF_Decoder *decoder, FT_Pos adx, FT_Pos ady, FT_Int bchar, FT_Int achar) {
	FT_Error 	 error;
	...
	error = cff_get_glyph_data(face, bchar_index, &charstring, &charstring_len);
	if (!error) {
	...
	error = cff_get_glyph_data(face, achar_index, &charstring, &charstring_len);
	if (!error) {
		error = cff_decoder_parse_charstrings(decoder, charstring, charstring_len);
...
cff_decoder_parse_charstrings(CFF_Decoder *decoder, FT_Byte *charstring_base, FT_ULong charstring_len) {
	...
	seed = (FT_Fixed)(uintptr_t)(char *)&seed ^ (FT_Fixed)(uintptr_t)(char *)&decoder ^ (FT_Fixed)(uintptr_t)(char *)&charstring_base;
	...
	while (ip < limit) {
		...
		if (v >= 32 || v == 28) {
			...
			if (v == 28) {
				...
				if (ip + 3 >= limit)
					goto Syntax_Error;
				val = ((FT_Int32)ip[0] << 24) | ((FT_Int32)ip[1] << 16) | ((FT_Int32)ip[2] << 8) | ip[3];
				ip += 4;
				shift = 0;
			}
			...
			*decoder->top++ = val;
			...
			case cff_op_vstemhm:
				...
				FT_TRACE4((op == cff_op_hstem ? " hstem" : (op == cff_op_vstem ? " vstem" : (op == cff_op_hstemhm ? " hstemhm" : " vstemhm"))));

				if (hinter)
					hinter->stems(hinter->hints, (op == cff_op_hstem || op == cff_op_hstemhm), num_args / 2, args);
			...
			case cff_op_cntrmask:
				...
				if (num_args > 0) {
					if (hinter)
						hinter->stems(hinter->hints, 0, num_args / 2, args);
...
#ifdef FT_DEBUG_LEVEL_TRACE
				{
					...
					for (maskbyte = 0; maskbyte < (FT_UInt)((decoder->num_hints + 7) >> 3); maskbyte++, ip++)
						FT_TRACE4(("%02X", *ip));
				}
#else
			...
			case cff_op_rlineto:
				...
				if (cff_builder_start_point(builder, x, y) || check_points(builder, num_args / 2))
					goto Memory_Error;
			...
			case cff_op_vlineto: {
				...
				FT_TRACE4((op == cff_op_hlineto ? " hlineto" : " vlineto"));

				if (cff_builder_start_point(builder, x, y) || check_points(builder, num_args))
					goto Memory_Error;
			...
			case cff_op_hvcurveto: {
				...
				FT_TRACE4((op == cff_op_vhcurveto ? " vhcurveto" : " hvcurveto"));
			...
			case cff_op_rlinecurve: {
				...
				if (cff_builder_start_point(builder, x, y) || check_points(builder, num_lines + 3))
					goto Memory_Error;
			...
			case cff_op_rcurveline: {
				...
				if (cff_builder_start_point(builder, x, y) || check_points(builder, num_curves * 3 + 2))
					goto Memory_Error;
			...
			case cff_op_hflex1: {
				...
				/* needs to be added..                                         */
				if (cff_builder_start_point(builder, x, y) || check_points(builder, 6))
					goto Memory_Error;
			...
			case cff_op_flex: {
				...
				for (count = 6; count > 0; count--) {
					x += args[0];
					y += args[1];
					cff_builder_add_point(builder, x, y, (FT_Bool)(count == 3 || count == 0));
					args += 2;
				}
			...
			case cff_op_endchar:
				...
				if (num_args == 4) {
					error = cff_operator_seac(decoder, args[0] >> 16, args[1] >> 16, (FT_Int)(args[2] >> 16), (FT_Int)(args[3] >> 16));
					args += 4;
				}
				...
				if (hinter) {
					...
					hinter->apply(hinter->hints, builder->current, (PSH_Globals)builder->hints_globals, (FT_Render_Mode)builder->hint_flags);
				}
			...
			case cff_op_sqrt:
				...
				if (args[0] > 0) {
					...
					for (;;) {
						new_root = (root + FT2_1_3_DivFix(args[0], root) + 1) >> 1;
...
cff_compute_max_advance(TT_Face face, FT_Int *max_advance) {
	...
	*max_advance = 0;
	...
	for (glyph_index = 0; glyph_index < face->root.num_glyphs;
		...
		error = cff_get_glyph_data(face, glyph_index, &charstring, &charstring_len);
		if (!error) {
