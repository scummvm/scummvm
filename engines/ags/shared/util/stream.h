...
public:
	...
	// Helper methods
	//-----------------------------------------------------
	inline int8 ReadInt8() override {
		return ReadByte();
	}

	inline size_t WriteInt8(int8 val) override {
		int ival = WriteByte(val);
	...
	inline size_t ReadArrayOfInt8(int8 *buffer, size_t count) override {
		return Read(buffer, count);
	}
	inline size_t WriteArrayOfInt8(const int8 *buffer, size_t count) override {
		return Write(buffer, count);
	}

	// Fill the requested number of bytes with particular value
	size_t WriteByteCount(uint8 b, size_t count);
...
public:
	...
	int ReadByte() override { return _stream->readByte(); }
	size_t Write(const void *buffer, size_t size) override { return 0; }
	int WriteByte(uint8 b) override { return 0; }

	int8 ReadInt8() override { return (int8)_stream->readByte(); }
	int16 ReadInt16() override { return _stream->readSint16LE(); }
	int ReadInt32() override { return _stream->readSint32LE(); }
	int64 ReadInt64() override { return _stream->readSint64LE(); }
	...
	size_t ReadArrayOfInt8(int8 *buffer, size_t count) override {
	...
	size_t WriteInt8(int8 val) override { return 0; }
	size_t WriteInt16(int16 val) override { return 0; }
	size_t WriteInt32(int val) override { return 0; }
	size_t WriteInt64(int64 val) override { return 0; }
	...
	size_t WriteArrayOfInt8(const int8 *buffer, size_t count) override { return 0; }
	size_t WriteArrayOfInt16(const int16 *buffer, size_t count) override { return 0; }
	size_t WriteArrayOfInt32(const int *buffer, size_t count) override { return 0; }
	size_t WriteArrayOfInt64(const int64 *buffer, size_t count) override { return 0; }
