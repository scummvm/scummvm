...
static const byte DOS_FONT_DATA[] = {
...
ScreenSurface::ScreenSurface(MortevielleEngine *vm) {
	...
	/* DOS System font */
	_fonts[kFontDOS]._fontData = (byte *)DOS_FONT_DATA;
	_fonts[kFontDOS]._width = DOS_FONT_WIDTH;
	_fonts[kFontDOS]._height = DOS_FONT_HEIGHT;
	_fonts[kFontDOS]._numChars = DOS_FONT_NUM_CHARS;
	/* Engine font */
	_fonts[kFontEngine]._fontData = nullptr;
	_fonts[kFontEngine]._width = FONT_WIDTH;
	_fonts[kFontEngine]._height = FONT_HEIGHT;
	_fonts[kFontEngine]._numChars = FONT_NUM_CHARS;
...
void ScreenSurface::readFontData(Common::File &f, int dataSize) {
	Font *font = &_fonts[kFontEngine];

	assert(dataSize == (font->_numChars * font->_height));
	::free(font->_fontData);
	font->_fontData = (byte *)malloc(dataSize);
	f.read(font->_fontData, dataSize);
}

int ScreenSurface::setFont(int index) {
	int oldFont = _curFont;
	_curFont = index;
	return oldFont;
}
 ...
 *		simulate the original 640x400 surface, all Y values have to be doubled
 */
void ScreenSurface::writeCharacter(const Common::Point &pt, unsigned char ch, int palIndex, int scaleFactor) {
	Font *font = &_fonts[_curFont];

	Graphics::Surface destSurface = lockArea(Common::Rect(pt.x, pt.y * scaleFactor,
		pt.x + font->_width, (pt.y + font->_height) * scaleFactor));

	// Get the start of the character to use
	assert((ch >= ' ') && (ch <= (unsigned char)(32 + font->_numChars)));
	const byte *charData = &(font->_fontData[((int)ch - 32) * font->_height]);

	// Loop through decoding each character's data
	for (int yp = 0; yp < font->_height; ++yp) {
		byte *lineP = (byte *)destSurface.getBasePtr(0, yp * scaleFactor);
		byte byteVal = *charData++;

		for (int xp = 0; xp < font->_width; ++xp, ++lineP, byteVal <<= 1) {
			if (byteVal & 0x80) {
				*lineP = palIndex;
				if (scaleFactor == kResolutionScaler)
...
int ScreenSurface::getStringWidth(const Common::String &s) {
	return s.size() * (_curFont == kFontEngine ? 6 : _fonts[_curFont]._width);
}
...
/**
 * Draw string with teh DOS system font
 */
void ScreenSurface::drawSystemString(const Common::String &l, int color) {
	...
	int oldFont = setFont(kFontDOS);
	for (int x = 0; (x < l.size()) && (l[x] != 0); ++x) {
		_vm->_screenSurface->writeCharacter(Common::Point(pt.x, pt.y), l[x], color, 1);
		pt.x += _fonts[kFontDOS]._width;
	}
	setFont(oldFont);
	_vm->_mouse->showMouse();
}
...
ScreenSurface::~ScreenSurface() {
	::free(_fonts[kFontEngine]._fontData);
	free();
}
