...
void EclipseEngine::drawDOSUI(Graphics::Surface *surface) {
	...
	if (deadline <= _countdown) {
		drawStringInSurface(message, 102, 135, black, transparent, surface);
		_temporaryMessages.push_back(message);
		_temporaryMessageDeadlines.push_back(deadline);
	} else if (!_currentAreaMessages.empty())
		drawStringInSurface(_currentArea->_name, 102, 135, black, transparent, surface);

	drawScoreString(score, 136, 6, black, transparent, surface);
	...
	drawStringInSurface(shiftStr("0", 'Z' - '$' + 1 - _angleRotationIndex), 79, 135, black, transparent, surface);
	drawStringInSurface(shiftStr("3", 'Z' - '$' + 1 - _playerStepIndex), 63, 135, black, transparent, surface);
	drawStringInSurface(shiftStr("7", 'Z' - '$' + 1 - _playerHeightNumber), 240, 135, black, transparent, surface);

	if (_shootingFrames > 0) {
		drawStringInSurface(shiftStr("4", 'Z' - '$' + 1), 232, 135, black, transparent, surface);
		drawStringInSurface(shiftStr("<", 'Z' - '$' + 1), 240, 135, black, transparent, surface);
	}
	...
	surface->fillRect(jarWater, cyan);

	drawIndicator(surface, 41, 4, 16);
	drawEclipseIndicator(surface, 228, 0, white, pink, cyan);
	surface->fillRect(Common::Rect(225, 168, 235, 187), transparent);
