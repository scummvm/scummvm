...
#define YWALK_SPEED 1
	...
	void ScalpelPerson::adjustSprite() {
		...
		if (_frameNumber >= (int)_walkSequences[_sequenceNumber]._sequences.size() || _walkSequences[_sequenceNumber][_frameNumber] == 0) {
	...
	ScalpelPeople::ScalpelPeople(SherlockEngine *vm)
	  : People(vm) {
			...
			if (objNum != -1) {
				...
				if (obj._seqSize < MAX_TALK_SEQUENCES) {
					...
					for (uint idx = 0; idx < MAX_TALK_SEQUENCES; ++idx) {
						...
						if (idx > 0 && !people._characters[speaker]._talkSequences[idx] && !people._characters[speaker]._talkSequences[idx - 1])
							break;
					}
