...
namespace Scalpel {
	...
	ScalpelInventory::ScalpelInventory(SherlockEngine *vm)
	  : Inventory(vm) {
		_invShapes.resize(6);
	...
	void ScalpelInventory::highlight(int index, byte color) {
		...
		bb.SHtransBlitFrom(frame, Common::Point(6 + slot * 52 + ((47 - frame._width) / 2), 163 + ((33 - frame._height) / 2)));
	...
	void ScalpelInventory::putInv(InvSlamMode slamIt) {
		...
		for (int idx = _invIndex; idx < _holdings && (idx - _invIndex) < (int)_invShapes.size(); ++idx) {
			...
			bb.SHtransBlitFrom(frame, Common::Point(6 + itemNum * 52 + ((47 - frame._width) / 2), 163 + ((33 - frame._height) / 2)));
		}
