...
namespace Scalpel {
	...
	ScalpelUserInterface::ScalpelUserInterface(SherlockEngine *vm)
	  : UserInterface(vm) {
	...
	void ScalpelUserInterface::drawInterface(int bufferNum) {
		...
		if (bufferNum == 3)
			screen._backBuffer2.SHfillRect(Common::Rect(0, INFO_LINE,
			                                            SHERLOCK_SCREEN_WIDTH, INFO_LINE + 10),
			                               INFO_BLACK);
	}
	...
	void ScalpelUserInterface::handleInput() {
		...
		if (!talk._scriptMoreFlag) { // Don't if scripts are running
			if (((events._rightPressed || events._rightReleased) && _helpStyle) || (!_helpStyle && !_menuCounter)) {
				// Handle any default commands if we're in STD_MODE
				if (_menuMode == STD_MODE) {
					if (pt.y < CONTROLS_Y && (events._rightPressed || (!_helpStyle && !events._released)) && (_bgFound != -1) && (_bgFound < 1000) && (scene._bgShapes[_bgFound]._defaultCommand || !scene._bgShapes[_bgFound]._description.empty())) {
						...
						if (_help > 5)
							_help = -1;
					} else if (pt.y < CONTROLS_Y && ((events._rightReleased && _helpStyle) || (events._released && !_helpStyle)) && (_bgFound != -1 && _bgFound < 1000) && (scene._bgShapes[_bgFound]._defaultCommand || !scene._bgShapes[_bgFound]._description.empty())) {
		...
		if (events._pressed || events._released || events._rightPressed || _keyPress || _pause) {
			if (((events._released && (_helpStyle || _help == -1)) || (events._rightReleased && !_helpStyle)) && (pt.y <= CONTROLS_Y) && (_menuMode == STD_MODE)) {
				...
				if (scene.checkForZones(pt, SCRIPT_ZONE) != 0 || scene.checkForZones(pt, NOWALK_ZONE) != 0) {
			...
			if ((!_windowOpen && !_menuCounter && pt.y > CONTROLS_Y) || _keyPress) {
	...
	void ScalpelUserInterface::clearInfo() {
		if (_infoFlag) {
			_vm->_screen->vgaBar(Common::Rect(IS_3DO ? 33 : 16, INFO_LINE,
			                                  SHERLOCK_SCREEN_WIDTH - (IS_3DO ? 33 : 19), INFO_LINE + 10),
			                     INFO_BLACK);
			_infoFlag = false;
			_oldLook = -1;
		}
		...
		if (mousePos.y < CONTROLS_Y && (temp = _bgFound) != -1) {
			if (temp != _oldLook) {
				...
				if (!tempStr.empty() && tempStr[0] != ' ') {
					...
					if ((_menuMode == INV_MODE || _menuMode == USE_MODE || _menuMode == GIVE_MODE) && (inv._invMode == INVMODE_USE || inv._invMode == INVMODE_GIVE)) {
						...
						} else if (temp >= 0 && temp < 1000 && _selector != -1 && scene._bgShapes[temp]._aType == PERSON) {
					...
					if (saves.promptForDescription(_selector)) {
						...
						if (!talk._talkToAbort) {
							screen._backBuffer1.fillRect(Common::Rect(6, CONTROLS_Y + 11 + (_selector - saves._savegameIndex) * 10,
							                                          SHERLOCK_SCREEN_WIDTH - 2, CONTROLS_Y + 20 + (_selector - saves._savegameIndex) * 10),
							                             INV_BACKGROUND);
				...
				if (_selector != -1) {
					...
					if (saves._envMode == SAVEMODE_LOAD) {
						...
						if (saves.promptForDescription(_selector)) {
							...
							if (!talk._talkToAbort) {
								screen._backBuffer1.fillRect(Common::Rect(6, CONTROLS_Y + 11 + (_selector - saves._savegameIndex) * 10,
								                                          317, CONTROLS_Y + 20 + (_selector - saves._savegameIndex) * 10),
								                             INV_BACKGROUND);
		...
		if (events._released || _keyboardInput) {
			if ((found == 0 && events._released) || _key == inv._hotkeyExit) {
				...
				if ((mousePos.y < CONTROLS_Y1) && (inv._invMode == INVMODE_LOOK) && (_find >= 0) && (_find < 1000)) {
					if (!scene._bgShapes[_find]._examine.empty() && scene._bgShapes[_find]._examine[0] >= ' ')
						inv.refreshInv();
	...
	void ScalpelUserInterface::doTalkControl() {
		...
		if (events._pressed || events._released) {
			...
			for (_selector = talk._talkIndex; _selector < (int)talk._statements.size() && !found; ++_selector) {
				if (mousePos.y > talk._statements[_selector]._talkPos.top && mousePos.y < talk._statements[_selector]._talkPos.bottom)
					found = true;
			}
			--_selector;
			if (!found)
