...
void ScalpelUserInterface::doEnvControl() {
	...
	if (events._released || _actionPress) {
		...
		} else if (((found == 3 && events._released) || _action == kActionScalpelFilesUp) && saves._savegameIndex) {
			bool moreActions;
			do {
				...
				if (events.actionHit()) {
					...
					moreActions = _action == kActionScalpelFilesUp;
				}
			} while ((saves._savegameIndex) && moreActions);
		} else if (((found == 4 && events._released) || _action == kActionScalpelFilesDown) && saves._savegameIndex < (MAX_SAVEGAME_SLOTS - ONSCREEN_FILES_COUNT)) {
			bool moreActions;
			do {
				saves._savegameIndex++;
				screen._backBuffer1.fillRect(Common::Rect(3, CONTROLS_Y + 11, SHERLOCK_SCREEN_WIDTH - 2,
				...
				if (events.actionHit()) {
					...
					moreActions = _action == kActionScalpelFilesDown;
				}
...
void ScalpelUserInterface::doTalkControl() {
	...
	if (_actionPress) {
		...
		if (_action == kActionScalpelTalkExit || _action == kActionScalpelTalkUp || _action == kActionScalpelTalkDown) {
			_actionInput = true;
		} else {
			_selector = -1;
		}
	...
	if (events._released || _keyboardInput || _actionInput) {
		if (((Common::Rect(99, CONTROLS_Y, 138, CONTROLS_Y + 10).contains(mousePos) && events._released)
			...
			_windowBounds.top = CONTROLS_Y1;
				...
				|| _action == kActionScalpelTalkUp) && talk._moreTalkUp) {
			...
			screen.slamRect(Common::Rect(5, CONTROLS_Y, SHERLOCK_SCREEN_WIDTH - 5, SHERLOCK_SCREEN_HEIGHT - 2));
				...
				|| _action == kActionScalpelTalkDown) && talk._moreTalkDown) {
			do {
				++talk._talkIndex;
