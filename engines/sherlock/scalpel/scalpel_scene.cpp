...
namespace Scalpel {
	...
	void ScalpelScene::drawAllShapes() {
		...
		for (int idx = 0; idx < MAX_CHARACTERS; ++idx) {
			...
			if (p._type == CHARACTER && p._walkLoaded) {
				bool flipped = IS_SERRATED_SCALPEL && (p._sequenceNumber == WALK_LEFT || p._sequenceNumber == STOP_LEFT || p._sequenceNumber == WALK_UPLEFT || p._sequenceNumber == STOP_UPLEFT || p._sequenceNumber == WALK_DOWNRIGHT || p._sequenceNumber == STOP_DOWNRIGHT);

				screen.getBackBuffer()->SHtransBlitFrom(*p._imageFrame, Common::Point(p._position.x / FIXED_INT_MULTIPLIER, p._position.y / FIXED_INT_MULTIPLIER - p.frameHeight()), flipped);
			}
		}

		// Draw all static and active shapes that are NORMAL and are in front of the player
		for (uint idx = 0; idx < _bgShapes.size(); ++idx) {
			if ((_bgShapes[idx]._type == ACTIVE_BG_SHAPE || _bgShapes[idx]._type == STATIC_BG_SHAPE) && _bgShapes[idx]._misc == NORMAL_FORWARD)
		...
		for (uint idx = 0; idx < _canimShapes.size(); ++idx) {
			if ((_canimShapes[idx]->_type == ACTIVE_BG_SHAPE || _canimShapes[idx]->_type == STATIC_BG_SHAPE) && _canimShapes[idx]->_misc == NORMAL_FORWARD)
				screen.getBackBuffer()->SHtransBlitFrom(*_canimShapes[idx]->_imageFrame, _canimShapes[idx]->_position,
				                                        _canimShapes[idx]->_flags & OBJ_FLIPPED);
		}
		...
		for (uint idx = 0; idx < _bgShapes.size(); ++idx) {
			...
			if ((_bgShapes[idx]._type == ACTIVE_BG_SHAPE || _bgShapes[idx]._type == STATIC_BG_SHAPE) && _bgShapes[idx]._misc == FORWARD)
				screen.getBackBuffer()->SHtransBlitFrom(*_bgShapes[idx]._imageFrame, _bgShapes[idx]._position,
				                                        _bgShapes[idx]._flags & OBJ_FLIPPED);
		}
		...
		for (uint idx = 0; idx < _canimShapes.size(); ++idx) {
			if ((_canimShapes[idx]->_type == ACTIVE_BG_SHAPE || _canimShapes[idx]->_type == STATIC_BG_SHAPE) && _canimShapes[idx]->_misc == FORWARD)
				screen.getBackBuffer()->SHtransBlitFrom(*_canimShapes[idx]->_imageFrame, _canimShapes[idx]->_position,
				                                        _canimShapes[idx]->_flags & OBJ_FLIPPED);
		}
	...
	void ScalpelScene::checkBgShapes() {
		...
		for (uint idx = 0; idx < _canimShapes.size(); ++idx) {
			...
			if (obj._type == STATIC_BG_SHAPE || obj._type == ACTIVE_BG_SHAPE) {
				if ((obj._flags & 5) == 1) {
					obj._misc = (pt.y < (obj._position.y + obj._imageFrame->_frame.h - 1)) ? NORMAL_FORWARD : NORMAL_BEHIND;
	...
	void ScalpelScene::doBgAnim() {
		...
		if (people[HOLMES]._type == CHARACTER && people[HOLMES]._walkLoaded && people._holmesOn) {
			...
			bool flipped = people[HOLMES]._sequenceNumber == WALK_LEFT || people[HOLMES]._sequenceNumber == STOP_LEFT || people[HOLMES]._sequenceNumber == WALK_UPLEFT || people[HOLMES]._sequenceNumber == STOP_UPLEFT || people[HOLMES]._sequenceNumber == WALK_DOWNRIGHT || people[HOLMES]._sequenceNumber == STOP_DOWNRIGHT;
	...
	int ScalpelScene::startCAnim(int cAnimNum, int playRate) {
		...
		if (cAnim._name.size() > 0 && cAnim._type != NO_SHAPE) {
			...
			if (!res.isInCache(fname)) {
				...
				Common::SeekableReadStream *imgStream = !_compressed ? roomStream->readStream(cAnim._dataSize) : Resources::decompressLZ(*roomStream, cAnim._dataSize);
