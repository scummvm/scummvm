...
#define JOURNAL_BAR_WIDTH 450
	...
	TattooJournal::TattooJournal(SherlockEngine *vm)
	  : Journal(vm) {
	...
	void TattooJournal::drawScrollBar() {
		...
		if (numPages <= 1) {
			...
			barX = (r.width() - BUTTON_SIZE * 2 - 6 - barWidth) * FIXED_INT_MULTIPLIER / (numPages - 1) * (_page - 1) / FIXED_INT_MULTIPLIER + r.left + 3 + BUTTON_SIZE;
