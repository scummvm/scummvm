...
#define FILES_LINES_COUNT 5
	...
	void WidgetFiles::show(SaveMode mode) {
		...
		if (_vm->_showOriginalSavesDialog) {
			...
			_bounds = Common::Rect(SHERLOCK_SCREEN_WIDTH * 2 / 3, (_surface.fontHeight() + 1) * (FILES_LINES_COUNT + 1) + 17);
	...
	void WidgetFiles::handleEvents() {
		...
		if (Common::Rect(_bounds.left, _bounds.top + _surface.fontHeight() + 14,
		      ...
		      .contains(mousePos)) {
			_selector = (mousePos.y - _bounds.top - _surface.fontHeight() - 14) / (_surface.fontHeight() + 1) + _savegameIndex;
		} else {
			_selector = -1;
		}
