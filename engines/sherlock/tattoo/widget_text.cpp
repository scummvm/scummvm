...
namespace Tattoo {
	...
	WidgetText::WidgetText(SherlockEngine *vm)
	  : WidgetBase(vm) {
	}
	...
	WidgetMessage::WidgetMessage(SherlockEngine *vm)
	  : WidgetBase(vm) {
		_menuCounter = 0;
	}
	...
	void WidgetMessage::handleEvents() {
		...
		if (events._pressed || events._released || events._rightPressed || events._rightReleased || ui._keyState.keycode || !_menuCounter) {
			// Close the window
			banishWindow();
