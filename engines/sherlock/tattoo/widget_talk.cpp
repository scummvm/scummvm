...
void WidgetTalk::handleEvents() {
	...
	if (events._released || events._rightReleased || action == kActionTattooTalkExit || hotkey) {
		events.clearEvents();
		_dialogTimer = 0;
		...
		if ((_outsideMenu && !_bounds.contains(mousePos)) || action == kActionTattooTalkExit) {
			if (action == kActionTattooTalkExit)
				_selector = -1;
