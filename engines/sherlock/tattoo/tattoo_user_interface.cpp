...
namespace Tattoo {
	...
	TattooUserInterface::TattooUserInterface(SherlockEngine *vm)
	  : UserInterface(vm)
	  , _inventoryWidget(vm)
	  , _messageWidget(vm)
	  , _textWidget(vm)
		...
		if (_mask != nullptr) {
			...
			for (int idx = 0; idx < PALETTE_COUNT; ++idx) {
				...
				for (int pal = 0; pal < PALETTE_COUNT; ++pal) {
					int d = (r - cMap[pal * 3]) * (r - cMap[pal * 3]) + (g - cMap[pal * 3 + 1]) * (g - cMap[pal * 3 + 1]) + (b - cMap[pal * 3 + 2]) * (b - cMap[pal * 3 + 2]);
	...
	void TattooUserInterface::doBgAnimEraseBackground() {
		...
		if (_mask != nullptr) {
			...
			for (uint idx = 0; idx < scene._bgShapes.size(); ++idx) {
				...
				if ((obj._type == ACTIVE_BG_SHAPE && (obj._maxFrames > 1 || obj._delta.x != 0 || obj._delta.y != 0)) || obj._type == HIDE_SHAPE || obj._type == REMOVE)
		...
		if ((people[people._walkControl]._position.x / FIXED_INT_MULTIPLIER - screen._currentScroll.x) < (SHERLOCK_SCREEN_WIDTH / 8) && people[people._walkControl]._delta.x < 0) {

			_targetScroll.x = (short)(people[people._walkControl]._position.x / FIXED_INT_MULTIPLIER - SHERLOCK_SCREEN_WIDTH / 8 - 250);
			if (_targetScroll.x < 0)
				_targetScroll.x = 0;
		}

		if ((people[people._walkControl]._position.x / FIXED_INT_MULTIPLIER - screen._currentScroll.x) > (SHERLOCK_SCREEN_WIDTH / 4 * 3) && people[people._walkControl]._delta.x > 0)
			_targetScroll.x = (short)(people[people._walkControl]._position.x / FIXED_INT_MULTIPLIER - SHERLOCK_SCREEN_WIDTH / 4 * 3 + 250);
