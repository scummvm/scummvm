...
#define NUM_INV_PER_LINE 4 // Number of inentory items per line in the dialog
	...
	void WidgetInventoryVerbs::load() {
		...
		for (uint idx = 0; idx < scene._bgShapes.size(); ++idx) {
			...
			if (obj._type != INVALID && obj._type != HIDDEN) {
				for (int useNum = 0; useNum < 6; ++useNum) {
					if (!obj._use[useNum]._verb.hasPrefix("*") && !obj._use[useNum]._target.compareToIgnoreCase(inv[_owner->_invSelect]._name)) {
		...
		for (int idx = 1; idx < MAX_CHARACTERS; ++idx) {
			for (int useNum = 0; useNum < 2; ++useNum) {
				if (!people[idx]._use[useNum]._target.compareToIgnoreCase(inv[_owner->_invSelect]._name) && !people[idx]._use[useNum]._verb.empty() && !people[idx]._use[useNum]._verb.hasPrefix(" ")) {
					bool found1 = false;
					for (uint cmdNum = 0; cmdNum < _inventCommands.size() && !found1; ++cmdNum) {
	...
	void WidgetInventoryVerbs::handleEvents() {
		...
		if (events._released || events._rightReleased || ui._keyState.keycode == Common::KEYCODE_ESCAPE) {
			...
			if (_outsideMenu || ui._keyState.keycode == Common::KEYCODE_ESCAPE) {
				...
				bool doFoolscap = !inv[_owner->_invSelect]._name.compareToIgnoreCase(FIXED(Inv6)) && !_inventCommands[_invVerbSelect].compareToIgnoreCase(FIXED(Solve));
				...
				if (doFoolscap) {
					...
					if (!_inventCommands[_invVerbSelect].compareToIgnoreCase(inv[_owner->_invSelect]._verb._verb) || !inv[_owner->_invSelect]._verb._target.compareToIgnoreCase("*SELF")) {
						inv.freeInv();
	...
	WidgetInventory::WidgetInventory(SherlockEngine *vm)
	  : WidgetBase(vm)
	  , _tooltipWidget(vm, this)
	  , _verbList(vm, this) {
		_invMode = 0;
		_invVerbMode = 0;
	...
	void WidgetInventory::drawInventory() {
		...
		for (int idx = 0, itemId = inv._invIndex; idx < NUM_INVENTORY_SHOWN; ++idx, ++itemId) {
			...
			if (itemId < inv._holdings) {
				...
				_surface.SHtransBlitFrom(img, Common::Point(pt.x + (INVENTORY_XSIZE - img._width) / 2, pt.y + (INVENTORY_YSIZE - img._height) / 2));
			}
		}
					...
					if (ui._bgFound != -1) {
						if (ui._personFound) {
							...
							for (int idx = 0; idx < 2; ++idx) {
								if (!person._use[idx]._verb.compareToIgnoreCase(_verb) && !person._use[idx]._target.compareToIgnoreCase(_invTarget)) {
							...
							for (int idx = 0; idx < 6; ++idx) {
								if (!ui._bgShape->_use[idx]._verb.compareToIgnoreCase(_verb) && !ui._bgShape->_use[idx]._target.compareToIgnoreCase(_invTarget)) {
