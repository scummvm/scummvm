...
#define CHARACTERS_INDEX 256
	...
	SavedNPCPath::SavedNPCPath(byte path[MAX_NPC_PATH], int npcIndex, int npcPause, const Point32 &position,
	  ...
	  , _npcPause(npcPause)
	  , _position(position)
	  , _npcFacing(npcFacing)
	  , _lookHolmes(lookHolmes) {
	...
	void TattooPerson::adjustSprite() {
		...
		if (!_walkCount && ui._exitZone != -1 && scene._walkedInScene && scene._goToScene == -1 && !_description.compareToIgnoreCase(people[HOLMES]._description)) {
	...
	void TattooPerson::walkToCoords(const Point32 &destPos, int destDir) {
		...
		if (ABS(_walkDest.x - (_position.x / FIXED_INT_MULTIPLIER)) > 8 || ABS(_walkDest.y - (_position.y / FIXED_INT_MULTIPLIER)) > 4) {
			goAllTheWay();
	...
	Common::Point TattooPerson::getSourcePoint() const {
		...
		return Common::Point(_position.x / FIXED_INT_MULTIPLIER + (_imageFrame ? _imageFrame->sDrawXSize(scaleVal) / 2 : 0), _position.y / FIXED_INT_MULTIPLIER);
	}
	...
	TattooPeople::TattooPeople(SherlockEngine *vm)
	  : People(vm) {
