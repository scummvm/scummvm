...
byte *Sword2Engine::fetchPsxBackground(uint32 location) {
	...
	file.seek(location * 4, Common::kSeekSet);
	...
	file.seek(screenOffset + ResHeader::size(), Common::kSeekSet);
	...
	file.seek(screenOffset + header.bgOffset + 4, Common::kSeekSet);
	dataOffset = file.readUint32LE();

	file.seek(screenOffset + header.bgOffset, Common::kSeekSet);
...
byte *Sword2Engine::fetchPsxParallax(uint32 location, uint8 level) {
	...
	file.seek(screenOffset + plxOffset, Common::kSeekSet);
	buffer = (byte *)malloc(totSize);
