...
public:
	...
	CTimerMsg(uint ticks, uint timerCtr, int actionVal, const CString &action) : CTimeMsg(ticks), _timerCtr(timerCtr), _actionVal(actionVal), _action(action) {}
...
public:
	...
	CMovementMsg(Common::KeyCode key) : _movement(getMovement(key)) {}
