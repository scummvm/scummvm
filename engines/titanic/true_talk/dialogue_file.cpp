...
DialogueResource *CDialogueFile::addToCache(int index) {
	if (_index.size() == 0 || index < 0 || index >= (int)_index.size() || _cache.empty())
		return nullptr;
...
bool CDialogueFile::read(DialogueResource *cacheEntry, byte *buffer, size_t bytesToRead) {
	...
	if (!cacheEntry || !cacheEntry->_active || !bytesToRead || (cacheEntry->_bytesRead + bytesToRead) > cacheEntry->_size)
		return false;
