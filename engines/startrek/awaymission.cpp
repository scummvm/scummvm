...
void StarTrekEngine::awayMissionSelectAction(bool openActionMenu) {
	...
	if (_awayMission.activeAction == ACTION_USE && _awayMission.activeObject == OBJECT_ICOMM && (_awayMission.crewDownBitset & (1 << OBJECT_KIRK)) == 0) {
		if (!walkActiveObjectToHotspot()) {
...
void StarTrekEngine::awayMissionUseObject(int16 clickedObject) {
	...
	if (!tryWalkToHotspot) {
		if ((activeIsCrewman && passiveIsCrewman) || (activeIsCrewman && passiveIsItem) || (activeIsItem && passiveIsItem)) {
	...
	case ACTION_USE:
		if (action.activeObject() != action.passiveObject()) {
			...
			case OBJECT_KIRK:
				...
				if (!(!_awayMission.disableWalking && _room->handleAction(ACTION_WALK, action.passiveObject(), 0, 0)) && !_room->handleAction(ACTION_GET, action.passiveObject(), 0, 0)) {
			...
			case OBJECT_MCCOY:
				if (!_room->handleAction(ACTION_USE, OBJECT_IMEDKIT, action.passiveObject(), 0) && !_room->handleAction(ACTION_USE, OBJECT_IMTRICOR, action.passiveObject(), 0)) {
