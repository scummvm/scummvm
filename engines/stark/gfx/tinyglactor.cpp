...
void TinyGLActorRenderer::render(const Math::Vector3d &position, float direction, const LightEntryArray &lights) {
	...
	for (Common::Array<Face *>::const_iterator face = faces.begin(); face != faces.end(); ++face) {
		...
		for (uint32 i = 0; i < numVertexIndices; i += 3) {
			uint32 index1 = vertexIndices[i + 0];
			uint32 index2 = vertexIndices[i + 1];
			uint32 index3 = vertexIndices[i + 2];
			auto vertex1 = _faceVBO[index1];
			auto vertex2 = _faceVBO[index2];
			auto vertex3 = _faceVBO[index3];

			tglBegin(TGL_TRIANGLES);
				tglColor3f(vertex1.r, vertex1.g, vertex1.b);
				if (tex) {
					tglTexCoord2f(vertex1.texS, vertex1.texT);
				}
				tglNormal3f(vertex1.nx, vertex1.ny, vertex1.nz);
				tglVertex3f(vertex1.x, vertex1.y, vertex1.z);

				tglColor3f(vertex2.r, vertex2.g, vertex2.b);
				if (tex) {
					tglTexCoord2f(vertex2.texS, vertex2.texT);
				}
				tglNormal3f(vertex2.nx, vertex2.ny, vertex2.nz);
				tglVertex3f(vertex2.x, vertex2.y, vertex2.z);

				tglColor3f(vertex3.r, vertex3.g, vertex3.b);
				if (tex) {
					tglTexCoord2f(vertex3.texS, vertex3.texT);
				}
				tglNormal3f(vertex3.nx, vertex3.ny, vertex3.nz);
				tglVertex3f(vertex3.x, vertex3.y, vertex3.z);
			tglEnd();
		}
	}
}
