...
void read_basic_palette(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		...
		errorExit("read_basic_palette: Unknown language");
...
void read_dialog_data(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		segmentStart = 0x1ddd0;
	else if (language == FR_FRA)
		segmentStart = 0x1e1a0;
	else if (language == DE_DEU)
		segmentStart = 0x1e230;
	else if (language == ES_ESP)
		segmentStart = 0x1de60;
	else if (language != EN_ANY)
		errorExit("read_dialog_data: Unknown language");
...
void read_talk_dialog_data(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		segmentStart = 0x1df20;
	else if (language == FR_FRA)
		segmentStart = 0x1e2f0;
	else if (language == DE_DEU)
		segmentStart = 0x1e380;
	else if (language == ES_ESP)
		segmentStart = 0x1dfb0;
	else if (language != EN_ANY)
		errorExit("read_talk_dialog_data: Unknown language");
...
void read_room_data(byte *&data, uint16 &totalSize) {
	...
	for (int index = 0; index < ROOM_NUM_ENTRIES; ++index) {
		...
		if ((FROM_LE_16(headerEntry.offset) != 0) && (FROM_LE_16(headerEntry.offset) != 0xffff) && (FROM_LE_16(headerEntry.roomNumber) != 0)) {
			...
			*offsetPtr++ = TO_LE_16(offset);
			...
			rec->clippingXEnd = (FROM_LE_16(buffer.clippingXEnd) == 0) ? 0 : TO_LE_16(FROM_LE_16(buffer.clippingXEnd) - 0x80);
			...
			lureExe.seek(dataSegment + walkAreaOffset + buffer.walkBoundsIndex * sizeof(RoomRectIn));
	...
	for (int tableNum = 0; tableNum < 4; ++tableNum) {
		...
		for (;;) {
			...
			if (r->scriptLoadFlag || (tableNum == 3))
				...
				int loadIndex = 0;
				while ((loadOffsets[loadIndex] != FROM_LE_16(entry.loadOffset)) && (loadOffsets[loadIndex] != 0))
					++loadIndex;
			...
			if (tableNum == 3) {
				...
				int procIndex = 0;
				while ((procList[procIndex] != FROM_LE_16(entry.tickProcOffset)) && (procList[procIndex] != 0xffff))
					++procIndex;

				if (procList[procIndex] == 0xffff) {
					if ((FROM_LE_16(entry.tickProcOffset) != 0xe00) && (FROM_LE_16(entry.tickProcOffset) != 2))
...
void read_room_exits(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		...
		errorExit("read_room_exits: Unknown language");
	...
	for (roomCtr = 0; roomCtr < NUM_ROOM_EXITS; ++roomCtr) {
		if (offsets[roomCtr] == 0) {
			...
			*offset++ = TO_LE_16(destIndex);
			...
			RoomExitHotspotOutputRecord *destP = (RoomExitHotspotOutputRecord *)(data + destIndex);
...
void read_room_exit_joins(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		...
		errorExit("read_room_exit_joins: Unknown language");
...
void read_anim_data(byte *&data, uint16 &totalSize) {
	...
	for (ctr = 0; ctr < animIndex; ++ctr) {
		...
		if ((FROM_LE_16(inRec.leftOffset) < 0x5000) || (FROM_LE_16(inRec.rightOffset) < 0x5000) || (abs(FROM_LE_16(inRec.leftOffset) - FROM_LE_16(inRec.rightOffset)) > 0x800) || (abs(FROM_LE_16(inRec.rightOffset) - FROM_LE_16(inRec.upOffset)) > 0x800) || (abs(FROM_LE_16(inRec.upOffset) - FROM_LE_16(inRec.downOffset)) > 0x800)) {
...
void read_script_data(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		segmentOffset = 0x1e020;
	else if (language == FR_FRA)
		segmentOffset = 0x1e3f0;
	else if (language == DE_DEU)
		segmentOffset = 0x1e480;
	else if (language == ES_ESP)
		segmentOffset = 0x1e0b0;
	else if (language != EN_ANY)
		errorExit("read_script_data: Unknown language");
...
void read_script2_data(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		segmentOffset = 0x19D90;
	else if (language == FR_FRA)
		segmentOffset = 0x1a160;
	else if (language == DE_DEU)
		segmentOffset = 0x1a1f0;
	else if (language == ES_ESP)
		segmentOffset = 0x19e20;
	else if (language != EN_ANY)
		errorExit("read_script2_data: Unknown language");
	lureExe.seek(segmentOffset);
...
void read_hotspot_script_offsets(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		...
		errorExit("read_hotspot_script_offsets: Unknown language");
...
void read_messages_segment(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		segmentOffset = 0x20c80;
	else if (language == FR_FRA)
		segmentOffset = 0x21050;
	else if (language == DE_DEU)
		segmentOffset = 0x210E0;
	else if (language == ES_ESP)
		segmentOffset = 0x20d10;
	else if (language != EN_ANY)
		errorExit("read_messages_segment: Unknown language");
...
void read_talk_headers(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		...
		errorExit("read_talk_headers: Unknown language");
	...
	for (entryCtr = 0; entryCtr < TALK_NUM_ENTRIES; ++entryCtr) {
		...
		for (subentryCtr = 0; subentryCtr < TALK_NUM_ENTRIES; ++subentryCtr) {
			if ((FROM_LE_16(entries[subentryCtr].offset) < currVal) && (FROM_LE_16(entries[subentryCtr].offset) > prevVal))
...
void read_talk_data(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		giveTalkIds = &italianGiveTalkIds[0];
	else if (language == FR_FRA)
		giveTalkIds = &frenchGiveTalkIds[0];
	else if (language == DE_DEU)
		giveTalkIds = &germanGiveTalkIds[0];
	else if (language == ES_ESP)
		giveTalkIds = &spanishGiveTalkIds[0];
	else if (language != EN_ANY)
		errorExit("read_talk_data: Unknown language");
	...
	for (talkCtr = 0; talkCtr < talkOffsetIndex; ++talkCtr) {
		...
		for (subentryCtr = 0; subentryCtr < talkOffsetIndex; ++subentryCtr) {
			if ((talkOffsets[subentryCtr] < currVal) && (talkOffsets[subentryCtr] > prevVal))
	...
	for (talkCtr = 0; talkCtr < numTalks; ++talkCtr) {
		...
		for (subentryCtr = 0; subentryCtr < numTalks; ++subentryCtr) {
			if ((responseOffset >= sortedList[subentryCtr]) && (responseOffset < sortedList[subentryCtr + 1])) {
				// Found a record
...
void read_room_pathfinding_data(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		...
		errorExit("read_room_pathfinding_data: Unknown language");
	lureExe.seek(dataSegment + dataStart);
...
void read_room_exit_hotspots_data(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		...
		errorExit("read_room_exit_hotspots_data: Unknown language");
	lureExe.seek(dataSegment + dataStart);
...
void save_fight_segment(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		fightSegment = 0x1c520;
	else if (language == FR_FRA)
		fightSegment = 0x1c8f0;
	else if (language == DE_DEU)
		fightSegment = 0x1c980;
	else if (language == ES_ESP)
		fightSegment = 0x1c5b0;
	else if (language != EN_ANY)
		errorExit("save_fight_segment: Unknown language");
...
const char *spanishTextStrings[NUM_TEXT_ENTRIES] = {
	...
	"Dale", "Habla con", "Dile a", "Compra", "Obsevar", "Observe", "Mirar por", "P\x97"
	                                                                            "dele",
	NULL,
	"Bebe", "Estado", "Ve a", "Regresa", "Sobornar", "Examinar",
	"Cr\x7f"
	"ditos",
	"Reiniciar", "Salvar juego", "Recuperar jue", "Abandonar", "Texto r\x98pido\x8b",
	"Texto lento \x8b", "Sonido activado   ", "Sonido desactivado ", "(nada)", " con ", " a ", " con ",
	"y luego", "eso es todo", "\x94"
	                          "Est\x98s seguro? (S/N)",
...
void save_text_strings(byte *&data, uint16 &totalSize) {
	...
	if (language == IT_ITA)
		textStrings = &italianTextStrings[0];
	else if (language == FR_FRA)
		textStrings = &frenchTextStrings[0];
	else if (language == DE_DEU)
		textStrings = &germanTextStrings[0];
	else if (language == ES_ESP)
		textStrings = &spanishTextStrings[0];
	else if (language != EN_ANY)
