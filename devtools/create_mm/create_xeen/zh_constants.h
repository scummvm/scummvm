...
public:
	...
	const char **RACE_NAMES() {
		static const char *_raceNames[] = { "\xa4\x48\xc3\xfe" /* 人類 */, "\xba\xeb\xc6\x46" /* 精靈 */, "\xb8\x47\xa4\x48"  /*  矮人 */,
						    "\xa6\x61\xba\xeb" /* 地精 */, "\xa5\x62\xc3\x7e\xa4\x48" /*  半獸人 */ };
		return _raceNames;
	}
	...
	const char **CONDITION_NAMES_M() {
		static const char *_conditionNamesM[] = {
			"Cursed", "Heart Broken", "Weak", "Poisoned", "Diseased",
				...
				"Paralyzed", "Unconscious", "Dead", "Stone", "Eradicated", "\xa5\xbf\xb1\x60" /* 正常 */
		};
	...
	const char *CHARACTER_DETAILS() {
		return "\x3l\f03\v039\t201%s\x0c""09\t003\v000%s : %s %s %s"
			   "\x3r\t053\v028\f%02u%u\fd\t103\f%02u%u\fd"
			   "\x3l\t145\f%02u%d\fd\t206\f15%u\fd\x3r"
			   "\t053\v051\f%02u%u\fd\t103\f%02u%u\fd"
			   "\x3l\t145\f%02u%u\fd\t206\f15%u\fd"
			   "\x3r\t053\v074\f%02u%u\fd\t103\f%02u%u\fd"
			   "\x3l\t145\f15%u\fd\t206\f15%u\fd"
			   "\x3r\t053\v097\f%02u%u\fd\t103\f%02u%u\fd"
			   "\x3l\t145\f15%u\fd\v085\t235\f15%u\xa4\xd1%s\fd"
			   "\x3r\t053\v120\f%02u%u\fd\t103\f%02u%u\fd"
			   "\x3l\t145\f15%u\fd\v108\t234\f%02u%s\fd"
			   "\t230%s%s%s%s\fd";
	}
	...
	const char *PARTY_GOLD() {
		return "\xaa\xf7\xb9\xf4\xc1\x60\xbc\xc6"; /* 金幣總數 */
	}
	...
	const char *CHARACTER_TEMPLATE() {
		return "\x1\f03\r\x3l"
			"\t027\v016\xa4\x4f\xb6\x71\t027\v039\xb4\xbc\xbc\x7a\t027\v062\xa4\x48\xae\xe6\t027\v085\xad\x40\xa4\x4f\t027\v108\xb3\x74\xab\xd7"
			"\t085\v016\xb7\xc7\xbd\x54\t085\v039\xb9\x42\xae\xf0\t085\v062\xa6\x7e\xc4\xd6\t085\v085\xb5\xa5\xaf\xc5\t085\v108\xa8\xbe\xc5\x40"
			"\t143\v016\xa5\xcd\xa9\x52\t143\v039\xaa\x6b\xa4\x4f\t143\v062\xa7\xdc\xa4\x4f\t143\v085\xa7\xde\xaf\xe0\t143\v108\xb8\x67\xbe\xfa"

			"\t201\v016\xb8\x67\xc5\xe7\xc2\x49\xbc\xc6"
			"\t201\v062\xc4\x5f\xa5\xdb\xc1\x60\xbc\xc6"
			"\t201\v085\xad\xb9\xaa\xab"
			"\t201\v108\xaa\xac\xaa\x70"
			"\f04\t271\v020\xaa\xab\xab\x7e"
			"\t271\v054\xa7\xd6\xc0\xcb"
			"\t271\v088\xa5\xe6\xb4\xab"
			"\t287\v102\xc2\xf7\t287\v115\xb6\x7d"
			"%s";
	}
	...
	class ZH_KeyConstants : public KeyConstants {
	public:
		class ZH_DialogsCharInfo : public DialogsCharInfo {
		public:
			int KEY_ITEM()     { return Common::KEYCODE_i; }
			int KEY_QUICK()    { return Common::KEYCODE_q; }
			int KEY_EXCHANGE() { return Common::KEYCODE_e; }
		};
		ZH_DialogsCharInfo *dialogsCharInfo() {
			if (!_dci)_dci = new ZH_DialogsCharInfo();
			return _dci;
		}

		class ZH_DialogsControlPanel : public DialogsControlPanel {
		public:
		...
		ZH_DialogsControlPanel *dialogsControlPanel() {
			if (!_dcp) _dcp = new ZH_DialogsControlPanel();
			return _dcp;
		}

		class ZH_DialogsCreateChar : public DialogsCreateChar {
		public:
		...
		ZH_DialogsCreateChar *dialogsCreateChar() {
			if (!_dcc) _dcc = new ZH_DialogsCreateChar();
			return _dcc;
		}

		class ZH_DialogsDifficulty : public DialogsDifficulty {
		public:
			int KEY_ADVENTURER() { return Common::KEYCODE_a; }
			int KEY_WARRIOR()    { return Common::KEYCODE_w; }
		};
		ZH_DialogsDifficulty *dialogsDifficulty() {
			if (!_dd) _dd = new ZH_DialogsDifficulty();
			return _dd;
		}

		class ZH_DialogsItems : public DialogsItems {
		public:
		...
		ZH_DialogsItems *dialogsItems() {
			if (!_di) _di = new ZH_DialogsItems();
			return _di;
		}

		class ZH_DialogsParty : public DialogsParty {
		public:
		...
		ZH_DialogsParty *dialogsParty() {
			if (!_dp) _dp = new ZH_DialogsParty();
			return _dp;
		}

		class ZH_DialogsQuests : public DialogsQuests {
		public:
		...
		ZH_DialogsQuests *dialogsQuests() {
			if (!_dq) _dq = new ZH_DialogsQuests();
			return _dq;
		}

		class ZH_DialogsQuickFight : public DialogsQuickFight {
		public:
			int KEY_NEXT() { return Common::KEYCODE_t; }
		};
		ZH_DialogsQuickFight *dialogsQuickFight() {
			if (!_dqf) _dqf = new ZH_DialogsQuickFight();
			return _dqf;
		}

		class ZH_DialogsSpells : public DialogsSpells {
		public:
		...
		ZH_DialogsSpells *dialogsSpells() {
			if (!_ds) _ds = new ZH_DialogsSpells();
			return _ds;
		}

		class ZH_Locations : public Locations {
		public:
		...
		ZH_Locations *locations() {
			if (!_l) _l = new ZH_Locations();
			return _l;
		}

		class ZH_CloudsOfXeenMenu : public CloudsOfXeenMenu {
		public:
		...
		ZH_CloudsOfXeenMenu *cloudsOfXeenMenu() {
			if (!_soxm)
				_soxm = new ZH_CloudsOfXeenMenu();
			return _soxm;
		}

	private:
		ZH_DialogsCharInfo     *_dci = NULL;
		ZH_DialogsControlPanel *_dcp = NULL;
		ZH_DialogsCreateChar   *_dcc = NULL;
		ZH_DialogsDifficulty   *_dd = NULL;
		ZH_DialogsItems        *_di = NULL;
		ZH_DialogsParty        *_dp = NULL;
		ZH_DialogsQuests       *_dq = NULL;
		ZH_DialogsQuickFight   *_dqf = NULL;
		ZH_DialogsSpells       *_ds = NULL;
		ZH_Locations           *_l = NULL;
		ZH_CloudsOfXeenMenu    *_soxm = NULL;
	};

	ZH_KeyConstants *keyConstants() {
		if (!_kc)_kc = new ZH_KeyConstants();
		return _kc;
	}

private:
	ZH_KeyConstants *_kc = NULL;
};
