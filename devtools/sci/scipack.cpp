...
test_file(char *filename) {
	...
	*dot = 0;
	...
	for (res_type = 0; res_type < sci_invalid_resource && scumm_stricmp(filename, sci_resource_types[res_type]); res_type++)
		;
...
int build_file_ids(int count, char **names) {
	int i;
	int error = 0;
...
write_uint16(int fd, unsigned int uint) {
	...
	if ((write(fd, &upper, 1) < 1) || (write(fd, &lower, 1) < 1)) {
		perror("While writing");
		exit(1);
	}
}

int write_files(int count, char **names) {
