...
# - The AUTHORS file
# - The gui/themes/credits.dat file
...
if ($#ARGV >= 0) {
	...
	$mode = "CPP" if ($ARGV[0] eq "--cpp");		# credits.dat (for use by about.cpp)
...
sub html_entities_to_cpp {
	...
	$text =~ s/&aacute;/\x{E1}/g;
	$text =~ s/&eacute;/\x{E9}/g;
	$text =~ s/&igrave;/\x{EC}/g;
	$text =~ s/&oacute;/\x{F3}/g;
	$text =~ s/&oslash;/\x{F8}/g;
	$text =~ s/&#322;/l/g;
	$text =~ s/&#347;/s/g;
	$text =~ s/&Scaron;/S/g;
	$text =~ s/&aring;/\x{E5}/g;
