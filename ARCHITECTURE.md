...
- The adjacent `0x383d0`-`0x384e0` range is now confirmed to be another overlapping run of tiny read/write dialogue-state wrappers over globals in the same contiguous block (`0xd2f00` through `0xd2f28`).
  ...
  - The native item/event branches are wider than the earlier engine stubs:
    ...
    - `DINER_BURNED` is `0x42b5`, `0x42b9`, response menu `0x2af`, optional `0x42c5` or `0x42cb`, then the common `0x42d2` tail.
    - `BARBER_POLE` is `0x426f`, `0x4274`, response menu `0x2b1`, optional `0x4281` or `0x4289`, then the common `0x428d` tail while also setting `ASSIGNED_DNA_TASK`.
