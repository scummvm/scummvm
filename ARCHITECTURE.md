...
- `handle_talk_to_mrs_potts @ 0x2ee70` is now bounded enough to rebuild from native flow.
  ...
  - The non-dead event followups are one-shot gates rather than terminal barks: `DAY_5` -> `0x2889`, `BURNED_TV_STATION` -> `0x2891`, `KARIN_KIDNAPED` -> `0x28dc` plus response menu `0x235`, and `DINER_BURNED` -> `0x290e`. After those gates, the keyword loop matches `0x237` through `0x24a`, with the `0x247` / `0x248` Lodge topics explicitly setting `get_set_discussed_lodge_topic`.
