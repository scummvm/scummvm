...
- The shared dialogue evidence/topic state shims behind those handlers are now partially explicit:
  - `get_set_shared_dialogue_state_d2e98` at `0x38230` is a neutral wrapper over `g_shared_dialogue_state_d2e98`; the only currently recovered gameplay reader is Herrill's `GASCAN` branch, where a zero value collapses the post-`0x30d8` continuation to `PC 0x3123`.
  ...
  - `get_set_shared_dialogue_state_d2eb0` at `0x382a0` is a neutral wrapper over `g_shared_dialogue_state_d2eb0`; Herrill's `GASCAN` response-1 branch uses it to decide whether `0x30eb` is inserted after `0x30e7`.
  ...
  - `get_set_shared_dialogue_state_d2eec` at `0x38380` is a neutral wrapper over `g_shared_dialogue_state_d2eec`; Herrill's `HAVE_LODGE_APP` continuation uses it to decide whether `0x3104` is inserted before `0x3109`.
