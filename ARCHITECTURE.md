...
- `run_load_game_menu` at `0x64910` is the load-game UI.
  ...
  - Load restore differs from ordinary room transitions for timers: `run_load_game_menu` calls `destroy_entity_list(&g_render_entity_list)`, then `free_loaded_world_data`; while freeing old `TimerRecord` nodes, `free_loaded_world_data` removes any preserved live global timer entity with the same name. The loaded slot then restores each `TIMER` record by name and only `room_setup` re-materializes timers whose `TimerRecord.arg1` matches the loaded room.
- `run_save_game_menu` at `0x632c0` is the matching save-game UI.
  ...
  - The remainder of the save blob persists the player combat-avatar state, record lists (`ANIM`, `FLAG`, `MONSTER`, `NPC`, `OBJECT`, `REGION`, `TIMER`), the talk-state blocks, ammo counters, and `g_current_music_path`; the timer portion writes each `timer_name`, `enabled`, and `current_value`.
...
- `load_dialogue_response_line` at `0x3a1a0` is the indexed `dialog.rsp` line loader used by the talk-to handlers.
  ...
  - `handle_talk_to_herrill @ 0x2dc00` is now bounded enough for a direct startup-room port.
    ...
    - The Inner Sanctum post-Muckey path has one native-only actor-state behavior outside the parsed room script: `HARVEST.SCR` skips the commented `{COMMAND "KILL_HERR2" ...}` line, but `update_actor_runtime_state` contains a hard-coded `HERRILL_LOG` removal path while Muckey combat state is active. ScummVM queues Herrill's live NPC death when `MUCKEY` completes death and triggers `ACTV_INNERSAN_DOOR`, giving the script's `KILL_HERMUCK_T` delay time to play the death bank before the `C002A`/`C002B` flic chain.
  ...
  - `handle_talk_to_moynahan @ 0x24a20` is now bounded enough for a direct startup-room port.
    ...
    - The item path has three real response-menu branches: `CASKET_PHOTO` / `CASKET_PHOTOCOPY` uses zero-based `dialog.rsp` lines `0x18c` and `0x18d`; the accepted trade calls `set_object_visibility_for_owner_or_room(..., visible=0)` for the presented photo and for `GLUE`, then calls `add_object_to_inventory(GLUE)` and sets `JUST_GOT_GLUE`; `LEDGER` / `LEDGER2` with `HAVE_BOTH_LEDGERS` uses zero-based `dialog.rsp` line `0x18e`; and note/checkbook evidence uses zero-based `dialog.rsp` line `0x190`. `PHOTO_OF_WHALEY_HERRILL`, `WED_MATCHES`, and the fallback item path stay linear.
  ...
  - `handle_talk_to_wasp_woman @ 0x2fde0` is now bounded enough for a direct startup-room port.
    ...
    - The hidden loop also clarifies the ambiguous BYE handling. The native code checks the selected topic against zero-based `dialog.rsp[0x301]` before each keyword dispatch, so the synthetic `BYE` selection exits silently even after the live keyword buffer has advanced to later states such as `0x30b`. The later zero-based `dialog.rsp[0x30e]` compare jumps back to the top of the loop; with the shipped `DIALOG.RSP`, both `0x301` and `0x30e` are the literal `BYE`, so the top-of-loop `0x301` compare exits before the menu is redrawn. Only an unmatched late typed topic falls through to `0x4c9a`.
...
- `update_actor_runtime_state` at `0x4d750` is the shared per-frame actor state machine for live actor entities.
  ...
  - For player resource-backed loadouts `2`, `3`, `4`, `5`, and `0xe`, the player-attack branch calls `consume_weapon_ammo_and_play_fire_or_empty_sound` on the zero-ammo and fire-commit checkpoints. That helper decrements the corresponding HUD/resource count when ammo is present, falls back to the empty-click sample when the count is already `0`, and the empty-ammo path returns before the later contact-resolution branch.
  ...
  - Empty-floor click targets are also component-based rather than diagonal:
    ...
    - when a pending horizontal click component is blocked, the state machine pushes a perpendicular Z waypoint around the blocking entity using `blocker.z - actor.z_extent - 1` or `blocker.z + blocker.z_extent + 1`, clears the pending X component, and records the blocker in the matching directional history slot
    - when a pending vertical/depth component is blocked, it pushes a perpendicular X waypoint using `blocker.left - actor.frame_width - 0x28` or `blocker.right + 0x28`, clears the pending Z component, and uses the blocker-history slots to avoid immediately rejecting the detour step against the same blocker
