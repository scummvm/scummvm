...
- `load_dialogue_response_line` at `0x3a1a0` is the indexed `dialog.rsp` line loader used by the talk-to handlers.
  ...
  - `handle_talk_to_stephanie @ 0x36710` is now bounded enough for a direct startup-room port.
    ...
    - The corrected raw-code tails show two hidden Stephanie continuations outside the current clean decompiler body.
      ...
      - The post-`KARIN_KIDNAPED` tail reuses that local spyhole flag to prepend `0x4970` / `0x4975`, optionally adds Mr. and Mrs. Potts's Tuesday-night alibi follow-ups (`0x4980` and `0x4984` / `0x4989` / `0x498f`), and only then checks the Karin-alive / Karin-dead payoff block.
