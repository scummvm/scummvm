...
- `load_dialogue_response_line` at `0x3a1a0` is the indexed `dialog.rsp` line loader used by the talk-to handlers.
  ...
  - `handle_talk_to_jimmy @ 0x28880` is a compact short-circuit handler with no keyword loop.
    - The first no-item visit clears `g_jimmy_first_no_item_line_pending`, conditionally plays `0x4a4c` if `g_jimmy_talk_state_block` is still clear, then always continues through `0x4a58`, sets `PAPER_CHK_1` and `GIVEN_PAPER_TODAY`, and opens three response menus from zero-based `dialog.rsp` lines `0xf5`, `0xf6`, and `0xf7`.
    ...
    - Presenting `NEWSPAPER` sets `GIVEN_PAPER_TODAY`, runs the `ACTV_HOUSE_EXIT` action tag, restores `NEWSPAPER` to its startup state, and on the first paper handoff plays `0x4a4c` before the random revisit bark `0x4acc` / `0x4a4b`.
    - The evidence-item path is otherwise linear: `PHOTO_OF_WHALEY_HERRILL` plays `0x4af8`, `LEDGER` / `LEDGER2` with `HAVE_BOTH_LEDGERS` and `CASKET_PHOTO` / `CASKET_PHOTOCOPY` converge on `0x4b00`, note/checkbook evidence plays `0x4b21`, `SNEAKERS` plays `0x4a9e` then falls through to `0x4af2`, and all other items fall back to `0x4af2`.
    - Later no-item visits remain a straight bark chain over inventory `SNEAKERS` without `BROOMKEY`, then `PAPER_CHK_4`, `PAPER_CHK_3`, `PAPER_CHK_2`, and the default `0x4b38`.
