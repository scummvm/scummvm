...
- The adjacent `0x383d0`-`0x384e0` range is now confirmed to be another overlapping run of tiny read/write dialogue-state wrappers over globals in the same contiguous block (`0xd2f00` through `0xd2f28`).
  ...
  - The `SCRATCHED_TUCKER` no-item path is not just that single bark. After `0x420d`, Sergeant continues with `0x421e`, opens zero-based `dialog.rsp` line `0x2b4`, and then either accepts the handoff with `0x422a` or pushes back through `0x422f` / `0x4239`.
