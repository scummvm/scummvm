...
- `load_dialogue_response_line` at `0x3a1a0` is the indexed `dialog.rsp` line loader used by the talk-to handlers.
  ...
  - `handle_talk_to_moynahan @ 0x24a20` is now bounded enough for a direct startup-room port.
    ...
    - Moynahan's keyword loop is typed-topic driven. Zero-based `dialog.rsp` line `0x199` is the explicit farewell topic behind `0x3e32`; `0x19a` / `0x19b` set the shared Mom day-5 good-cause state before `0x3d5f`; `0x19d..0x1a0`, `0x1a2..0x1a3`, and `0x1a9..0x1ab` each converge on single response lines; `0x1b2..0x1b3` is the day-5-only branch that writes the recovered shared `d2ec0` / `d2ec8` bits before `0x3df5`; `0x1b4..0x1b5` is room-specific to `MBLM`; `0x1b8..0x1be` and `0x1bf..0x1c2` are also day-5 gated; `0x1c4` loops silently on the current menu; everything else falls back to `0x3e39`.
