...
if(DETECTION_FULL)
  add_compile_definitions(DETECTION_FULL)
endif()

if(ENABLE_PLUGINS)
  add_compile_definitions(DYNAMIC_MODULES)
  # Default plugin prefix/suffix (can be overridden by toolkit)
  if(NOT DEFINED PLUGIN_PREFIX)
    set(PLUGIN_PREFIX "")
  endif()
  if(NOT DEFINED PLUGIN_SUFFIX)
    if(WIN32)
  ...
  add_compile_definitions(PLUGIN_PREFIX="${PLUGIN_PREFIX}")
  add_compile_definitions(PLUGIN_SUFFIX="${PLUGIN_SUFFIX}")
endif()

set(DYNAMIC_ENGINES "" CACHE STRING "Engines to build as dynamic plugins")
set(STATIC_ENGINES "" CACHE STRING "Engines to build as static builtin")
...
foreach(engine IN LISTS _engines)
  if("${_engine_${engine}_build}")
    ...
    # Determine build type (STATIC or MODULE)
    set(engine_build_type "STATIC")
    if(ENABLE_PLUGINS)
      if(engine IN_LIST DYNAMIC_ENGINES)
        set(engine_build_type "MODULE")
      elseif(engine IN_LIST STATIC_ENGINES)
        set(engine_build_type "STATIC")
      elseif(DEFAULT_DYNAMIC)
        set(engine_build_type "MODULE")
      endif()
    endif()
    set("_engine_${engine}_build_type" ${engine_build_type})
