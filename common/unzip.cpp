...
static int unzlocal_CheckCurrentFileCoherencyHeader(unz_s *s, uInt *piSizeVar,
	...
	*psize_local_extrafield = 0;
	...
	s->_stream->seek(s->cur_file_info_internal.offset_curfile + s->byte_before_the_zipfile, SEEK_SET);
	if (s->_stream->err())
		return UNZ_ERRNO;
...
int unzReadCurrentFile(unzFile file, voidp buf, unsigned len) {
	...
	while (pfile_in_zip_read_info->stream.avail_out > 0) {
		if ((pfile_in_zip_read_info->stream.avail_in == 0) && (pfile_in_zip_read_info->rest_read_compressed > 0)) {
			...
			if (uReadThis == 0)
				return UNZ_EOF;
			pfile_in_zip_read_info->_stream->seek(pfile_in_zip_read_info->pos_in_zipfile + pfile_in_zip_read_info->byte_before_the_zipfile, SEEK_SET);
		...
		if (pfile_in_zip_read_info->compression_method == 0) {
			...
			*/
			...
			pfile_in_zip_read_info->crc32_data = crc32(pfile_in_zip_read_info->crc32_data, bufBefore, (uInt)(uOutThis));
...
int unzGetLocalExtrafield(unzFile file, voidp buf, unsigned len) {
	...
	pfile_in_zip_read_info->_stream->seek(pfile_in_zip_read_info->offset_local_extrafield + pfile_in_zip_read_info->pos_local_extrafield, SEEK_SET);
	if (pfile_in_zip_read_info->_stream->err())
		return UNZ_ERRNO;
...
ZipArchive::ZipArchive(unzFile zipFile)
  : _zipFile(zipFile) {
	assert(_zipFile);
}
