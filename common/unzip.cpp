...
unzFile unzOpen(Common::SeekableReadStream *stream) {
	...
	us->_stream->seek(central_pos, Seek::SET);
	if (us->_stream->err())
		err=UNZ_ERRNO;
                                              ...
                                              char *szComment,  uLong commentBufferSize)
{
	...
	if (file==NULL)
		return UNZ_PARAMERROR;
	s=(unz_s*)file;
	s->_stream->seek(s->pos_in_central_dir+s->byte_before_the_zipfile, Seek::SET);
	if (s->_stream->err())
		err=UNZ_ERRNO;
...
static int unzlocal_CheckCurrentFileCoherencyHeader(unz_s* s, uInt* piSizeVar,
	...
	*psize_local_extrafield = 0;
	...
	s->_stream->seek(s->cur_file_info_internal.offset_curfile +
								s->byte_before_the_zipfile, Seek::SET);
	if (s->_stream->err())
		return UNZ_ERRNO;
	...
	while (pfile_in_zip_read_info->stream.avail_out>0) {
		if ((pfile_in_zip_read_info->stream.avail_in==0) &&
			...
			pfile_in_zip_read_info->_stream->seek(pfile_in_zip_read_info->pos_in_zipfile +
				pfile_in_zip_read_info->byte_before_the_zipfile, Seek::SET);
...
int unzGetLocalExtrafield(unzFile file, voidp buf, unsigned len) {
	...
	pfile_in_zip_read_info->_stream->seek(pfile_in_zip_read_info->offset_local_extrafield +
			  pfile_in_zip_read_info->pos_local_extrafield,Seek::SET);
	if (pfile_in_zip_read_info->_stream->err())
		return UNZ_ERRNO;
...
int unzGetGlobalComment(unzFile file, char *szComment, uLong uSizeBuf) {
	...
	s->_stream->seek(s->central_pos+22, Seek::SET);
	if (s->_stream->err())
		return UNZ_ERRNO;
