...
bool MemoryReadStream::seek(int32 offs, SeekWhence whence) {
	// Pre-Condition
	assert(_pos <= _size);
	switch (whence) {
	case kSeekEnd:
		// kSeekEnd works just like kSeekSet, only 'reversed',
		// i.e. from the end.
		offs = _size + offs;
		// Fall through
	case kSeekSet:
...
bool MemoryWriteStreamDynamic::seek(int32 offs, SeekWhence whence) {
	// Pre-Condition
	assert(_pos <= _size);
	switch (whence) {
	case kSeekEnd:
...
bool SeekableSubReadStream::seek(int32 offset, SeekWhence whence) {
	assert(_pos >= _begin);
	assert(_pos <= _end);
...
bool BufferedSeekableReadStream::seek(int32 offset, SeekWhence whence) {
