...
public:
	...
	inline SpanIterator(span_type *const span, const difference_type index) : _span(span),
	                                                                          _index(index) {
		if (span != nullptr) {
	...
	inline SpanIterator(const SpanIterator &other) : _span(other._span),
	                                                 _index(other._index) {}
...
class SpanBase : public SafeBool<Derived<ValueType>> {
	...
	friend class SpanInternal::SpanIterator;
...
public:
	...
	inline SpanImpl(const pointer data_, const size_type size_) : super_type(),
	                                                              _data(data_),
	                                                              _size(size_) {}

	template<typename Other>
	inline SpanImpl(const Other &other) : super_type(),
	                                      _data(other.data()),
	                                      _size(other.size()) {}
...
public:
	...
	inline NamedSpanImpl(const pointer data_,
	                     ...
	                     const size_type sourceByteOffset_ = 0) : super_type(data_, size_),
	                                                              _name(name_),
	                                                              _sourceByteOffset(sourceByteOffset_) {}

	template<typename Other>
	inline NamedSpanImpl(const Other &other) : super_type(other),
	                                           _name(other.name()),
