...
bool computeStreamMD5(ReadStream &stream, uint8 digest[16], uint32 length, ProgressUpdateCallback progressUpdateCallback, GUI::IntegrityDialog *dialog) {
	...
	while ((i = stream.read(buf, readlen)) > 0) {
		...
		if (progressUpdateCallback != nullptr && !progressUpdateCallback(dialog, i)) {
			return false;
		}
...
String computeStreamMD5AsString(ReadStream &stream, uint32 length, ProgressUpdateCallback progressUpdateCallback, GUI::IntegrityDialog *dialog) {
	String md5;
	uint8 digest[16];
	if (computeStreamMD5(stream, digest, length, progressUpdateCallback, dialog)) {
		for (int i = 0; i < 16; i++) {
