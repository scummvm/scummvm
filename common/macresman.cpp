...
String MacResManager::computeResForkMD5AsString(uint32 length, bool tail, ProgressUpdateCallback progressUpdateCallback, GUI::IntegrityDialog *dialog) const {
	if (!hasResFork())
		return String();
	...
	return computeStreamMD5AsString(resForkStream, MIN<uint32>(length, _resForkSize), progressUpdateCallback, dialog);
}
