...
int Indeo5Decoder::decodeMbInfo(IVIBandDesc *band, IVITile *tile) {
	...
	if (!refMb && ((band->_qdeltaPresent && band->_inheritQDelta) || band->_inheritMv))
		return -1;
		...
		for (x = tile->_xPos; x < (tile->_xPos + tile->_width); x += band->_mbSize) {
			...
			if (_ctx._gb->getBit()) {
				...
				if (band->_qdeltaPresent) {
					...
					} else if (mb->_cbp || (!band->_plane && !band->_bandNum && (_ctx._frameFlags & 8))) {
						mb->_qDelta = _ctx._gb->getVLC2<1>(_ctx._mbVlc._tab->_table, IVI_VLC_BITS);
						mb->_qDelta = IVI_TOSIGNED(mb->_qDelta);
					}
				}
			...
			s = band->_isHalfpel;
			if (mb->_type)
				if (x + (mb->_mvX >> s) + (y + (mb->_mvY >> s)) * band->_pitch < 0 || x + ((mb->_mvX + s) >> s) + band->_mbSize - 1 + (y + band->_mbSize - 1 + ((mb->_mvY + s) >> s)) * band->_pitch > band->_bufSize - 1) {
...
int Indeo5Decoder::decode_gop_header() {
	...
	for (p = 0; p <= 1; p++) {
		for (i = 0; i < (!p ? picConf._lumaBands : picConf._chromaBands); i++) {
			...
			band->_is2dTrans = band->_invTransform == IndeoDSP::ffIviInverseSlant8x8 || band->_invTransform == IndeoDSP::ffIviInverseSlant4x4;
