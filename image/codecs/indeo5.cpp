...
Indeo5Decoder::Indeo5Decoder(uint16 width, uint16 height, uint bitsPerPixel) : IndeoDecoderBase(width, height, bitsPerPixel) {
		...
		for (x = tile->_xPos; x < (tile->_xPos + tile->_width); x += band->_mbSize) {
			...
			if (mb->_type)
				if (x + (mb->_mvX >> s) + (y + (mb->_mvY >> s)) * band->_pitch < 0 ||
				    x + ((mb->_mvX + s) >> s) + band->_mbSize - 1 + (y + band->_mbSize - 1 + ((mb->_mvY + s) >> s)) * band->_pitch > band->_bufSize - 1) {
