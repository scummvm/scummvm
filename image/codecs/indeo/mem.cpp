 ...
 */
	uint32 bitswap32(uint32 x) {
		return (uint32)ffReverse[x & 0xFF] << 24 | (uint32)ffReverse[(x >> 8) & 0xFF] << 16 | (uint32)ffReverse[(x >> 16) & 0xFF] << 8 | (uint32)ffReverse[x >> 24];
	}
 ...
 */
	uint16 invertBits(uint16 val, int nbits) {
		...
		if (nbits <= 8) {
			...
			res = ((ffReverse[val & 0xFF] << 8) + (ffReverse[val >> 8])) >> (16 - nbits);
		}
