...
int Indeo4Decoder::decodePictureHeader() {
	...
	if (_ctx._mbVlc.decodeHuffDesc(&_ctx, _ctx._gb->getBit(), IVI_MB_HUFF) || _ctx._blkVlc.decodeHuffDesc(&_ctx, _ctx._gb->getBit(), IVI_BLK_HUFF))
		return -1;
...
int Indeo4Decoder::decodeBandHeader(IVIBandDesc *band) {
	...
	if (!band->_isEmpty) {
		...
		if (!_ctx._gb->getBit() || _ctx._frameType == IVI4_FRAMETYPE_INTRA) {
			...
			if ((uint)transformId >= FF_ARRAY_ELEMS(_transforms) || !_transforms[transformId]._invTrans) {
				warning("Transform %d", transformId);
				return -3;
			}
			if ((transformId >= 7 && transformId <= 9) || transformId == 17) {
		...
		else if (band->_blkVlc.decodeHuffDesc(&_ctx, 1, IVI_BLK_HUFF))
			return -1;
	...
	for (y = tile->_yPos; y < tile->_yPos + tile->_height; y += band->_mbSize) {
		...
		for (x = tile->_xPos; x < tile->_xPos + tile->_width; x += band->_mbSize) {
			...
			if (_ctx._gb->getBit()) {
				...
				} else if (_ctx._frameType == IVI4_FRAMETYPE_INTRA || _ctx._frameType == IVI4_FRAMETYPE_INTRA1) {
				...
				} else if (mb->_cbp || (!band->_plane && !band->_bandNum && _ctx._inQ)) {
					mb->_qDelta = _ctx._gb->getVLC2<1>(_ctx._mbVlc._tab->_table,
					                                   IVI_VLC_BITS);
					mb->_qDelta = IVI_TOSIGNED(mb->_qDelta);
				}
