...
bool CoktelDecoder::evaluateSeekFrame(int32 &frame, Common::SeekWhence whence) const {
	if (!isVideoLoaded())
		// Nothing to do
		return false;

	// Find the frame to which to seek
	if      (whence == Common::kSeekCur)
		frame += _curFrame;
	else if (whence == Common::kSeekEnd)
		frame = _frameCount - frame - 1;
	else if (whence == Common::kSeekSet)
		frame--;
	else
		return false;
...
bool PreIMDDecoder::seek(int32 frame, Common::SeekWhence whence, bool restart) {
...
bool IMDDecoder::seek(int32 frame, Common::SeekWhence whence, bool restart) {
	if (!evaluateSeekFrame(frame, whence))
		return false;
...
bool VMDDecoder::seek(int32 frame, Common::SeekWhence whence, bool restart) {
	if (!evaluateSeekFrame(frame, whence))
		return false;
