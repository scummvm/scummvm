...
IMDDecoder::IMDDecoder(Audio::Mixer *mixer, Audio::Mixer::SoundType soundType)
  : CoktelDecoder(mixer, soundType)
  , _stream(0)
  , _version(0)
  , _stdX(-1)
...
VMDDecoder::VMDDecoder(Audio::Mixer *mixer, Audio::Mixer::SoundType soundType)
  : CoktelDecoder(mixer, soundType)
  , _stream(0)
  , _version(0)
  , _flags(0)
...
bool VMDDecoder::seek(int32 frame, int whence, bool restart) {
	...
	// Restart sound
	if (_hasSound && (frame == -1) && ((_soundStage == kSoundNone) || (_soundStage == kSoundFinished))) {
...
bool VMDDecoder::renderFrame(Common::Rect &rect) {
	...
	if (type & 0x80) {
		...
		srcBuffer = 1;
		_videoBufferLen[srcBuffer] = deLZ77(_videoBuffer[srcBuffer], dataPtr, dataSize, _videoBufferSize);
...
void VMDDecoder::blit16(const Graphics::Surface &srcSurf, Common::Rect &rect) {
	...
	const byte *src = (const byte *)srcSurf.getPixels() + (srcRect.top * srcSurf.pitch) + srcRect.left * _bytesPerPixel;
...
public:
	VMD_ADPCMStream(Common::SeekableReadStream *stream, DisposeAfterUse::Flag disposeAfterUse,
	  ...
	  : Audio::DVI_ADPCMStream(stream, disposeAfterUse, stream->size(), rate, channels, 0) {
...
Common::SeekableReadStream *VMDDecoder::getEmbeddedFile(const Common::String &fileName) const {
	...
	Common::MemoryReadStream *stream = new Common::MemoryReadStream(data, file->realSize, DisposeAfterUse::YES);
...
AdvancedVMDDecoder::VMDVideoTrack::VMDVideoTrack(VMDDecoder *decoder)
  : _decoder(decoder) {
}
