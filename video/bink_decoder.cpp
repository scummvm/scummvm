...
void BinkDecoder::readNextPacket() {
	...
	for (uint32 i = 0; i < _audioTracks.size(); i++) {
		...
		if (audioPacketLength >= 4) {
			...
			audio.bits = new Common::BitStream32LELSB(new Common::SeekableSubReadStream(_bink,
			                                                                            audioPacketStart + 4, audioPacketEnd),
			                                          DisposeAfterUse::YES);
	...
	frame.bits = new Common::BitStream32LELSB(new Common::SeekableSubReadStream(_bink,
	                                                                            videoPacketStart, videoPacketEnd),
	                                          DisposeAfterUse::YES);
...
BinkDecoder::BinkVideoTrack::BinkVideoTrack(uint32 width, uint32 height, const Graphics::PixelFormat &format, uint32 frameCount, const Common::Rational &frameRate, bool swapPlanes, bool hasAlpha, uint32 id) : _frameCount(frameCount), _frameRate(frameRate), _swapPlanes(swapPlanes), _hasAlpha(hasAlpha), _id(id) {
	_curFrame = -1;
...
BinkDecoder::BinkAudioTrack::BinkAudioTrack(BinkDecoder::AudioInfo &audio, Audio::Mixer::SoundType soundType) : AudioTrack(soundType),
                                                                                                                _audioInfo(&audio) {
