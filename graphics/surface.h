...
struct Surface {
	 ...
	 * @param target	the target image. In most cases this is the framebuffer.
	 *
	 * @param posX		the position on the X-axis in the target image in pixels 
	 *					where the image is supposed to be rendered.
	 *					The default value is 0.
	 *
	 * @param posY		the position on the Y-axis in the target image in pixels 
	 *					where the image is supposed to be rendered.
	 *					The default value is 0.
	 *
	 ...
	 *
	 * @param partRect	Pointer on Common::Rect which specifies the section to be rendered.
	 *					If the whole image has to be rendered the Pointer is NULL.
	 *					This refers to the unflipped and unscaled image.
	 *					The default value is NULL.
	 *
	 * @param Color		an ARGB color value, which determines the parameters for 
	 *					the color modulation und alpha blending.
	 *					The alpha component of the color determines the alpha blending 
	 *					parameter (0 = no covering, 255 = full covering).
	 *					The color components determines the color for color modulation.
	 *					The default value is BS_ARGB(255, 255, 255, 255) 
	 *					(full covering, no color modulation).
	 ...
	 *					The images will be scaled if the output width of the screen 
	 *					section differs from the image section.
	 ...
	 * @remarks	The current implementation only supports 32 bpp images in 16, 8, 0, 24 
	 * (ARGB)-order although, as long as the source and target have the same pixelFormat, 
	 * and alpha is located at left-shift 0, (i.e. ARGB vs BGRA, but not RGBA or ABGR) 
	 * it should work.
	 */
	Common::Rect blit(Graphics::Surface &target, int posX = 0, int posY = 0,
	                  int flipping = FLIP_NONE,
	                  const Common::Rect *partRect = NULL,
	                  uint color = ARGB_TO_COLOR(255, 255, 255, 255),
	 ...
	 */
	void applyColorKey(uint8 rKey, uint8 gKey, uint8 bKey, bool overwriteAlpha = false);

	/**
	 * Scales a passed surface, creating a new surface with the result
	 * @param srcImage      Source image to scale
	 * @param xSize			The target width
	 * @param ySize			The target height
	 * @remarks Caller is responsible for freeing the returned surface
	 */
	Graphics::Surface *scale(int xSize, int ySize) const;
private:
