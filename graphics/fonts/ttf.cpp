...
TTFFont::TTFFont()
	: _initialized(false), _face(), _ttfFile(0), _width(0), _height(0), _ascent(0),
...
Font *loadTTFFont(Common::SeekableReadStream *stream, DisposeAfterUse::Flag disposeAfterUse, int size, TTFSizeMode sizeMode, uint dpi, TTFRenderMode renderMode, const uint32 *mapping, bool stemDarkening) {
	TTFFont *font = new TTFFont();

	if (!font->load(stream, disposeAfterUse, size, sizeMode, dpi, renderMode, mapping, stemDarkening)) {
		delete font;
		return 0;
	}
...
Font *loadTTFFontFromArchive(const Common::String &filename, int size, TTFSizeMode sizeMode, uint dpi, TTFRenderMode renderMode, const uint32 *mapping) {
	...
	Font *font = loadTTFFont(f, DisposeAfterUse::YES, size, sizeMode, dpi, renderMode, mapping);
	if (!font) {
		delete archive;
		delete f;
		return nullptr;
	...
	// HACK: We currently assume that ZipArchive always loads the whole file into memory, so we can delete the aechive here.
	delete archive;
	return font;
}
...
Font *findTTFace(const Common::Array<Common::String> &files, const Common::U32String &faceName,
	...
	if (!font->load(bestTTFFile, DisposeAfterUse::YES, size, sizeMode, dpi,
