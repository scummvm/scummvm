...
#define IS_IN_CLIP(x, y) (_clippingArea.left <= (x) && (x) < _clippingArea.right && _clippingArea.top <= (y) && (y) < _clippingArea.bottom)
...
VectorRendererSpec<PixelType>::
    VectorRendererSpec(PixelFormat format) : _format(format),
...
inline void VectorRendererSpec<PixelType>::
	...
	if (alpha == 0xff) {
		...
		*ptr = ((dR << _format.rShift) & _redMask) | ((dG << _format.gShift) & _greenMask) | ((dB << _format.bShift) & _blueMask) | ((dA << _format.aShift) & _alphaMask);
...
void VectorRendererSpec<PixelType>::
	...
	if (Base::_fillMode != kFillDisabled && Base::_shadowOffset && x + r + Base::_shadowOffset < Base::_activeSurface->w && y + r + Base::_shadowOffset < Base::_activeSurface->h) {
		if (useClippingVersions)
...
void VectorRendererSpec<PixelType>::
	...
	if (Base::_fillMode != kFillDisabled && Base::_shadowOffset && x + w + Base::_shadowOffset < Base::_activeSurface->w && y + h + Base::_shadowOffset < Base::_activeSurface->h) {
		if (useClippingVersions)
...
void VectorRendererSpec<PixelType>::
	...
	if (Base::_fillMode != kFillDisabled && Base::_shadowOffset && x + w + Base::_shadowOffset + 1 < Base::_activeSurface->w && y + h + Base::_shadowOffset + 1 < Base::_activeSurface->h && h > (Base::_shadowOffset + 1) * 2) {
		if (useOriginal) {
