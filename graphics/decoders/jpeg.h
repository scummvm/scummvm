...
/*
 * This code is based on jpgd.h coming from jpeg-compressor
...
// ScummVM stream class.
class jpeg_decoder_scummvm_stream : public jpeg_decoder_stream {

	Common::SeekableReadStream *_stream;

public:
	jpeg_decoder_scummvm_stream(Common::SeekableReadStream *stream) : _stream(stream) { }

	virtual ~jpeg_decoder_scummvm_stream() { }
...
Graphics::Surface *decompress_jpeg_image_from_stream(jpeg_decoder_stream *pStream, int *width, int *height, int *actual_comps, int req_comps);
