...
public:
	MarginedBitmap(int width, int height, T marginValue) : _width(width),
	                                                       _height(height),
	...
	MarginedBitmap(const MarginedBitmap &rhs) : _width(rhs._width),
	                                            _height(rhs._height),
	                                            _stride(rhs._stride),
	                                            _buffer(rhs._buffer),
	                                            _origin(calculateOrigin()) {}
...
#define EQUALS(x, y) (pixel[y * stride + x] == *pixel)
	...
	return (EQUALS(-1, 0) ? 0x01 : 0x00) | (EQUALS(-1, -1) ? 0x02 : 0x00) | (EQUALS(0, -1) ? 0x04 : 0x00) | (EQUALS(1, -1) ? 0x08 : 0x00) | (EQUALS(1, 0) ? 0x10 : 0x00) | (EQUALS(1, 1) ? 0x20 : 0x00) | (EQUALS(0, 1) ? 0x40 : 0x00) | (EQUALS(-1, 1) ? 0x80 : 0x00);
