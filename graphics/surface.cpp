...
void Surface::create(int32 width, int32 height, const PixelFormat &f) {
	free();
...
void Surface::init(int32 width, int32 height, int32 newPitch, void *newPixels, const PixelFormat &f) {
	assert(width >= 0 && height >= 0);
	w = width;
	h = height;
	pitch = newPitch;
...
Graphics::Surface *Surface::scale(uint32 newWidth, uint32 newHeight, bool filtering) const {
